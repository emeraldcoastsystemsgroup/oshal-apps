...
export const BAKE_OFF_LIMITS = Object.freeze({
  nameMax: 120,
  promptMin: 20,
  promptMax: 20000,
  referenceMax: 20000,
...
/** A validated bake-off job: the prompt to race, the rubric to grade it by, and the economics. */
export interface BakeOffSpec {
  name: string;
  prompt: string;
  rubric: string[];
  ...
  /** Explicit lane subset (bot agent ids). Empty = every lane the caller may reach. */
  laneAgentIds: string[];
}

/** One lane's raw result as the engine persists it (cost/model observed, never declared). */
export interface LaneResultRow {
  laneBot: string;
  laneAgentId: string;
  laneHarness: string;
  laneProvider: string;
...
/** A lane reduced to the fields the ranking actually reasons over. */
export interface LaneSummary {
  ...
  ok: boolean;
  costUsd: number;
  /** False when the lane failed or reported a non-positive cost — excluded from savings math. */
  costKnown: boolean;
  score: number;
  judgeMode: string | null;
  totalTokens: number;
...
/** Why a lane was excluded from the ranked set. */
export type LaneRejectionReason = 'lane-failed' | 'below-quality-bar' | 'cost-unknown';

/** A rejected lane plus the single reason that disqualified it. */
export interface RejectedLane extends LaneSummary {
  reason: LaneRejectionReason;
}

/** Why no recommendation could be made. `null` when one was. */
...
/** The report's headline: which lane to use, against what, and what it saves. */
export interface Recommendation {
  winner: LaneSummary | null;
  baseline: LaneSummary | null;
  perRunSavingUsd: number | null;
  monthlySavingUsd: number | null;
  judgeMode: string;
  blocked: RecommendationBlock | null;
  caveats: string[];
}
...
function microMoney(n: number): number {
 ...
 */
export function normalizeRubric(raw: unknown): string[] | null {
  if (raw === undefined || raw === null) return [...BAKE_OFF_DEFAULTS.rubric];
  if (!Array.isArray(raw)) return null;
  const cleaned = raw
    .filter((r): r is string => typeof r === 'string')
    .map((r) => r.trim())
    .filter((r) => r.length > 0)
    .slice(0, BAKE_OFF_LIMITS.rubricMax);
...
export function validateJobSpec(
  raw: unknown,
): { ok: true; spec: BakeOffSpec } | { ok: false; error: string } {
  const body = (raw ?? {}) as Record<string, unknown>;
  const name = String(body.name ?? '').trim().slice(0, BAKE_OFF_LIMITS.nameMax);
  if (!name) return { ok: false, error: 'name is required' };

  const prompt = String(body.prompt ?? '').trim();
  if (prompt.length < BAKE_OFF_LIMITS.promptMin) {
    return { ok: false, error: `prompt must be at least ${BAKE_OFF_LIMITS.promptMin} characters — a shorter prompt cannot separate one lane from another` };
  }
  if (prompt.length > BAKE_OFF_LIMITS.promptMax) {
    return { ok: false, error: `prompt exceeds ${BAKE_OFF_LIMITS.promptMax} characters` };
  }

  const rubric = normalizeRubric(body.rubric);
  if (!rubric) return { ok: false, error: 'rubric must be a non-empty array of criteria strings' };

  const referenceRaw = typeof body.reference === 'string' ? body.reference.trim() : '';
  const laneAgentIds = Array.isArray(body.laneAgentIds)
    ? body.laneAgentIds.filter((v): v is string => typeof v === 'string' && v.trim().length > 0).map((v) => v.trim())
    : [];
  ...
  return {
    ...
    spec: {
      ...
      prompt,
      rubric,
      reference: referenceRaw ? referenceRaw.slice(0, BAKE_OFF_LIMITS.referenceMax) : null,
      qualityBar: clamp(num(body.qualityBar, BAKE_OFF_DEFAULTS.qualityBar), 0, 100),
      monthlyVolume: Math.round(clamp(num(body.monthlyVolume, BAKE_OFF_DEFAULTS.monthlyVolume), 1, BAKE_OFF_LIMITS.volumeMax)),
      laneAgentIds: laneAgentIds.slice(0, BAKE_OFF_DEFAULTS.maxLanes),
    },
  };
}
 ...
 */
export function laneKey(lane: { laneBot?: string; bot?: string; laneHarness?: string; harness?: string; laneProvider?: string; provider?: string }): string {
 ...
 */
export function summarizeResult(row: LaneResultRow): LaneSummary {
 ...
 */
export function judgeModeConsensus(summaries: LaneSummary[]): 'llm' | 'lexical-fallback' | 'mixed' | 'none' {
  const modes = new Set(summaries.filter((s) => s.ok && s.judgeMode).map((s) => s.judgeMode as string));
  if (modes.size === 0) return 'none';
  if (modes.size > 1) return 'mixed';
  const only = [...modes][0];
  return only === JUDGE_MODE_LLM ? JUDGE_MODE_LLM : JUDGE_MODE_LEXICAL;
}
...
export function rankLanes(
  ...
  qualityBar: number,
): { qualifying: LaneSummary[]; rejected: RejectedLane[] } {
  const qualifying: LaneSummary[] = [];
  const rejected: RejectedLane[] = [];
  for (const lane of summaries) {
    if (!lane.ok) rejected.push({ ...lane, reason: 'lane-failed' });
    else if (lane.score < qualityBar) rejected.push({ ...lane, reason: 'below-quality-bar' });
    else if (!lane.costKnown) rejected.push({ ...lane, reason: 'cost-unknown' });
    else qualifying.push(lane);
  }
 ...
 */
export function projectSaving(
  baselineCostUsd: number,
  winnerCostUsd: number,
  monthlyVolume: number,
): { perRunSavingUsd: number; monthlySavingUsd: number } {
...
function pickBaseline(qualifying: LaneSummary[]): LaneSummary {
...
function baseCaveats(summaries: LaneSummary[], rejected: RejectedLane[], mode: string): string[] {
  const caveats = ['One sample per lane — this is a probe, not a benchmark. Re-run before acting on a narrow margin.'];
  if (mode === JUDGE_MODE_LEXICAL) {
  ...
  if (failed.length) caveats.push(`${failed.length} lane(s) failed and were excluded: ${failed.join(', ')}.`);
...
export function recommendLane(
  summaries: LaneSummary[],
  opts: { qualityBar: number; monthlyVolume: number },
): Recommendation {
  const mode = judgeModeConsensus(summaries);
  const qualityBar = clamp(num(opts?.qualityBar, BAKE_OFF_DEFAULTS.qualityBar), 0, 100);
  const monthlyVolume = Math.max(1, Math.round(num(opts?.monthlyVolume, BAKE_OFF_DEFAULTS.monthlyVolume)));
  ...
  if (!summaries.length) return { ...empty, blocked: 'no-results', caveats };
  if (mode === 'mixed') {
...
export function buildReport(
  ...
  lanes: LaneSummary[];
  qualifying: LaneSummary[];
  rejected: RejectedLane[];
  recommendation: Recommendation;
} {
  const lanes = (rows ?? []).map(summarizeResult);
  const { qualifying, rejected } = rankLanes(lanes, clamp(num(opts?.qualityBar, BAKE_OFF_DEFAULTS.qualityBar), 0, 100));
