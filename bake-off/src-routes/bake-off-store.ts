...
import type { BakeOffSpec, LaneResultRow } from './bake-off-scoring';

const log = createChildLogger({ module: 'bake-off-store' });

/** Tables this package owns. Order matters — results reference runs, runs reference jobs. */
const TABLES = ['bake_off_jobs', 'bake_off_runs', 'bake_off_results'] as const;
 ...
 */
export async function ensureBakeOffSchema(pool: Pool): Promise<void> {
  const statements = [SCHEMA_SQL, ...TABLES.flatMap((t) => buildOwnerRlsPolicyStatements(t, 'owner_sub'))];
...
/** A stored job row as the routes hand it back. */
export interface BakeOffJob extends BakeOffSpec {
  id: string;
  ownerSub: string;
  createdAt: string;
  updatedAt: string;
...
function toJob(r: Record<string, any>): BakeOffJob {
  return {
    id: String(r.id),
 ...
 */
export async function insertJob(pool: Pool, ownerSub: string, spec: BakeOffSpec): Promise<BakeOffJob> {
  const { rows } = await pool.query(
    `INSERT INTO bake_off_jobs (owner_sub, name, prompt, rubric, reference, quality_bar, monthly_volume, lane_agent_ids)
     VALUES ($1,$2,$3,$4::jsonb,$5,$6,$7,$8::jsonb) RETURNING *`,
    [ownerSub, spec.name, spec.prompt, JSON.stringify(spec.rubric), spec.reference,
 ...
 */
export async function listJobs(pool: Pool, ownerSub: string): Promise<BakeOffJob[]> {
  const { rows } = await pool.query(
    'SELECT * FROM bake_off_jobs WHERE owner_sub = $1 ORDER BY created_at DESC LIMIT 200',
    [ownerSub],
  );
 ...
 */
export async function getJob(pool: Pool, ownerSub: string, jobId: string): Promise<BakeOffJob | null> {
 ...
 */
export async function deleteJob(pool: Pool, ownerSub: string, jobId: string): Promise<boolean> {
 ...
 */
export async function openRun(pool: Pool, ownerSub: string, jobId: string, lanesRequested: number): Promise<string> {
  const { rows } = await pool.query(
    `INSERT INTO bake_off_runs (job_id, owner_sub, status, lanes_requested) VALUES ($1,$2,'running',$3) RETURNING id`,
    [jobId, ownerSub, lanesRequested],
  );
...
export async function closeRun(
  pool: Pool, runId: string, status: 'complete' | 'failed', lanesCompleted: number, error?: string | null,
): Promise<void> {
  await pool.query(
    `UPDATE bake_off_runs SET status = $2, lanes_completed = $3, error = $4, finished_at = NOW() WHERE id = $1`,
    [runId, status, lanesCompleted, error ?? null],
  );
}

/** One lane result plus the fields only the store persists (output text, rationale, dimensions). */
export interface LaneResultInsert extends LaneResultRow {
  output?: string | null;
 ...
 * @returns Nothing.
 */
export async function insertResult(pool: Pool, ownerSub: string, runId: string, r: LaneResultInsert): Promise<void> {
  await pool.query(
    `INSERT INTO bake_off_results (run_id, owner_sub, lane_bot, lane_agent_id, lane_harness, lane_provider,
    ...
    [runId, ownerSub, r.laneBot, r.laneAgentId, r.laneHarness, r.laneProvider, r.observedModel ?? null,
...
/** A run header as the routes hand it back. */
...
function toRun(r: Record<string, any>): BakeOffRun {
  return {
 ...
 * @returns The runs.
 */
export async function listRuns(pool: Pool, ownerSub: string, jobId: string): Promise<BakeOffRun[]> {
  const { rows } = await pool.query(
    'SELECT * FROM bake_off_runs WHERE job_id = $1 AND owner_sub = $2 ORDER BY started_at DESC LIMIT 50',
    [jobId, ownerSub],
  );
...
export async function getRun(
  pool: Pool, ownerSub: string, jobId: string, runId: string | null,
): Promise<BakeOffRun | null> {
  const { rows } = runId
...
export async function listResults(
  pool: Pool, ownerSub: string, runId: string, includeOutput: boolean,
): Promise<Array<LaneResultInsert & { output: string | null }>> {
  const { rows } = await pool.query(
    'SELECT * FROM bake_off_results WHERE run_id = $1 AND owner_sub = $2 ORDER BY created_at',
    [runId, ownerSub],
  );
