...
export async function ensureDroneSchema(pool: Pool): Promise<void> {
  await runRuntimeSchemaBootstrap({
    ...
    CREATE TABLE IF NOT EXISTS drone_messages (
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
  `,
      ...buildOwnerRlsPolicyStatements('drone_missions', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('drone_command_log', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('drone_conversations', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('drone_messages', 'user_sub'),
    ],
    requirements: [
...
interface DraftEnvelope { say: string; mission: unknown | null; fleetMission: unknown | null }
...
function buildOperatorPrompt(o: {
  ...
  telemetry: DroneTelemetry; fence: Geofence; missions: Array<{ name: string; status: string }>;
  droneId: string; fleetLine: string;
}): string {
  const t = o.telemetry;
...
export function parseOperatorEnvelope(text: string): DraftEnvelope {
  const fallbackSay = (text || '').trim();
  const o = extractEnvelopeJson(text);
  if (!o) return { say: fallbackSay, mission: null, fleetMission: null };
  return { say: sayOr(o, fallbackSay), mission: o.mission ?? null, fleetMission: (o as Record<string, unknown>).fleetMission ?? null };
}
 ...
 */
function checkFleetDraft(raw: unknown, drone: DroneService): { plan: FleetMissionPlan | null; errors: string[] } {
  const { plan, errors } = normalizeFleetMissionDraft(raw);
  if (!plan) return { plan: null, errors };
  const all: string[] = [];
  for (const a of plan.assignments) {
    try {
      const st = drone.getState(a.droneId);
      validateMission(a.plan, drone.fence, st.telemetry.home).forEach((e) => all.push(`[${a.droneId}] ${e}`));
    } catch (err) {
      all.push((err as Error).message);
    }
  }
  all.push(...validateFleetSeparation(plan.assignments));
...
function onlineHomes(drone: DroneService): Record<string, GeoPoint> {
  const homes: Record<string, GeoPoint> = {};
  for (const f of drone.listFleet()) if (f.online && f.telemetry) homes[f.droneId] = f.telemetry.home;
  return homes;
}
 ...
 * share one gate.
 */
function checkShowDraft(raw: unknown, drone: DroneService): { timeline: ShowTimeline | null; errors: string[] } {
  const homes = onlineHomes(drone);
  const { timeline, errors } = normalizeShowDraft(raw, homes);
  if (!timeline) return { timeline: null, errors };
  const all = validateShowTimeline(timeline, drone.fence, homes);
...
export function createDroneRoutes(ctx: AppContext): Router {
  const router = Router();
  const pool = ctx.pool;
  const store = new ConciergeStore(pool, 'drone', false);
  const drone = new DroneService();
  const showRunner = new FleetShowRunner(drone);
  ...
  const viewer =
    ...
    async (req, res) => {
      ...
      try { await fn(req, res, sub); }
        ...
        if (err instanceof DroneValidationError) { res.status(422).json({ error: 'geofence/mission validation failed', errors: err.errors }); return; }
        if (err instanceof DroneCommandError) { res.status(409).json({ error: err.message }); return; }
        logger.error({ err, path: req.path }, 'drone route error');
        res.status(500).json({ error: err.message || 'internal error' });
      }
    };

  const audit = async (sub: string, command: string, detail: unknown, outcome: 'ok' | 'rejected'): Promise<void> => {
    await pool.query(
      `INSERT INTO drone_command_log (user_sub, command, detail, outcome) VALUES ($1,$2,$3,$4)`,
      [sub, command, JSON.stringify(detail ?? null), outcome],
    ).catch((err) => logger.error({ err, command }, 'drone audit insert failed'));
  };
   ...
   */
  const command = (name: string, run: (req: Request, droneId: string, confirm: boolean) => Promise<unknown>): RequestHandler =>
    viewer(async (req, res, sub) => {
      const droneId = String(req.params.droneId || req.body?.droneId || DEFAULT_DRONE_ID);
      ...
      try {
        ...
        if (err instanceof DroneCommandError || err instanceof DroneValidationError) {
  ...
  const registerCommands = (base: string): void => {
  ...
  router.post('/show/draft', viewer(async (req, res, sub) => {
    let raw: unknown = req.body?.timeline;
    if (!raw && req.body?.preset) {
      if (String(req.body.preset) !== 'ballet') { res.status(422).json({ error: `unknown show preset "${req.body.preset}"` }); return; }
      const homes = onlineHomes(drone);
      const requested: string[] = Array.isArray(req.body?.droneIds) && req.body.droneIds.length
        ? req.body.droneIds.map(String)
        : Object.keys(homes);
      const missing = requested.filter((id) => !homes[id]);
      if (missing.length) { res.status(422).json({ error: 'show needs online drones', errors: missing.map((id) => `drone "${id}" is not online`) }); return; }
      raw = generateBalletShow(requested.map((id) => ({ droneId: id, home: homes[id] })));
    }
    if (!raw) { res.status(400).json({ error: 'body needs a timeline or a preset' }); return; }
    const { timeline, errors } = checkShowDraft(raw, drone);
    if (!timeline) { res.status(422).json({ error: 'invalid show timeline', errors }); return; }
    const r = await pool.query(
      `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'show','draft') RETURNING *`,
      [sub, timeline.name, JSON.stringify(timeline)]);
  ...
  router.post('/show/retask', viewer(async (req, res, sub) => {
    ...
    const confirm = req.body?.confirm === true;
    const homes = onlineHomes(drone);
    const { timeline, errors } = normalizeShowDraft(req.body?.timeline, homes);
    if (!timeline) { res.status(422).json({ error: 'invalid retask timeline', errors }); return; }
    ...
    const hwRoster = st.drones.filter((id) => { try { return drone.getState(id).provider !== 'sim'; } catch { return false; } });
    ...
    const liveById: Record<string, GeoPoint> = {};
    for (const id of st.drones) {
      try { liveById[id] = drone.getState(id).telemetry.position; }
      catch (err) { logger.warn({ err, droneId: id }, 'retask — roster drone has no readable state'); }
    }
    const vErrors = validateRetask(timeline, drone.fence, homes, liveById, st.drones);
    if (vErrors.length) {
    ...
    try {
      ...
      if (err instanceof DroneCommandError || err instanceof DroneValidationError) {
        const reason = err instanceof DroneValidationError ? err.errors.join('; ') : err.message;
    ...
    await audit(sub, 'show-retask', { name: timeline.name, cues: timeline.cues, drones: showParticipants(timeline) }, 'ok');
    res.json({ ok: true, show: showRunner.status() });
  }));
  ...
  router.get('/fleet/patterns', viewer(async (_req, res) => {
    res.json({ patterns: FLEET_PATTERNS, defaults: { sizeM: 160, altBaseM: 30, altStepM: 12, speedMps: 8 } });
  }));
  router.post('/fleet/patterns/:patternId/draft', viewer(async (req, res, sub) => {
    ...
    const { plan: generated, errors: genErrors } = generateFleetPattern({
    ...
    if (!generated) { res.status(422).json({ error: 'pattern generation failed', errors: genErrors }); return; }
    const { plan: fleetPlan, errors } = checkFleetDraft(generated, drone);
    if (!fleetPlan) { res.status(422).json({ error: 'generated pattern failed the fleet gate', errors }); return; }
    const r = await pool.query(
      `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'pattern','draft') RETURNING *`,
      [sub, fleetPlan.name, JSON.stringify(fleetPlan)]);
    res.json({ mission: r.rows[0] });
  }));
  ...
  router.post('/nodes/heartbeat', async (req: Request, res: Response) => {
    if (!hasValidServiceSecret(req)) { res.status(401).json({ error: 'drone-node heartbeat requires the swarm service secret' }); return; }
    try {
      const b = (req.body || {}) as Partial<DroneNodeHeartbeat>;
      ...
      const ack = drone.ingestHeartbeat({
        ...
        telemetry: b.telemetry as DroneTelemetry,
...
type Audit = (sub: string, command: string, detail: unknown, outcome: 'ok' | 'rejected') => Promise<void>;

function registerMissionRoutes(router: Router, viewer: Viewer, pool: Pool, drone: DroneService, audit: Audit, showRunner: FleetShowRunner): void {
  router.get('/missions', viewer(async (_req, res, sub) => {
    const rows = (await pool.query(
      `SELECT mission_id, name, plan, status, source, created_at, updated_at, last_flown_at
  ...
  router.post('/missions', viewer(async (req, res, sub) => {
    ...
    if (isFleetMissionShape(req.body)) {
      const { plan: fleetPlan, errors } = checkFleetDraft(req.body, drone);
      if (!fleetPlan) { res.status(422).json({ error: 'invalid fleet mission', errors }); return; }
      const r = await pool.query(
        `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'manual','ready') RETURNING *`,
        [sub, fleetPlan.name, JSON.stringify(fleetPlan)]);
      res.json({ mission: r.rows[0] });
      return;
    }
    const { plan, errors } = normalizeMissionDraft(req.body);
    if (!plan) { res.status(422).json({ error: 'invalid mission', errors }); return; }
    const fenceErrors = validateMission(plan, drone.fence, drone.getState().telemetry.home);
    if (fenceErrors.length) { res.status(422).json({ error: 'geofence/mission validation failed', errors: fenceErrors }); return; }
    const r = await pool.query(
      `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'manual','ready') RETURNING *`,
      [sub, plan.name, JSON.stringify(plan)]);
    res.json({ mission: r.rows[0] });
  }));
  ...
  router.post('/missions/:missionId/execute', viewer(async (req, res, sub) => {
    const droneId = String(req.body?.droneId || DEFAULT_DRONE_ID);
    const confirm = req.body?.confirm === true;
    const r = await pool.query(`SELECT * FROM drone_missions WHERE mission_id = $1 AND user_sub = $2`, [req.params.missionId, sub]);
    if (!r.rows.length) { res.status(404).json({ error: 'mission not found' }); return; }
    ...
    if (isShowShape(r.rows[0].plan)) {
      const { timeline, errors } = checkShowDraft(r.rows[0].plan, drone);
      if (!timeline) { res.status(422).json({ error: 'stored show failed re-validation', errors }); return; }
      try {
    ...
    if (isFleetMissionShape(r.rows[0].plan)) {
      const { plan: fleetPlan, errors } = normalizeFleetMissionDraft(r.rows[0].plan);
      if (!fleetPlan) { res.status(422).json({ error: 'stored fleet mission is malformed', errors }); return; }
    ...
    const { plan, errors } = normalizeMissionDraft(r.rows[0].plan);
    if (!plan) { res.status(422).json({ error: 'stored mission is malformed', errors }); return; }
    try {
      await drone.startMission(plan as MissionPlan, droneId, confirm);
    } catch (err) {
      if (err instanceof DroneCommandError || err instanceof DroneValidationError) {
        const reason = err instanceof DroneValidationError ? err.errors.join('; ') : err.message;
...
function registerChatRoutes(router: Router, viewer: Viewer, ctx: AppContext, pool: Pool, store: ConciergeStore, drone: DroneService): void {
  router.post('/chat', viewer(async (req, res, sub) => {
    ...
    const missions = (await pool.query(
      `SELECT name, status FROM drone_missions WHERE user_sub = $1 ORDER BY updated_at DESC LIMIT 10`, [sub])).rows;
    ...
    let droneId = String(req.body?.droneId || DEFAULT_DRONE_ID);
    let state: ReturnType<DroneService['getState']>;
    try { state = drone.getState(droneId); }
    catch { droneId = DEFAULT_DRONE_ID; state = drone.getState(); }
    ...
    let raw = '';
    try {
      const result = await (ctx as any).orchestrator.processMessage(`drone-${sub}-${randomUUID()}`, prompt, {
        agenticMode: true, autoApprove: false, source: 'drone', agentId: OPERATOR_AGENT_ID, userSub: sub,
      });
      raw = String(result?.response || '').trim();
    } catch (e) { logger.error({ e }, 'drone operator orchestrate failed'); }
    const env = parseOperatorEnvelope(raw);
    if (!env.say) env.say = "I couldn't reach the flight-planning brain just now — try again in a moment.";

    let draft: { missionId: string; plan: MissionPlan | FleetMissionPlan } | null = null;
    ...
    if (env.fleetMission) {
      ...
      if (fleetPlan) {
        const r = await pool.query(
          `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'concierge','draft') RETURNING mission_id`,
          [sub, fleetPlan.name, JSON.stringify(fleetPlan)]);
        draft = { missionId: r.rows[0].mission_id, plan: fleetPlan };
      }
    } else if (env.mission) {
      const { plan, errors } = normalizeMissionDraft(env.mission);
      draftErrors = errors;
      if (plan) {
        const fenceErrors = validateMission(plan, state.fence, state.telemetry.home);
        if (fenceErrors.length) draftErrors = fenceErrors;
        else {
          const r = await pool.query(
            `INSERT INTO drone_missions (user_sub, name, plan, source, status) VALUES ($1,$2,$3,'concierge','draft') RETURNING mission_id`,
            [sub, plan.name, JSON.stringify(plan)]);
