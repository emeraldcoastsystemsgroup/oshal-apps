...
const definitions: Array<[number,string,string,string]> = [[0,"saved","watchlist-titles","Want to watch"]];
...
function item(row:any):any {const title=clip(row.title,100); if(!title)return null; return {text:title, detail:'Saved to watchlist '+stamp(row.created_at)+(row.year?' · '+clip(row.year,10):''), context:{title:'Plan a movie night',notes:'Selected saved movie: '+title+'. Discuss suitable food or music for this viewing plan.'}};}
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 ...
 router.get('/',async(req,res)=>{
  ...
  const items:any[]=recent.status==='fulfilled'?recent.value.rows.map(item).filter(Boolean).slice(0,3).map(({context,...entry}:any)=>({...entry,tone:'neutral',fix:"movies-concierge",actions:["plan-meal","plan-music"].map(integration=>({integration,context}))})):[];
  const failed=results.filter(r=>r.status==='rejected').length;
  if(failed)items.push({text:'Some saved data cannot be checked.',tone:'warn',fix:"movies-concierge"});
  else if(!items.length)items.push({text:"No saved preparation items to review yet.",tone:'neutral',fix:"movies-concierge"});
