...
/** @description Dollar value for a clue row (0-based). */
function valueForRow(row) { return (row + 1) * BASE; }
...
/** @description The host may only build content in the lobby or when the final is due. */
function canGenerate(state) { return ['lobby', 'final-setup'].indexOf(state.phase) >= 0; }
...
function generatePrompt(state, ctx = {}) {
  ...
  return [
    'MODE: generate. Build a Jeopardy board: exactly ' + CATEGORIES + ' categories, each with exactly ' + ROWS + ' clues, easiest first.',
...
function normalizeCategory(raw) {
  var clues = (Array.isArray(raw && raw.clues) ? raw.clues : [])
    ...
    .slice(0, ROWS)
    .map(function (c, row) {
      return { clue: String(c.clue).slice(0, 300), answer: String(c.answer).slice(0, 120), value: valueForRow(row), used: false, isDaily: false };
    });
...
function ingestGenerated(state, json, now = Date.now()) {
  ...
  var categories = (Array.isArray(json && json.categories) ? json.categories : []).slice(0, CATEGORIES).map(normalizeCategory)
    .filter(function (c) { return c.clues.length === ROWS; });
  if (categories.length < 2) return { ok: false, error: 'BAD_BOARD' };
  var dailyCat = Math.floor(Math.random() * categories.length);
  var dailyRow = Math.floor(Math.random() * ROWS);
  categories[dailyCat] = { ...categories[dailyCat], clues: categories[dailyCat].clues.map(function (c, r) { return r === dailyRow ? { ...c, isDaily: true } : c; }) };
  var next = {
    ...state, round: (Number(state.round) || 0) + 1, phase: 'board',
    board: { ...state.board, categories: categories, pick: null, control: null, missed: [], wager: null, winner: null },
  };
  return {
    ok: true, state: director.cut(buzzer.clear(next), 'board', null, now),
    event: { kind: 'milestone', content: 'Round ' + next.round + ': ' + categories.map(function (c) { return c.title; }).join(' | ') },
    host: { mode: 'banter', payload: { categories: categories.map(function (c) { return c.title; }) } },
  };
}
...
function reduceWager(state, action, actor, now) {
  ...
  var ceiling = Math.max(scoreOf(state, actor.seatId), ROWS * BASE);
...
function applyClueRuling(state, correct, actor, now) {
  ...
  if (correct) {
    ...
    won.phase = boardExhausted(won.board) ? 'final-setup' : 'board';
  ...
  moved.phase = boardExhausted(moved.board) ? 'final-setup' : 'board';
...
function onTimeout(state, timer, now = Date.now(), ctx = {}) {
  ...
  if (timer.kind === 'wager') {
    ...
    var ceiling = Math.max(scoreOf(state, seatId), ROWS * BASE);
