...
import { accessibleConnections, isConnectionExpired, type ConnectionRow } from '@/app/routes/connector-tenancy';
...
export function identitySummary(rows: ConnectionRow[], now = Date.now()) {
  const expired = rows.filter(r => isConnectionExpired(r, now)).length;
  const expiring = rows.filter(r => {
    if (!r.expiry || r.refresh_token) return false;
  ...
  const items = [];
  if (!rows.length) items.push({ text: 'No accounts saved. Open Identity to connect the services you use.', tone: 'neutral', fix: 'identity-home' });
  if (expired) items.push({ metricId: 'reconnect', text: `${expired} authorization(s) have expired and cannot renew automatically.`, tone: 'warn', fix: 'identity-home' });
  if (expiring) items.push({ metricId: 'expires-7d', text: `${expiring} nonrenewable authorization(s) expire within seven days.`, tone: 'warn', fix: 'identity-home' });
...
export function createIdentitySummaryRoutes(ctx: AppContext): Router {
  const router = Router();
  router.get('/summary', async (req, res) => {
    ...
    try { res.json(identitySummary(await accessibleConnections(ctx.pool, String(sub)))); }
    catch { res.status(503).json({ error: 'Connection metadata cannot be checked.' }); }
  });
  return router;
}
