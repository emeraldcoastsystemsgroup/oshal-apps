...
const definitions: Array<[number,string,string,string]> = [[0,"pending","pending-meal-items","Meal items to review"],[2,"day","meal-handoffs-24h","Checkout links / 24h"],[2,"five_days","meal-handoffs-5d","Checkout links / 5 days"]];
...
function item(row:any):any {if(!row.title)return null;const notes=clip('Meal choice to discuss: '+clip(row.title,200)+'; quantity '+row.quantity+(row.store_name?' from '+clip(row.store_name,100):'')+'. This is a cart item, not a placed order.',700);return {text:clip(row.title,120),detail:'In your meal cart · '+stamp(row.created_at),context:{title:'Prepare for my meal choice',notes}};}
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 ...
 router.get('/',async(req,res)=>{
  ...
  const items:any[]=recent.status==='fulfilled'?recent.value.rows.map(item).filter(Boolean).slice(0,3).map(({context,...entry}:any)=>({...entry,tone:'neutral',fix:"eats-concierge",actions:["plan-shopping","plan-movie","plan-music"].map(integration=>({integration,context}))})):[];
  const failed=results.filter(r=>r.status==='rejected').length;
  if(failed)items.push({text:'Some saved data cannot be checked.',tone:'warn',fix:"eats-concierge"});
  else if(!items.length)items.push({text:"No saved preparation items to review yet.",tone:'neutral',fix:"eats-concierge"});
