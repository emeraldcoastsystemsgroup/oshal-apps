...
/** One candlestick, exactly the shape the framework's `getCandles` returns. */
...
/** The framework's `getCandles`, injected so this module stays framework-free. */
export type CandleReader = (
  seriesTicker: string, marketTicker: string, startTs: number, endTs: number, periodInterval: 1 | 60 | 1440,
) => Promise<Candle[]>;

/** An announced hand whose market has not settled yet. */
export interface OpenAlert {
  ticker: string;
  seriesTicker: string | null;
  side: 'yes' | 'no';
  /** The ask on OUR side at announcement — what the alert quoted. */
  announcedPrice: number | null;
  announcedAt: string;
  title: string | null;
}

/** Where the "current" price came from. `mid` is best; `mean` is the weakest. */
export type PopBasis = 'mid' | 'close' | 'mean';

/**
 * A market's newest usable price. This is the half of a pop that belongs to the MARKET rather than
 * to whoever asked, which makes it the half that is safe to share between callers — and so the only
 * half the per-process cache is allowed to hold.
 */
export interface LatestPrice {
  price: number;
  basis: PopBasis;
  /** End of the candle the price came from, epoch seconds. */
  endPeriodTs: number;
}

/** One market's move since it was announced, on the side that was picked. */
export interface Pop {
  ticker: string;
  side: 'yes' | 'no';
  announcedPrice: number;
  currentPrice: number;
  /** Signed move in cents on OUR side. Positive = the market moved toward the pick. */
  moveCents: number;
  /** Signed move as a fraction of the announced price; null when that price was zero. */
  moveFraction: number | null;
  toward: boolean;
  basis: PopBasis;
  /** End of the candle the current price came from, ISO. */
  asOf: string | null;
}

/** What the route serves: the pops it could price, and an honest account of what it did not. */
export interface PopResult {
  generatedAt: string;
  pops: Pop[];
  /** Open alerts found. */
  open: number;
  /** Markets a candle read was attempted or served from cache for. */
  checked: number;
  /** Open alerts left unpriced because the per-read cap was hit. */
  deferred: number;
  /** Markets whose candle read failed or returned nothing usable. */
  unavailable: number;
  cap: number;
}

/** At most this many candle reads per request — the public tier is shared with the scan. */
export const POP_MAX_MARKETS = 12;

/** Minimum gap between candle reads, ms. ~2.9 rps, under the documented ~3 rps public tier. */
export const POP_MIN_INTERVAL_MS = 350;

/** How long a priced market stays good for. */
export const POP_CACHE_TTL_MS = 5 * 60 * 1000;

/** Never ask for more than this much history in one candle window. */
export const POP_MAX_SPAN_DAYS = 30;

/**
 * Per-process memo + rate-limit clock. Held by the route so spacing survives across requests.
 *
 * `entries` memoizes the MARKET (`LatestPrice`), deliberately not a `Pop`: one cache serves every
 * caller and is keyed by ticker, and a Pop embeds the announced price and side of the caller it was
 * built for. Caching the market keeps the thing that made the cache worth having — one vendor fetch
 * per ticker no matter how many surfaces are open — while the caller-specific derivation happens per
 * request. A null entry is a market that could not be priced, memoized so a dead series is not
 * re-asked every refresh.
 */
export interface PopCache {
  entries: Map<string, { atMs: number; latest: LatestPrice | null }>;
  lastCallMs: number;
}
 ...
 */
export function createPopCache(): PopCache {
 ...
 */
export function sidePrice(yesPrice: number, side: 'yes' | 'no'): number {
 ...
 */
export function candleYesPrice(candle: Candle): { price: number; basis: PopBasis } | null {
  const bid = candle.yesBidClose;
  const ask = candle.yesAskClose;
  if (usable(bid) && usable(ask)) return { price: ((bid as number) + (ask as number)) / 2, basis: 'mid' };
  if (usable(candle.close)) return { price: candle.close as number, basis: 'close' };
  if (usable(candle.mean)) return { price: candle.mean as number, basis: 'mean' };
  return null;
}
 ...
 */
export function latestYesPrice(candles: Candle[]): LatestPrice | null {
  for (let i = candles.length - 1; i >= 0; i -= 1) {
    const c = candles[i];
    const p = candleYesPrice(c);
    if (p) return { price: p.price, basis: p.basis, endPeriodTs: c.endPeriodTs };
  }
  return null;
}
 ...
 */
export function popFrom(alert: OpenAlert, latest: LatestPrice): Pop | null {
  const announced = alert.announcedPrice;
  if (!usable(announced)) return null;
  const from = announced as number;
 ...
 */
export function candleWindow(announcedAt: string, nowMs: number): { startTs: number; endTs: number; periodInterval: 60 } {
  const endTs = Math.floor(nowMs / 1000);
  const announcedMs = Date.parse(announcedAt);
  const floorMs = nowMs - POP_MAX_SPAN_DAYS * 86_400_000;
...
export async function readOpenAlerts(
  pool: QueryablePool, userSub: string, strategy: string, limit = 50,
): Promise<OpenAlert[]> {
  const { rows } = await pool.query(
    `SELECT a.ticker, a.detail, a.created_at,
    ...
    [userSub, strategy, Math.min(200, Math.max(1, limit))]);
  return rows.map((r) => {
    ...
    return {
      ...
      announcedAt: new Date(r.created_at as string).toISOString(),
 ...
 */
export function seriesFromTicker(ticker: string): string | null {
...
export async function fetchPops(
  ...
  const cap = Number.isFinite(opts.cap) && (opts.cap as number) > 0 ? Math.floor(opts.cap as number) : POP_MAX_MARKETS;
  const minIntervalMs = Number.isFinite(opts.minIntervalMs) ? Math.max(0, opts.minIntervalMs as number) : POP_MIN_INTERVAL_MS;
  const ttl = Number.isFinite(opts.cacheTtlMs) ? Math.max(0, opts.cacheTtlMs as number) : POP_CACHE_TTL_MS;
  ...
  for (const alert of alerts) {
    ...
    if (hit && now() - hit.atMs < ttl) {
      ...
      if (cached) pops.push(cached); else unavailable += 1;
      continue;
    }
    if (reads >= cap) { deferred += 1; continue; }
    if (!alert.seriesTicker) { unavailable += 1; continue; }
    ...
    if (pop) pops.push(pop); else unavailable += 1;
  }
