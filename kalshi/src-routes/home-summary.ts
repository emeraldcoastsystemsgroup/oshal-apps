...
export function createHomeSummaryRoutes(ctx: AppContext): Router {
  ...
  router.get('/', async (req, res) => {
    ...
    const actions = (title: string, notes: string) => ['review-finance','prepare-document','review-research'].map(integration => ({ integration, context: { title, notes } }));
    const items: any[] = alerts.status === 'fulfilled' ? alerts.value.rows.map((a: any) => {
    ...
    if (orders.status === 'fulfilled') for (const o of orders.value.rows) {
    ...
    if (failed) items.push({ text: 'Some saved market records cannot be checked.', tone: 'warn', fix: 'kalshi-home' });
    else if (!items.length) items.push({ text: 'No personal alerts or placement records in the last five days.', tone: 'neutral', fix: 'kalshi-home' });
