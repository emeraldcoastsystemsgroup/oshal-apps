...
exports.DEFAULT_PAPER_BANKROLL = exports.MIN_GRADED_FOR_VERDICT = exports.ROLLING_WINDOW = void 0;
exports.quadraticFee = quadraticFee;
exports.paperBankrollStart = paperBankrollStart;
exports.stakeReturn = stakeReturn;
exports.nextBankroll = nextBankroll;
exports.bankrollWalk = bankrollWalk;
exports.bankrollReturn = bankrollReturn;
exports.dailySeries = dailySeries;
exports.daysToVerdict = daysToVerdict;
exports.gradedPerDay = gradedPerDay;
exports.groupByStrategy = groupByStrategy;
exports.readGradedRows = readGradedRows;
exports.readStrategyMeta = readStrategyMeta;
exports.readBookBankroll = readBookBankroll;
exports.readBooks = readBooks;
exports.trendSeries = trendSeries;
exports.ROLLING_WINDOW = 50;
exports.MIN_GRADED_FOR_VERDICT = 30;
/** Kalshi's quadratic taker fee per contract, dollars. */
function quadraticFee(price) {
 ...
 */
exports.DEFAULT_PAPER_BANKROLL = 1000;
/**
 ...
 */
function paperBankrollStart(env = process.env) {
    const raw = Number(env.KALSHI_PAPER_BANKROLL_START);
    return Number.isFinite(raw) && raw > 0 ? raw : exports.DEFAULT_PAPER_BANKROLL;
}
/**
 ...
 */
function stakeReturn(price, pnl) {
 ...
 */
function nextBankroll(bankroll, stakeFraction, price, pnl) {
 ...
 */
function bankrollWalk(rows, start) {
    const s = Number.isFinite(start) && start > 0 ? start : exports.DEFAULT_PAPER_BANKROLL;
    let bankroll = s;
    let sized = 0;
    for (const r of rows) {
 ...
 */
function bankrollReturn(start, end) {
 ...
 */
function dailySeries(rows, bankrollStart = exports.DEFAULT_PAPER_BANKROLL) {
    const out = [];
    const window = [];
    let cum = 0;
    let bank = Number.isFinite(bankrollStart) && bankrollStart > 0 ? bankrollStart : exports.DEFAULT_PAPER_BANKROLL;
    for (const r of rows) {
        ...
        if (window.length > exports.ROLLING_WINDOW)
            window.shift();
        ...
        if (last && last.day === day) {
            ...
            last.pnl += r.pnl;
            last.cumPnl = cum;
            last.brier = meanInto(last.brier, last.n - 1, r.brier);
            last.marketBrier = meanInto(last.marketBrier, last.n - 1, r.marketBrier);
            last.rollingHit = hit;
            last.rollingBreakeven = breakeven;
            last.bankroll = bank;
        }
        else {
...
function meanInto(current, count, sample) {
 ...
 */
function daysToVerdict(graded, gradedPerDay) {
    if (graded >= exports.MIN_GRADED_FOR_VERDICT)
        return 0;
    if (!gradedPerDay || gradedPerDay <= 0)
        return null;
    return Math.ceil((exports.MIN_GRADED_FOR_VERDICT - graded) / gradedPerDay);
}
/** Observed grading pace: settled rows per day between the first prediction and now (≥ 1 day). */
function gradedPerDay(graded, firstAtIso, nowMs) {
    if (!firstAtIso || graded <= 0)
        return null;
...
async function readGradedRows(pool, sinceDays) {
    const { rows } = await pool.query(`SELECT strategy, graded_at, ((side = 'yes') = settled_yes) AS won, market_prob::float AS price,
      ...
      ORDER BY strategy, graded_at, id`, sinceDays ? [String(sinceDays)] : []);
    return rows.map((r) => ({
        strategy: String(r.strategy), gradedAt: new Date(r.graded_at).toISOString(), won: r.won === true,
...
async function readStrategyMeta(pool) {
    const { rows } = await pool.query(`SELECT strategy, count(*) FILTER (WHERE NOT settled)::int AS pending, count(*) FILTER (WHERE settled)::int AS graded,
 ...
 */
async function readBookBankroll(pool, start) {
    const { rows } = await pool.query(`SELECT market_prob::float AS price, pnl_per_contract::float AS pnl, stake_fraction::float AS stake_fraction
       FROM kalshi_predictions
...
async function readBooks(pool, bankrollStart = exports.DEFAULT_PAPER_BANKROLL) {
    const { rows: led } = await pool.query(`SELECT count(DISTINCT strategy)::int AS strategies, count(*)::int AS predictions,
    ...
    const { rows: ord } = await pool.query(`SELECT env, count(*)::int AS orders, count(*) FILTER (WHERE kalshi_status = 'blocked')::int AS blocked,
    ...
    const by = (env) => ord.find((r) => r.env === env) ?? { orders: 0, blocked: 0, contracts: 0, notional: 0 };
    const demo = by('demo');
    const live = by('live');
    const l = led[0] ?? { strategies: 0, predictions: 0, graded: 0, pnl: 0 };
    return {
        paperAuto: {
 ...
 */
async function trendSeries(pool, opts) {
    const nowMs = opts.nowMs ?? Date.now();
    const start = Number.isFinite(opts.bankrollStart) && opts.bankrollStart > 0
        ? opts.bankrollStart : exports.DEFAULT_PAPER_BANKROLL;
    ...
    const names = new Set([...grouped.keys(), ...meta.keys()]);
    const strategies = [...names].sort().map((strategy) => {
    ...
    return {
        generatedAt: new Date(nowMs).toISOString(), windowDays: opts.sinceDays, minGraded: exports.MIN_GRADED_FOR_VERDICT,
        bankrollStart: start, strategies, books,
    };
}
//# sourceMappingURL=kalshi-trends.js.map