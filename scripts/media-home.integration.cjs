 ...
 * daily-trade-recap: the host nightly writes no ticket and no workflow run, so this fixture keeps
 * completed workflow_runs of that ticket type in place as the defect's own witness - the tiles must
 * count the recorded trading session, the published report journal row and the parked approval
 * tickets instead, and must not reflect those runs. */
...
(async()=>{
 ...
 try{
  ...
  // The ACTUAL schemas, read from the code that owns them: the equity store's own bootstrap
  // statement and the journal CLI's own DDL constant. Neither table is RLS-walled (core migration
  // 112 excludes both: host report CLIs read them over the app DSN with no GUC stamp), so the
  // route's explicit user_sub filter is the whole isolation boundary and is proven below.
  const equitySrc=fs.readFileSync(path.join(core,'src/app/trading-daily-equity-store.ts'),'utf8');
  const equityDDL=equitySrc.match(/`(CREATE TABLE IF NOT EXISTS oshal_trading_daily_equity [\s\S]*?)`/);assert.ok(equityDDL,'Use the actual equity schema');await admin.query(equityDDL[1]);
  const journalSrc=fs.readFileSync(path.join(core,'scripts/oshal-strategy-journal.js'),'utf8');
  const journalDDL=journalSrc.match(/`(CREATE TABLE IF NOT EXISTS oshal_trading_strategy_journal [\s\S]*?)`/);assert.ok(journalDDL,'Use the actual journal schema');await admin.query(journalDDL[1]);
  const ticketSrc=fs.readFileSync(path.join(core,'scripts/migrations/100-ticket-family-base-schema.sql'),'utf8');
  await admin.query(ticketSrc.slice(ticketSrc.indexOf('CREATE TABLE IF NOT EXISTS tickets'),ticketSrc.indexOf('CREATE TABLE IF NOT EXISTS ticket_task_links')));
  for(const sql of buildOwnerRlsPolicyStatements('tickets','owner_sub'))await admin.query(sql);
  ...
  // Eastern calendar days, the unit both recap records are keyed on. Day 0 is TODAY: the after-close
  // recap has not run yet, so a session recorded today must never be reported as a miss. Day -8 is
  // outside the seven-day window. Only day -1 published a report, so day -2 is the missed session.
  const etDay=n=>new Date(Date.now()+n*86400000).toLocaleDateString('en-CA',{timeZone:'America/New_York'});
  for(const user of ['alice','bob']){
   ...
   for(const day of [0,-1,-2,-8])await admin.query("INSERT INTO oshal_trading_daily_equity(user_sub,mode,et_day,equity) VALUES($1,'paper',$2::date,1000)",[user,etDay(day)]);
   await admin.query("INSERT INTO oshal_trading_strategy_journal(user_sub,et_day,kind,summary,source) VALUES($1,$2::date,'report',$3,'daily-report')",[user,etDay(-1),'Daily report published for '+user]);
   await admin.query("INSERT INTO oshal_trading_strategy_journal(user_sub,et_day,kind,summary,source) VALUES($1,$2::date,'note',$3,'operator-cli')",[user,etDay(-2),'A hand-written note is not a published report for '+user]);
   for(const [status,group] of [['approval_required','approval_required'],['complete','complete']])
    await admin.query("INSERT INTO tickets(ticket_id,title,status,state_group,ticket_type,owner_sub,created_at,updated_at) VALUES($1,$2,$3,$4,'daily-trade-recap',$5,$6,$6)",[crypto.randomUUID(),'Daily trade recap '+status+' '+user,status,group,user,at(-24)]);
  ...
  const get=async(app,sub='alice')=>{let handler;const module={exports:{}};new Function('require','module','exports',fs.readFileSync(path.join(root,app,'routes/home-summary.js'),'utf8'))(()=>({Router:()=>({get:(_,fn)=>handler=fn})}),module,module.exports);module.exports.createHomeSummaryRoutes({pool,authorization:{currentActor:()=>({sub,isActive:true}),authorize:async()=>({allowed:true})}});const res={statusCode:200,setHeader(){},status(n){this.statusCode=n;return this;},json(body){this.body=body;}};await as(sub,()=>handler({oidc:{user:{sub},isAuthenticated:()=>true}},res));return res;};
  const expected={'brand-graphics':['0','0','3'],'creative-studio':['0','0','3'],'daily-trade-recap':['1','1','2','1'],lora:['1','1','0'],'portrait-studio':['0','0','3'],'print-ingest':['4','0','3'],storage:['github','oshal-local']};
  ...
  // The defect itself: three completed workflow_runs of this exact ticket type and owner are in the
  // fixture, and nothing on Home may reflect them - production never writes one.
  const recap=(await get('daily-trade-recap')).body;
  assert.ok(recap.items.some(i=>i.tone==='warn'&&new RegExp('No recap recorded for '+etDay(-2)).test(i.text)),'the session that published no report must be named');
  assert.ok(recap.items.some(i=>i.tone==='neutral'&&new RegExp('Recap recorded for '+etDay(-1)).test(i.text)),'the session that did publish must be recorded');
  assert.equal(recap.items.some(i=>new RegExp('No recap recorded for '+etDay(0)).test(i.text)),false,'today has not had its close yet');
  assert.ok(recap.items.some(i=>i.tone==='warn'&&/approval_required/.test(i.detail)),'the parked ticket the workflow-run states could never see');
  // Only the journal is withdrawn: the counts that need it go Unavailable while the ticket evidence
  // that does not need it is retained.
  await admin.query(`REVOKE SELECT ON oshal_trading_strategy_journal FROM ${schema}`);const partial=await get('daily-trade-recap');assert.equal(partial.statusCode,200);assert.equal(partial.body.partial,true);assert.equal(partial.body.metrics[0].value,'Unavailable');assert.equal(partial.body.metrics[3].value,'1');
