...
const definitions: Array<[number,string,string,string]> = [[0,"day","ride-handoffs-24h","Ride links / 24h"],[0,"five_days","ride-handoffs-5d","Ride links / 5 days"]];
...
function item(row:any):any {if(!row.dropoff)return null;const notes=clip('Destination: '+clip(row.dropoff,350)+'; Pickup: '+clip(row.pickup,350)+'; Saved handoff '+stamp(row.created_at)+'; booking not confirmed.',1200);return {text:clip(row.dropoff,120),detail:'Ride preparation · '+stamp(row.created_at),context:{title:'Plan around my saved ride request',notes}};}
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 ...
 router.get('/',async(req,res)=>{
  ...
  const items:any[]=recent.status==='fulfilled'?recent.value.rows.map(item).filter(Boolean).slice(0,3).map(({context,...entry}:any)=>({...entry,tone:'neutral',fix:"rides-concierge",actions:["plan-meal","plan-trip"].map(integration=>({integration,context}))})):[];
  const failed=results.filter(r=>r.status==='rejected').length;
  if(failed)items.push({text:'Some saved data cannot be checked.',tone:'warn',fix:"rides-concierge"});
  else if(!items.length)items.push({text:"No saved preparation items to review yet.",tone:'neutral',fix:"rides-concierge"});
