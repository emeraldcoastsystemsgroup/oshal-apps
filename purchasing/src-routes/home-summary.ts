...
const definitions: Array<[number,string,string,string]> = [[0,"lists","active-shopping-lists","Active lists"],[2,"pending","pending-shopping-items","Items to review"],[3,"day","shopping-handoffs-24h","Checkout links / 24h"],[3,"five_days","shopping-handoffs-5d","Checkout links / 5 days"]];
...
function item(row:any):any {if(!row.title)return null;return {text:clip(row.title,120),detail:clip(row.name,100)+' · Saved '+stamp(row.created_at),context:{title:'Discuss a meal from my shopping list',notes:clip('Selected shopping item: '+clip(row.title,300)+'; quantity '+row.quantity+'. Decide whether this item is relevant to a meal; it has not been purchased.',700)}};}
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 ...
 router.get('/',async(req,res)=>{
  ...
  const items:any[]=recent.status==='fulfilled'?recent.value.rows.map(item).filter(Boolean).slice(0,3).map(({context,...entry}:any)=>({...entry,tone:'neutral',fix:"shop-concierge",actions:["plan-meal"].map(integration=>({integration,context}))})):[];
  const failed=results.filter(r=>r.status==='rejected').length;
  if(failed)items.push({text:'Some saved data cannot be checked.',tone:'warn',fix:"shop-concierge"});
  else if(!items.length)items.push({text:"No saved preparation items to review yet.",tone:'neutral',fix:"shop-concierge"});
