...
/** One stored brief as the surface reads it. */
export interface StoredMeetingBrief {
  eventId: string;
  title: string;
  startsAt: string;
  builtAt: string;
 ...
 */
export function storedBrief(row: Record<string, unknown>): StoredMeetingBrief | null {
  const document = row.brief && typeof row.brief === 'object' ? row.brief as Record<string, unknown> : null;
  ...
  if (!deliveryText.trim() || !eventId) return null;
 ...
 */
export function createMeetingBriefViewRoutes(ctx: AppContext): Router {
  const router = Router();
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      const briefs = result.rows.map(storedBrief).filter((brief): brief is StoredMeetingBrief => brief !== null);
