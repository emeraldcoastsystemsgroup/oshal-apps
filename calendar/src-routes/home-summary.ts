...
export function calendarEvidence(snapshot:any, now=new Date()):any {
 const saved=snapshot && Number.isFinite(new Date(snapshot.synced_at).getTime()) && new Date(snapshot.synced_at)<=now;
 const events:any[]=saved&&Array.isArray(snapshot.events)?snapshot.events:[];
 ...
 const items:any[]=upcoming.slice(0,3).map(e=>{const title=e.title.slice(0,120),detail=e.start+(e.allDay?' / all day':'')+' / synced '+new Date(snapshot.synced_at).toISOString();return {text:title,detail,fix:'calendar-review',actions:['plan-gift','plan-meal','plan-trip','prepare-meeting'].map(integration=>({integration,context:{title,notes:(detail+'. Prepare for this event. Confirm the occasion, preferences, budget, timing and participants with me. Do not assume a gift is appropriate.').slice(0,2000),...(typeof e.url==='string'&&/^https:\/\/calendar\.google\.com\//.test(e.url)?{sourceUrl:e.url}:{})}}))};});
...
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 const r=Router();r.get('/',async(req,res)=>{res.setHeader('Cache-Control','no-store');const o=(req as any).oidc,sub=o?.user?.sub;if(!sub||o?.isAuthenticated?.()!==true){res.status(401).json({error:'not_authenticated'});return;}
 try{const q=await ctx.pool.query({text:'SELECT events,synced_at FROM calendar_preparation_snapshots WHERE user_sub = $1',values:[sub],query_timeout:1800} as any);res.json(calendarEvidence(q.rows[0]));}catch{res.status(503).json({error:'Calendar snapshot unavailable'});}});return r;
}
