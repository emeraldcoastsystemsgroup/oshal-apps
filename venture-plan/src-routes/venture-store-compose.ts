...
import type {
  Assumption as EngineAssumption, AssumptionLedger, AssumptionUnit, Confidence as EngineConfidence,
} from './venture-assumptions';
import { SOURCE_CONFIDENCE_CEILING, buildLedger, isWeakerConfidence } from './venture-assumptions';
import type { BomComponent, PriceBreak, SupplierTerms } from './venture-bom';
import type { Channel } from './venture-channels';
import type { VentureModelInput } from './venture-model';
import { withAssumption } from './venture-model';
import type { Assumption, BomLine, HeadcountRow, Scenario, Vendor, Venture } from './venture-types';

/** Engine assumption units. Anything else is stored as an unweighted ratio. */
const ENGINE_UNITS: readonly AssumptionUnit[] = Object.freeze([
...
export const ASSEMBLY_COST_KEY = 'bom.final-assembly.cost';

/** Whole dollars to micro-dollars, for the structural defaults below. */
const D = (dollars: number): number => Math.round(dollars * 1_000_000);
 ...
 */
export function engineConfidence(a: Assumption): EngineConfidence {
  if (a.sourceKind === 'model-estimate') return a.confidence === 'low' ? 'guessed' : 'estimated';
  if (a.sourceKind === 'vendor-quote') return 'quoted';
  const claimed: EngineConfidence = a.confidence === 'high' ? 'observed' : a.confidence === 'medium' ? 'benchmarked' : 'estimated';
  ...
  const ceiling = SOURCE_CONFIDENCE_CEILING[engineSource(a).kind] ?? 'estimated';
  return isWeakerConfidence(ceiling, claimed) ? ceiling : claimed;
}

/** Translate a store source kind into the engine's tagged source union. */
function engineSource(a: Assumption): EngineAssumption['source'] {
 ...
 */
export function toEngineLedger(rows: readonly Assumption[]): ReturnType<typeof buildLedger> {
  const engine: EngineAssumption[] = [];
  for (const a of rows) {
    if (a.valueNum === null) continue;
    const unit = (ENGINE_UNITS as readonly string[]).includes(a.unit) ? (a.unit as AssumptionUnit) : 'ratio';
  ...
  return buildLedger(engine);
}

/** Build the supplier terms for one BOM line, or an explicit "unassigned" stub. */
function supplierFor(line: BomLine, vendors: ReadonlyMap<string, Vendor>): SupplierTerms {
...
function priceBreaksFor(line: BomLine): PriceBreak[] {
...
function componentFor(
  line: BomLine, byParent: ReadonlyMap<string, BomLine[]>, vendors: ReadonlyMap<string, Vendor>,
  path: string, bindings: Array<{ assumptionId: string; path: string }>,
): BomComponent {
...
export function buildBomTree(
  productName: string, lines: readonly BomLine[], vendors: ReadonlyMap<string, Vendor>,
): { root: BomComponent; bindings: Array<{ assumptionId: string; path: string }> } {
  const byParent = new Map<string, BomLine[]>();
  const roots: BomLine[] = [];
  for (const l of lines) {
    if (!l.parentLineId) { roots.push(l); continue; }
    const bucket = byParent.get(l.parentLineId);
    if (bucket) bucket.push(l);
    else byParent.set(l.parentLineId, [l]);
  }
  const bindings: Array<{ assumptionId: string; path: string }> = [
    { assumptionId: ASSEMBLY_COST_KEY, path: 'product.bom.priceBreaks.0.unitCostMicros' },
  ];
  const root: BomComponent = {
    id: 'final-assembly', name: `${productName} (final assembly)`,
    qtyPerParent: 1, discrete: true, scrapRateRatio: 0,
    priceBreaks: [{ minQty: 0, unitCostMicros: 0, assumptionRef: ASSEMBLY_COST_KEY }],
    supplier: {
 ...
 */
const STRUCTURAL_BINDINGS: ReadonlyArray<{ assumptionId: string; path: string }> = Object.freeze([
...
function num(structure: Record<string, unknown>, key: string, fallback: number, min: number, max: number): number {
...
function defaultChannels(structure: Record<string, unknown>): Channel[] {
  const supplied = structure.channels;
  if (Array.isArray(supplied) && supplied.length) return supplied as Channel[];
  return [{
...
function seasonFor(structure: Record<string, unknown>, sellWindowStart: string) {
  ...
  return {
    ...
    postSeasonPolicy: structure.postSeasonPolicy === 'carry' ? ('carry' as const) : ('liquidate' as const),
...
function rolesFor(rows: readonly HeadcountRow[], horizonStart: string, horizonMonths: number) {
...
/** Everything the composer needs, already read from the store. */
export interface ComposeSources {
  ...
  /** Modelling date. A PARAMETER — nothing downstream reads a clock. */
  onDate: string;
}
 ...
 */
export function composeModelInput(s: ComposeSources): {
  input: VentureModelInput;
  missingAssumptionKeys: string[];
} {
  const structure = (s.venture.spec?.structure ?? {}) as Record<string, unknown>;
  ...
  const runQty = Math.round(
    s.scenario?.volumeUnits ?? num(structure, 'runQtyUnits', DEFAULTS.runQtyUnits, 1, 10_000_000),
  );
  const shelfMicros = s.scenario?.retailPriceMicros
    ?? Math.round(num(structure, 'shelfPriceMicros', DEFAULTS.shelfPriceMicros, 1, 1e12));

  const input: VentureModelInput = {
    ledger: built.ledger,
...
function landedShape(structure: Record<string, unknown>): VentureModelInput['landed'] {
  return {
    incoterm: (['EXW', 'FCA', 'FOB', 'CIF', 'DDP'] as const).includes(structure.incoterm as never)
      ? (structure.incoterm as 'FOB') : 'FOB',
...
function demandShape(structure: Record<string, unknown>, runQty: number): VentureModelInput['demand'] {
...
function opexByMonth(structure: Record<string, unknown>, start: string, months: number): Record<string, number> {
...
function applyOverrides(input: VentureModelInput, scenario: Scenario | null): VentureModelInput {
  if (!scenario) return input;
  let next = input;
  for (const [key, value] of Object.entries(scenario.overrides ?? {})) {
    ...
    if (!next.ledger.byId[key]) continue;
    next = withAssumption(next, key, value);
  }
  return next;
}
 ...
 */
export function missingKeys(input: VentureModelInput): string[] {
  const referenced = new Set<string>();
  const walk = (c: BomComponent): void => {
    for (const b of c.priceBreaks) referenced.add(b.assumptionRef);
    for (const r of c.supplier.assumptionRefs) referenced.add(r);
    for (const kid of c.children ?? []) walk(kid);
  };
  walk(input.product.bom);
  for (const r of input.landed.freight.assumptionRefs) referenced.add(r);
  for (const r of input.landed.duty.assumptionRefs) referenced.add(r);
  for (const c of input.channels) for (const r of c.assumptionRefs) referenced.add(r);
  for (const sc of input.demand.scenarios) referenced.add(sc.assumptionRef);
  for (const r of input.season.assumptionRefs) referenced.add(r);
  for (const role of input.roles) for (const r of role.assumptionRefs) referenced.add(r);
 ...
 */
export function hashableInputs(ledger: AssumptionLedger, runQtyUnits: number): Record<string, number> {
  const out: Record<string, number> = { '@runQtyUnits': runQtyUnits };
  for (const id of ledger.order) out[id] = ledger.byId[id].value;
  return out;
}
