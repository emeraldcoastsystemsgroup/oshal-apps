...
import {
  ledgerStats, weakestConfidence, type AssumptionLedger, type AssumptionUnit, type Confidence, type LedgerStats,
} from './venture-assumptions';
import type { BomRollup } from './venture-bom';
import type { ChannelWaterfall } from './venture-channels';
import type { DemandResult } from './venture-demand';
import type { BreakEven, FinancialsResult } from './venture-financials';
import type { HeadcountResult } from './venture-headcount';
import type { LandedCost } from './venture-landed';
import type { ScheduleResult } from './venture-schedule';
import { formatUsd, microsToCents, type Micros } from './venture-primitives';

/** What kind of number a figure is. Documents style these differently. */
export type FigureKind = 'computed' | 'assumption' | 'count';

/** One registered printable number. */
export interface Figure {
  ...
  unit: AssumptionUnit;
  kind: FigureKind;
  /** How it was derived, e.g. `runRecurringMicros / runQtyUnits`. */
  formula?: string;
  assumptionRefs: string[];
  /** Weakest confidence anywhere in the chain — a chain is worth its worst link. */
  confidence: Confidence | null;
}

/** Every figure, keyed by id. */
export type FigureRegistry = Record<string, Figure>;

/** The generated provenance report a document header prints. */
export interface Traceability {
  totalFigures: number;
  computedFigures: number;
  assumptionFigures: number;
  /** Figures whose chain reaches a model-authored or guessed money assumption. */
  softFigureIds: string[];
  /** Figures whose chain reaches an assumption that is not registered at all. */
  unsourcedFigureIds: string[];
  ledger: LedgerStats;
}

/** How much of this model is measured rather than guessed. Derived, never set. */
export type ModelPosture = 'actual' | 'quoted' | 'estimate';

/** Thrown when a document names a figure the engine never computed. */
export class MissingFigureError extends Error {
  /**
   * @description Build the missing-figure error.
   * @param figureId - The unknown figure id the template asked for.
   */
  constructor(public readonly figureId: string) {
    super(`document references figure "${figureId}", which the engine did not compute`);
    this.name = 'MissingFigureError';
  }
}

/** The engine results a registry is built from. */
export interface FigureSources {
  ...
  /** The season profile's own assumption references — sell-through, window, recovery rate. */
  seasonRefs: readonly string[];
}

/** Distinct, sorted union of several reference lists. */
function union(...lists: Array<readonly string[] | undefined>): string[] {
  const set = new Set<string>();
  for (const l of lists) if (l) for (const r of l) if (r) set.add(r);
  return [...set].sort();
}
 ...
 */
export function referenceGroups(s: FigureSources): {
  bom: string[]; landed: string[]; channel: string[]; demand: string[];
  season: string[]; org: string[]; all: string[];
} {
...
function fig(
  ledger: AssumptionLedger, id: string, label: string, value: number | null,
  unit: AssumptionUnit, kind: FigureKind, formula: string, refs: readonly string[],
): Figure | null {
  if (value === null || !Number.isFinite(value)) return null;
  const known = refs.map((r) => ledger.byId[r]?.confidence).filter((c): c is Confidence => Boolean(c));
  return { id, label, value, unit, kind, formula, assumptionRefs: [...refs].sort(), confidence: weakestConfidence(known) };
}

/** BOM and landed-cost figures. */
function costFigures(s: FigureSources): Array<Figure | null> {
  const L = s.ledger;
...
function marketFigures(s: FigureSources): Array<Figure | null> {
  const L = s.ledger;
  const out: Array<Figure | null> = [
  ...
  for (const w of s.waterfalls) {
    ...
    out.push(
      fig(L, `channel.${w.channelId}.shelfPriceMicros`, `${w.channelId}: shelf price`, w.shelfPriceMicros, 'micros', 'computed', 'as priced', refs),
      fig(L, `channel.${w.channelId}.wholesaleMicros`, `${w.channelId}: wholesale price`, w.wholesaleMicros, 'micros', 'computed', 'shelf x (1 - retailer margin) x (1 - distributor margin)', refs),
      fig(L, `channel.${w.channelId}.netWholesaleMicros`, `${w.channelId}: net wholesale after allowances`, w.netWholesaleMicros, 'micros', 'computed', 'wholesale - every allowance', refs),
      fig(L, `channel.${w.channelId}.totalFeeMicros`, `${w.channelId}: channel deductions per unit`, w.totalFeeMicros, 'micros', 'computed', 'sum of the fee stack at this price', refs),
      fig(L, `channel.${w.channelId}.contributionPerUnitMicros`, `${w.channelId}: contribution per unit`, w.contributionPerUnitMicros, 'micros', 'computed', 'gross revenue - fees - landed cost', refs),
      fig(L, `channel.${w.channelId}.contributionBps`, `${w.channelId}: contribution rate`, w.contributionBps, 'bps', 'computed', 'contribution / gross revenue', refs),
    );
  }
  return out;
}
...
function planFigures(s: FigureSources): Array<Figure | null> {
  const L = s.ledger;
  ...
  return [
    ...
    fig(L, 'ledger.softMoneyCount', 'Money assumptions nobody has quoted', ledgerStats(s.ledger).softMoneyIds.length, 'count', 'count', 'model-authored or guessed money assumptions', []),
  ];
}
 ...
 */
export function computeTraceability(figures: FigureRegistry, ledger: AssumptionLedger): Traceability {
  const stats = ledgerStats(ledger);
  const soft = new Set(stats.softMoneyIds);
  const softFigureIds: string[] = [];
  const unsourcedFigureIds: string[] = [];
  let computedFigures = 0;
  let assumptionFigures = 0;
  for (const id of Object.keys(figures).sort()) {
    const f = figures[id];
    if (f.kind === 'computed') computedFigures += 1;
    if (f.kind === 'assumption') assumptionFigures += 1;
    ...
    if (f.kind === 'computed' && f.assumptionRefs.length === 0) unsourcedFigureIds.push(id);
    else if (f.assumptionRefs.some((r) => !ledger.byId[r])) unsourcedFigureIds.push(id);
    else if (f.assumptionRefs.some((r) => soft.has(r)) || f.confidence === 'estimated' || f.confidence === 'guessed') softFigureIds.push(id);
  }
  return {
 ...
 */
export function derivePosture(ledger: AssumptionLedger): ModelPosture {
  const money = ledger.order.map((id) => ledger.byId[id]).filter((a) => a.unit === 'micros');
  if (!money.length) return 'estimate';
  if (money.every((a) => a.confidence === 'observed')) return 'actual';
  if (money.every((a) => a.confidence === 'quoted' || a.confidence === 'observed')) return 'quoted';
  return 'estimate';
}
 ...
 */
export function renderFigureTokens(template: string, figures: FigureRegistry): string {
  return template.replace(TOKEN_RE, (_match, id: string, format?: string) => {
    const f = figures[id];
    if (!f) throw new MissingFigureError(id);
 ...
 */
export function formatFigure(f: Figure, format?: string): string {
  const mode = format ?? (f.unit === 'micros' ? 'usd' : f.unit === 'bps' ? 'pct' : 'raw');
  if (mode === 'usd') return formatUsd(f.value as Micros);
  if (mode === 'cents') return String(microsToCents(f.value as Micros));
  if (mode === 'pct') return `${(f.value / 100).toFixed(2)}%`;
  if (mode === 'bps') return `${f.value} bps`;
 ...
 */
export function referencedFigureIds(template: string): string[] {
  const ids = new Set<string>();
  for (const m of template.matchAll(TOKEN_RE)) ids.add(m[1]);
  return [...ids].sort();
}
