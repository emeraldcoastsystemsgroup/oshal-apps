...
exports.roleMonthlyMicros = roleMonthlyMicros;
exports.roleActiveIn = roleActiveIn;
exports.computeHeadcount = computeHeadcount;
 ...
 */
function roleMonthlyMicros(r) {
    const fte = Math.max(0, Number.isFinite(r.fteRatio) ? r.fteRatio : 0);
    const burden = r.kind === 'employee' ? Math.max(0, r.burdenBps) : 0;
    const loadedAnnual = (0, venture_primitives_1.addMicros)(r.annualBaseMicros, (0, venture_primitives_1.applyBps)(r.annualBaseMicros, burden));
    return (0, venture_primitives_1.scaleMicros)((0, venture_primitives_1.divMicros)(loadedAnnual, 12) ?? 0, fte);
}
/**
 ...
 */
function roleActiveIn(r, month) {
    if ((0, venture_primitives_1.ymCompare)(month, r.startMonth) < 0)
        return false;
    return !r.endMonth || (0, venture_primitives_1.ymCompare)(month, r.endMonth) <= 0;
}
/**
 ...
 */
function computeHeadcount(roles, horizon) {
    const byMonth = {};
    let peakFte = 0;
    let peakFteMonth = null;
    for (const month of horizon) {
        ...
        const recruiting = roles
            ...
            .map((r) => r.oneTimeRecruitMicros);
        const costMicros = (0, venture_primitives_1.addMicros)(...active.map(roleMonthlyMicros), ...recruiting);
    ...
    return {
        byMonth,
        totalMicros: (0, venture_primitives_1.addMicros)(...horizon.map((m) => byMonth[m]?.costMicros ?? 0)),
        peakFte,
        peakFteMonth,
        issues: [],
...
//# sourceMappingURL=venture-headcount.js.map