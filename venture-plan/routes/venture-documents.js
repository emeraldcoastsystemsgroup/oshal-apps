...
exports.MissingFiguresError = void 0;
exports.postureHeader = postureHeader;
exports.renderDocument = renderDocument;
exports.renderRegisterCsv = renderRegisterCsv;
...
class MissingFiguresError extends Error {
    docKey;
    figureIds;
    /**
...
exports.MissingFiguresError = MissingFiguresError;
/** Format one figure for a markdown table cell. */
function cell(f) {
    return (0, venture_figures_1.formatFigure)(f);
}
...
function figuresSection(s, heading, keys, optional, missing, cited) {
    const rows = ['| Figure | Value | How it was derived | Rests on |', '|---|---|---|---|'];
    for (const key of keys) {
        ...
        for (const r of f.assumptionRefs)
            cited.add(r);
...
function tableSection(s, heading, table) {
    const rows = Array.isArray(s.tables[table]) ? s.tables[table] : [];
    ...
    const body = rows.slice(0, 400).map((r) => `| ${cols.map((c) => (MONEY_COLUMNS.has(c) && typeof r[c] === 'number' ? (0, venture_primitives_1.formatUsd)(r[c]) : md(r[c]))).join(' | ')} |`);
    return `## ${heading}\n\n${[head, rule, ...body].join('\n')}\n`;
}
/** Render the assumption register, optionally narrowed to some domains. */
function assumptionSection(s, heading, domains, cited) {
...
function proseSection(s, heading, proseKey) {
 ...
 */
function renderDocument(spec, s) {
    const missing = [];
    const cited = new Set();
    const parts = [postureHeader(s, spec)];
    const sections = [];
    ...
    if (missing.length)
        throw new MissingFiguresError(spec.key, missing);
    ...
    return {
        ...
        bodyMd,
        sections,
        unverifiedNumbers: (0, venture_bot_contracts_1.verifyProseNumbers)(spec.sections.filter((x) => x.kind === 'prose')
            .map((x) => s.prose[x.proseKey] ?? '').join('\n'), knownValues(s)),
 ...
 */
function renderRegisterCsv(assumptions) {
 ...
 */
function buildTables(model, stored, tornado = []) {
    return {
...
//# sourceMappingURL=venture-documents.js.map