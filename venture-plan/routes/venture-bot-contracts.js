...
exports.resolveSourceKind = resolveSourceKind;
exports.parseAssumptionOutput = parseAssumptionOutput;
exports.parseBomOutput = parseBomOutput;
exports.parseOpsOutput = parseOpsOutput;
exports.parseScopeOutput = parseScopeOutput;
exports.parseProseOutput = parseProseOutput;
exports.verifyProseNumbers = verifyProseNumbers;
const logger_1 = require("@/shared/logger");
const venture_types_1 = require("./venture-types");
const log = (0, logger_1.createChildLogger)({ module: 'venture-bot-contracts' });
...
function unknownKey(row, allowed) {
 ...
 */
function resolveSourceKind(claimed, sourceUrl, sourceDetail) {
    if (!(0, venture_types_1.isSourceKind)(claimed))
        return 'model-estimate';
    if (claimed === 'model-estimate')
        return 'model-estimate';
...
function parseAssumptionRow(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return { reject: 'not an object' };
    const r = raw;
    const bad = unknownKey(r, ASSUMPTION_KEYS);
    if (bad)
        return { reject: `unknown key "${bad}" — a bot may not return a computed result` };
    const key = typeof r.key === 'string' ? r.key.trim() : '';
    if (!/^[a-z0-9][a-z0-9.\-]{2,158}$/i.test(key))
        return { reject: `bad key "${key}"` };
    if (!(0, venture_types_1.isDomain)(r.domain))
        return { reject: `key ${key}: unknown domain` };
    if (!(0, venture_types_1.isConfidence)(r.confidence))
        return { reject: `key ${key}: unknown confidence` };
    const valueNum = fin(r.valueNum);
    const valueText = typeof r.valueText === 'string' ? r.valueText : null;
    if (valueNum === null && !valueText)
        return { reject: `key ${key}: no value` };
...
function collect(payload, field, parse) {
    ...
    for (const entry of raw.slice(0, 400)) {
        ...
        if (out.row)
            ...
            rejected.push(out.reject ?? 'rejected');
    }
    return { rows, rejected };
}
/**
 ...
 */
function parseAssumptionOutput(text) {
    ...
    if (rejected.length)
        log.warn({ rejected: rejected.slice(0, 20), count: rejected.length }, 'assumption rows rejected');
    return { ok: true, rows, rejected };
}
/** Parse one BOM line draft, or explain why it was rejected. */
function parseBomRow(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return { reject: 'not an object' };
    const r = raw;
    const bad = unknownKey(r, BOM_KEYS);
    if (bad)
        return { reject: `unknown key "${bad}"` };
    const ref = typeof r.ref === 'string' ? r.ref.trim().slice(0, 32) : '';
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,31}$/.test(ref))
        return { reject: `bad ref "${ref}"` };
    if (typeof r.partName !== 'string' || !r.partName.trim())
        return { reject: `${ref}: no part name` };
    if (!(0, venture_types_1.isConfidence)(r.confidence))
        return { reject: `${ref}: unknown confidence` };
    ...
    if (low === null || high === null)
        return { reject: `${ref}: a cost line must carry a low/high band` };
    if (low < 0 || high < low)
        return { reject: `${ref}: band is negative or inverted` };
    return {
        row: {
            ...
            moq: fin(r.moq) === null ? null : Math.round(fin(r.moq)),
            leadTimeDays: fin(r.leadTimeDays) === null ? null : Math.round(fin(r.leadTimeDays)),
            toolingCostMicros: Math.max(0, Math.round(fin(r.toolingCostMicros) ?? 0)),
            toolingLifeUnits: fin(r.toolingLifeUnits) === null ? null : Math.round(fin(r.toolingLifeUnits)),
...
function parseVendorRow(raw) {
    ...
    if (typeof r.name !== 'string' || !r.name.trim())
        return { reject: 'no vendor name' };
    if (!(0, venture_types_1.isConfidence)(r.confidence))
        return { reject: `${r.name}: unknown confidence` };
    return {
        row: {
            ...
            moq: fin(r.moq) === null ? null : Math.round(fin(r.moq)),
            leadTimeDays: fin(r.leadTimeDays) === null ? null : Math.round(fin(r.leadTimeDays)),
            qualificationDays: fin(r.qualificationDays) === null ? null : Math.round(fin(r.qualificationDays)),
...
function parseBomOutput(text) {
    ...
    if (rejected.length)
        log.warn({ rejected: rejected.slice(0, 20), count: rejected.length }, 'BOM rows rejected');
    return { ok: true, rows: lines.rows, rejected, vendors: vendors.rows };
}
/** Parse one schedule task. */
function parseTaskRow(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return { reject: 'not an object' };
    const r = raw;
    ...
    if (bad)
        return { reject: `unknown key "${bad}"` };
    if (typeof r.name !== 'string' || !r.name.trim())
        return { reject: 'no task name' };
    if (!(0, venture_types_1.isConfidence)(r.confidence))
        return { reject: `${r.name}: unknown confidence` };
    const days = fin(r.durationDays);
    if (days === null || days < 0)
        return { reject: `${r.name}: no duration` };
    return {
        row: {
            ...
            dependsOn: Array.isArray(r.dependsOn) ? r.dependsOn.filter((d) => typeof d === 'string').slice(0, 20) : [],
            confidence: r.confidence,
        },
    };
}
/** Parse one planned role. */
function parseRoleRow(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return { reject: 'not an object' };
    const r = raw;
    ...
    if (typeof r.role !== 'string' || !r.role.trim())
        return { reject: 'no role name' };
    if (!(0, venture_types_1.isConfidence)(r.confidence))
        return { reject: `${r.role}: unknown confidence` };
    const base = fin(r.baseSalaryMicros);
    if (base === null || base < 0)
        return { reject: `${r.role}: no base salary` };
...
function parseOpsOutput(text) {
    ...
    if (rejected.length)
        log.warn({ rejected: rejected.slice(0, 20), count: rejected.length }, 'ops rows rejected');
 ...
 */
function parseScopeOutput(text) {
    const payload = extractJsonObject(text) ?? {};
    const str = (v, max) => (typeof v === 'string' && v.trim() ? v.trim().slice(0, max) : undefined);
    const list = (v) => (Array.isArray(v) ? v.filter((x) => typeof x === 'string').map((x) => x.slice(0, 400)).slice(0, 20) : []);
    return {
 ...
 */
function parseProseOutput(text) {
    const payload = extractJsonObject(text);
    ...
    for (const [k, v] of Object.entries(sections)) {
        if (typeof v === 'string' && v.trim())
            out[k.slice(0, 64)] = v.trim().slice(0, 8000);
    }
    return out;
}
...
function numeralValue(digits, suffix) {
 ...
 */
function verifyProseNumbers(prose, values) {
    ...
    for (const m of String(prose ?? '').matchAll(NUMERAL_RE)) {
        ...
        if (!Number.isFinite(value))
            continue;
        if (Number.isInteger(value) && value >= 0 && value <= 100 && !m[0].includes('$'))
            continue;
        ...
        if (allowed.has(Math.round(value * 100) / 100))
            continue;
        unmatched.add(m[0].trim());
    }
    return [...unmatched].slice(0, 100);
}
//# sourceMappingURL=venture-bot-contracts.js.map