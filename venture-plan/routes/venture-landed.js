...
exports.INCOTERM_BUYER_PAYS = exports.LANDED_LEG_ORDER = void 0;
exports.containersFor = containersFor;
exports.computeLandedCost = computeLandedCost;
exports.maxExWorksForLanded = maxExWorksForLanded;
exports.landedLeg = landedLeg;
exports.logisticsUnitMicros = logisticsUnitMicros;
 ...
 */
exports.INCOTERM_BUYER_PAYS = {
    EXW: exports.LANDED_LEG_ORDER,
 ...
 */
function containersFor(units, f) {
    if (!(units > 0) || !(f.unitsPerContainer > 0))
        return 0;
    return Math.ceil(units / f.unitsPerContainer);
}
/** Internal: the freight decision and its cost, before any other leg. */
function computeFreight(units, f) {
    const issues = [];
    if (!(units > 0))
        return { mode: 'fcl', containers: 0, fill: 0, costMicros: 0, basis: 'no shipment', issues };
    ...
    if (units < perContainer * threshold) {
        ...
        if (chargeable > rawCbm) {
            issues.push((0, venture_issues_1.issue)('lcl-minimum-chargeable', 'warn', 'landed:freight', `The shipment measures ${rawCbm.toFixed(3)} CBM but the carrier bills a ${f.lclMinimumChargeableCbm} CBM minimum, so freight is charged on volume that is not being shipped.`, { actualCbm: (0, venture_primitives_1.roundHalfUp)(rawCbm * 1000) / 1000, chargeableCbm: chargeable }));
        }
        return {
            mode: 'lcl', containers: 0, fill: units / perContainer,
            costMicros: (0, venture_primitives_1.scaleMicros)(f.lclCostPerCbmMicros, chargeable),
    ...
    if (fill < 0.7) {
        issues.push((0, venture_issues_1.issue)('partial-container', 'warn', 'landed:freight', `${containers} container(s) ship ${Math.round(fill * 100)}% full; freight per unit is carrying empty space.`, { containers, fillPct: Math.round(fill * 100) }));
    }
    return {
        mode: 'fcl', containers, fill,
        costMicros: (0, venture_primitives_1.scaleMicros)(f.containerCostMicros, containers),
        basis: `${containers} x ${f.containerType}`, issues,
    };
}
/** Internal: build one leg record, zeroed when the seller pays it. */
function leg(key, totalMicros, units, buyerPays, basis, assumptionRefs) {
    const total = buyerPays ? totalMicros : 0;
    return {
        key, totalMicros: total, perUnitMicros: (0, venture_primitives_1.divMicros)(total, units), paidBy: buyerPays ? 'buyer' : 'seller',
...
function computeLandedCost(input) {
    ...
    const pays = new Set(exports.INCOTERM_BUYER_PAYS[input.incoterm] ?? exports.INCOTERM_BUYER_PAYS.EXW);
    const issues = [];
    ...
    if (input.duty.customsValueBasis === 'cif') {
        issues.push((0, venture_issues_1.issue)('customs-basis-assumed', 'info', 'landed:duty', 'Duty is assessed on a CIF value here; the United States assesses on FOB, so confirm the declared basis with the broker before relying on the duty figure.', { basis: 'cif' }));
    }
    ...
    legs.push(leg('drayage', (0, venture_primitives_1.scaleMicros)(input.drayagePerContainerMicros, containerCount), units, pays.has('drayage'), `${containerCount} x drayage`, []));
    legs.push(leg('warehouseIn', (0, venture_primitives_1.scaleMicros)(input.warehouseInPerUnitMicros, units), units, pays.has('warehouseIn'), `${units} x receiving`, []));
    const buyerTotalMicros = (0, venture_primitives_1.addMicros)(...legs.map((l) => l.totalMicros));
    const sellerPaidLegs = legs.filter((l) => l.paidBy === 'seller').map((l) => l.key);
    for (const key of sellerPaidLegs) {
        issues.push((0, venture_issues_1.issue)('seller-paid-leg', 'info', 'landed:incoterm', `Under ${input.incoterm} the seller pays "${key}", so it is excluded from buyer landed cost — it is inside the unit price, not free.`, { incoterm: input.incoterm, legKey: key }));
    }
    if (units === 0) {
        issues.push((0, venture_issues_1.issue)('zero-volume', 'warn', 'landed', 'Zero units ship, so there is no per-unit landed cost.', { units: 0 }));
    }
    const dutyish = (0, venture_primitives_1.addMicros)(...legs.filter((l) => DUTY_LIKE.has(l.key)).map((l) => l.totalMicros));
    return {
        units, mode: freight.mode, containers: freight.containers, containerFillRatio: freight.fill,
        legs, customsValueMicros: customs, buyerTotalMicros, buyerUnitMicros: (0, venture_primitives_1.divMicros)(buyerTotalMicros, units),
        sellerPaidLegs,
        effectiveDutyBps: (0, venture_primitives_1.bpsOf)(dutyish, customs),
        issues,
        assumptionRefs: [...new Set([...input.freight.assumptionRefs, ...input.duty.assumptionRefs])].sort(),
    };
}
const DUTY_LIKE = new Set(['duty', 'additionalTariff', 'mpf', 'hmf']);
/**
 ...
 */
function originLegs(input, units, containerCount, freight, pays) {
    const legs = [
        leg('exWorks', (0, venture_primitives_1.scaleMicros)(input.exWorksUnitMicros, units), units, true, `${units} x unit price`, []),
        leg('originInland', (0, venture_primitives_1.scaleMicros)(input.originInlandPerContainerMicros, containerCount), units, pays.has('originInland'), `${containerCount} x origin inland`, []),
    ...
    const insuredValue = (0, venture_primitives_1.addMicros)(...legs.map((l) => l.totalMicros));
    const premium = Math.max((0, venture_primitives_1.applyBps)(insuredValue, Math.max(0, input.insuranceBps)), input.insuranceMinimumMicros);
    legs.push(leg('marineInsurance', premium, units, pays.has('marineInsurance'), `${input.insuranceBps} bps of insured value, minimum premium applied`, []));
    return legs;
}
/**
 ...
 */
function customsValue(basis, legs, pays) {
    const include = basis === 'cif'
        ? ['exWorks', 'originInland', 'exportClearance', 'oceanFreight', 'marineInsurance']
        : ['exWorks', 'originInland', 'exportClearance'];
    return (0, venture_primitives_1.addMicros)(...legs.filter((l) => include.includes(l.key) && (l.key === 'exWorks' || pays.has(l.key))).map((l) => l.totalMicros));
}
/**
 ...
 */
function governmentCharges(d, customs, units, pays) {
    const mpfRaw = (0, venture_primitives_1.applyBps)(customs, Math.max(0, d.mpfBps));
    const mpf = Math.min(Math.max(mpfRaw, d.mpfMinMicros), d.mpfMaxMicros);
    return [
        leg('duty', (0, venture_primitives_1.applyBps)(customs, Math.max(0, d.htsDutyBps)), units, pays.has('duty'), `${d.htsDutyBps} bps of customs value (HS ${d.htsCode})`, d.assumptionRefs),
        leg('additionalTariff', (0, venture_primitives_1.applyBps)(customs, Math.max(0, d.additionalTariffBps)), units, pays.has('additionalTariff'), `${d.additionalTariffBps} bps trade-remedy add-on`, d.assumptionRefs),
        leg('mpf', mpf, units, pays.has('mpf'), mpf === d.mpfMinMicros ? 'MPF at the per-entry floor' : mpf === d.mpfMaxMicros ? 'MPF at the per-entry ceiling' : `${d.mpfBps} bps MPF`, []),
        leg('hmf', (0, venture_primitives_1.applyBps)(customs, Math.max(0, d.hmfBps)), units, pays.has('hmf'), `${d.hmfBps} bps HMF`, []),
    ];
}
/**
 ...
 */
function maxExWorksForLanded(targetLandedUnitMicros, input) {
    const units = Number.isFinite(input.units) ? Math.max(0, Math.trunc(input.units)) : 0;
    if (units === 0) {
        return {
            exWorksUnitMicros: 0, method: 'closed-form',
            issues: [(0, venture_issues_1.issue)('zero-volume', 'warn', 'landed:inversion', 'Zero units ship, so no ex-works target exists.', { units: 0 })],
        };
    }
    const target = (0, venture_primitives_1.scaleMicros)(targetLandedUnitMicros, units);
    const total = (x) => computeLandedCost({ ...input, exWorksUnitMicros: x }).buyerTotalMicros;
    const t0 = total(0);
    ...
    if (!(slope > 0)) {
        return {
            exWorksUnitMicros: 0, method: 'closed-form',
            issues: [(0, venture_issues_1.issue)('inversion-impossible', 'block', 'landed:inversion', 'Landed cost does not rise with the ex-works price, so no maximum factory price can be derived from this shipment shape.', {})],
        };
    }
    ...
    if (linear < 0) {
        return {
            exWorksUnitMicros: 0, method: 'closed-form',
            issues: [(0, venture_issues_1.issue)('inversion-impossible', 'block', 'landed:inversion', 'Freight, duty and handling alone already exceed the target landed cost; no factory price, not even zero, reaches it.', { targetLandedUnitMicros, fixedLandedMicros: t0 })],
        };
    }
 ...
 */
function bisectExWorks(target, seed, total) {
    let lo = 0;
    let hi = Math.max(1, seed * 2 + 1_000_000);
    if (total(lo) > target) {
        return {
            exWorksUnitMicros: 0, method: 'bisection',
            issues: [(0, venture_issues_1.issue)('inversion-impossible', 'block', 'landed:inversion', 'Even a zero factory price lands above the target once freight, duty and handling are paid.', { targetTotalMicros: target })],
        };
    }
    for (let i = 0; i < 64 && total(hi) <= target; i += 1)
        hi *= 2;
    ...
    return {
        ...
        issues: [(0, venture_issues_1.issue)('rounding-residual', 'info', 'landed:inversion', 'A fee cap boundary made landed cost only piecewise affine in the factory price, so the maximum was found by bisection over the real model rather than by algebra.', {})],
    };
}
/**
 ...
 */
function logisticsUnitMicros(landed) {
    const goods = landedLeg(landed, 'exWorks')?.totalMicros ?? 0;
    return (0, venture_primitives_1.divMicros)((0, venture_primitives_1.subMicros)(landed.buyerTotalMicros, goods), landed.units);
}
//# sourceMappingURL=venture-landed.js.map