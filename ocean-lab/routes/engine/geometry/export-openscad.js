...
exports.toScadIdentifier = toScadIdentifier;
exports.formatScadValue = formatScadValue;
exports.toOpenScad = toOpenScad;
const logger_1 = require("@/shared/logger");
const format_number_1 = require("./format-number");
const vector_math_1 = require("./vector-math");
const log = (0, logger_1.createChildLogger)({ module: 'shared/geometry/export-openscad' });
...
function formatScadValue(value) {
    if (typeof value === 'number')
        return (0, format_number_1.formatFloat)(value);
    if (NUMERIC_VECTOR.test(value.trim()))
        return value.trim();
...
function parameterLines(parameters) {
    return Object.entries(parameters).map(([key, value], index) => `${toScadIdentifier(key, index)} = ${formatScadValue(value)};`);
}
/**
...
function toOpenScad(mesh, options = {}) {
    ...
    const points = mesh.vertices.map((v) => `      [${(0, format_number_1.formatFloat)(v.x)}, ${(0, format_number_1.formatFloat)(v.y)}, ${(0, format_number_1.formatFloat)(v.z)}]`);
    const faces = mesh.triangles.map((triangle, index) => {
        (0, vector_math_1.resolveTriangle)(mesh, triangle, index);
