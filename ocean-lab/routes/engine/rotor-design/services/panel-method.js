...
exports.panelSolveCl = panelSolveCl;
exports.panelLiftLine = panelLiftLine;
exports.liftLineCl = liftLineCl;
const logger_1 = require("@/shared/logger");
const log = (0, logger_1.createChildLogger)({ module: 'features/rotor-design/panel-method' });
...
function buildPanels(section) {
    ...
    for (let i = 1; i < clockwise.length; i += 1)
        if (clockwise[i].x > clockwise[start].x)
            start = i;
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        if (length === 0)
            throw new RangeError(`Panel ${i} has zero length; the profile repeats a point`);
...
function buildInfluence(panels) {
    ...
    const sourceNormal = Array.from({ length: n }, () => new Array(n).fill(0));
    const sourceTangent = Array.from({ length: n }, () => new Array(n).fill(0));
    const vortexNormal = new Array(n).fill(0);
    const vortexTangent = new Array(n).fill(0);
    for (let i = 0; i < n; i += 1) {
...
function solveLinearSystem(matrix, rightSides) {
    ...
    for (let col = 0; col < n; col += 1) {
        ...
        if (Math.abs(rows[pivot][col]) < 1e-14)
            throw new RangeError(`Panel system is singular at column ${col}`);
        [rows[col], rows[pivot]] = [rows[pivot], rows[col]];
        for (let r = col + 1; r < n; r += 1) {
            const factor = rows[r][col] / rows[col][col];
            if (factor === 0)
                continue;
            for (let c = col; c < n + k; c += 1)
 ...
 */
function assembleSystem(panels, influence) {
    const n = panels.count;
    const matrix = Array.from({ length: n + 1 }, () => new Array(n + 1).fill(0));
    for (let i = 0; i < n; i += 1) {
        for (let j = 0; j < n; j += 1)
            matrix[i][j] = influence.sourceNormal[i][j];
...
function kuttaRightSide(panels, alphaRad) {
    ...
    for (let i = 0; i < n; i += 1)
        side[i] = -(ux * -panels.sines[i] + uy * panels.cosines[i]);
 ...
 */
function panelSolveCl(section, alphaRad) {
 ...
 * @throws RangeError when the profile is degenerate or the system is singular.
 */
function panelLiftLine(section) {
    const panels = buildPanels(section);
    const influence = buildInfluence(panels);
    const matrix = assembleSystem(panels, influence);
    ...
    const sinCoefficient = (clProbe - cosCoefficient * Math.cos(LIFT_LINE_PROBE_RAD)) / Math.sin(LIFT_LINE_PROBE_RAD);
    const line = {
        cosCoefficient,
        sinCoefficient,
