...
exports.DEFAULT_SECTION_STATIONS = void 0;
exports.nacaThickness = nacaThickness;
exports.nacaCamber = nacaCamber;
exports.cosineStations = cosineStations;
exports.sectionRingLength = sectionRingLength;
exports.nacaSection = nacaSection;
const logger_1 = require("@/shared/logger");
const log = (0, logger_1.createChildLogger)({ module: 'features/rotor-design/naca-section' });
/**
...
function nacaThickness(x, thickness) {
    if (!(x >= 0 && x <= 1))
        throw new RangeError(`NACA thickness needs 0 <= x <= 1, received ${x}`);
    if (!(thickness >= 0))
        throw new RangeError(`NACA thickness fraction must be >= 0, received ${thickness}`);
...
function cosineStations(count) {
    ...
    for (let i = 0; i < count; i += 1)
        stations.push(0.5 * (1 - Math.cos((Math.PI * i) / (count - 1))));
    stations[0] = 0;
    stations[count - 1] = 1;
    return stations;
}
 ...
 */
function sectionRingLength(stationsPerSurface) {
 ...
 */
function nacaSection(spec, stationsPerSurface = exports.DEFAULT_SECTION_STATIONS) {
