...
exports.BETZ_LIMIT = void 0;
exports.prandtlTipLoss = prandtlTipLoss;
exports.prandtlHubLoss = prandtlHubLoss;
exports.axialInduction = axialInduction;
exports.solveBemt = solveBemt;
exports.cpCurve = cpCurve;
const logger_1 = require("@/shared/logger");
const naca_section_1 = require("./naca-section");
const section_polar_1 = require("./section-polar");
const log = (0, logger_1.createChildLogger)({ module: 'features/rotor-design/bemt-solver' });
 ...
 */
function prandtlTipLoss(bladeCount, radiusM, tipRadiusM, sinPhi) {
 ...
 */
function prandtlHubLoss(bladeCount, radiusM, hubRadiusM, sinPhi) {
    const s = Math.max(Math.abs(sinPhi), 1e-9);
 ...
 */
function prandtlLossFactor(context, sinPhi) {
...
function axialInduction(kappa, lossFactor) {
    ...
    const g3 = u + 2 * f - 25 / 9;
    if (Math.abs(g3) < 1e-9)
        return 1 - 1 / (2 * Math.sqrt(Math.max(g2, 1e-12)));
 ...
 */
function tangentialInduction(kappaPrime) {
 ...
 */
function evaluateResidual(context, phi) {
    const { element, settings } = context;
    const alpha = phi - element.betaRad;
    const { cl, cd } = (0, section_polar_1.sectionPolar)(element.section, alpha, context.reynolds, {
    ...
    const kappaPrime = (element.solidity * tangential) / (4 * lossFactor * clampAwayFromZero(sinPhi * cosPhi));
    const a = axialInduction(kappa, lossFactor);
    const aPrime = tangentialInduction(kappaPrime);
    const residual = sinPhi / clampAwayFromZero(1 - a) - cosPhi / (context.localSpeedRatio * clampAwayFromZero(1 + aPrime));
 ...
 */
function bisectInflowAngle(context) {
    for (const [low, high] of candidateBrackets()) {
        ...
        if (!Number.isFinite(loValue) || !Number.isFinite(hiValue) || loValue * hiValue > 0)
            continue;
 ...
 */
function inflowAtReynolds(context, flow, reynolds) {
    const working = { ...context, reynolds };
    ...
    return {
        ...
        solution,
        relativeSpeed,
        impliedReynolds: (0, section_polar_1.sectionReynolds)(relativeSpeed, working.element.chordM, flow.kinematicViscosityM2S),
    };
}
/**
 ...
 */
function reynoldsAgrees(candidate) {
 ...
 */
function bisectReynolds(context, flow) {
 ...
 */
function reportNoConvergence(context) {
    log.error({
 ...
 * @returns A non-converged element result.
 */
function emptyElement(element, reynolds) {
    return {
 ...
 */
function buildElementResult(settled, flow) {
...
function interpolateStation(stations, radiusFrac) {
    ...
    let upper = 1;
    while (upper < stations.length - 1 && stations[upper].radiusFrac < radiusFrac)
        upper += 1;
...
function buildElements(blade, count) {
    if (blade.stations.length < 2)
        throw new RangeError('A blade needs at least 2 design stations');
...
function resolveOptions(options) {
    return {
        ...
        panelStations: options.panelStations ?? naca_section_1.DEFAULT_SECTION_STATIONS,
    };
}
/**
 ...
 */
function solveBemt(blade, flow, options = {}) {
    if (!(flow.freeStreamMs > 0))
        throw new RangeError(`BEMT needs a positive free stream, received ${flow.freeStreamMs}`);
    ...
    const results = elements.map((element) => {
        ...
        const context = {
            ...
            reynolds: (0, section_polar_1.sectionReynolds)(seed, element.chordM, flow.kinematicViscosityM2S),
            aspectRatio,
            settings,
        };
...
function integrateRotor(blade, flow, elements) {
    ...
    if (!allConverged) {
        log.error({ failed: elements.filter((e) => !e.converged).length, total: elements.length }, 'BEMT solve did not converge on every element — the integrated result is NOT usable');
    }
    return result;
}
/**
 ...
 */
function cpCurve(blade, flow, lambdaValues, options = {}) {
    const points = lambdaValues.map((lambda) => {
        if (!(lambda > 0))
            throw new RangeError(`Tip-speed ratio must be positive, received ${lambda}`);
    ...
    if (failed.length > 0) {
        log.error({ failedPoints: failed.map((point) => ({ lambda: point.lambda, failedElements: point.failedElements })), total: points.length }, 'Cp(λ) sweep contains points integrated over a partly unsolved blade — read allConverged before plotting cp');
    }
    return points;
}
