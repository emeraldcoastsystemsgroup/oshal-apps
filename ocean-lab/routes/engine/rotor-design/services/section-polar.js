...
exports.NOMINAL_ASPECT_RATIO = exports.SEAWATER_KINEMATIC_VISCOSITY_M2S = void 0;
exports.sectionReynolds = sectionReynolds;
exports.skinFrictionCoefficient = skinFrictionCoefficient;
exports.estimateSectionDrag = estimateSectionDrag;
exports.maxLiftCoefficient = maxLiftCoefficient;
exports.sectionLiftLine = sectionLiftLine;
exports.sectionPolar = sectionPolar;
const logger_1 = require("@/shared/logger");
const naca_section_1 = require("./naca-section");
const panel_method_1 = require("./panel-method");
const viterna_1 = require("./viterna");
const log = (0, logger_1.createChildLogger)({ module: 'features/rotor-design/section-polar' });
/** @description Kinematic viscosity of seawater at coastal temperature and salinity, m²/s. */
exports.SEAWATER_KINEMATIC_VISCOSITY_M2S = 1.05e-6;
 ...
 */
function sectionReynolds(relativeSpeedMs, chordM, kinematicViscosityM2S = exports.SEAWATER_KINEMATIC_VISCOSITY_M2S) {
    if (!(chordM > 0))
        throw new RangeError(`Section Reynolds needs a positive chord, received ${chordM}`);
 ...
 */
function skinFrictionCoefficient(reynolds) {
 ...
 */
function estimateSectionDrag(thickness, reynolds, cl, clMinDrag) {
 ...
 */
function sectionLiftLine(spec, stationsPerSurface = naca_section_1.DEFAULT_SECTION_STATIONS) {
    const key = `${spec.maxCamber}:${spec.camberPos}:${spec.thickness}:${stationsPerSurface}`;
    const cached = liftLineCache.get(key);
    if (cached)
        return cached;
    const line = (0, panel_method_1.panelLiftLine)((0, naca_section_1.nacaSection)(spec, stationsPerSurface));
 ...
 */
function stallAngles(line, reynolds) {
 ...
 * @returns `{ cl, cd }`.
 */
function sectionPolar(spec, alphaRad, reynolds, options = {}) {
    const line = sectionLiftLine(spec, options.stationsPerSurface ?? naca_section_1.DEFAULT_SECTION_STATIONS);
    const aspectRatio = options.aspectRatio ?? exports.NOMINAL_ASPECT_RATIO;
    const clMinDrag = (0, panel_method_1.liftLineCl)(line, 0);
    ...
    if (alpha <= positive && alpha >= negative) {
        const cl = (0, panel_method_1.liftLineCl)(line, alpha);
 ...
 * {@link viternaExtend}.
 */
function handoverPoint(spec, line, reynolds, aspectRatio, stallAngle, clMinDrag) {
    const clStall = (0, panel_method_1.liftLineCl)(line, stallAngle);
 ...
 */
function stalledCoefficients(spec, line, alpha, reynolds, aspectRatio, stallAngle, clMinDrag) {
    const point = handoverPoint(spec, line, reynolds, aspectRatio, stallAngle, clMinDrag);
    const sign = stallAngle >= 0 ? 1 : -1;
    const mirrored = (0, viterna_1.viternaExtend)(point, sign * alpha);
