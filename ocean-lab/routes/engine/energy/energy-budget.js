...
exports.DEFAULT_DURATION_HOURS = void 0;
exports.totalDrawW = totalDrawW;
exports.simulateEnergyBudget = simulateEnergyBudget;
exports.recommendStorageWh = recommendStorageWh;
const logger_1 = require("@/shared/logger");
const logger = (0, logger_1.createChildLogger)({ module: 'shared:energy-budget' });
/** @description Default run: 30 days, long enough to contain a full multi-day environmental cycle. */
exports.DEFAULT_DURATION_HOURS = 720;
/**
 ...
 */
function requirePositive(value, field, maximum = Infinity) {
...
function resolveRun(options) {
    const durationHours = options.durationHours ?? exports.DEFAULT_DURATION_HOURS;
...
function stepStore(state, storage, netWh, floorWh, stepSeconds) {
    ...
    if (netWh >= 0) {
        ...
        state.socWh -= drawn;
        if (drawn < deficit)
            state.brownoutSeconds += stepSeconds;
    }
}
/**
...
function simulateEnergyBudget(design, options = {}) {
    ...
    logger.info({ label: verdict.label, perpetual: verdict.perpetual, marginRatio: verdict.marginRatio, minSocFraction: verdict.minSocFraction }, 'energy budget simulated');
 ...
 * @returns The verdict.
 */
function buildVerdict(label, capacityWh, state, durationHours) {
    return {
 ...
 */
function recommendStorageWh(design, options = {}, maxCapacityWh = 100_000) {
    requirePositive(maxCapacityWh, 'maxCapacityWh');
    const at = (capacityWh) => simulateEnergyBudget({ ...design, storage: { ...design.storage, capacityWh } }, options).verdict;
