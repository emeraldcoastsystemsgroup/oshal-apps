...
exports.parseBemtOptions = parseBemtOptions;
exports.createBladeStudioAssetRoutes = createBladeStudioAssetRoutes;
exports.createRotorRoutes = createRotorRoutes;
const express_1 = require("express");
const rotor_design_1 = require("./engine/rotor-design");
...
const logger = (0, logger_1.createChildLogger)({ module: 'rotor-routes' });
/**
...
exports.ROTOR_LIMITS = {
    ...
    maxNameLength: harvest_routes_1.HARVEST_LIMITS.maxNameLength,
    /** Longest load list accepted. Shared with the harvest surface. */
    maxLoads: harvest_routes_1.HARVEST_LIMITS.maxLoads,
    /** Longest tidal constituent list accepted. Shared with the harvest surface. */
    maxConstituents: harvest_routes_1.HARVEST_LIMITS.maxConstituents,
    /** Samples actually returned by POST /harvest. Shared with the harvest surface. */
    maxResponseSamples: harvest_routes_1.HARVEST_LIMITS.maxResponseSamples,
};
/**
 ...
 */
class RotorInputError extends Error {
    /**
...
exports.RotorInputError = RotorInputError;
/**
 ...
 */
class RotorSolutionError extends Error {
    /**
...
exports.RotorSolutionError = RotorSolutionError;
/**
 ...
 */
function optInt(src, key, label, min, max) {
    const value = optNum(src, key, label, min, max);
    if (value === undefined)
        return undefined;
    if (!Number.isInteger(value))
        bad(`${label}.${key} must be a whole number, received ${value}`);
    return value;
}
/**
 ...
 */
function optBool(src, key, label) {
    const raw = src[key];
    if (raw === undefined || raw === null)
        return undefined;
    if (typeof raw !== 'boolean')
        bad(`${label}.${key} must be a boolean`);
    return raw;
}
/**
...
function text(src, key, label) {
    ...
    if (value.length > exports.ROTOR_LIMITS.maxNameLength) {
        bad(`${label}.${key} must be at most ${exports.ROTOR_LIMITS.maxNameLength} characters`);
    }
    return value;
}
/**
 ...
 * @returns The array.
 */
function list(src, key, label, min, max) {
    const raw = src[key];
    if (!Array.isArray(raw))
        bad(`${label}.${key} must be an array`);
    const values = raw;
    if (values.length < min)
        bad(`${label}.${key} needs at least ${min} entries, received ${values.length}`);
 ...
 */
function parseNacaSpec(raw, at) {
    const spec = asRecord(raw, at);
    const maxCamber = num(spec, 'maxCamber', at, -exports.ROTOR_LIMITS.maxCamber, exports.ROTOR_LIMITS.maxCamber);
    ...
    return {
        maxCamber,
        camberPos,
        thickness: num(spec, 'thickness', at, exports.ROTOR_LIMITS.minThickness, exports.ROTOR_LIMITS.maxThickness),
    };
}
/**
 ...
 */
function parseStation(raw, at) {
    const station = asRecord(raw, at);
    return {
        radiusFrac: num(station, 'radiusFrac', at, 0, 1),
        chordM: num(station, 'chordM', at, exports.ROTOR_LIMITS.minChordM, exports.ROTOR_LIMITS.maxChordM),
        twistDeg: num(station, 'twistDeg', at, -exports.ROTOR_LIMITS.maxTwistDeg, exports.ROTOR_LIMITS.maxTwistDeg),
 ...
 */
function parseBlade(raw) {
    const blade = asRecord(raw, 'blade');
    const tipRadiusM = num(blade, 'tipRadiusM', 'blade', exports.ROTOR_LIMITS.minTipRadiusM, exports.ROTOR_LIMITS.maxTipRadiusM);
    const hubRadiusM = num(blade, 'hubRadiusM', 'blade', 0, exports.ROTOR_LIMITS.maxTipRadiusM);
    ...
    const bladeCount = num(blade, 'bladeCount', 'blade', 1, exports.ROTOR_LIMITS.maxBladeCount);
    if (!Number.isInteger(bladeCount))
        bad(`blade.bladeCount must be a whole number, received ${bladeCount}`);
    const stations = list(blade, 'stations', 'blade', exports.ROTOR_LIMITS.minStations, exports.ROTOR_LIMITS.maxStations).map((entry, i) => parseStation(entry, `blade.stations[${i}]`));
    return {
        tipRadiusM,
        hubRadiusM,
        bladeCount,
        stations,
        pitchDeg: num(blade, 'pitchDeg', 'blade', -exports.ROTOR_LIMITS.maxPitchDeg, exports.ROTOR_LIMITS.maxPitchDeg),
    };
}
/**
...
function parseFlow(raw) {
    ...
    return {
        freeStreamMs: num(flow, 'freeStreamMs', 'flow', exports.ROTOR_LIMITS.minFreeStreamMs, exports.ROTOR_LIMITS.maxFreeStreamMs),
        rotationRadS: num(flow, 'rotationRadS', 'flow', 0, exports.ROTOR_LIMITS.maxRotationRadS),
        densityKgM3: num(flow, 'densityKgM3', 'flow', exports.ROTOR_LIMITS.minDensityKgM3, exports.ROTOR_LIMITS.maxDensityKgM3),
        kinematicViscosityM2S: num(flow, 'kinematicViscosityM2S', 'flow', exports.ROTOR_LIMITS.minKinematicViscosityM2S, exports.ROTOR_LIMITS.maxKinematicViscosityM2S),
    };
}
/**
 ...
 */
function parseBemtOptions(raw, stationCount, solveCount) {
    const opts = raw === undefined || raw === null ? {} : asRecord(raw, 'options');
    const elementCount = optInt(opts, 'elementCount', 'options', exports.ROTOR_LIMITS.minElementCount, exports.ROTOR_LIMITS.maxElementCount) ?? 48;
    const panelStations = optInt(opts, 'panelStations', 'options', exports.ROTOR_LIMITS.minPanelStations, exports.ROTOR_LIMITS.maxPanelStations) ??
        rotor_design_1.DEFAULT_SECTION_STATIONS;
    const panelWork = stationCount * panelStations ** 3;
    if (panelWork > exports.ROTOR_LIMITS.maxPanelWorkUnits) {
        bad(`options.panelStations against ${stationCount} stations asks for ${panelWork} panel work units; the cap ` +
            `is ${exports.ROTOR_LIMITS.maxPanelWorkUnits} — lower panelStations or use fewer stations`);
    }
    const elementSolves = elementCount * Math.max(1, solveCount);
    if (elementSolves > exports.ROTOR_LIMITS.maxElementSolves) {
        bad(`options.elementCount across ${solveCount} solve(s) asks for ${elementSolves} element solves; the cap is ` +
            `${exports.ROTOR_LIMITS.maxElementSolves} — lower elementCount or sweep fewer tip-speed ratios`);
    }
    return {
        elementCount,
        panelStations,
 ...
 */
function parseLambdas(raw) {
    return list(raw, 'lambdas', 'body', 1, exports.ROTOR_LIMITS.maxLambdas).map((entry, i) => {
        const at = `body.lambdas[${i}]`;
        if (typeof entry !== 'number' || !Number.isFinite(entry))
            bad(`${at} must be a finite number`);
        const lambda = entry;
        if (!(lambda > 0) || lambda > exports.ROTOR_LIMITS.maxLambda) {
            bad(`${at} must be greater than 0 and at most ${exports.ROTOR_LIMITS.maxLambda}, received ${lambda}`);
        }
        return lambda;
    });
}
 ...
 */
function parseLoads(src) {
    return list(src, 'loads', 'body', 1, exports.ROTOR_LIMITS.maxLoads).map((raw, i) => {
...
function parseTidalSite(raw) {
    ...
    const constituents = list(site, 'constituents', 'body.site', 1, exports.ROTOR_LIMITS.maxConstituents).map((entry, i) => {
...
exports.ROTOR_EXPORT_FORMATS = ['stl', 'stl-binary', 'obj', 'scad', 'dxf'];
/**
 ...
 */
function estimateExportSize(stationCount, sectionPoints) {
    const ringLength = (0, rotor_design_1.sectionRingLength)(sectionPoints);
    return {
        ringLength,
 ...
 */
function bladeToDxfSections(blade, sectionPoints) {
    ...
    return sorted.map((station, index) => {
        ...
        const points = (0, rotor_design_1.nacaSection)(station.section, sectionPoints).points.map((p) => {
            const x = (p.x - rotor_design_1.PITCH_AXIS_CHORD_FRACTION) * station.chordM;
 ...
 */
function renderMeshExport(mesh, format, name, parameters) {
    if (format === 'stl-binary') {
        const bytes = (0, geometry_1.toStlBinary)(mesh);
        return {
            format,
            filename: `${name}.stl`,
    ...
    const rendered = format === 'stl' ? (0, geometry_1.toStlAscii)(mesh, name) : format === 'obj' ? (0, geometry_1.toObj)(mesh, name) : (0, geometry_1.toOpenScad)(mesh, { name, parameters });
 ...
 */
function renderExport(blade, format, sectionPoints, name) {
    const size = estimateExportSize(blade.stations.length, sectionPoints);
    if (size.triangles > exports.ROTOR_LIMITS.maxExportTriangles) {
        bad(`body.sectionPoints against ${blade.stations.length} stations would emit ${size.triangles} facets; the cap ` +
            `is ${exports.ROTOR_LIMITS.maxExportTriangles} — lower sectionPoints or use fewer stations`);
    }
    if (format === 'dxf') {
        const drawing = (0, geometry_1.toDxfR12)(bladeToDxfSections(blade, sectionPoints));
        return {
            format,
            filename: `${name}.dxf`,
    ...
    const mesh = (0, rotor_design_1.bladeToMesh)(blade, sectionPoints);
    return {
        ...renderMeshExport(mesh, format, name, scadParameters(blade)),
        mesh: {
            vertices: mesh.vertices.length,
            triangles: mesh.triangles.length,
            validation: (0, geometry_1.validateMesh)(mesh),
        },
    };
}
/**
 ...
 */
function assertUsableForBudget(bemt) {
    if (!bemt.allConverged) {
        throw new RotorSolutionError('the BEMT solve did not converge on every element, so its Cp is an integral over a partly missing ' +
            'rotor — change the operating point or the discretisation rather than consuming this number');
    }
    if (!(bemt.cp > 0)) {
        throw new RotorSolutionError(`the rotor extracts no net power at this operating point (Cp ${bemt.cp}) — it is being driven, not ` +
 ...
 */
function turbineFromSolve(blade, bemt, drivetrain) {
    return {
 ...
 */
const CONSTANT_CP_NOTE = 'Cp is solved once, at the posted flow, and then held constant across the tide — which assumes the ' +
...
function handleSolve(req, res) {
    ...
    const bemt = (0, rotor_design_1.solveBemt)(blade, flow, options);
    if (bemt.cp > rotor_design_1.BETZ_LIMIT) {
        logger.warn({ cp: bemt.cp, betzLimit: rotor_design_1.BETZ_LIMIT, tipSpeedRatio: bemt.tipSpeedRatio }, 'BEMT solved a Cp above the Betz limit');
    }
    res.json({ options, betzLimit: rotor_design_1.BETZ_LIMIT, aboveBetzLimit: bemt.cp > rotor_design_1.BETZ_LIMIT, bemt });
}
/**
...
function handleCpCurve(req, res) {
    ...
    res.json({
        options,
        betzLimit: rotor_design_1.BETZ_LIMIT,
        points,
        peak,
...
function handleExport(req, res) {
    ...
    if (typeof format !== 'string' || !exports.ROTOR_EXPORT_FORMATS.includes(format)) {
        bad(`body.format must be one of ${exports.ROTOR_EXPORT_FORMATS.join(', ')}`);
    }
    const blade = parseBlade(body.blade);
    const sectionPoints = optInt(body, 'sectionPoints', 'body', exports.ROTOR_LIMITS.minSectionPoints, exports.ROTOR_LIMITS.maxSectionPoints) ??
        rotor_design_1.DEFAULT_SECTION_STATIONS;
    const name = body.name === undefined || body.name === null ? 'oshal_blade' : text(body, 'name', 'body');
    res.json(renderExport(blade, format, sectionPoints, name));
}
/**
...
function handleHarvest(req, res) {
    ...
    const options = (0, harvest_routes_1.parseHarvestOptions)(body.options, 'marine');
    const blade = parseBlade(body.blade);
    const flow = parseFlow(body.flow);
    ...
    const aboveBetzLimit = bemt.cp > rotor_design_1.BETZ_LIMIT;
    if (aboveBetzLimit) {
        logger.warn({ cp: bemt.cp, betzLimit: rotor_design_1.BETZ_LIMIT }, 'substituting an above-Betz Cp into the marine budget');
    }
    const result = (0, marine_1.simulatePowerBudget)({ ...design, turbine }, options);
    logger.info({ cp: bemt.cp, tipSpeedRatio: bemt.tipSpeedRatio, perpetual: result.verdict.perpetual }, 'solved a rotor and closed the marine energy loop on its Cp');
    res.json({
        bemt,
        turbine,
        betzLimit: rotor_design_1.BETZ_LIMIT,
        ...
        samples: (0, harvest_routes_1.downsampleSamples)(result.samples, exports.ROTOR_LIMITS.maxResponseSamples),
 ...
 */
function isInputRejection(error) {
    return error instanceof RotorInputError || error instanceof harvest_routes_1.HarvestInputError;
}
/**
 ...
 */
function rotorRoute(operation, handler) {
    return (req, res) => {
 ...
 */
const BLADE_STUDIO_SCRIPTS = ['blade-studio-gl.js', 'blade-studio.js'];
/**
 ...
 */
function createBladeStudioAssetRoutes(opts = {}) {
    const router = (0, express_1.Router)();
    const guard = opts.requiresAuth ?? surface_files_1.passthrough;
    for (const file of BLADE_STUDIO_SCRIPTS) {
        const resolved = (0, surface_files_1.surfaceFile)(opts.appPackageDir, file);
        logger.info({ file, resolved }, 'Resolved a blade studio engine script');
        router.get(`/${file}`, guard, (0, surface_files_1.serveSurfaceFile)(resolved, 'application/javascript'));
    }
    return router;
}
/**
 ...
 */
function createRotorRoutes(opts = {}) {
    const router = (0, express_1.Router)();
    const guard = opts.requiresAuth ?? surface_files_1.passthrough;
