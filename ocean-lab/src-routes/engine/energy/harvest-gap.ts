...
function crossingSeconds(
  ...
  for (let i = 0; i < CROSSING_REFINEMENTS; i += 1) {
    const mid = (lo + hi) / 2;
    if (harvestAt(mid) < cutInW === hiBelow) hi = mid;
    else lo = mid;
  }
  return (lo + hi) / 2;
}
...
export function longestGapHours(harvestAt: HarvestSampler, cutInW: number, spanHours: number, stepSeconds = 60): number {
  if (!Number.isFinite(spanHours) || spanHours <= 0) throw new Error(`spanHours must be a finite positive number, got ${spanHours}`);
  if (!Number.isFinite(stepSeconds) || stepSeconds <= 0) throw new Error(`stepSeconds must be a finite positive number, got ${stepSeconds}`);
  ...
  if (gapStart !== null) longest = Math.max(longest, spanSeconds - gapStart);
  return longest / 3600;
}
