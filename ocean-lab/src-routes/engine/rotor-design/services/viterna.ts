...
import type { SectionCoefficients, ViternaStallPoint } from '../model/rotor-types';
 ...
 */
export function viternaDragMax(aspectRatio: number): number {
  if (!(aspectRatio > 0)) throw new RangeError(`Viterna needs a positive aspect ratio, received ${aspectRatio}`);
  return aspectRatio > AR_CEILING ? CD_MAX_LIMIT : 1.11 + 0.018 * aspectRatio;
}

/** @description The four Viterna coefficients, derived once from a stall point. */
interface ViternaCoefficients {
  /** `A1` — the `sin 2α` lift term. */
  a1: number;
  /** `A2` — the `cos²α / sin α` lift term that carries the stall point. */
  a2: number;
  /** `B1` — the `sin²α` drag term, equal to the flat-plate ceiling. */
  b1: number;
  /** `B2` — the `cos α` drag term that carries the stall point. */
  b2: number;
}
 ...
 */
function viternaCoefficients(stall: ViternaStallPoint): ViternaCoefficients {
 ...
 */
function viternaBranch(c: ViternaCoefficients, alphaRad: number): SectionCoefficients {
 ...
 * @returns Section coefficients.
 */
function reversedBranch(c: ViternaCoefficients, alphaRad: number): SectionCoefficients {
 ...
 * @returns Section coefficients.
 */
function tailBranch(c: ViternaCoefficients, alphaRad: number, alphaStallRad: number): SectionCoefficients {
 ...
 */
export function viternaExtend(stall: ViternaStallPoint, alphaRad: number): SectionCoefficients {
  ...
  if (magnitude < stall.alphaStallRad) {
    throw new RangeError(
      `viternaExtend is defined for |alpha| >= alphaStall (${stall.alphaStallRad}); received ${wrapped}`,
    );
  }
  const c = viternaCoefficients(stall);
  let forward: SectionCoefficients;
  if (magnitude <= Math.PI / 2) forward = viternaBranch(c, magnitude);
  else if (magnitude <= Math.PI - stall.alphaStallRad) forward = reversedBranch(c, magnitude);
  else forward = tailBranch(c, magnitude, stall.alphaStallRad);
