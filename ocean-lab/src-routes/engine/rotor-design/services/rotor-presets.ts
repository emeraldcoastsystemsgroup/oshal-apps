...
import type { BladeSpec, BladeStation, FlowCondition, NacaSpec } from '../model/rotor-types';
import { SEAWATER_KINEMATIC_VISCOSITY_M2S, sectionLiftLine } from './section-polar';

const log = createChildLogger({ module: 'features/rotor-design/rotor-presets' });

/** @description Seawater density at coastal temperature and salinity, kg/m³. */
export const SEAWATER_DENSITY_KGM3 = 1025;

/** @description Inputs to {@link betzOptimalBlade}. */
export interface BetzBladeOptions {
  /** Tip radius, metres. */
  tipRadiusM: number;
  /** Blade count. */
  bladeCount: number;
  /** Tip-speed ratio the blade is optimised for. */
  designTipSpeedRatio: number;
  /** Hub radius as a fraction of tip radius. Default 0.25. */
  hubRadiusFrac?: number;
  /** Lift coefficient the sections are designed to fly at. Default 1.0. */
  designLiftCoefficient?: number;
  /** Number of design stations from hub to tip. Default 13. */
  stationCount?: number;
  /** Camber of every section. Default 0.04 (a 44xx-family blade). */
  maxCamber?: number;
  /** Chordwise position of maximum camber. Default 0.4. */
  camberPos?: number;
  /** Thickness fraction at the root — thick, because the root carries the bending moment. Default 0.18. */
  rootThickness?: number;
  /** Thickness fraction at the tip. Default 0.12. */
  tipThickness?: number;
}
 ...
 */
function quantiseThickness(thickness: number): number {
 ...
 */
function betzStation(radiusFrac: number, options: Required<BetzBladeOptions>): BladeStation {
  ...
  const chordM =
    (8 * Math.PI * radiusM * (1 - Math.cos(phi))) / (options.bladeCount * options.designLiftCoefficient);
  const spanFrac = (radiusFrac - options.hubRadiusFrac) / Math.max(1 - options.hubRadiusFrac, 1e-9);
  const section: NacaSpec = {
    maxCamber: options.maxCamber,
    camberPos: options.camberPos,
    thickness: quantiseThickness(
      options.rootThickness + spanFrac * (options.tipThickness - options.rootThickness),
    ),
  };
  const line = sectionLiftLine(section);
 ...
 * not usable.
 */
export function betzOptimalBlade(options: BetzBladeOptions): BladeSpec {
  const resolved: Required<BetzBladeOptions> = {
    hubRadiusFrac: 0.25,
  ...
  if (!(resolved.tipRadiusM > 0)) throw new RangeError(`Tip radius must be positive, got ${resolved.tipRadiusM}`);
  if (!(resolved.bladeCount >= 1)) throw new RangeError(`Blade count must be >= 1, got ${resolved.bladeCount}`);
  ...
  if (!(resolved.stationCount >= 2)) throw new RangeError(`Need at least 2 stations, got ${resolved.stationCount}`);
  ...
  for (let i = 0; i < resolved.stationCount; i += 1) {
    const radiusFrac =
      resolved.hubRadiusFrac + ((1 - resolved.hubRadiusFrac) * i) / (resolved.stationCount - 1);
    stations.push(betzStation(radiusFrac, resolved));
  }
  log.debug(
    { tipRadiusM: resolved.tipRadiusM, bladeCount: resolved.bladeCount, lambda: resolved.designTipSpeedRatio },
    'generated a Betz-optimal blade',
  );
  return {
 ...
 */
export function scaleBlade(blade: BladeSpec, tipRadiusM: number): BladeSpec {
  if (!(tipRadiusM > 0)) throw new RangeError(`Scaled tip radius must be positive, received ${tipRadiusM}`);
...
export const REFERENCE_DESIGN_TIP_SPEED_RATIO = 5;
 ...
 */
export const REFERENCE_DESIGN_LIFT_COEFFICIENT = 0.7;

/** @description Tip radius of {@link referenceTidalRotor}, metres — 150 mm, i.e. it fits a print bed. */
export const REFERENCE_TIP_RADIUS_M = 0.15;
 ...
 */
export function referenceTidalRotor(): BladeSpec {
  return betzOptimalBlade({
    tipRadiusM: REFERENCE_TIP_RADIUS_M,
    bladeCount: 3,
    designTipSpeedRatio: REFERENCE_DESIGN_TIP_SPEED_RATIO,
    hubRadiusFrac: 0.25,
    designLiftCoefficient: REFERENCE_DESIGN_LIFT_COEFFICIENT,
    stationCount: 13,
  });
}
 ...
 */
export function referenceTidalFlow(): FlowCondition {
  return {
    freeStreamMs: 1,
    rotationRadS: (REFERENCE_DESIGN_TIP_SPEED_RATIO * 1) / REFERENCE_TIP_RADIUS_M,
    densityKgM3: SEAWATER_DENSITY_KGM3,
