...
import type { MeshValidation, MeshValidationOptions, TriMesh } from './geometry-types';
import { meshDiagonal, meshVolume } from './mesh-metrics';
import { resolveTriangle, triangleArea } from './vector-math';

const log = createChildLogger({ module: 'shared/geometry/mesh-validate' });

/** @description Relative area floor: facets below `RELATIVE_AREA_FLOOR · diagonal²` are slivers. */
const RELATIVE_AREA_FLOOR = 1e-12;

/** @description How one undirected edge is used by the facets that touch it. */
interface EdgeUse {
  /** Total facet uses. Exactly 2 on a closed manifold. */
  count: number;
  /** Uses traversing the edge from the lower index to the higher one. */
  forward: number;
}
 ...
 */
function buildEdgeCensus(mesh: TriMesh): Map<string, EdgeUse> {
  const census = new Map<string, EdgeUse>();
  for (let i = 0; i < mesh.triangles.length; i += 1) {
    const triangle = mesh.triangles[i];
    resolveTriangle(mesh, triangle, i);
    for (let e = 0; e < 3; e += 1) {
 ...
 * @throws RangeError when a facet references a vertex that does not exist.
 */
function findDegenerate(mesh: TriMesh, areaEpsilon: number): number[] {
  const found: number[] = [];
  for (let i = 0; i < mesh.triangles.length; i += 1) {
    const [a, b, c] = resolveTriangle(mesh, mesh.triangles[i], i);
    if (triangleArea(a, b, c) <= areaEpsilon) found.push(i);
  }
  return found;
}
 ...
 */
function defaultAreaEpsilon(mesh: TriMesh): number {
  if (mesh.vertices.length === 0) return 0;
  const diagonal = meshDiagonal(mesh);
 ...
 */
export function validateMesh(mesh: TriMesh, options: MeshValidationOptions = {}): MeshValidation {
  ...
  for (const triangle of mesh.triangles) {
    for (const index of triangle) referenced.add(index);
  }
  ...
  for (const use of census.values()) {
    if (use.count === 1) boundaryEdges += 1;
    else if (use.count > 2) nonManifoldEdges += 1;
    ...
    if (use.count === 2 && use.forward !== 1) consistentWinding = false;
  }
  ...
  const outwardFacing = watertight && consistentWinding && meshVolume(mesh) > 0;
  const verdict: MeshValidation = {
    watertight,
    openEdges,
