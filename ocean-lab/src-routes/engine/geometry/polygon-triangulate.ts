...
import type { Point2D, Triangle, Vec3 } from './geometry-types';
import { crossVec, dotVec, normalizeVec, subVec, vecLength } from './vector-math';

const log = createChildLogger({ module: 'shared/geometry/polygon-triangulate' });

/** @description Relative floor on a candidate ear's area, scaled by the ring's own extent. */
const EAR_AREA_FLOOR = 1e-15;

/** @description A ring projected onto its own plane, with the basis that put it there. */
interface RingPlane {
  /** The ring's points in plane coordinates, in ring order. */
  points: Point2D[];
  /** `2 · area` of the projected ring — positive by construction of the basis. */
  twiceArea: number;
}
 ...
 */
export function ringNormal(ring: ReadonlyArray<Vec3>): Vec3 {
  ...
  const helper =
    Math.abs(normal.x) <= Math.abs(normal.y) && Math.abs(normal.x) <= Math.abs(normal.z)
      ...
      : Math.abs(normal.y) <= Math.abs(normal.z)
        ? { x: 0, y: 1, z: 0 }
        : { x: 0, y: 0, z: 1 };
  const uAxis = normalizeVec(crossVec(helper, normal));
  const vAxis = crossVec(normal, uAxis);
  const origin = ring[0];
  const points = ring.map((point) => {
    const local = subVec(point, origin);
    return { x: dotVec(local, uAxis), y: dotVec(local, vAxis) };
  });
 ...
 */
function insideTriangle(point: Point2D, a: Point2D, b: Point2D, c: Point2D): boolean {
 ...
 */
function convexCandidates(
  points: ReadonlyArray<Point2D>,
  remaining: ReadonlyArray<number>,
  areaFloor: number,
): number[] {
  const count = remaining.length;
  const scored: Array<{ at: number; area: number }> = [];
  for (let at = 0; at < count; at += 1) {
    const area = cross2(
      points[remaining[(at - 1 + count) % count]],
      points[remaining[at]],
      points[remaining[(at + 1) % count]],
    );
    if (area > areaFloor) scored.push({ at, area });
  }
 ...
 */
function isClippable(points: ReadonlyArray<Point2D>, remaining: ReadonlyArray<number>, at: number): boolean {
  const count = remaining.length;
  ...
  for (let i = 0; i < count; i += 1) {
    if (i === at || i === before || i === after) continue;
    if (insideTriangle(points[remaining[i]], prev, here, next)) return false;
  }
  return true;
}
 ...
 */
function clipEars(points: ReadonlyArray<Point2D>, areaFloor: number): Triangle[] | null {
  const remaining = points.map((_, index) => index);
  const triangles: Triangle[] = [];
  while (remaining.length > 3) {
    const count = remaining.length;
    const picked = convexCandidates(points, remaining, areaFloor).find((at) =>
      isClippable(points, remaining, at),
    );
 ...
 */
export function triangulateRing(ring: ReadonlyArray<Vec3>): Triangle[] | null {
  if (ring.length < 3) {
