...
import type { LoftOptions, Section2D, SectionPlacement, Triangle, TriMesh, Vec3 } from './geometry-types';
import { flipWinding, meshVolume } from './mesh-metrics';
import { triangulateRing } from './polygon-triangulate';
import { addVec, scaleVec, vecLength, subVec } from './vector-math';

const log = createChildLogger({ module: 'shared/geometry/mesh-loft' });
 ...
 */
export function placeSection(section: Section2D, placement: SectionPlacement): Vec3[] {
  if (section.points.length === 0) throw new RangeError('placeSection requires a non-empty section');
  const uAxis = placement.uAxis ?? { x: 1, y: 0, z: 0 };
  const vAxis = placement.vAxis ?? { x: 0, y: 1, z: 0 };
  if (vecLength(uAxis) === 0 || vecLength(vAxis) === 0) {
  ...
  return section.points.map((point) => {
    ...
    return addVec(placement.origin, addVec(scaleVec(uAxis, u), scaleVec(vAxis, v)));
  });
}
 ...
 */
function dropRepeatedClose(ring: ReadonlyArray<Vec3>, epsilon: number): Vec3[] {
  const points = ring.map((point) => ({ ...point }));
  const first = points[0];
  const last = points[points.length - 1];
  if (points.length > 3 && vecLength(subVec(last, first)) <= epsilon) points.pop();
  return points;
}
...
function normalizeRings(
  sections: ReadonlyArray<ReadonlyArray<Vec3>>,
  closeRing: boolean,
  epsilon: number,
): Vec3[][] {
  ...
  rings.forEach((ring, index) => {
    if (ring.length !== expected) {
      throw new RangeError(
        `Section ${index} has ${ring.length} points but section 0 has ${expected}; a lofted strip needs matching point counts`,
      );
    }
  });
  return rings;
 ...
 */
function ringEdges(count: number, closeRing: boolean): Array<[number, number]> {
  const edges: Array<[number, number]> = [];
  const limit = closeRing ? count : count - 1;
  for (let i = 0; i < limit; i += 1) edges.push([i, (i + 1) % count]);
  return edges;
}
...
function appendCentroidFan(
  ...
  const centroid = ring.reduce((sum, point) => addVec(sum, point), { x: 0, y: 0, z: 0 });
  const centroidIndex = vertices.length;
  vertices.push(scaleVec(centroid, 1 / ring.length));
  for (let i = 0; i < ring.length; i += 1) {
    const next = (i + 1) % ring.length;
    triangles.push(
      end === 'start' ? [centroidIndex, base + next, base + i] : [centroidIndex, base + i, base + next],
    );
  }
}
...
function appendCap(
  ...
  const capped = triangulateRing(ring);
  if (!capped) {
    log.error(
      { points: ring.length, end },
      'cap ring is not a simple polygon — falling back to a centroid fan, whose facets may overlap',
    );
...
export function loftSections(
  ...
  const rings = normalizeRings(sections, closeRing, options.weldEpsilon ?? DEFAULT_WELD_EPSILON);
  const perRing = rings[0].length;
  const edges = ringEdges(perRing, closeRing);
  const vertices: Vec3[] = rings.flat();
  const triangles: Triangle[] = [];
  for (let s = 0; s + 1 < rings.length; s += 1) appendStrip(triangles, edges, s * perRing, (s + 1) * perRing);
  const lastBase = (rings.length - 1) * perRing;
  if (capStart) appendCap(vertices, triangles, rings[0], 0, 'start');
  if (capEnd) appendCap(vertices, triangles, rings[rings.length - 1], lastBase, 'end');
  const built: TriMesh = { vertices, triangles };
  const closed = closeRing && capStart && capEnd;
  const inverted = closed && meshVolume(built) < 0;
  log.debug(
    { sections: rings.length, perRing, vertices: vertices.length, triangles: triangles.length, closed, inverted },
    'lofted sections into a triangle mesh',
  );
  return inverted ? flipWinding(built) : built;
}
