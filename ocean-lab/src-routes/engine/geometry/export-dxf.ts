...
import type { DxfSection } from './geometry-types';
import { formatFixed } from './format-number';

const log = createChildLogger({ module: 'shared/geometry/export-dxf' });

/** @description R12's version string; the token importers key their parser off. */
export const DXF_R12_VERSION = 'AC1009';
 ...
 */
export function toDxfLayerName(name: string, ordinal: number): string {
  const cleaned = name.trim().toUpperCase().replace(/[^A-Z0-9_$-]+/g, '_').replace(/^_+|_+$/g, '');
  if (cleaned.length === 0) return `SECTION_${ordinal}`;
 ...
 */
function tablesSection(layers: ReadonlyArray<string>): string[] {
 ...
 */
function polylineEntity(section: DxfSection, layer: string): string[] {
  const elevation = section.elevation ?? 0;
  const lines = [
    ...group(0, 'POLYLINE'),
    ...group(8, layer),
    ...group(66, '1'),
    ...group(10, formatFixed(0)),
    ...group(20, formatFixed(0)),
    ...group(30, formatFixed(elevation)),
  ...
  for (const point of section.points) {
    lines.push(
      ...group(0, 'VERTEX'),
      ...group(8, layer),
      ...group(10, formatFixed(point.x)),
      ...group(20, formatFixed(point.y)),
      ...group(30, formatFixed(elevation)),
    );
  }
 ...
 */
export function toDxfR12(sections: ReadonlyArray<DxfSection>): string {
  ...
  const lines = [
    ...
    ...group(2, 'HEADER'),
    ...group(9, '$ACADVER'),
    ...group(1, DXF_R12_VERSION),
    ...group(9, '$INSBASE'),
    ...group(10, formatFixed(0)),
    ...group(20, formatFixed(0)),
    ...group(30, formatFixed(0)),
    ...group(0, 'ENDSEC'),
