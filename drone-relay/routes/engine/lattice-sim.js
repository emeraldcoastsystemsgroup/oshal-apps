...
exports.latticeSpacingM = latticeSpacingM;
exports.simulateLattice = simulateLattice;
...
const BASE = { x: 0, y: 0 };
const at = (d, e) => Math.hypot(d.x - e.x, d.y - e.y);
const nodeOf = (w, index) => w.geo.nodes[index - 1];
function makeDrone(id, role, state, p, slot, enduranceS) {
    const s = at(p, BASE);
...
function initDrones(w, scenario) {
    ...
    for (let i = 1; i <= Math.max(0, w.plan.sparesAvailable); i += 1)
        drones.push(makeDrone(`s${i}`, 'relay', 'base', BASE, null, w.spec.enduranceS));
    ...
    for (const ev of scenario.events) {
        if (ev.atS > w.t - dt && ev.atS <= w.t) {
            ...
            if (d && d.state !== 'failed') {
                ...
                log(w, 'fail', `${d.id} failed ${Math.round(d.s)} m from the base`, d.id);
            }
        }
    }
}
function tip(w) {
    return w.drones.find((d) => d.role === 'tip');
}
function activeRelays(w) {
...
function reachFrom(w, moved) {
    const edge = w.plan.hardRangeM;
    const pos = (d) => (moved && moved.d === d ? moved : d);
    const relays = activeRelays(w);
    const parent = new Map();
    const queue = [{ id: 'base', p: BASE }];
    for (let q = 0; q < queue.length; q += 1) {
        for (const d of relays) {
            if (parent.has(d.id) || at(pos(d), queue[q].p) > edge)
                continue;
...
function tipLink(w, parent, moved) {
    const t = tip(w);
    let best = at(t, BASE) <= w.plan.hardRangeM ? { id: 'base', m: at(t, BASE) } : null;
    for (const d of activeRelays(w)) {
        if (!parent.has(d.id))
            continue;
        const m = at(moved && moved.d === d ? moved : d, t);
        if (m <= w.plan.hardRangeM && (!best || m < best.m))
            best = { id: d.id, m };
    }
    return best;
}
 ...
 */
function rank(w, d) {
    return at(d, BASE) <= w.geo.spacingM / 2 ? 0 : w.geo.nodes[(0, lattice_1.nearestNode)(w.geo, d) - 1].depth;
}
/**
...
function routeTree(w) {
    ...
    for (;;) {
        ...
        for (const d of relays) {
            ...
            if (!tree.has(d.id) && b.m <= edge && (!pick || b.m < best.get(pick.id).m - 1e-9))
                pick = d;
        }
        if (!pick)
            return tree;
        tree.set(pick.id, best.get(pick.id));
        for (const d of relays) {
            const m = at(d, pick);
            if (!tree.has(d.id) && m < best.get(d.id).m - 1e-9)
                best.set(d.id, { from: pick.id, m });
        }
    }
}
...
function linkPass(w) {
    ...
    const note = (from, to, m) => {
        const margin = (0, chain_1.hopMarginDb)(w.spec, w.plan, m);
...
function controllerTargets(w) {
    const list = roster(w);
    const t = tip(w);
    const wanted = (0, lattice_1.latticeAssign)(w.geo, list.length, t.reachable ? t : { x: t.lastX, y: t.lastY });
    ...
    for (const d of list) {
        ...
        if (old && old.slot) {
            out.set(d.id, old.slot);
            taken.add(old.slot);
            tired.add(old.id);
        }
    }
    for (const d of list)
        if (!out.has(d.id) && !tired.has(d.id) && d.slot && wantedSet.has(d.slot) && !taken.has(d.slot)) {
            out.set(d.id, d.slot);
            taken.add(d.slot);
        }
    const free = list.filter((d) => !out.has(d.id) && !tired.has(d.id));
    for (const slot of wanted) {
        if (taken.has(slot) || !free.length)
            continue;
        const n = nodeOf(w, slot);
        let best = 0;
        free.forEach((d, i) => { if (at({ x: d.lastX, y: d.lastY }, n) < at({ x: free[best].lastX, y: free[best].lastY }, n) - 1e-9)
            best = i; });
...
function dispatchPass(w, targets) {
    ...
    for (const d of w.drones) {
        ...
        if (slot !== undefined && at(d, nodeOf(w, slot)) <= 5) {
            old.state = 'returning';
            old.slot = null;
            w.metrics.swaps += 1;
            w.awaitingRestore = true;
            log(w, 'release', `${old.id} released home; ${d.id} holds its slot`, old.id);
            d.swapFor = null;
        }
    }
}
/** Step `d` toward a point by at most `stepM`; returns the metres moved. */
function stepToward(d, goal, stepM) {
    const m = at(d, goal);
...
function localPass(w, d, dt) {
    ...
    if (d.innerLinkOk)
        d.movedInwardM = 0;
    const action = (0, node_policy_1.decideLocal)({ innerLinkOk: d.innerLinkOk, innerLostForS: d.innerLostForS, movedInwardM: d.movedInwardM, hopM: w.geo.spacingM, remainingS: d.remainingS, returnTimeS: at(d, BASE) / w.spec.cruiseMps, reserveS: w.spec.reserveS, detectS: w.spec.detectS, rtlAfterS: w.spec.rtlAfterS });
...
function guardedMove(w, d, goal, dt) {
    ...
    const pointAtF = (k) => ({ d, x: from.x + (goal.x - from.x) * k, y: from.y + (goal.y - from.y) * k });
    const keeps = (k) => {
...
function tipPosition(w, flownM) {
    const approachM = w.approach.length > 1 ? (0, path_1.distance)(w.approach[0], w.approach[1]) : 0;
    if (flownM < approachM)
        return (0, path_1.pointAt)(w.approach, flownM);
    const loopM = w.geo.surveyM;
    return (0, path_1.pointAt)(w.loop, loopM > 0 ? (flownM - approachM) % loopM : 0);
}
function motionPass(w, targets, dt) {
    for (const d of activeRelays(w)) {
        ...
        const slot = targets.get(d.id);
        if (slot !== undefined)
            guardedMove(w, d, nodeOf(w, slot), dt);
    }
    const t = tip(w);
    if (t.state !== 'active')
        return;
...
function accountPass(w, dt) {
    ...
    if (!reach && w.awaitingGapDetect && w.outageStart !== null && w.t - w.outageStart >= w.spec.staleS) {
        ...
        log(w, 'gap-detected', 'controller: the tip is stale — the lattice is broken');
    }
    if (reach && w.outageStart !== null) {
    ...
    if (restored && w.awaitingRestore) {
        ...
        log(w, 'restored', 'every slot held and the tip in reach');
    }
}
 ...
 * @returns Metres.
 */
function latticeSpacingM(spec, plan) {
    return spec.spacingFactor * (0, transports_1.rangeAtMarginM)(plan.transport, spec.requiredMarginDb, spec.pathLossExponent);
}
function makeWorld(spec, plan, scenario) {
    const geo = (0, lattice_1.latticeGeometry)(spec.area, latticeSpacingM(spec, plan));
    const w = {
 ...
 */
function simulateLattice(spec, plan, scenario) {
...
//# sourceMappingURL=lattice-sim.js.map