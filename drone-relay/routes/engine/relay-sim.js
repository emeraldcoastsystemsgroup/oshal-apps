...
exports.SCENARIO_LIMITS = void 0;
exports.validateScenario = validateScenario;
exports.rosterIds = rosterIds;
exports.chainOrderTo = chainOrderTo;
exports.simulate = simulate;
const chain_1 = require("./chain");
const node_policy_1 = require("./node-policy");
const path_1 = require("./path");
const spec_error_1 = require("./spec-error");
const lattice_sim_1 = require("./lattice-sim");
const tree_sim_1 = require("./tree-sim");
/** @description The scenario ranges the validator enforces. */
exports.SCENARIO_LIMITS = { durationS: { min: 10, max: 7200, default: 600 }, dtS: { min: 0.1, max: 5, default: 0.5 }, sampleS: { min: 0.5, max: 60, default: 1 }, maxEvents: 32 };
/**
 ...
 */
function validateScenario(input, plan) {
    const m = input && typeof input === 'object' ? input : {};
    const L = exports.SCENARIO_LIMITS;
    const durationS = (0, spec_error_1.numberIn)(m.durationS, 'durationS', L.durationS.min, L.durationS.max, L.durationS.default);
    const dtS = (0, spec_error_1.numberIn)(m.dtS, 'dtS', L.dtS.min, L.dtS.max, L.dtS.default);
    const sampleS = (0, spec_error_1.numberIn)(m.sampleS, 'sampleS', L.sampleS.min, L.sampleS.max, L.sampleS.default);
    const startDeployed = m.startDeployed === undefined ? true : m.startDeployed === true;
    const raw = m.events === undefined ? [] : m.events;
    if (!Array.isArray(raw))
        throw new spec_error_1.SpecError('events', 'events must be a list');
    if (raw.length > L.maxEvents)
        throw new spec_error_1.SpecError('events', `at most ${L.maxEvents} events`);
    const roster = new Set(rosterIds(plan));
    const events = raw.map((e, i) => {
        const ev = e && typeof e === 'object' ? e : {};
        if (ev.kind !== 'fail')
            throw new spec_error_1.SpecError(`events[${i}].kind`, 'only "fail" events are supported');
        const drone = String(ev.drone ?? '');
        if (!roster.has(drone))
            throw new spec_error_1.SpecError(`events[${i}].drone`, `unknown drone "${drone}" (roster: ${[...roster].join(', ')})`);
        return { atS: (0, spec_error_1.numberIn)(ev.atS, `events[${i}].atS`, 0, durationS, 0), kind: 'fail', drone };
    });
 ...
 * @returns The ids.
 */
function rosterIds(plan) {
    const ids = [];
    for (let i = 1; i <= plan.relaysNeeded; i += 1)
        ids.push(`r${i}`);
    for (let i = 1; i <= plan.groundNodes.length; i += 1)
        ids.push(`g${i}`);
    for (let i = 1; i <= Math.max(0, plan.sparesAvailable); i += 1)
        ids.push(`s${i}`);
    if (plan.tree)
        plan.tree.branches.forEach((b) => ids.push(`tip${b.index}`));
    else
        ids.push('tip');
    return ids;
}
 ...
 * @returns The order.
 */
function chainOrderTo(plan, dst) {
    if (plan.lattice)
        return latticeOrderTo(plan, dst);
    if (!plan.tree)
        return [...plan.slots.map((s) => `r${s.index}`), 'tip'];
    const trunk = plan.slots.filter((s) => !s.branch).map((s) => `r${s.index}`);
    const along = (b) => [...plan.slots.filter((s) => s.branch === b).map((s) => `r${s.index}`), `tip${b}`];
...
function latticeOrderTo(plan, dst) {
    ...
    const route = [];
    for (let n = start; bySlot.has(n); n = bySlot.get(n).cell?.parent ?? 0)
        route.unshift(`r${n}`);
    return dst === 'tip' ? [...route, 'tip'] : route;
}
const LAUNCH_INTERVAL_S = 5;
const RESTORE_TOLERANCE_M = 2;
function makeDrone(id, role, state, s, enduranceS) {
...
function initDrones(spec, plan, scenario) {
    const drones = [];
    ...
    for (let i = 1; i <= Math.max(0, plan.sparesAvailable); i += 1)
        drones.push(makeDrone(`s${i}`, 'relay', 'base', 0, spec.enduranceS));
    ...
    for (const ev of scenario.events) {
        if (ev.atS > w.t - dt && ev.atS <= w.t) {
            ...
            if (d && d.state !== 'failed') {
                ...
                log(w, 'fail', `${d.id} failed at ${Math.round(d.s)} m`, d.id);
            }
        }
    }
}
...
function linkPass(w) {
    ...
    for (const d of members(w)) {
        const dist = (0, path_1.distance)((0, path_1.pointAt)(w.spec.path, prevS), (0, path_1.pointAt)(w.spec.path, d.s));
...
function tipReachable(w) {
    const tip = w.drones.find((d) => d.id === 'tip');
...
function controllerTargets(w) {
    ...
    if (tipReachable(w)) {
        ...
        w.tipTargetS = (0, chain_1.allowedTipS)(w.spec, w.plan, inChain);
        const elastic = ground.length ? (0, chain_1.elasticTargetsAround)(w.tipTargetS, k, ground) : (0, chain_1.elasticTargets)(w.tipTargetS, k);
    ...
    const inward = ground.length ? (0, chain_1.spreadAround)(anchor, connected.length, ground) : (0, chain_1.spread)(anchor, connected.length);
 ...
 */
function commandOuterSegment(w, targets, lost, anchor) {
    if (!controlDirect(w) || !lost.length)
        return;
...
function dispatchPass(w, targets) {
    ...
    for (const d of w.drones) {
        ...
        if (target !== undefined && Math.abs(d.s - target) <= 5) {
...
function localPass(w, d, dt, commanded) {
    ...
    if (action.action === 'rtl' && commanded && action.reason !== 'battery')
        return 'idle';
    if (action.action === 'shift-in' && commanded)
        return 'idle';
...
function motionPass(w, targets, dt) {
    ...
    list.forEach((d, i) => {
        ...
        if (d.role === 'ground')
            return;
        if (localPass(w, d, dt, commanded) === 'moved')
            return;
        if (!d.reachable && !commanded)
            return;
        ...
        const before = d.s;
        d.s = (0, chain_1.guardMove)(d.s + step, innerS, outerS, w.plan.hardRangeM);
...
function groundPass(w, dt) {
    for (const d of w.drones) {
        ...
        if (d.state === 'active' || d.state === 'returning')
            d.remainingS = Math.max(0, d.remainingS - dt * drawFactor(w, d));
...
function accountPass(w, hops, dt) {
    ...
    if (!reach && w.outageStart === null) {
        ...
        w.awaitingRestore = true;
        log(w, 'tip-lost', 'the tip is out of reach');
    }
    if (!reach && w.awaitingGapDetect && w.outageStart !== null && w.t - w.outageStart >= knownWindowS(w)) {
        w.awaitingGapDetect = false;
        w.metrics.gapDetectedAtS.push(round1(w.t));
        log(w, 'gap-detected', 'controller: the tip is stale — chain broken');
    }
    if (reach && w.outageStart !== null) {
        ...
        log(w, 'reconnected', 'the tip is reachable again');
    }
    const tip = w.drones.find((d) => d.id === 'tip');
    ...
    if (restored && w.awaitingRestore) {
        ...
        log(w, 'restored', 'every hop at the design spacing and the tip back on station');
    }
}
 ...
 */
function simulate(spec, plan, scenario) {
    if (!plan.feasible)
        throw new spec_error_1.SpecError('plan', `the plan is not feasible: ${plan.reasons.join('; ')}`);
    if (plan.tree && spec.branches)
        return (0, tree_sim_1.simulateTree)(spec, plan, scenario);
    if (plan.lattice && spec.area)
        return (0, lattice_sim_1.simulateLattice)(spec, plan, scenario);
    const w = {
...
//# sourceMappingURL=relay-sim.js.map