...
export function createHomeSummaryRoutes(ctx: AppContext): Router {
  ...
  router.get('/', async (req, res) => {
    ...
    if (failed) items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: 'drone-relay' });
    else if (!items.length) items.push({ text: 'No relay chains designed yet. Open the app to size one.', tone: 'neutral', fix: 'drone-relay' });
