...
import { type ChainPlan, type ChainSpec, type CourierPlan, type Slot } from './chain';
import { type SimResult } from './relay-sim';

const n0 = (v: number): string => String(Math.round(v));
const n1 = (v: number): string => (Math.round(v * 10) / 10).toFixed(1);

function budgetSection(spec: ChainSpec, plan: ChainPlan): string {
...
function controlLine(spec: ChainSpec, plan: ChainPlan): string {
  const c = plan.control;
  if (!c) return 'Control plane: in band — heartbeats and commands ride the chain, so a broken chain is also a silent tip until it closes.';
...
function postureLine(spec: ChainSpec, plan: ChainPlan): string {
  if (spec.posture !== 'perch') return 'relays hover at their slots.';
...
function courierLine(spec: ChainSpec, plan: ChainPlan, c: CourierPlan): string {
...
function treeLine(plan: ChainPlan): string {
  const t = plan.tree;
  if (!t) return '';
...
function latticeLine(plan: ChainPlan): string {
...
function tipRows(plan: ChainPlan): string[] {
  const row = (id: string, s: Slot): string => `| ${id} | ${n0(s.s)} | ${n1(s.pt.x)}, ${n1(s.pt.y)}, ${n1(s.pt.z)} |`;
  if (plan.lattice) return [row('tip (where the sweep starts)', plan.tip)];
  return plan.tree ? plan.tree.branches.map((b) => row(`tip${b.index} (branch ${b.index})`, b.tip)) : [row('tip', plan.tip)];
}

function chainSection(spec: ChainSpec, plan: ChainPlan): string {
...
function policySection(spec: ChainSpec): string {
  return [
...
function resultSection(spec: ChainSpec, sim: SimResult | null): string {
  if (!sim) return '## 4. Last run\n\nNo scenario has been run for this plan yet.';
  const m = sim.metrics;
 ...
 * @returns Markdown.
 */
export function designMarkdown(spec: ChainSpec, plan: ChainPlan, sim: SimResult | null): string {
  return [
