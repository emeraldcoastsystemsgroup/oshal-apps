...
import { type Pt, distance } from './path';
import { SpecError } from './spec-error';

/** @description What the validator and the tiling hold an area to. */
export const LATTICE_LIMITS = { minPoints: 3, maxPoints: 64, maxExtentM: 20_000, maxGridCells: 40_000, maxSlots: 256 } as const;

/** @description One lattice slot: a grid node, its hop depth from the base and the neighbour one hop nearer (0 = the base). */
export interface LatticeNode {
  ...
  depth: number;
  parent: number;
  /** A node only there to join the lattice to the base (outside the area's cover). */
  feeder: boolean;
}

/** @description The tiled area. */
export interface LatticeGeometry {
  spacingM: number;
  /** Half a cell diagonal: the farthest a point of the area is from its nearest slot. */
  coverM: number;
  nodes: LatticeNode[];
  maxDepth: number;
  areaM2: number;
  /** The tip's sweep, a closed loop (it flies back to the first point and goes round again). */
  survey: Pt[];
  surveyM: number;
}
...
export function polygonArea(poly: Pt[]): number {
  let twice = 0;
  for (let a = 0; a < poly.length; a += 1) { const p = poly[a]; const q = poly[(a + 1) % poly.length]; twice += p.x * q.y - q.x * p.y; }
 ...
 * @returns True inside.
 */
export function pointInPolygon(p: { x: number; y: number }, poly: Pt[]): boolean {
  let inside = false;
...
function segmentPointM(p: { x: number; y: number }, a: Pt, b: Pt): number {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
 ...
 * @returns Metres.
 */
export function distanceToArea(p: { x: number; y: number }, poly: Pt[]): number {
  if (pointInPolygon(p, poly)) return 0;
  let best = Number.POSITIVE_INFINITY;
  for (let a = 0; a < poly.length; a += 1) best = Math.min(best, segmentPointM(p, poly[a], poly[(a + 1) % poly.length]));
  return best;
}
...
function edgesCross(a1: Pt, a2: Pt, b1: Pt, b2: Pt): boolean {
  const d1 = cross(b1, b2, a1);
  const d2 = cross(b1, b2, a2);
  const d3 = cross(a1, a2, b1);
  const d4 = cross(a1, a2, b2);
  if (((d1 > 0 && d2 < 0) || (d1 < 0 && d2 > 0)) && ((d3 > 0 && d4 < 0) || (d3 < 0 && d4 > 0))) return true;
  const on = (p: Pt, q: Pt, r: Pt): boolean => Math.abs(cross(p, q, r)) < EPS && Math.min(p.x, q.x) - EPS <= r.x && r.x <= Math.max(p.x, q.x) + EPS && Math.min(p.y, q.y) - EPS <= r.y && r.y <= Math.max(p.y, q.y) + EPS;
 ...
 */
export function validateArea(input: unknown): Pt[] {
  const L = LATTICE_LIMITS;
  if (!Array.isArray(input) || input.length < L.minPoints) throw new SpecError('area', `area needs at least ${L.minPoints} points: the polygon the tip surveys`);
  if (input.length > L.maxPoints) throw new SpecError('area', `area may carry at most ${L.maxPoints} points`);
  const poly = input.map((raw, i) => {
    const p = raw && typeof raw === 'object' ? (raw as Record<string, unknown>) : {};
    const pt = { x: Number(p.x), y: Number(p.y), z: 0 };
    if (![pt.x, pt.y].every(Number.isFinite)) throw new SpecError(`area[${i}]`, 'every area point needs finite x and y');
    if (Math.abs(pt.x) > 50_000 || Math.abs(pt.y) > 50_000) throw new SpecError(`area[${i}]`, 'area point is out of range');
    return pt;
  });
  poly.forEach((p, i) => { if (distance(p, poly[(i + 1) % poly.length]) < 1) throw new SpecError(`area[${(i + 1) % poly.length}]`, 'area edges must be at least one metre'); });
  for (let a = 0; a < poly.length; a += 1) {
    for (let b = a + 2; b < poly.length; b += 1) {
      if (a === 0 && b === poly.length - 1) continue;
      if (edgesCross(poly[a], poly[a + 1], poly[b], poly[(b + 1) % poly.length])) throw new SpecError('area', `the area's edges ${a}–${a + 1} and ${b}–${(b + 1) % poly.length} cross: give the polygon's points in order around it`);
    }
  }
  if (polygonArea(poly) < 1) throw new SpecError('area', 'the area encloses less than a square metre');
  const xs = poly.map((p) => p.x);
  const ys = poly.map((p) => p.y);
  if (Math.max(...xs) - Math.min(...xs) > L.maxExtentM || Math.max(...ys) - Math.min(...ys) > L.maxExtentM) throw new SpecError('area', `the area spans more than ${L.maxExtentM} m`);
  return poly;
}

/** Grid nodes whose cell touches the area: inside it, or within half a cell diagonal of an edge. */
function coveringNodes(area: Pt[], g: number): Set<string> {
  ...
  if ((i1 - i0 + 1) * (j1 - j0 + 1) > LATTICE_LIMITS.maxGridCells) throw new SpecError('area', `the area needs more than ${LATTICE_LIMITS.maxGridCells} grid cells at a ${Math.round(g)} m hop: survey a smaller area or use a longer hop`);
  const out = new Set<string>();
  for (let i = i0; i <= i1; i += 1) for (let j = j0; j <= j1; j += 1) if ((i !== 0 || j !== 0) && distanceToArea({ x: i * g, y: j * g }, area) <= r + EPS) out.add(key(i, j));
  return out;
}

/** Breadth-first over `allowed` from the base, neighbours in a fixed order; returns depth and parent per node. */
function bfs(allowed: Set<string>): Map<string, { depth: number; parent: string }> {
  const seen = new Map<string, { depth: number; parent: string }>([[key(0, 0), { depth: 0, parent: '' }]]);
  const queue = [[0, 0]];
  for (let q = 0; q < queue.length; q += 1) {
    const [i, j] = queue[q];
    const depth = (seen.get(key(i, j)) as { depth: number }).depth;
...
function feederRun(joined: Set<string>, wanted: Set<string>, box: { i0: number; i1: number; j0: number; j1: number }): string[] {
  const from = new Map<string, string>();
  const queue: Array<[number, number]> = [];
  for (const k of [...joined].sort()) { const [i, j] = k.split(',').map(Number); queue.push([i, j]); from.set(k, ''); }
  for (let q = 0; q < queue.length; q += 1) {
    const [i, j] = queue[q];
    for (const [di, dj] of STEPS) {
      const ni = i + di;
      const nj = j + dj;
      const k = key(ni, nj);
      if (ni < box.i0 || ni > box.i1 || nj < box.j0 || nj > box.j1 || from.has(k)) continue;
      from.set(k, key(i, j));
      if (wanted.has(k)) { const run: string[] = []; for (let c = from.get(k) as string; c && !joined.has(c); c = from.get(c) as string) run.push(c); return run; }
      queue.push([ni, nj]);
    }
  }
  return [];
...
function joinToBase(cover: Set<string>): Set<string> {
  ...
  for (let guard = 0; guard <= cover.size; guard += 1) {
    ...
    if (!missing.size) return feeder;
    for (const k of feederRun(joined, missing, box)) feeder.add(k);
  }
  return feeder;
}
 ...
 */
export function surveyPath(area: Pt[], laneM: number): Pt[] {
  const ys = area.map((p) => p.y);
  ...
  for (let n = 0; n < passes; n += 1) {
    ...
    xs.sort((a, b) => a - b);
    const legs: Pt[] = [];
    for (let k = 0; k + 1 < xs.length; k += 2) legs.push({ x: xs[k], y, z: 0 }, { x: xs[k + 1], y, z: 0 });
    out.push(...(n % 2 ? legs.reverse() : legs));
  }
  if (!out.length) out.push({ x: area.reduce((s, p) => s + p.x, 0) / area.length, y: area.reduce((s, p) => s + p.y, 0) / area.length, z: 0 });
  return out;
}
 ...
 * @returns The lattice.
 */
export function latticeGeometry(area: Pt[], spacingM: number): LatticeGeometry {
  const cover = coveringNodes(area, spacingM);
  const feeder = joinToBase(cover);
  const all = new Set([...cover, ...feeder]);
  if (all.size > LATTICE_LIMITS.maxSlots) throw new SpecError('area', `the area tiles into ${all.size} slots at a ${Math.round(spacingM)} m hop; at most ${LATTICE_LIMITS.maxSlots}`);
  const tree = bfs(all);
  const order = [...tree.keys()].filter((k) => k !== key(0, 0));
  const indexOf = new Map(order.map((k, n) => [k, n + 1]));
  const nodes = order.map((k, n): LatticeNode => {
    const [i, j] = k.split(',').map(Number);
    const t = tree.get(k) as { depth: number; parent: string };
    return { index: n + 1, i, j, x: i * spacingM, y: j * spacingM, depth: t.depth, parent: t.parent === key(0, 0) ? 0 : (indexOf.get(t.parent) as number), feeder: feeder.has(k) };
  });
  const survey = surveyPath(area, spacingM);
  const loop = [...survey, survey[0]];
  let surveyM = 0;
  for (let n = 1; n < loop.length; n += 1) surveyM += distance(loop[n - 1], loop[n]);
 ...
 */
export function nearestNode(geo: LatticeGeometry, p: { x: number; y: number }): number {
  let best = geo.nodes[0];
  for (const n of geo.nodes) if (Math.hypot(n.x - p.x, n.y - p.y) < Math.hypot(best.x - p.x, best.y - p.y) - EPS) best = n;
  return best.index;
}
 ...
 */
export function latticeRoute(geo: LatticeGeometry, index: number): number[] {
  const route: number[] = [];
  for (let n = index; n > 0; n = geo.nodes[n - 1].parent) route.unshift(n);
  return route;
}
 ...
 */
export function latticeAssign(geo: LatticeGeometry, k: number, tip: { x: number; y: number }): number[] {
