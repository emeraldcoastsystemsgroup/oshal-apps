...
/** One channel's watch frequency plus a few representative video titles. */
...
/** The top channels for a single calendar month — drives the rising/fading read. */
export interface MonthStat {
  month: string; // YYYY-MM
  topChannels: Array<{ channel: string; count: number }>;
}

/** Compact, prompt-sized distillation of a watch history. Stays well under a few KB. */
export interface WatchAggregate {
  ...
  recentTitles: Array<{ title: string; channel: string; time: string }>;
}

/** One normalized watch event extracted from a raw Takeout row. */
interface WatchRow {
  title: string;
  channel: string | null;
  time: string | null;
 ...
 */
function coerceEntries(raw: string | unknown): unknown[] {
 ...
 */
function extractRow(entry: unknown): WatchRow | null {
  if (!entry || typeof entry !== 'object') return null;
  const e = entry as Record<string, unknown>;
  const rawTitle = typeof e.title === 'string' ? e.title : '';
  if (!rawTitle) return null;
  const title = rawTitle.replace(/^Watched\s+/, '').trim();
  const subs = Array.isArray(e.subtitles) ? (e.subtitles as Array<Record<string, unknown>>) : [];
...
function topPairs(counts: Map<string, number>, n: number): Array<{ channel: string; count: number }> {
...
function buildMonthlyTrend(byMonth: Map<string, Map<string, number>>): MonthStat[] {
  return [...byMonth.keys()]
    .sort()
    .slice(-TREND_MONTHS)
    .map((month) => ({
      month,
      topChannels: topPairs(byMonth.get(month) as Map<string, number>, TREND_CHANNELS_PER_MONTH),
    }));
}
 ...
 */
export function parseTakeoutWatchHistory(raw: string | unknown): WatchAggregate {
  const entries = coerceEntries(raw);
  const channelCounts = new Map<string, number>();
  const channelSamples = new Map<string, string[]>();
  const byMonth = new Map<string, Map<string, number>>();
  const dated: WatchRow[] = [];
  let total = 0, from: string | null = null, to: string | null = null;

  for (const entry of entries) {
    const row = extractRow(entry);
    if (!row) continue;
    total += 1;
    if (row.time) { dated.push(row); if (!from || row.time < from) from = row.time; if (!to || row.time > to) to = row.time; }
    ...
    if (samples.length < SAMPLES_PER_CHANNEL && row.title && !samples.includes(row.title)) samples.push(row.title);
  ...
  const topChannels: ChannelStat[] = topPairs(channelCounts, TOP_CHANNELS)
  ...
  const recentTitles = dated
    ...
    .map((r) => ({ title: r.title, channel: r.channel as string, time: r.time as string }));
