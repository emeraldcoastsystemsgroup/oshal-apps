...
exports.loadShowLibrary = loadShowLibrary;
exports.createVideoPumpRoutes = createVideoPumpRoutes;
...
const series_pump_1 = require("@/app/series-pump");
const vids_node_availability_1 = require("@/app/vids-node-availability");
const logger = (0, logger_1.createChildLogger)({ module: 'video-pump-routes' });
...
function showsDir(appPackageDir) {
    const candidates = [
 ...
 */
function loadShowLibrary(dir) {
    const shows = [];
    const rejected = [];
    let files = [];
    try {
        files = fs.readdirSync(dir).filter((f) => f.endsWith('.yaml')).sort();
    }
    catch {
    ...
    for (const file of files) {
        let show;
        try {
            show = js_yaml_1.default.load(fs.readFileSync(path.join(dir, file), 'utf8'));
        }
        catch (err) {
            rejected.push({ file, why: `unreadable: ${err.message}` });
            continue;
        }
        if (!show?.slug || !show.title || !show.premise) {
            rejected.push({ file, why: 'slug, title and premise are all required' });
            continue;
        }
        const cast = Array.isArray(show.cast) ? show.cast : [];
        if (cast.length < 2) {
            rejected.push({ file, why: 'a show needs at least two characters' });
            continue;
        }
        const pointers = (0, series_pipeline_1.resolveSpeakerPointers)(cast);
        const seen = new Map();
        let clash = null;
        for (const [name, ptr] of pointers) {
            if (seen.has(ptr))
                clash = `${name} and ${seen.get(ptr)} both resolve to "the ${ptr}"`;
            seen.set(ptr, name);
        }
        if (clash) {
            rejected.push({ file, why: `${clash} — make each description's first clause end in that character's own noun` });
            continue;
        }
        shows.push(show);
    }
...
async function upsertShow(ctx, sub, show) {
    const { rows } = await ctx.pool.query(`INSERT INTO video_pump_shows
     ...
     RETURNING (created_at = updated_at) AS is_new`, [sub, show.slug, show.title, show.premise, show.styleLock ?? null,
 ...
 */
function createVideoPumpRoutes(ctx) {
    const router = (0, express_1.Router)();
    const dir = showsDir(ctx.appPackageDir);
    ...
    router.get('/shows', async (req, res) => {
        ...
        try {
            ...
            const { rows } = await ctx.pool.query(`SELECT slug, title, enabled, standing_authorization, daily_cap, min_interval_minutes,
           ...
           FROM video_pump_shows WHERE user_sub = $1 ORDER BY slug`, [sub]);
            const enrolled = new Map(rows.map((r) => [String(r.slug), r]));
            res.json({
                ok: true,
                shows: shows.map((s) => ({
     ...
     */
    router.post('/shows/import', async (req, res) => {
        const sub = callerSub(req);
        if (!sub) {
            res.status(401).json({ error: 'not_authenticated' });
            return;
     ...
     */
    router.post('/shows/:slug/enroll', async (req, res) => {
        const sub = callerSub(req);
        if (!sub) {
            res.status(401).json({ error: 'not_authenticated' });
            return;
        ...
        if (!Number.isInteger(dailyCap) || dailyCap < 0 || dailyCap > 24) {
            res.status(400).json({ error: 'dailyCap must be an integer 0–24' });
            return;
        }
        if (!Number.isInteger(interval) || interval < 0) {
            res.status(400).json({ error: 'minIntervalMinutes must be a non-negative integer' });
            return;
        }
        try {
            const { rows } = await ctx.pool.query(`UPDATE video_pump_shows
            SET enabled = $3, standing_authorization = $4, daily_cap = $5,
                min_interval_minutes = $6, updated_at = now()
          WHERE user_sub = $1 AND slug = $2
          RETURNING slug, enabled, standing_authorization, daily_cap, min_interval_minutes`, [sub, String(req.params.slug), b.enabled !== false, Boolean(b.standingAuthorization), dailyCap, interval]);
            if (!rows.length) {
                res.status(404).json({ error: 'show not found — import the library first' });
                return;
            }
    ...
    router.post('/shows/:slug/resume', async (req, res) => {
        const sub = callerSub(req);
        if (!sub) {
            res.status(401).json({ error: 'not_authenticated' });
            return;
        ...
        try {
            const { rows } = await ctx.pool.query(`UPDATE video_pump_shows SET paused_reason = NULL, consecutive_failures = 0, updated_at = now()
    ...
    router.get('/status', async (req, res) => {
        ...
        try {
            const node = await (0, vids_node_availability_1.checkVidsNodeAvailability)(ctx.pool, { skipProbe: req.query.probe !== '1' });
            const due = await (0, series_pump_1.pickNextShow)(ctx.pool);
            const runs = await ctx.pool.query(`SELECT show_slug, episode_title, joke_seed, outcome, outcome_stage, outcome_reason,
                drive_url, duration_ms, created_at
           FROM video_pump_runs WHERE user_sub = $1 ORDER BY created_at DESC LIMIT 25`, [sub]);
            const totals = await ctx.pool.query(`SELECT outcome, count(*)::int AS n FROM video_pump_runs WHERE user_sub = $1 GROUP BY outcome`, [sub]);
            res.json({
                ok: true,
                ...
                totals: totals.rows.reduce((a, r) => { a[r.outcome] = r.n; return a; }, {}),
                runs: runs.rows,
            });
        }
        catch (err) {
    ...
    router.post('/run', async (req, res) => {
        ...
        try {
            const cycle = await (0, series_pump_1.runPumpOnce)(ctx);
...
//# sourceMappingURL=video-pump-routes.js.map