...
function createPortraitStudioRoutes(ctx) {
    ...
    router.get('/artifacts', async (req, res) => {
        ...
        try {
            const rows = await ctx.pool.query(`SELECT portrait_id FROM ps_portraits WHERE user_sub = $1 AND status = 'done'
         AND output_path IS NOT NULL ORDER BY created_at DESC, portrait_id DESC LIMIT 51 OFFSET $2`, [sub, Number(cursor)]);
    ...
    router.get('/readiness', async (req, res) => {
        ...
        try {
           ...
           FROM ps_portraits WHERE user_sub = $1`, [sub]);
