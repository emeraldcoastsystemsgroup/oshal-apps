...
exports.portraitActor = portraitActor;
exports.requirePortraitPermission = requirePortraitPermission;
exports.registerPortraitAuthorization = registerPortraitAuthorization;
exports.createPortraitAuthorizationRoutes = createPortraitAuthorizationRoutes;
/**
 * CHANGE LOG
 * -----------------------------------------------------------------------------
...
const ACTIONS = ['view', 'read', 'create', 'change', 'delete', 'export', 'email', 'artist'];
 ...
 */
function portraitActor(ctx) {
    const actor = ctx.authorization?.currentActor();
    if (!actor?.isActive || !actor.sub || !actor.issuer)
        throw Object.assign(new Error('portrait_verified_identity_required'), { status: 401 });
    return actor;
}
 ...
 */
async function requirePortraitPermission(ctx, action, resourceId) {
    portraitActor(ctx);
    for (const permission of new Set(['portrait.view', `portrait.${action}`])) {
        ...
        if (!decision.allowed)
            throw Object.assign(new Error('portrait_permission_denied'), { status: 403 });
    }
}
 ...
 */
function registerPortraitAuthorization(ctx) {
    if (!ctx.authorization)
        throw new Error('Portrait Studio requires application-authorization');
    ctx.authorization.registerResource('portraits', { authorize: async ({ actor, operation, grant }) => {
            if (!actor.isActive || !actor.issuer || !actor.sub || grant.scope !== 'own' || operation.tenantId)
                return false;
            const id = operation.resourceId ?? /^\/portraits\/([^/]+)(?:\/|$)/.exec(operation.path ?? '')?.[1];
            if (!id)
                return true; // Collections below always constrain their SQL to this exact principal.
            if (!UUID.test(id))
                return false;
            const found = await ctx.pool.query('SELECT portrait_id FROM ps_portraits WHERE portrait_id = $1 AND user_sub = $2 AND owner_issuer = $3', [id, actor.sub, actor.issuer]);
...
function createPortraitAuthorizationRoutes(ctx) {
    ...
    router.patch('/portraits/:id', (0, express_1.json)({ limit: '4kb' }), async (req, res) => {
        try {
            ...
            if (typeof title !== 'string' || title.trim().length > 120 || Object.keys(req.body).some(key => key !== 'title')) {
                res.status(400).json({ error: 'Only a title of at most 120 characters may be changed' });
                return;
            }
            const result = await ctx.pool.query('UPDATE ps_portraits SET title = $4, updated_at = NOW() WHERE portrait_id = $1 AND user_sub = $2 AND owner_issuer = $3 RETURNING portrait_id, title', [req.params.id, actor.sub, actor.issuer, title.trim()]);
            if (!result.rows.length) {
                res.status(404).json({ error: 'portrait_not_found' });
                return;
            }
            await requirePortraitPermission(ctx, 'change', req.params.id);
