...
import { withRetries, isTransientVendorError, withTimeout, Semaphore, passportSize, isValidEmailAddress, PASSPORT_SIZES } from './portrait-ops';
...
const generationSlots = new Semaphore(Math.max(1, parseInt(process.env.PORTRAIT_STUDIO_MAX_CONCURRENT || '4', 10) || 4));
 ...
 */
async function ensureSchema(ctx: AppContext): Promise<void> {
 ...
 */
async function sweepStuckRows(ctx: AppContext, force = false): Promise<number> {
  const now = Date.now();
  if (!force && now - lastSweepMs < 60_000) return 0;
  lastSweepMs = now;
  try {
    const r = await ctx.pool.query(
      `UPDATE ps_portraits SET status = 'failed',
    ...
    if (r.rowCount) logger.warn({ swept: r.rowCount }, 'swept stuck portrait rows to failed');
    return r.rowCount ?? 0;
  } catch (err) {
 ...
 */
async function failRow(ctx: AppContext, id: string, message: string): Promise<void> {
  try {
    await ctx.pool.query(
      `UPDATE ps_portraits SET status = 'failed', error = $2, updated_at = NOW() WHERE portrait_id = $1`,
 ...
 */
async function runGeneration(ctx: AppContext, id: string, sub: string, prompt: string, source: Buffer): Promise<void> {
  ...
  try {
    ...
    const attempt = (): Promise<StoryboardImageResult> => withTimeout(
      provider.generateWithMeta
        ...
        : provider.generate(prompt, source).then((image) => ({ image, costUsd: null, model: `${provider.id}-default` })),
      VENDOR_TIMEOUT_MS,
      'image generation',
    );
    ...
    await ctx.pool.query(
      ...
      [id, outPath, `${provider.id}:${result.model}`, result.costUsd],
    );
 ...
 */
function sendImage(res: Response, filePath: string, download: boolean): void {
 ...
 */
async function passportCrop(filePath: string, size: number): Promise<Buffer> {
  return sharp(filePath).resize(size, size, { fit: 'cover', position: 'attention' }).png().toBuffer();
}
...
export function createPortraitStudioRoutes(ctx: AppContext): Router {
  ...
  router.get('/portraits/:id/export', async (req, res) => {
    try {
      const sub = callerSub(req);
      if (!sub) { res.status(401).json({ error: 'unauthenticated' }); return; }
      const size = passportSize(req.query.size);
      if (!size) { res.status(400).json({ error: `size must be one of: ${PASSPORT_SIZES.join(', ')}` }); return; }
      const row = await ownedRow(ctx, req.params.id, sub);
      if (!row || row.status !== 'done') { res.status(404).json({ error: 'portrait not found' }); return; }
  ...
  router.post('/portraits/:id/email', async (req, res) => {
    try {
      ...
      const size = wantsResize ? passportSize(body.size) : null;
      if (wantsResize && !size) {
        res.status(400).json({ error: `size must be one of: ${PASSPORT_SIZES.join(', ')} — or omitted to send the original` });
        return;
      }
      const row = await ownedRow(ctx, req.params.id, sub);
      if (!row || row.status !== 'done') { res.status(404).json({ error: 'portrait not found' }); return; }
