 ...
 */
async function formatCrop(filePath: string, width: number, height: number): Promise<Buffer> {
  return sharp(filePath).resize(width, height, { fit: 'cover', position: 'attention' }).png().toBuffer();
}
