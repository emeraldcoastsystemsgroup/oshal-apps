...
import { resolveSkillProfileByApp, composeSkillProfilePrompt } from '@/shared/skill-profiles';
...
import { sendGmail, summarizeGmailMetadata } from '@/app/routes/email-routes';
import { confirmationRequiredPayload, hasExplicitWriteConfirmation } from '@/shared/security/explicit-write-confirmation';
import { decryptSessionValue, encryptSessionValue, isSessionSecretRequiredError } from './session-crypto';

/** Load-time-only fallback for frameworks predating ctx.appPackageDir (D10). */
const LOAD_TIME_PACKAGE_DIR = process.env.OSHAL_APP_PACKAGE_DIR || '';

const logger = createChildLogger({ module: 'email-app-routes' });
...
async function resolveToken(req: Request, res: Response, ctx: AppContext): Promise<string | null> {
...
function extractBody(payload: unknown): string {
  ...
  if (p.mimeType === 'text/html' && p.body?.data) return stripHtml(b64url(p.body.data));
  return '';
}
...
async function eventsToday(token: string): Promise<Array<{ summary: string; start: string; location: string }>> {
  const now = new Date();
  ...
  return ((data.items as Array<{ summary?: string; start?: { dateTime?: string; date?: string }; location?: string }>) || []).map((e) => ({
...
function topSenders(msgs: MailSummary[]): Array<{ name: string; count: number }> {
...
function digestText(msgs: MailSummary[], events: Array<{ summary: string; start: string; location: string }>): string {
  ...
  for (const e of events) lines.push(`- ${e.start} ${e.summary}${e.location ? ` @ ${e.location}` : ''}`);
  return lines.join('\n');
}
...
async function runOnBot(ctx: AppContext, kind: string, sub: string, prompt: string): Promise<string> {
  ...
  const text =
    kind === 'summary'
      ? composeSkillProfilePrompt(prompt, 'summarize', resolveSkillProfileByApp('email-summarizer', 'summarize'))
      : prompt;
  const result = await executeBotOrInline(ctx, botClient, EMAIL_BOT_AGENT_ID, {
    text,
 ...
 */
export async function ensureEmailSchema(pool: AppContext['pool']): Promise<void> {
  await runRuntimeSchemaBootstrap({
    ...
    statements: [
      ...
      ...buildOwnerRlsPolicyStatements('oshal_email_digests', 'user_sub'),
    ],
    requirements: [
...
async function storeDigest(pool: AppContext['pool'], sub: string, summary: string): Promise<void> {
  await pool.query(
    `INSERT INTO oshal_email_digests (user_sub, summary, updated_at) VALUES ($1, $2, now())
     ON CONFLICT (user_sub) DO UPDATE SET summary = $2, updated_at = now()`,
    [sub, encryptSessionValue(summary)],
  );
}

/** Read the cached digest for a user, or null. */
async function readDigest(pool: AppContext['pool'], sub: string): Promise<{ summary: string; updatedAt: string } | null> {
  const row = (await pool.query('SELECT summary, updated_at FROM oshal_email_digests WHERE user_sub = $1', [sub])).rows[0];
  if (!row?.summary) return null;
  try {
    return { summary: decryptSessionValue(row.summary), updatedAt: row.updated_at };
  } catch (err) {
    if (isSessionSecretRequiredError(err)) throw err;
 ...
 */
export function createEmailRoutes(ctx: AppContext): Router {
  const router = Router();
  ...
  router.get('/message/:id', async (req, res) => {
    ...
    try {
      ...
      const metadata = summarizeGmailMetadata(req.params.id, m);
      res.json({
        ...metadata,
        to: header(m, 'To'),
  ...
  router.post('/draft', async (req, res) => {
    ...
    const tone = (req.body as { tone?: string })?.tone || 'professional and concise';
    if (!messageId) {
  ...
  router.post('/send', async (req, res) => {
    if (!hasExplicitWriteConfirmation(req.body)) { res.status(428).json(confirmationRequiredPayload('no-send', 'Sending email')); return; }
    const token = await resolveToken(req, res, ctx);
    if (!token) return;
    const b = (req.body || {}) as { to?: string; subject?: string; body?: string; confirm?: boolean; attachment?: { filename: string; contentBase64: string; mimeType?: string } };
    const subject = String(b.subject || '').trim();
    const bodyText = String(b.body || '');
    if (!subject && !bodyText) { res.status(400).json({ error: 'subject or body required' }); return; }
    try {
      ...
      if (!to) { res.status(400).json({ error: 'no recipient and could not resolve your own address' }); return; }
      const sent = await sendGmail(token, { to, subject: subject || '(no subject)', body: bodyText, attachment: b.attachment });
  ...
  router.get('/social/profile', async (req, res) => {
    ...
    const token = await getValidAccessToken(ctx.pool, sub, 'facebook');
    if (!token) {
      if (req.query.surface === '1') {
    ...
    try {
      ...
      const me = (await r.json()) as { id?: string; name?: string; picture?: { data?: { url?: string } } };
