...
import type { PumpkinPreset } from './pumpkin-engine-types';

/** Built-in preset names are reserved — a user "save" of one of these clones to a new slug. */
export const BUILTIN_PRESET_NAMES = ['inflatable', 'screen', 'classic', 'friendly', 'evil'] as const;
 ...
 */
export const BUILTIN_PRESETS: Record<string, PumpkinPreset> = {
  inflatable: {
    name: 'inflatable',
...
function color(v: unknown, dflt: string): string {
 ...
 */
export function normalizePreset(raw: unknown, name: string): PumpkinPreset {
  const r = (raw ?? {}) as Record<string, any>;
  const base = BUILTIN_PRESETS.screen;
  const c = r.colors ?? {};
  const f = r.face ?? {};
  const m = r.motion ?? {};
  const g = r.glow ?? {};
...
export function builtinPreset(name: string): PumpkinPreset | null {
