...
export const ROOM_TTL_MS = 90 * 1000;
 ...
 */
export const ROOM_MAX_IDLE_MS = 15 * 60 * 1000;
...
/**
 * A room listener (one SSE transport). Returning EXACTLY `false` means "this transport is dead, drop
 * me" — the only reliable signal available, because writing to a destroyed socket does not throw.
 * Returning `undefined` means healthy (do NOT treat falsy as dead).
 */
export type PumpkinRoomListener = (evt: PumpkinEvent) => boolean | void;
...
interface PumpkinRoom {
  ...
  token: string;
  lastSeen: number;
  listeners: Set<PumpkinRoomListener>;
}
...
export interface PumpkinRoomInfo {
  ...
  label: string;
  live: boolean;
  /** How many SSE transports are attached RIGHT NOW. 0 = registered but nobody is watching. */
  listeners: number;
}

/** The outcome of resolving which room(s) a headless caller meant. */
export type SpeakTargetResolution =
  | { ok: true; targets: PumpkinRoomInfo[] }
...
export class PumpkinRoomRegistry {
  ...
  private readonly sweepTimer = setInterval(() => this.sweep(), SWEEP_INTERVAL_MS);
   ...
   */
  private fanout(entry: PumpkinRoom, evt: PumpkinEvent): number {
    const dead: PumpkinRoomListener[] = [];
    let delivered = 0;
    for (const l of entry.listeners) {
      try {
        if (l(evt) === false) dead.push(l);
        else delivered += 1;
      } catch (err) {
    ...
    for (const l of dead) entry.listeners.delete(l);
    return delivered;
  }

  /** Tell a room's listeners it is gone, then drop it. A client MUST re-register, not reconnect. */
  private evict(k: string, entry: PumpkinRoom): void {
    for (const l of entry.listeners) {
      try {
        l({ type: 'evicted' });
      } catch (err) {
  ...
  private sweep(): void {
    ...
    for (const [k, v] of this.rooms) {
      ...
      if (idle > ROOM_MAX_IDLE_MS) this.evict(k, v);
    }
  }
   ...
   */
  has(sub: string, room: string): boolean {
   ...
   */
  subscribe(sub: string, room: string, listener: PumpkinRoomListener): (() => void) | null {
    const entry = this.rooms.get(key(sub, roomSlug(room)));
   ...
   */
  pushTrusted(sub: string, room: string, evt: PumpkinEvent): number {
    const entry = this.rooms.get(key(sub, roomSlug(room)));
    if (!entry) return -1;
    return this.fanout(entry, evt);
  }
 ...
 */
export function resolveSpeakTargets(sub: string, requested?: string): SpeakTargetResolution {
  const rooms = pumpkinRooms.list(sub);
  const live = rooms.filter((r) => r.live);
  const want = String(requested ?? '').trim();
  if (want === '*') {
    if (live.length === 0) return { ok: false, status: 409, body: { error: 'no_live_pumpkin', rooms: [] } };
  ...
  if (want) {
    ...
    if (!hit) return { ok: false, status: 404, body: { error: 'unknown_room', room: want, rooms } };
    return { ok: true, targets: [hit] };
  }
  if (live.length === 0) return { ok: false, status: 409, body: { error: 'no_live_pumpkin', rooms: [] } };
  if (live.length === 1) return { ok: true, targets: live };
  return {
    ok: false,
    status: 409,
