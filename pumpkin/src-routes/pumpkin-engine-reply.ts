...
import type { PumpkinExpression, PumpkinReply } from './pumpkin-engine-types';
 ...
 */
function capSpoken(raw: string): string {
  ...
  if (sentEnd >= MAX_SPOKEN * 0.5) return head.slice(0, sentEnd + 1).trim();
 ...
 */
export function parsePumpkinReply(raw: string): PumpkinReply {
  const text = String(raw || '').trim();
  const block = firstJsonObject(text);
  let say = '';
  let expression: PumpkinExpression = 'neutral';
  let intensity = 0.5;
  if (block) {
    try {
      const obj = JSON.parse(block) as Record<string, unknown>;
      if (typeof obj.say === 'string') say = obj.say.trim();
      if (typeof obj.expression === 'string' && EXPRESSIONS.includes(obj.expression as PumpkinExpression)) {
        expression = obj.expression as PumpkinExpression;
      }
  ...
  if (!say) say = block ? FALLBACK_LINE : text || FALLBACK_LINE;
