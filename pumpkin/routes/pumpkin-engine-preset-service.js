...
const pumpkin_engine_presets_1 = require("./pumpkin-engine-presets");
const logger = (0, logger_1.createChildLogger)({ module: 'pumpkin-preset-service' });
...
class PumpkinPresetService {
     ...
     */
    async listPresets(sub) {
        const builtins = Object.values(pumpkin_engine_presets_1.BUILTIN_PRESETS).map((p) => JSON.parse(JSON.stringify(p)));
        try {
            const r = await this.pool.query(`SELECT config FROM pumpkin_presets WHERE user_sub = $1 ORDER BY name`, [sub]);
            const customs = r.rows.map((row) => row.config);
     ...
     */
    async getPreset(sub, name) {
        const builtin = (0, pumpkin_engine_presets_1.builtinPreset)(name);
        if (builtin)
            return builtin;
        try {
     ...
     */
    async savePreset(sub, name, raw) {
        let target = slug(name);
        if (!target)
            target = 'custom';
        if (pumpkin_engine_presets_1.BUILTIN_PRESETS[target])
            target = `${target}-custom`;
        const preset = (0, pumpkin_engine_presets_1.normalizePreset)(raw, target);
        try {
            await this.pool.query(`INSERT INTO pumpkin_presets (user_sub, name, config) VALUES ($1, $2, $3)
         ON CONFLICT (user_sub, name) DO UPDATE SET config = EXCLUDED.config, updated_at = NOW()`, [sub, target, JSON.stringify(preset)]);
        }
        catch (err) {
...
exports.PumpkinPresetService = PumpkinPresetService;
//# sourceMappingURL=pumpkin-engine-preset-service.js.map