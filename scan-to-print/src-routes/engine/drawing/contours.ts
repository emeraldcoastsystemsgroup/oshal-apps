...
import { type Mask, maskAt } from '../raster/raster-types';

/** @description A directed boundary edge between two pixel-corner points. */
interface Edge { from: Point2D; to: Point2D; used: boolean }

/** @description Map key for a corner point. */
const keyOf = (p: Point2D): string => `${p.x},${p.y}`;
 ...
 */
function boundaryEdges(mask: Mask): Edge[] {
  const edges: Edge[] = [];
  for (let y = 0; y < mask.height; y += 1) {
    for (let x = 0; x < mask.width; x += 1) {
      if (maskAt(mask, x, y) === 0) continue;
      if (maskAt(mask, x, y - 1) === 0) edges.push({ from: { x, y }, to: { x: x + 1, y }, used: false });
      if (maskAt(mask, x + 1, y) === 0) edges.push({ from: { x: x + 1, y }, to: { x: x + 1, y: y + 1 }, used: false });
      if (maskAt(mask, x, y + 1) === 0) edges.push({ from: { x: x + 1, y: y + 1 }, to: { x, y: y + 1 }, used: false });
      if (maskAt(mask, x - 1, y) === 0) edges.push({ from: { x, y: y + 1 }, to: { x, y }, used: false });
    }
  }
  return edges;
}
...
function turn(a: Point2D, b: Point2D): number {
 ...
 */
function pickNext(candidates: Edge[], heading: Point2D): Edge {
...
function simplifyLoop(points: Point2D[]): Point2D[] {
  const out: Point2D[] = [];
 ...
 */
export function traceContours(mask: Mask): Point2D[][] {
  const edges = boundaryEdges(mask);
  const outgoing = new Map<string, Edge[]>();
  for (const edge of edges) {
    const key = keyOf(edge.from);
    const list = outgoing.get(key);
    if (list) list.push(edge); else outgoing.set(key, [edge]);
  }
  const loops: Point2D[][] = [];
 ...
 */
export function loopsToPath(loops: Point2D[][], scale: number, offset: Point2D, decimals = 3): string {
