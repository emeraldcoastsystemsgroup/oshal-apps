...
import { type Mask, type MaskStats, createMask, maskStats } from './raster-types';
import type { Raster } from './raster-types';

/** @description Knobs for {@link extractSilhouette}. Defaults suit a phone photo on a plain surface. */
export interface SilhouetteOptions {
  /** Fraction of the shorter side sampled as the background frame. Default 0.03. */
  borderFraction?: number;
  /** Floor on the Otsu threshold (0..255 colour-distance units) so a flat image never flips. Default 14. */
  minThreshold?: number;
  /** Morphological open/close radius in pixels. 0 disables. Default 1. */
  morphologyRadius?: number;
  /** Fill enclosed background pockets — a silhouette is a solid outline. Default true. */
  fillHoles?: boolean;
}

/** @description What the extractor decided and why, so the UI can show the person the evidence. */
export interface SilhouetteResult {
  /** The object mask, one byte per pixel. */
  mask: Mask;
  /** Colour-distance threshold actually used (0..255). */
  threshold: number;
  /** Estimated background colour. */
  background: { r: number; g: number; b: number };
  /** Count, coverage and bounds of the mask. */
  stats: MaskStats;
  /** Human-readable capture problems; empty when the photo looks usable. */
  warnings: string[];
}
 ...
 */
export function estimateBackground(raster: Raster, borderFraction: number): { r: number; g: number; b: number } {
  ...
  for (let y = 0; y < raster.height; y += 1) {
    ...
    for (let x = 0; x < raster.width; x += 1) {
      if (!onRowFrame && x >= frame && x < raster.width - frame) continue;
 ...
 */
export function distanceMap(raster: Raster, background: { r: number; g: number; b: number }): Uint8Array {
 ...
 */
function morph(mask: Mask, dilate: boolean): Mask {
    ...
    for (let x = 0; x < mask.width; x += 1) {
      ...
      for (let dy = -1; dy <= 1 && (dilate ? hit === 0 : hit === 1); dy += 1) {
        for (let dx = -1; dx <= 1; dx += 1) {
          ...
          if (dilate && v === 1) { hit = 1; break; }
          if (!dilate && v === 0) { hit = 0; break; }
        }
      }
 ...
 */
export function openClose(mask: Mask, radius: number): Mask {
  let current = mask;
  for (let i = 0; i < radius; i += 1) current = morph(current, false);
  for (let i = 0; i < radius; i += 1) current = morph(current, true);
  for (let i = 0; i < radius; i += 1) current = morph(current, true);
  for (let i = 0; i < radius; i += 1) current = morph(current, false);
  return current;
}
 ...
 */
export function labelComponents(mask: Mask): { labels: Int32Array; sizes: number[] } {
  ...
  for (let start = 0; start < labels.length; start += 1) {
    ...
    while (head < tail) {
      ...
      for (const n of neighbours) {
        if (n >= 0 && mask.data[n] === 1 && labels[n] === 0) { labels[n] = id; queue[tail++] = n; }
      }
    }
  }
  return { labels, sizes };
...
export function largestComponent(mask: Mask): Mask {
  ...
  let keep = 0;
  for (let id = 1; id < sizes.length; id += 1) if (sizes[id] > sizes[keep]) keep = id;
  const out = createMask(mask.width, mask.height);
  if (keep === 0) return out;
  for (let i = 0; i < labels.length; i += 1) out.data[i] = labels[i] === keep ? 1 : 0;
  return out;
}
...
export function fillHoles(mask: Mask): Mask {
  ...
  const seed = (at: number): void => {
    if (mask.data[at] === 0 && reached[at] === 0) { reached[at] = 1; queue[tail++] = at; }
  };
  for (let x = 0; x < mask.width; x += 1) { seed(x); seed((mask.height - 1) * mask.width + x); }
  for (let y = 0; y < mask.height; y += 1) { seed(y * mask.width); seed(y * mask.width + mask.width - 1); }
  while (head < tail) {
    const at = queue[head++];
    const x = at % mask.width;
    const y = (at - x) / mask.width;
    if (x > 0) seed(at - 1);
    if (x < mask.width - 1) seed(at + 1);
    if (y > 0) seed(at - mask.width);
    if (y < mask.height - 1) seed(at + mask.width);
  }
  const out = createMask(mask.width, mask.height);
  for (let i = 0; i < out.data.length; i += 1) out.data[i] = mask.data[i] === 1 || reached[i] === 0 ? 1 : 0;
  return out;
}

/** @description Capture problems worth telling the person about, derived from the mask alone. */
function captureWarnings(stats: MaskStats, mask: Mask, threshold: number, minThreshold: number): string[] {
  const warnings: string[] = [];
  if (stats.pixels === 0) {
    warnings.push('No object found: the photo looks uniform. Use a plain background that contrasts with the object.');
    return warnings;
  }
  if (threshold <= minThreshold) warnings.push('Low contrast between object and background; the outline may be unreliable.');
  if (stats.coverage < 0.02) warnings.push('The object is very small in the frame; move closer so it fills more of the photo.');
  if (stats.coverage > 0.9) warnings.push('The object fills almost the whole frame; the background estimate is unreliable.');
 ...
 */
export function extractSilhouette(raster: Raster, options: SilhouetteOptions = {}): SilhouetteResult {
  ...
  let mask = createMask(raster.width, raster.height);
  for (let i = 0; i < distances.length; i += 1) mask.data[i] = distances[i] > threshold ? 1 : 0;
  if (radius > 0) mask = openClose(mask, radius);
  mask = largestComponent(mask);
  if (options.fillHoles ?? true) mask = fillHoles(mask);
