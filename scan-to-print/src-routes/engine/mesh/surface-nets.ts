...
import type { TriMesh, Triangle, Vec3 } from '../geometry/geometry-types';
import { type OccupancyGrid, gridIndex, voxelCenter } from '../grid/occupancy-grid';

/** @description Mesher knobs. */
export interface SurfaceNetsOptions {
  /** Laplacian relaxation passes. Default 2. 0 keeps the raw dual vertices. */
  smoothIterations?: number;
  /** Relaxation step, 0..1. Default 0.5. */
  smoothLambda?: number;
}

/** @description Per-cell scratch: dual vertex index and the cell's clamp box. */
interface CellRecord { vertex: number; lo: Vec3; hi: Vec3 }

/** @description The three cyclic axis orders (a, b, c) with unit index strides for cells. */
const AXES: ReadonlyArray<readonly [0 | 1 | 2, 0 | 1 | 2, 0 | 1 | 2]> = [[0, 1, 2], [1, 2, 0], [2, 0, 1]];
...
function cellIndex(grid: OccupancyGrid, i: number, j: number, k: number): number {
  return i + (grid.nx - 1) * (j + (grid.ny - 1) * k);
}

/** @description Place the dual vertex of one mixed cell; returns null for a uniform cell. */
function cellVertex(grid: OccupancyGrid, i: number, j: number, k: number): { p: Vec3; lo: Vec3; hi: Vec3 } | null {
  const occ = CORNERS.map(([di, dj, dk]) => grid.data[gridIndex(grid, i + di, j + dj, k + dk)]);
  let solid = 0;
  for (const v of occ) solid += v;
  if (solid === 0 || solid === 8) return null;
  const pos = CORNERS.map(([di, dj, dk]) => voxelCenter(grid, i + di, j + dj, k + dk));
  const sum = { x: 0, y: 0, z: 0 };
  let crossings = 0;
  for (const [a, b] of EDGES) {
    if (occ[a] === occ[b]) continue;
    sum.x += (pos[a].x + pos[b].x) / 2; sum.y += (pos[a].y + pos[b].y) / 2; sum.z += (pos[a].z + pos[b].z) / 2;
    crossings += 1;
  }
...
function placeVertices(grid: OccupancyGrid, cells: Int32Array, vertices: Vec3[], boxes: CellRecord[]): void {
...
function quadCells(grid: OccupancyGrid, cells: Int32Array, ijk: [number, number, number], order: readonly [number, number, number]): [number, number, number, number] | null {
  const [, b, c] = order;
  const limit = [grid.nx - 1, grid.ny - 1, grid.nz - 1];
  const pick = (db: number, dc: number): number => {
    const q: [number, number, number] = [ijk[0], ijk[1], ijk[2]];
    q[b] -= db;
    q[c] -= dc;
    if (q[0] < 0 || q[1] < 0 || q[2] < 0 || q[0] >= limit[0] || q[1] >= limit[1] || q[2] >= limit[2]) return -1;
    return cells[cellIndex(grid, q[0], q[1], q[2])];
  };
  const v = [pick(0, 0), pick(1, 0), pick(1, 1), pick(0, 1)];
  return v.some((x) => x < 0) ? null : (v as [number, number, number, number]);
}

/** @description Pass 2: one quad (two triangles) per crossing grid edge, wound solid→empty outward. */
function emitQuads(grid: OccupancyGrid, cells: Int32Array, triangles: Triangle[]): void {
    ...
    for (let j = 0; j < grid.ny; j += 1) {
      for (let i = 0; i < grid.nx; i += 1) {
        ...
        for (const order of AXES) {
          const a = order[0];
          const n = [i, j, k][a];
          if (n + 1 >= [grid.nx, grid.ny, grid.nz][a]) continue;
          const there = grid.data[at + stride[a]];
          if (here === there) continue;
          const q = quadCells(grid, cells, [i, j, k], order);
          if (!q) continue;
          const [v0, v1, v2, v3] = q;
          if (here === 1) triangles.push([v0, v1, v2], [v0, v2, v3]);
          else triangles.push([v0, v3, v2], [v0, v2, v1]);
        }
      }
    }
  }
...
function smooth(vertices: Vec3[], triangles: Triangle[], boxes: CellRecord[], iterations: number, lambda: number): void {
  const neighbours: number[][] = vertices.map(() => []);
  for (const [a, b, c] of triangles) {
    neighbours[a].push(b, c); neighbours[b].push(a, c); neighbours[c].push(a, b);
  }
  const clamp = new Map(boxes.map((r) => [r.vertex, r] as const));
  for (let it = 0; it < iterations; it += 1) {
    const next = vertices.map((v, idx) => {
      const n = neighbours[idx];
      if (n.length === 0) return v;
      const mean = { x: 0, y: 0, z: 0 };
      for (const q of n) { mean.x += vertices[q].x; mean.y += vertices[q].y; mean.z += vertices[q].z; }
      mean.x /= n.length; mean.y /= n.length; mean.z /= n.length;
      const box = clamp.get(idx) as CellRecord;
      return {
    ...
    for (let idx = 0; idx < vertices.length; idx += 1) vertices[idx] = next[idx];
  }
}
 ...
 */
export function surfaceNets(grid: OccupancyGrid, options: SurfaceNetsOptions = {}): TriMesh {
  const iterations = options.smoothIterations ?? 2;
  const lambda = options.smoothLambda ?? 0.5;
  if (!Number.isInteger(iterations) || iterations < 0 || iterations > 20) throw new RangeError('smoothIterations must be an integer 0..20');
  if (!(lambda >= 0 && lambda <= 1)) throw new RangeError('smoothLambda must be within 0..1');
  const cells = new Int32Array((grid.nx - 1) * (grid.ny - 1) * (grid.nz - 1)).fill(-1);
  const vertices: Vec3[] = [];
  const boxes: CellRecord[] = [];
  placeVertices(grid, cells, vertices, boxes);
  const triangles: Triangle[] = [];
  emitQuads(grid, cells, triangles);
  if (iterations > 0 && vertices.length > 0) smooth(vertices, triangles, boxes, iterations, lambda);
