 ...
 * -----------------------------------------------------------------------------
 * 1 | maintainer@emeraldcoastsystemsgroup.com   | Vendored verbatim from the ocean-lab geometry slice: the vector
 *                     |                             | primitives the mesher, the validator and the exporters share.
 *                     |                             | Free functions over plain objects rather than a Vector class —
 *                     |                             | every consumer is a pure transform, and a class would only add
 *                     |                             | allocation and a `this` to get wrong inside a triangle loop.
 */
...
/**
 * @description Construct a vector so call sites read as geometry (`vec3(0, 0, 1)`).
 * @param x - Cartesian X.
 * @param y - Cartesian Y.
 * @param z - Cartesian Z.
...
/**
 * @description Cross product, right-handed. The single place the winding convention is realised:
 * every outward-normal and signed-volume claim in this engine bottoms out here.
 * @param a - Left operand.
 * @param b - Right operand.
...
/**
 * @description Unit vector, or the zero vector when the input has no length. Zero rather than NaN
 * is deliberate: a zero normal is the STL convention for "derive it from the winding", so a
 * degenerate facet still writes a legal file.
...
/**
 * @description The un-normalised facet normal `(b − a) × (c − a)`.
 ...
 * goes, so an out-of-range index fails at the bad data instead of as `undefined.x` later.
