...
import { type OccupancyGrid, gridIndex } from './occupancy-grid';
import type { DepthMap } from './depth-carver';
import { type ViewName, VIEW_FRAMES, VIEW_NAMES, axisVector, isViewName, projectToView, readAxis } from './views';

/** @description Pinhole intrinsics in pixels. Pixel (col, row) has its centre at (col + 0.5, row + 0.5). */
export interface PinholeIntrinsics {
  /** Focal length along image columns, pixels. */
  fx: number;
  /** Focal length along image rows, pixels. */
  fy: number;
  /** Principal point column, pixels from the image's left edge. */
  cx: number;
  /** Principal point row, pixels from the image's top edge. */
  cy: number;
}

/**
 * @description Where the camera is and which way it faces, in the world frame. The camera frame is
 * +X image right, +Y image DOWN, +Z forward (the optical axis); ARKit-style +Y up / -Z forward
 * frames must be converted by the caller (negate the second and third columns).
 */
export interface CameraPose {
  /** Camera centre, world millimetres. */
  positionMm: Vec3;
  /** Camera-to-world rotation, row-major 3x3: its columns are the camera's X, Y, Z axes in the world. */
  rotation: readonly number[];
}

/** @description `z`: distance along the optical axis (ARKit sceneDepth, most ToF). `range`: Euclidean length of the ray. */
export type DepthKind = 'z' | 'range';

/** @description A perspective range image in millimetres; NaN or +-Infinity means no return. */
export interface PerspectiveRangeImage {
  /** Pixel columns. */
  width: number;
  /** Pixel rows. */
  height: number;
  /** The camera's pinhole model. */
  intrinsics: PinholeIntrinsics;
  /** The camera's pose in the world frame. */
  pose: CameraPose;
  /** What each value measures. */
  depthKind: DepthKind;
  /** `width x height` values, row-major, millimetres. */
  data: Float32Array;
}

/** @description Published bounds for re-projection, echoed the same way the reconstruction limits are. */
export const REPROJECTION_LIMITS = Object.freeze({
  /**
...
/** @description The orthographic map to fill: the same geometry fields a {@link DepthMap} carries. */
export interface OrthographicTarget {
  /** Canonical view to project into; default the one nearest the optical axis. */
  view?: ViewName;
  ...
  vCenterMm: number;
  /** Sensor-plane look coordinate, mm; default the camera centre's own. */
  planeMm?: number;
  /** Lean limit for this call, degrees; see {@link REPROJECTION_LIMITS}. */
  maxLeanDeg?: number;
}

/** @description What happened to every pixel, so a thin result can be explained. */
export interface ReprojectionStats {
  /** The canonical view the map is in. */
  view: ViewName;
  /** Angle between the optical axis and that view's look axis, degrees. */
  poseAngleDeg: number;
  /** Pixels with a finite return. */
  samples: number;
  /** Samples that landed in the map (before the z-buffer kept the shallowest). */
  landed: number;
  /** Samples dropped because their ray leaned past the limit. */
  leanDropped: number;
  /** Samples dropped because they lie behind the sensor plane. */
  behindPlane: number;
  /** Samples dropped because they fall outside the map. */
  outsideMap: number;
  /** Largest lean of any sample that landed, degrees. */
  maxLeanDeg: number;
}
...
function requireRotation(r: readonly number[]): void {
  if (!Array.isArray(r) || r.length !== 9 || !r.every(Number.isFinite)) throw new RangeError('pose.rotation must be nine finite numbers (row-major 3x3)');
  const col = (c: number): Vec3 => ({ x: r[c], y: r[3 + c], z: r[6 + c] });
  ...
  if (off.some((x) => Math.abs(x) > ORTHONORMAL_TOLERANCE)) throw new RangeError('pose.rotation must be a proper rotation (orthonormal columns, determinant +1)');
}

/** @description Refuse an image whose shape, intrinsics or depth kind cannot be back-projected. */
function requireImage(image: PerspectiveRangeImage): void {
  const { width, height, intrinsics: k, data } = image;
  if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0) throw new RangeError('Range image width and height must be positive integers');
  if (width * height > REPROJECTION_LIMITS.maxPixels) throw new RangeError(`Range image exceeds ${REPROJECTION_LIMITS.maxPixels} pixels`);
  if (!(data instanceof Float32Array) || data.length !== width * height) throw new RangeError('Range image data must be a Float32Array of width x height values');
  if (!k || !(k.fx > 0) || !(k.fy > 0) || !Number.isFinite(k.fx) || !Number.isFinite(k.fy)) throw new RangeError('intrinsics fx and fy must be positive numbers of pixels');
  if (!Number.isFinite(k.cx) || !Number.isFinite(k.cy)) throw new RangeError('intrinsics cx and cy must be finite pixel coordinates');
  if (image.depthKind !== 'z' && image.depthKind !== 'range') throw new RangeError('depthKind must be "z" or "range"');
  const p = image.pose?.positionMm;
  if (!p || ![p.x, p.y, p.z].every(Number.isFinite)) throw new RangeError('pose.positionMm must be a finite world point');
  requireRotation(image.pose.rotation);
}

/** @description Refuse a target map the carver could not index. */
function requireTarget(t: OrthographicTarget): number {
  if (t.view !== undefined && !isViewName(t.view)) throw new RangeError(`target view must be one of ${VIEW_NAMES.join(', ')}`);
  if (!(t.mmPerPx > 0) || !Number.isFinite(t.mmPerPx)) throw new RangeError('target mmPerPx must be a positive number');
  if (!Number.isInteger(t.width) || !Number.isInteger(t.height) || t.width <= 0 || t.height <= 0) throw new RangeError('target width and height must be positive integers');
  if (![t.uCenterPx, t.vCenterPx, t.uCenterMm, t.vCenterMm].every(Number.isFinite)) throw new RangeError('target centre pixel and centre millimetres must be finite');
  if (t.planeMm !== undefined && !Number.isFinite(t.planeMm)) throw new RangeError('target planeMm must be finite');
  const bound = REPROJECTION_LIMITS.maxLeanDeg;
  const lean = t.maxLeanDeg ?? bound.default;
  if (!Number.isFinite(lean) || lean < bound.min || lean > bound.max) throw new RangeError(`maxLeanDeg must be between ${bound.min} and ${bound.max} degrees`);
  return lean;
}

/** @description Camera vector to world: `R v`. */
function toWorld(r: readonly number[], v: Vec3): Vec3 {
 ...
 */
export function nearestCanonicalView(pose: CameraPose): { view: ViewName; angleDeg: number } {
  requireRotation(pose.rotation);
  const forward = toWorld(pose.rotation, { x: 0, y: 0, z: 1 });
  let best: ViewName = VIEW_NAMES[0];
  let bestDot = -Infinity;
  for (const name of VIEW_NAMES) {
    const look = axisVector(VIEW_FRAMES[name].look);
...
function pixelRay(k: PinholeIntrinsics, col: number, row: number): { dir: Vec3; invNorm: number } {
 ...
 */
export function reprojectToCanonicalView(image: PerspectiveRangeImage, target: OrthographicTarget): { map: DepthMap; stats: ReprojectionStats } {
  requireImage(image);
  ...
  const look = axisVector(frame.look);
  const planeMm = target.planeMm ?? readAxis(image.pose.positionMm, frame.look);
  const map: DepthMap = {
  ...
  const stats: ReprojectionStats = { view, poseAngleDeg: nearest.angleDeg, samples: 0, landed: 0, leanDropped: 0, behindPlane: 0, outsideMap: 0, maxLeanDeg: 0 };
...
function splatSample(image: PerspectiveRangeImage, map: DepthMap, stats: ReprojectionStats, s: { col: number; row: number; value: number; look: Vec3; cosLimit: number }): void {
  ...
  if (cosLean < s.cosLimit) { stats.leanDropped += 1; return; }
  const along = image.depthKind === 'z' ? s.value / invNorm : s.value;
  const p = image.pose.positionMm;
  const world: Vec3 = { x: p.x + rayWorld.x * along, y: p.y + rayWorld.y * along, z: p.z + rayWorld.z * along };
  const frame = VIEW_FRAMES[map.view];
  const depth = readAxis(world, frame.look) - map.planeMm;
  if (depth < 0) { stats.behindPlane += 1; return; }
  const { u, v } = projectToView(world, frame);
...
function solidAtWorld(grid: OccupancyGrid, p: Vec3): boolean {
 ...
 */
export function renderPerspectiveDepth(grid: OccupancyGrid, camera: Omit<PerspectiveRangeImage, 'data'>): PerspectiveRangeImage {
  const image: PerspectiveRangeImage = { ...camera, data: new Float32Array(camera.width * camera.height).fill(NaN) };
  requireImage(image);
  ...
  for (let row = 0; row < camera.height; row += 1) {
    for (let col = 0; col < camera.width; col += 1) {
      ...
      for (let s = span.enter; s <= span.exit; s += step) {
        if (!solidAtWorld(grid, { x: origin.x + w.x * s, y: origin.y + w.y * s, z: origin.z + w.z * s })) continue;
