...
import { type OccupancyGrid, createOccupancyGrid, gridIndex, MAX_VOXELS_PER_AXIS } from './occupancy-grid';

/** @description Raw vertex positions, interleaved xyz, in the file's own units. */
...
/** @description Bounds for the parser. */
export interface ParsePlyOptions {
  /** Refuse files with more vertices than this. Default 5,000,000. */
  maxPoints?: number;
}
...
/** @description One property in an element declaration. */
interface PlyProperty { name: string; type: string; list?: { countType: string; itemType: string } }
/** @description One element declaration. */
interface PlyElement { name: string; count: number; properties: PlyProperty[] }
/** @description The parsed header. */
interface PlyHeader { format: 'ascii' | 'binary_little_endian' | 'binary_big_endian'; elements: PlyElement[]; bodyOffset: number }
...
function parseHeader(bytes: Uint8Array): PlyHeader {
  ...
  if (!probe.startsWith('ply') || endAt < 0) throw new RangeError('Not a PLY file (missing ply/end_header)');
  let bodyOffset = endAt + endToken.length;
  if (probe[bodyOffset] === '\r') bodyOffset += 1;
  if (probe[bodyOffset] === '\n') bodyOffset += 1;
  const header: PlyHeader = { format: 'ascii', elements: [], bodyOffset };
  for (const raw of probe.slice(0, endAt).split(/\r?\n/)) {
    const parts = raw.trim().split(/\s+/);
    if (parts[0] === 'format') header.format = parts[1] as PlyHeader['format'];
    else if (parts[0] === 'element') header.elements.push({ name: parts[1], count: Number(parts[2]), properties: [] });
    else if (parts[0] === 'property' && header.elements.length > 0) {
      const element = header.elements[header.elements.length - 1];
      if (parts[1] === 'list') element.properties.push({ name: parts[4], type: 'list', list: { countType: parts[2], itemType: parts[3] } });
...
function readScalar(view: DataView, at: number, type: string, little: boolean): number {
  switch (type) {
...
function readBinary(bytes: Uint8Array, header: PlyHeader, out: Float32Array): void {
...
function readAscii(bytes: Uint8Array, header: PlyHeader, out: Float32Array): void {
  ...
  for (const element of header.elements) {
    ...
    for (let n = 0; n < element.count; n += 1) {
      for (const prop of element.properties) {
        if (prop.list) { at += 1 + Number(tokens[at]); continue; }
        if (isVertex && (prop.name === 'x' || prop.name === 'y' || prop.name === 'z')) {
 ...
 */
export function parsePly(bytes: Uint8Array, options: ParsePlyOptions = {}): PointCloud {
  const header = parseHeader(bytes);
  const vertex = header.elements.find((e) => e.name === 'vertex');
  if (!vertex) throw new RangeError('PLY file has no vertex element');
  const names = new Set(vertex.properties.map((p) => p.name));
  if (!names.has('x') || !names.has('y') || !names.has('z')) throw new RangeError('PLY vertex element lacks x/y/z');
  const maxPoints = options.maxPoints ?? 5_000_000;
  if (!Number.isInteger(vertex.count) || vertex.count <= 0) throw new RangeError('PLY vertex count must be a positive integer');
  if (vertex.count > maxPoints) throw new RangeError(`PLY has ${vertex.count} points; the limit is ${maxPoints}`);
  const xyz = new Float32Array(vertex.count * 3);
  if (header.format === 'ascii') readAscii(bytes, header, xyz);
  else readBinary(bytes, header, xyz);
  for (let i = 0; i < xyz.length; i += 1) if (!Number.isFinite(xyz[i])) throw new RangeError('PLY contains a non-finite coordinate');
  return { count: vertex.count, xyz };
}

/** @description How to bring a scanner's frame into the world frame. */
export interface VoxelizeOptions {
  /** Voxel edge, millimetres. */
  voxelMm: number;
  /** Multiply file units to get millimetres (1000 for a metres export). Default 1. */
  unitScale?: number;
  /** The file's up axis. `y` (ARKit / most phone exports) is rotated onto world Z. Default `z`. */
  up?: 'y' | 'z';
  /**
   * Empty shell thickness in voxels. Default 2 — it must EXCEED the closing radius passed to
   * {@link fillSolidFromSurface}: the morphology never writes the outermost layer, so a surface
   * sitting right against a one-voxel shell cannot dilate outward and the erosion pass eats it.
   */
  padding?: number;
}

/** @description The voxelised cloud and what was learned about it. */
export interface VoxelizedCloud {
  /** Surface voxels set to 1; nothing filled yet. */
  grid: OccupancyGrid;
  /** Object extent in millimetres after scaling and re-orientation. */
  sizeMm: Vec3;
...
function transformPoints(cloud: PointCloud, unitScale: number, up: 'y' | 'z'): { pts: Float32Array; min: Vec3; max: Vec3 } {
 ...
 */
export function voxelizePointCloud(cloud: PointCloud, options: VoxelizeOptions): VoxelizedCloud {
  const unitScale = options.unitScale ?? 1;
  const { pts, min, max } = transformPoints(cloud, unitScale, options.up ?? 'z');
  const sizeMm: Vec3 = { x: max.x - min.x, y: max.y - min.y, z: max.z - min.z };
  if (!(sizeMm.x > 0) || !(sizeMm.y > 0) || !(sizeMm.z > 0)) throw new RangeError('Point cloud has zero extent along an axis');
  const largest = Math.max(sizeMm.x, sizeMm.y, sizeMm.z);
  if (largest / options.voxelMm > MAX_VOXELS_PER_AXIS - 2) {
    throw new RangeError(`Voxel ${options.voxelMm} mm is too fine for a ${largest.toFixed(1)} mm object (limit ${MAX_VOXELS_PER_AXIS} per axis)`);
  }
  const padding = options.padding ?? 2;
  const grid = createOccupancyGrid(sizeMm, options.voxelMm, padding, 0);
  const shift: Vec3 = { x: -(min.x + max.x) / 2, y: -(min.y + max.y) / 2, z: -min.z };
  ...
  const clampIndex = (raw: number, count: number): number => Math.min(count - padding - 1, Math.max(padding, raw));
  let pointCount = 0;
...
function morph3(grid: OccupancyGrid, dilate: boolean): void {
...
function lowestOccupiedLayer(grid: OccupancyGrid): number {
  const layer = grid.nx * grid.ny;
  for (let k = 0; k < grid.nz; k += 1) {
    const base = k * layer;
    for (let at = base; at < base + layer; at += 1) if (grid.data[at] === 1) return k;
  }
  return -1;
}
 ...
 */
function capLayerHoles(grid: OccupancyGrid, k: number): number {
  const layer = grid.nx * grid.ny;
  const base = k * layer;
  ...
  for (let j = 0; j < grid.ny; j += 1) {
    for (let i = 0; i < grid.nx; i += 1) {
      if (i !== 0 && i !== grid.nx - 1 && j !== 0 && j !== grid.ny - 1) continue;
      const at = i + grid.nx * j;
      if (grid.data[base + at] === 0 && reached[at] === 0) { reached[at] = 1; queue[tail++] = at; }
    }
  }
  while (head < tail) {
    const at = queue[head++];
    ...
    const steps: Array<[boolean, number]> = [[i < grid.nx - 1, 1], [i > 0, -1], [j < grid.ny - 1, grid.nx], [j > 0, -grid.nx]];
    for (const [open, step] of steps) {
      if (!open) continue;
      const q = at + step;
      if (grid.data[base + q] === 0 && reached[q] === 0) { reached[q] = 1; queue[tail++] = q; }
    }
  }
  let capped = 0;
  for (let at = 0; at < layer; at += 1) {
    if (grid.data[base + at] === 0 && reached[at] === 0) { grid.data[base + at] = 1; capped += 1; }
  }
  return capped;
}

/** @description Caller assertions about the scan that change how the solid is inferred. */
export interface FillSolidOptions {
  /**
   * Treat the object's lowest occupied layer as resting on a solid bed plane and cap its holes
   * before the flood. Use it for a one-sided capture (phone LiDAR of an object on a table), where
   * the missing underside is an artefact of the viewpoint and not a feature of the part. It seals
   * that one layer only, so a genuine gap anywhere else still leaks and is still reported.
   */
  sealBase?: boolean;
}
 ...
 */
export function fillSolidFromSurface(
  grid: OccupancyGrid,
  closeRadius = 1,
  options: FillSolidOptions = {},
): { interiorFilled: number; closed: boolean; sealedBase: boolean } {
  if (!Number.isInteger(closeRadius) || closeRadius < 0) throw new RangeError('closeRadius must be a non-negative integer');
  if (Math.min(grid.nx, grid.ny, grid.nz) < 2 * (closeRadius + 1) + 1) throw new RangeError('Grid is too small for the requested closing radius');
  for (let r = 0; r < closeRadius; r += 1) morph3(grid, true);
  for (let r = 0; r < closeRadius; r += 1) morph3(grid, false);
  let sealedBase = false;
  if (options.sealBase) {
    const k = lowestOccupiedLayer(grid);
    if (k < 0) throw new RangeError('Base sealing needs at least one solid voxel; the point cloud produced an empty grid');
  ...
  while (head < tail) {
    ...
    for (let d = 0; d < 6; d += 1) {
      if (!open[d]) continue;
      const q = at + stride[d];
      if (grid.data[q] === 0 && reached[q] === 0) { reached[q] = 1; queue[tail++] = q; }
    }
  }
  let interiorFilled = 0;
  for (let at = 0; at < grid.data.length; at += 1) {
    if (grid.data[at] === 0 && reached[at] === 0) { grid.data[at] = 1; interiorFilled += 1; }
  }
