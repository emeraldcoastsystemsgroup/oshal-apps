 ...
 * arithmetic.
 */
export const PRINT_CHECK_LIMITS = Object.freeze({
 ...
 */
export const OVERHANG_READING_BAND_DEG = Object.freeze({ under: 1, over: 6 });

/** @description The machine the part is being checked against. */
export interface PrintCheckOptions {
  /** Nozzle diameter, millimetres: one extruded bead is this wide. */
  nozzleMm?: number;
  /** Layer height, millimetres. */
  layerHeightMm?: number;
  /** How many perimeters a wall must be able to hold. */
  perimeters?: number;
  /** Radius, in voxels, of the sphere the underside normal is averaged over. */
  normalRadiusVoxels?: number;
}

/** @description One measured quantity per check, its derived limit, and the verdict. */
export interface PrintChecks {
  /** Nozzle diameter used, millimetres. */
  nozzleMm: number;
  /** Layer height used, millimetres. */
  layerHeightMm: number;
  /** Perimeter count used. */
  perimeters: number;
  /** Thinnest wall found, millimetres (twice the smallest maximal inscribed sphere radius). */
  minWallMm: number;
  /** Where that wall is, world millimetres; null when the solid has no medial ridge. */
  minWallAtMm: Vec3 | null;
  /** The floor a wall must clear: `perimeters x nozzleMm`. */
  minWallLimitMm: number;
  /** Steepest downward-facing surface above the bed, degrees from vertical (0 = wall, 90 = ceiling). */
  largestOverhangDeg: number;
  /** Where that surface is, world millimetres; null when nothing overhangs. */
  largestOverhangAtMm: Vec3 | null;
  /** The ceiling an overhang must stay under: `atan(nozzleMm / layerHeightMm)`, degrees. */
  overhangLimitDeg: number;
  /** The thinnest wall is under the floor. */
  thinWall: boolean;
  /** The steepest overhang is over the ceiling. */
  steepOverhang: boolean;
  /** One sentence per failing check; empty when the part passes. These become drawing notes. */
  failures: string[];
}

/** @description A published bound: default, inclusive minimum, inclusive maximum. */
interface Bound { default: number; min: number; max: number }

/** @description Reject a machine number that is not a finite value inside its published bound. */
function requireNumber(value: number | undefined, bound: Bound, label: string): number {
...
function requireCount(value: number | undefined, bound: Bound, label: string): number {
  if (value === undefined) return bound.default;
...
function distanceLine(f: Float64Array, n: number, out: Float64Array, v: Int32Array, z: Float64Array): void {
  let k = 0;
  v[0] = 0;
  z[0] = -Infinity;
  z[1] = Infinity;
...
function sweepAxis(field: Float64Array, n: number, count: number, indexOf: (line: number, q: number) => number): void {
  const f = new Float64Array(n);
  const out = new Float64Array(n);
  const v = new Int32Array(n);
  const z = new Float64Array(n + 1);
  for (let line = 0; line < count; line += 1) {
    for (let q = 0; q < n; q += 1) f[q] = field[indexOf(line, q)];
    distanceLine(f, n, out, v, z);
    for (let q = 0; q < n; q += 1) field[indexOf(line, q)] = out[q];
  }
}
 ...
 */
export function squaredDistanceToEmpty(grid: OccupancyGrid): Float64Array {
  const { nx, ny, nz } = grid;
  const far = (nx + ny + nz) ** 2;
  const field = new Float64Array(nx * ny * nz);
  for (let at = 0; at < field.length; at += 1) field[at] = grid.data[at] === 1 ? far : 0;
...
function onMedialRidge(grid: OccupancyGrid, field: Float64Array, i: number, j: number, k: number): boolean {
  const d = field[gridIndex(grid, i, j, k)];
  for (let dk = -1; dk <= 1; dk += 1) {
    for (let dj = -1; dj <= 1; dj += 1) {
      for (let di = -1; di <= 1; di += 1) {
        if (field[gridIndex(grid, i + di, j + dj, k + dk)] > d) return false;
      }
    }
  }
  return true;
 ...
 */
function measureMinWall(grid: OccupancyGrid, field: Float64Array): { mm: number; at: Vec3 | null } {
  let best = Infinity;
  let at: Vec3 | null = null;
  for (let k = 1; k < grid.nz - 1; k += 1) {
    for (let j = 1; j < grid.ny - 1; j += 1) {
      for (let i = 1; i < grid.nx - 1; i += 1) {
        const here = gridIndex(grid, i, j, k);
        if (grid.data[here] === 0 || field[here] >= best) continue;
        if (!onMedialRidge(grid, field, i, j, k)) continue;
        best = field[here];
        at = voxelCenter(grid, i, j, k);
      }
    }
  }
  if (at === null) return { mm: 0, at: null };
 ...
 */
function solidForNormal(grid: OccupancyGrid, i: number, j: number, k: number, base: number): boolean {
  if (k < base) return true;
  if (i < 0 || j < 0 || i >= grid.nx || j >= grid.ny || k >= grid.nz) return false;
 ...
 */
function outwardNormal(grid: OccupancyGrid, at: { i: number; j: number; k: number }, radius: number, base: number): Vec3 {
  ...
  for (let dk = -radius; dk <= radius; dk += 1) {
    for (let dj = -radius; dj <= radius; dj += 1) {
      for (let di = -radius; di <= radius; di += 1) {
        ...
        if (len === 0 || len > radius || !solidForNormal(grid, at.i + di, at.j + dj, at.k + dk, base)) continue;
        n.x -= di / len;
        n.y -= dj / len;
        n.z -= dk / len;
      }
...
function lowestSolidLayer(grid: OccupancyGrid): number {
  const plane = grid.nx * grid.ny;
  for (let k = 0; k < grid.nz; k += 1) {
    for (let at = k * plane; at < (k + 1) * plane; at += 1) if (grid.data[at] === 1) return k;
  }
  return -1;
}

/** @description The steepest downward-facing surface above the bed layer, degrees from vertical. */
function measureOverhang(grid: OccupancyGrid, radius: number): { deg: number; at: Vec3 | null } {
...
function describeFailures(c: Omit<PrintChecks, 'failures'>, voxelMm: number): string[] {
 ...
 */
export function evaluatePrintChecks(grid: OccupancyGrid, options: PrintCheckOptions = {}): PrintChecks {
  const nozzleMm = requireNumber(options.nozzleMm, PRINT_CHECK_LIMITS.nozzleMm, 'nozzleMm');
  const layerHeightMm = requireNumber(options.layerHeightMm, PRINT_CHECK_LIMITS.layerHeightMm, 'layerHeightMm');
  const perimeters = requireCount(options.perimeters, PRINT_CHECK_LIMITS.perimeters, 'perimeters');
  const radius = requireCount(options.normalRadiusVoxels, PRINT_CHECK_LIMITS.normalRadiusVoxels, 'normalRadiusVoxels');
  if (layerHeightMm > nozzleMm) throw new RangeError('layerHeightMm must not exceed nozzleMm; a bead cannot be taller than the orifice that lays it');
  if (lowestSolidLayer(grid) < 0) throw new RangeError('Print checks need at least one solid voxel');
