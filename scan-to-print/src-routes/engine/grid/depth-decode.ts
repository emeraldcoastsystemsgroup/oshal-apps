...
import { type ViewName, VIEW_NAMES, isViewName } from './views';

/** @description Published bounds for an uploaded range image. */
export const DEPTH_UPLOAD_LIMITS = Object.freeze({
  ...
  formats: ['png16', 'f32le'] as const,
});

/** @description An accepted encoding. */
export type DepthFormat = (typeof DEPTH_UPLOAD_LIMITS.formats)[number];

/** @description The geometry that places a range image in the world frame, as sent with the upload. */
export interface DepthUploadHeader {
  /** The encoding of the bytes. */
  format: DepthFormat;
  /** Canonical view the sensor looked from. */
  view: ViewName;
  /** Pixel columns. */
  width: number;
  /** Pixel rows. */
  ...
  mmPerPx: number;
  /** Pixel column where world `u` = `uCenterMm`. */
  uCenterPx: number;
  /** Pixel row where world `v` = `vCenterMm`. */
  vCenterPx: number;
  /** World `u` at the centre pixel, mm; default 0 (the centre pixel is where the world origin lands). */
  uCenterMm: number;
  /** World `v` at the centre pixel, mm; default 0. */
  vCenterMm: number;
  /** Signed-look-axis coordinate of the sensor plane (depth 0), mm. */
  planeMm: number;
  /** Millimetres per stored unit. */
  depthScale: number;
}
...
function numberField(raw: Record<string, unknown>, key: string, fallback?: number): number {
  ...
  if (!Number.isFinite(n) || (typeof value === 'string' && value.trim() === '')) throw new RangeError(`Depth header field ${key} must be a finite number`);
  return n;
}
 ...
 */
export function parseDepthHeader(raw: Record<string, unknown>, size?: { width: number; height: number }): DepthUploadHeader {
  const format = raw.format;
  if (!DEPTH_UPLOAD_LIMITS.formats.includes(format as DepthFormat)) throw new RangeError(`Depth header field format must be one of ${DEPTH_UPLOAD_LIMITS.formats.join(', ')}`);
  if (!isViewName(raw.view)) throw new RangeError(`Depth header field view must be one of ${VIEW_NAMES.join(', ')}`);
  ...
  if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0) throw new RangeError('Depth header width and height must be positive integers');
  if (size && (width !== size.width || height !== size.height)) throw new RangeError(`Depth header says ${width}x${height} but the image is ${size.width}x${size.height}`);
  if (width * height > DEPTH_UPLOAD_LIMITS.maxPixels) throw new RangeError(`Depth image exceeds ${DEPTH_UPLOAD_LIMITS.maxPixels} pixels`);
  const mmPerPx = numberField(raw, 'mmPerPx');
  if (!(mmPerPx > 0)) throw new RangeError('Depth header field mmPerPx must be positive');
  const bound = DEPTH_UPLOAD_LIMITS.depthScale;
  const depthScale = numberField(raw, 'depthScale', bound.default);
  if (depthScale < bound.min || depthScale > bound.max) throw new RangeError(`Depth header field depthScale must be between ${bound.min} and ${bound.max}`);
  return {
    format: format as DepthFormat, view: raw.view, width, height, mmPerPx, depthScale,
...
function emptyMap(h: DepthUploadHeader): DepthMap {
  return {
 ...
 */
export function depthMapFromFloat32(bytes: Uint8Array, header: DepthUploadHeader): DepthMap {
  if (bytes.length !== header.width * header.height * 4) throw new RangeError(`A ${header.width}x${header.height} float32 range image is ${header.width * header.height * 4} bytes, not ${bytes.length}`);
 ...
 */
export function depthMapFromUint16(values: Uint16Array, header: DepthUploadHeader): DepthMap {
  if (values.length !== header.width * header.height) throw new RangeError(`A ${header.width}x${header.height} 16-bit range image has ${header.width * header.height} samples, not ${values.length}`);
  const map = emptyMap(header);
  for (let i = 0; i < map.data.length; i += 1) map.data[i] = values[i] === 0 ? NaN : values[i] * header.depthScale;
  return map;
}
