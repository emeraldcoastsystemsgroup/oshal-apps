...
exports.chooseScale = chooseScale;
exports.scaleLabel = scaleLabel;
exports.renderEngineeringDrawing = renderEngineeringDrawing;
const format_number_1 = require("../geometry/format-number");
...
exports.SHEET = Object.freeze({ width: 420, height: 297, margin: 10, titleWidth: 150, titleHeight: 40 });
/** @description Standard drawing scales, descending. */
const SCALES = [10, 5, 2, 1, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01];
...
function viewsArea() {
    const x = exports.SHEET.margin;
    const y = exports.SHEET.margin;
    const w = exports.SHEET.width - 2 * exports.SHEET.margin;
    const h = exports.SHEET.height - 2 * exports.SHEET.margin - exports.SHEET.titleHeight;
...
function chooseScale(input) {
    ...
    for (const name of Object.keys(input.views)) {
        const { uMm, vMm } = (0, views_1.viewExtent)(views_1.VIEW_FRAMES[name], input.sizeMm);
...
function scaleLabel(scale) {
    if (scale >= 1)
        return `${Number(scale.toFixed(2))}:1`;
...
function placeView(name, mask, voxelMm, scale) {
    const { bbox } = (0, raster_types_1.maskStats)(mask);
    if (!bbox)
        return null;
    const { x, y, cw, ch } = viewsArea();
    const [col, row] = CELL[name];
    const centre = { x: x + (col + 0.5) * cw, y: y + (row + 0.5) * ch };
    const px = voxelMm * scale;
    const bboxCentre = { x: (bbox.minX + bbox.maxX + 1) / 2, y: (bbox.minY + bbox.maxY + 1) / 2 };
    const offset = { x: centre.x - bboxCentre.x * px, y: centre.y - bboxCentre.y * px };
    return {
        x1: offset.x + bbox.minX * px, y1: offset.y + bbox.minY * px,
        x2: offset.x + (bbox.maxX + 1) * px, y2: offset.y + (bbox.maxY + 1) * px,
        path: (0, contours_1.loopsToPath)((0, contours_1.traceContours)(mask), px, offset),
    };
}
/** @description A horizontal dimension: extension lines, arrowed line, centred label above. */
function dimensionH(x1, x2, yLine, yFrom, label) {
    const a = 1.5;
    return [
...
function dimensionV(y1, y2, xLine, xFrom, label) {
    const a = 1.5;
    const mid = (y1 + y2) / 2;
    return [
...
function renderDimensions(input, placed) {
    const parts = [];
    const mm = (v) => (0, format_number_1.formatFixed)(v, 1);
    ...
    if (top)
        parts.push(dimensionV(top.y1, top.y2, top.x2 + 6, top.x2 + 1, mm(input.sizeMm.y)));
    return parts.join('');
}
/** @description Every placed view outline plus its label. */
function renderViews(input, scale) {
    const { x, y, cw, ch } = viewsArea();
    const placed = {};
    const parts = [];
...
function titleBlock(input, scale) {
    const x = exports.SHEET.width - exports.SHEET.margin - exports.SHEET.titleWidth;
    const y = exports.SHEET.height - exports.SHEET.margin - exports.SHEET.titleHeight;
    const rows = [
        ...
        ['VOXEL', `${(0, format_number_1.formatFixed)(input.voxelMm, 2)} mm`], ['VOLUME', `${(0, format_number_1.formatFixed)(input.volumeMm3 / 1000, 2)} cm³`],
    ...
    return [
        `<rect x="${x}" y="${y}" width="${exports.SHEET.titleWidth}" height="${exports.SHEET.titleHeight}" class="frame"/>`,
        ...cells,
        `<text x="${x + 2}" y="${y + exports.SHEET.titleHeight - 2}" class="tb"><tspan class="tbk">PROJECTION</tspan> THIRD ANGLE</text>`,
        projectionSymbol(x + 52, y + exports.SHEET.titleHeight - 4),
        `<text x="${x + exports.SHEET.titleWidth - 2}" y="${y + exports.SHEET.titleHeight - 2}" class="tb" text-anchor="end">oshal scan-to-print</text>`,
    ].join('');
}
/** @description Numbered notes, bottom left, dimension provenance first. */
function notesBlock(input) {
    const x = exports.SHEET.margin;
    const y = exports.SHEET.height - exports.SHEET.margin - exports.SHEET.titleHeight;
    const width = exports.SHEET.width - 2 * exports.SHEET.margin - exports.SHEET.titleWidth;
    const provenance = `Extents: X ${(0, format_number_1.formatFixed)(input.sizeMm.x, 1)} (${input.sources.x}), Y ${(0, format_number_1.formatFixed)(input.sizeMm.y, 1)} (${input.sources.y}), Z ${(0, format_number_1.formatFixed)(input.sizeMm.z, 1)} (${input.sources.z}).`;
    const lines = [provenance, ...input.notes].slice(0, 6).map((n, i) => `${i + 1}. ${n.length > 150 ? `${n.slice(0, 147)}…` : n}`);
    return [
        `<rect x="${x}" y="${y}" width="${width}" height="${exports.SHEET.titleHeight}" class="frame"/>`,
 ...
 */
function renderEngineeringDrawing(input) {
    const scale = chooseScale(input);
    const views = renderViews(input, scale);
    return [
        `<svg xmlns="http://www.w3.org/2000/svg" width="${exports.SHEET.width}mm" height="${exports.SHEET.height}mm" viewBox="0 0 ${exports.SHEET.width} ${exports.SHEET.height}">`,
        `<title>${esc(input.partName)} — engineering drawing</title>`,
        `<style>${STYLE}</style>`,
        `<rect x="0" y="0" width="${exports.SHEET.width}" height="${exports.SHEET.height}" fill="#fff"/>`,
        `<rect x="${exports.SHEET.margin}" y="${exports.SHEET.margin}" width="${exports.SHEET.width - 2 * exports.SHEET.margin}" height="${exports.SHEET.height - 2 * exports.SHEET.margin}" class="frame"/>`,
        views.svg,
...
//# sourceMappingURL=engineering-drawing.js.map