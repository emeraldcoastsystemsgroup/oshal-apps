...
exports.suggestSquareOnFrames = suggestSquareOnFrames;
const silhouette_carver_1 = require("./silhouette-carver");
const views_1 = require("./views");
/** @description Reject frames, views or dimensions the scorer cannot read. */
function requireInputs(frames, views, known) {
    if (!Array.isArray(frames) || frames.length === 0)
        throw new RangeError('At least one frame is required');
    for (const f of frames) {
        if (!Number.isInteger(f?.pixels) || f.pixels < 0)
            throw new RangeError('Each frame needs a non-negative integer pixel count');
        if (f.sequence !== undefined && !Number.isInteger(f.sequence))
            throw new RangeError('A frame sequence must be an integer clip number');
        const b = f.bbox;
        if (b !== null && !(Number.isInteger(b?.minX) && Number.isInteger(b.minY) && b.maxX >= b.minX && b.maxY >= b.minY))
            throw new RangeError('Each frame bbox must be null or integer bounds with max >= min');
    }
    if (!Array.isArray(views) || views.length === 0)
        throw new RangeError('At least one candidate view is required');
    if (new Set(views).size !== views.length || !views.every(views_1.isViewName))
        throw new RangeError('Candidate views must be distinct canonical view names');
    const axes = new Set();
    for (const d of known) {
        if (!['x', 'y', 'z'].includes(d?.axis) || axes.has(d.axis) || !(d.mm > 0) || !Number.isFinite(d.mm))
            throw new RangeError('Known dimensions must be distinct axes with positive millimetres');
        axes.add(d.axis);
    }
}
/**
 ...
 */
function neighbour(frames, from, step) {
    const clip = frames[from].sequence ?? 0;
    for (let i = from + step; i >= 0 && i < frames.length; i += step) {
        if ((frames[i].sequence ?? 0) !== clip)
            return undefined;
        if (frames[i].pixels > 0)
            return frames[i];
    }
    return undefined;
}
 ...
 */
function turnScores(frames) {
    return frames.map((f, i) => {
        if (f.pixels === 0)
            return { turn: -Infinity, interior: false };
        const around = [neighbour(frames, i, -1), neighbour(frames, i, 1)].filter((n) => n !== undefined).map((n) => n.pixels);
        if (around.length === 0)
            return { turn: 0, interior: false };
...
function skewFor(view, box, sizes) {
 ...
 */
function suggestSquareOnFrames(frames, views, known, options = {}) {
    requireInputs(frames, views, known);
    const excluded = new Set(options.exclude ?? []);
    const sizes = Object.fromEntries(known.map((d) => [d.axis, d.mm]));
    const turn = turnScores(frames);
    const maxSkew = -Math.log(1 - silhouette_carver_1.SCALE_DISAGREEMENT);
    const taken = new Set();
    return views.map((view) => {
        const ranked = [];
        frames.forEach((f, index) => {
            if (f.bbox === null || f.pixels === 0)
                return;
...
//# sourceMappingURL=frame-suggest.js.map