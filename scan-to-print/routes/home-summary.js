...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        if (failed)
            items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: 'scan-to-print' });
        else if (!items.length)
            items.push({ text: 'No objects scanned yet. Open the app to begin.', tone: 'neutral', fix: 'scan-to-print' });
