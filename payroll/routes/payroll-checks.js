...
exports.STALE_AFTER_MONTHS = void 0;
exports.amountInWords = amountInWords;
exports.allocateCheckNumbers = allocateCheckNumbers;
...
function amountInWords(cents) {
    ...
    if (total < 0)
        throw new Error('A check cannot be written for a negative amount.');
    const dollars = Math.floor(total / 100);
    const remainder = String(total % 100).padStart(2, '0');
    if (dollars === 0)
        return `ZERO AND ${remainder}/100`;
    const groups = [];
    let n = dollars;
    let scale = 0;
    while (n > 0) {
 ...
 */
async function allocateCheckNumbers(pool, sub, count) {
    ...
    const r = await pool.query(`UPDATE payroll_company
      ...
      WHERE user_sub = $1
      RETURNING check_next_number - $2 AS start`, [sub, n]);
    const start = Number(r.rows[0]?.start || 0);
    if (!start)
        throw new Error('No company settings row — cannot allocate check numbers.');
    return Array.from({ length: n }, (_, i) => start + i);
}
/** Format cents as $#,##0.00. */
function figures(cents) {
...
function addMonths(date, months) {
    ...
    return new Date(Date.UTC(d.getUTCFullYear(), targetMonth, Math.min(d.getUTCDate(), end))).toISOString().slice(0, 10);
}
/**
...
function checkDocument(input) {
    ...
    if (!checkNumber)
        warnings.push('No check number was allocated for this payment.');
    const earnings = input.earnings || [];
    const deductions = input.deductions || [];
    const taxes = input.taxes || [];
    const grossCents = Number(input.grossCents ?? earnings.reduce((a, l) => a + l.currentCents, 0));
    const withheld = [...deductions, ...taxes].reduce((a, l) => a + l.currentCents, 0);
    if (grossCents && grossCents - withheld !== amountCents) {
        warnings.push(`The stub does not foot: gross ${figures(grossCents)} less ${figures(withheld)} withheld is `
    ...
    return {
        ...
        staleLegend: `VOID AFTER ${exports.STALE_AFTER_MONTHS} MONTHS — ${addMonths(String(input.date), exports.STALE_AFTER_MONTHS)}`,
        earnings,
        deductions,
        taxes,
        grossCents,
        ...
        micrLine: null,
        micrNote: 'No MICR line is generated. Print onto your bank\'s pre-encoded check stock: the governing '
...
//# sourceMappingURL=payroll-checks.js.map