...
exports.FL_EFILE_EMPLOYEE_THRESHOLD = exports.FL_INSTALLMENT_FEE_CENTS = exports.FL_INTEREST_DAILY_FACTOR_2026 = exports.FL_LATE_PENALTY_PER_30_DAYS_CENTS = exports.FL_INITIAL_RATE_PCT = exports.FL_MAX_RATE_PCT = exports.FL_MIN_RATE_PCT = exports.FL_WAGE_BASE_CENTS = void 0;
exports.isValidFloridaSsn = isValidFloridaSsn;
exports.rt6Dates = rt6Dates;
exports.formRt6 = formRt6;
exports.rt6InstallmentDates = rt6InstallmentDates;
 ...
 */
exports.FL_WAGE_BASE_CENTS = 700_000;
/**
 ...
 */
exports.FL_MIN_RATE_PCT = 0.10;
exports.FL_MAX_RATE_PCT = 5.40;
/** A new Florida employer pays 2.7% until roughly ten quarters are reported. */
exports.FL_INITIAL_RATE_PCT = 2.70;
/**
 ...
 */
exports.FL_EFILE_EMPLOYEE_THRESHOLD = 10;
/* ── helpers ─────────────────────────────────────────────────────────────── */
 ...
 */
function isValidFloridaSsn(ssn) {
    const d = String(ssn ?? '').replace(/\D/g, '');
    if (d.length !== 9)
        return false;
    if (/^0+$/.test(d))
        return false;
    if (d.startsWith('000') || d.startsWith('666') || d.startsWith('9'))
        return false;
    if (d.slice(3, 5) === '00')
        return false;
    if (d.slice(5) === '0000')
 ...
 */
function rt6Dates(year, quarter) {
 ...
 */
async function formRt6(pool, sub, company, year, quarter, opts = {}) {
    const { from, to } = quarterBounds(year, quarter);
    const warnings = [];
    const stateCode = String(company.state_code || '').toUpperCase();
    if (stateCode !== 'FL') {
        warnings.push(`This company's work state is ${stateCode || 'unset'}, not FL. The RT-6 is a Florida return; `
            + 'the figures below are computed but do not describe a filing obligation in that state.');
    }
    const ratePct = Number(company.suta_rate_pct ?? exports.FL_INITIAL_RATE_PCT);
    const rateWithinBounds = ratePct >= exports.FL_MIN_RATE_PCT && ratePct <= exports.FL_MAX_RATE_PCT;
    if (!rateWithinBounds) {
        warnings.push(`The reemployment tax rate on file is ${ratePct}%, outside Florida's 2026 range of `
            + `${exports.FL_MIN_RATE_PCT}% to ${exports.FL_MAX_RATE_PCT}%. Check the rate notice from the Department of `
            + 'Revenue — a short-term-compensation employer is the one documented exception to the cap.');
    }
    const wageBaseCents = Number(company.suta_wage_base_cents ?? exports.FL_WAGE_BASE_CENTS);
    if (wageBaseCents !== exports.FL_WAGE_BASE_CENTS) {
        warnings.push(`The wage base on file is $${(wageBaseCents / 100).toFixed(2)}; Florida's is `
            + `$${(exports.FL_WAGE_BASE_CENTS / 100).toFixed(2)} per employee per calendar year.`);
    }
    ...
    const penaltyCents = lateBlocks * exports.FL_LATE_PENALTY_PER_30_DAYS_CENTS;
    const interestCents = cents(taxDueCents * exports.FL_INTEREST_DAILY_FACTOR_2026 * lateDays);
    const feeCents = opts.installment ? exports.FL_INSTALLMENT_FEE_CENTS : 0;
    const totalCents = taxDueCents + penaltyCents + interestCents + feeCents;
    const invalidSsns = employees.filter((e) => !e.ssnValid).length;
    if (invalidSsns) {
        warnings.push(`${invalidSsns} employee record(s) have a missing or structurally invalid Social Security `
    ...
    const mustFileElectronically = employees.length >= exports.FL_EFILE_EMPLOYEE_THRESHOLD;
    if (mustFileElectronically) {
        warnings.push(`With ${employees.length} employees this report must be filed and paid electronically `
            + '(10 or more in any quarter during Florida\'s prior fiscal year, July 1 – June 30).');
    }
    if (lateBlocks > 0) {
        warnings.push(`Filed ${lateDays} day(s) after the penalty-after date of ${penaltyAfter}: `
            + `${lateBlocks} × $25 penalty. Florida's documents state no maximum for this penalty.`);
    }
    if (fallsOnWeekend(penaltyAfter)) {
        warnings.push(`The penalty-after date ${penaltyAfter} falls on a weekend. Florida's rule for a postmark `
    ...
    const lines = [
        ...
        },
        {
            ...
            note: `11% for 2026, applied as a daily factor of ${exports.FL_INTEREST_DAILY_FACTOR_2026}.`,
        },
        {
            line: '8',
    ...
    return {
        ...
        caveat: 'This is a worksheet, not a filed return. oshal does not transmit to the Florida Department '
 ...
 */
async function employeeWageDetail(pool, sub, year, from, to, wageBaseCents) {
    const rows = (await pool.query(`SELECT e.employee_id, e.first_name, e.last_name, e.ssn_last4,
      ...
      ORDER BY e.last_name, e.first_name`, [sub, year, from, to])).rows;
    return rows.map((r) => {
...
async function sutaAccrued(pool, sub, from, to) {
    const r = await pool.query(`SELECT COALESCE(SUM(l.suta_cents), 0) AS suta
       FROM payroll_run_lines l
       JOIN payroll_runs r ON r.run_id = l.run_id AND r.user_sub = l.user_sub
      WHERE l.user_sub = $1 AND r.status = 'paid' AND r.pay_date BETWEEN $2 AND $3`, [sub, from, to]);
 ...
 */
function rt6InstallmentDates(year, quarter) {
...
//# sourceMappingURL=payroll-rt6.js.map