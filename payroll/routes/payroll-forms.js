...
exports.form941 = form941;
exports.form940 = form940;
exports.w2Document = w2Document;
...
const SS_COMBINED = payroll_tax_tables_1.SS_RATE * 2;
const MEDICARE_COMBINED = payroll_tax_tables_1.MEDICARE_RATE * 2;
 ...
 */
async function form941(pool, sub, year, quarter) {
    const { from, to } = quarterBounds(year, quarter);
    const t = (await pool.query(`SELECT COUNT(DISTINCT l.employee_id) AS employees,
      ...
      WHERE l.user_sub = $1 AND r.status = 'paid' AND r.pay_date BETWEEN $2 AND $3`, [sub, from, to])).rows[0];
    ...
    const addlWages = payroll_tax_tables_1.ADDL_MEDICARE_RATE > 0 ? cents(n('addl_withheld') / payroll_tax_tables_1.ADDL_MEDICARE_RATE) : 0;
    ...
    const l5d = cents(addlWages * payroll_tax_tables_1.ADDL_MEDICARE_RATE);
 ...
 */
async function form940(pool, sub, year) {
    const rows = (await pool.query(`SELECT l.employee_id,
    ...
    for (const r of rows) {
        ...
        totalWages += w;
        excess += Math.max(0, w - payroll_tax_tables_1.FUTA_WAGE_BASE_CENTS); // per-employee base
        futaAccrued += Number(r.futa || 0);
    }
    const taxable = Math.max(0, totalWages - excess);
    const computed = cents(taxable * payroll_tax_tables_1.FUTA_RATE);
    const lines = [
 ...
 */
async function w2Document(pool, sub, employee, company, year, revealed) {
    const t = (await pool.query(`SELECT COALESCE(SUM(l.fit_taxable_cents + l.supplemental_taxable_cents),0) b1,
      ...
      WHERE l.user_sub = $1 AND l.employee_id = $2 AND r.status = 'paid'
        AND date_part('year', r.pay_date) = $3`, [sub, employee.employee_id, year])).rows[0];
    const missing = [];
    if (!revealed.ein)
    ...
    const fmtEin = (e) => (e ? `${e.slice(0, 2)}-${e.slice(2)}` : '');
    const fmtSsn = (s) => (s ? `${s.slice(0, 3)}-${s.slice(3, 5)}-${s.slice(5)}` : '');
    const addr = (r) => [r.address_line1, r.address_line2,
    ...
    return {
        ...
        boxes: {
            1: n('b1'), 2: n('b2'), 3: Math.min(n('b3'), payroll_tax_tables_1.SS_WAGE_BASE_CENTS), 4: n('b4'),
...
//# sourceMappingURL=payroll-forms.js.map