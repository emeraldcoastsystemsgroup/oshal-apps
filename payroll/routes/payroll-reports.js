...
exports.depositSchedule = depositSchedule;
exports.deductionRegister = deductionRegister;
exports.laborDistribution = laborDistribution;
 ...
 */
async function runJournal(pool, sub, runId) {
    const run = (await pool.query('SELECT run_id, pay_date FROM payroll_runs WHERE run_id = $1 AND user_sub = $2', [runId, sub])).rows[0];
    if (!run)
        return null;
    const t = (await pool.query(`SELECT COALESCE(SUM(l.gross_cents),0) gross, COALESCE(SUM(l.fit_cents),0) fit,
    ...
    const lines = [];
    const push = (account, label, debit, credit) => {
        if (debit === 0 && credit === 0)
            return;
    ...
    return {
        runId: String(run.run_id), payDate: (0, payroll_store_1.isoDate)(run.pay_date), lines,
...
function nextBusinessDay(iso) {
    ...
    for (let i = 0; i < 7; i += 1) {
        ...
        if (day !== 0 && day !== 6)
            return d;
        d = (0, payroll_store_1.addDays)(d, 1);
    }
    return d;
}
 ...
 */
async function depositSchedule(pool, sub, year, depositorStatus) {
    const rows = (await pool.query(`SELECT r.pay_date,
    ...
    return rows.map((r) => {
        const payDate = (0, payroll_store_1.isoDate)(r.pay_date);
        const liabilityCents = Number(r.liability || 0);
        if (liabilityCents >= NEXT_DAY_THRESHOLD_CENTS) {
            return {
                payDate, liabilityCents, dueDate: nextBusinessDay((0, payroll_store_1.addDays)(payDate, 1)),
        ...
        if (depositorStatus === 'semiweekly') {
            ...
            let due = (0, payroll_store_1.addDays)(payDate, 1);
            const wanted = isWedToFri ? 3 : 5; // Wednesday or Friday
            for (let i = 0; i < 14; i += 1) {
                if (new Date(`${due}T00:00:00Z`).getUTCDay() === wanted)
                    break;
                due = (0, payroll_store_1.addDays)(due, 1);
            }
...
async function deductionRegister(pool, sub, from, to) {
    return (await pool.query(`SELECT d.code, COUNT(DISTINCT d.employee_id) AS employees,
...
async function laborDistribution(pool, sub, from, to) {
    return (await pool.query(`SELECT COALESCE(NULLIF(e.department,''),'(unassigned)') AS department,
...
//# sourceMappingURL=payroll-reports.js.map