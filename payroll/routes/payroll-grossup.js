 ...
 */
function grossUp(targetNetCents, code, baseEarnings, deductions, w4, opts) {
    const warnings = [];
    const target = Math.max(0, Math.trunc(Number(targetNetCents) || 0));
    const at = (grossCents) => (0, payroll_payrun_1.computeCheck)([...baseEarnings, { code, hours: 0, rateCents: 0, amountCents: grossCents, workweek: 1 }], deductions, w4, opts);
    const baseline = (0, payroll_payrun_1.computeCheck)(baseEarnings, deductions, w4, opts);
    ...
    while (lo < hi) {
        ...
        iterations += 1;
        if (at(mid).netCents >= target)
            hi = mid;
        else
            lo = mid + 1;
        if (iterations > 200)
            break; // defensive; bisection over cents converges long before this
    }
    const check = at(lo);
...
//# sourceMappingURL=payroll-grossup.js.map