...
import { DEFAULT_SUTA_RATE_PCT, DEFAULT_SUTA_WAGE_BASE_CENTS } from './payroll-tax-tables';

type Pool = AppContext['pool'];

/** Run lifecycle statuses. */
export const RUN_DRAFT = 'draft';
export const RUN_PAID = 'paid';
...
export const KIND_OFFCYCLE = 'offcycle';
export const KIND_FINAL = 'final';
export const KIND_VOID = 'void';

/** Kinds that behave as a normal editable draft. */
export const EDITABLE_KINDS = [KIND_REGULAR, KIND_OFFCYCLE, KIND_FINAL];

/** Create/upgrade every payroll table (idempotent), with RLS at the chokepoint. */
export async function ensurePayrollSchema(pool: Pool): Promise<void> {
  await runRuntimeSchemaBootstrap({
    ...
    statements: [
      ...
      `CREATE TABLE IF NOT EXISTS payroll_company (
        ...
        suta_rate_pct NUMERIC NOT NULL DEFAULT ${DEFAULT_SUTA_RATE_PCT},
        suta_wage_base_cents BIGINT NOT NULL DEFAULT ${DEFAULT_SUTA_WAGE_BASE_CENTS},
      ...
      ...['taxable_wages', 'ss_taxable', 'deferral', 'supplemental', 'gross', 'fit', 'ss', 'medicare', 'state'].map(
        (c) => `ALTER TABLE payroll_employees ADD COLUMN IF NOT EXISTS prior_ytd_${c}_cents BIGINT NOT NULL DEFAULT 0`,
      ),
      ...
      `ALTER TABLE payroll_runs ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT '${KIND_REGULAR}'`,
      ...
      ...['k401', 'health', 'roth', 'supplemental_fit', 'deferral', 'tips', 'qualified_ot',
        'garnishment', 'disposable', 'supplemental_taxable', 'imputed', 'cash_earnings', 'nontaxable_paid'].map(
        (c) => `ALTER TABLE payroll_run_lines ADD COLUMN IF NOT EXISTS ${c}_cents BIGINT NOT NULL DEFAULT 0`,
      ),
      ...
      ...buildOwnerRlsPolicyStatements('payroll_company', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_employees', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_bank_accounts', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_deduction_elections', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_runs', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_run_lines', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_line_earnings', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_line_deductions', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_payments', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_audit', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_ach_events', 'user_sub'),
      ...buildOwnerRlsPolicyStatements('payroll_filings', 'user_sub'),
    ],
    requirements: [
