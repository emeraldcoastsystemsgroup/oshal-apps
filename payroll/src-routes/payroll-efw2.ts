...
export const EFW2_RECORD_LEN = 512;
 ...
 */
export const EFW2_VERIFIED_TAX_YEARS: readonly number[] = Object.freeze([2025]);

/** Why a given year is not available, in words an operator can act on. */
export function efw2UnavailableReason(taxYear: number): string {
  ...
  return `${base}Retrieve SSA Publication 42-007 for tax year ${taxYear} and add its layout. `
    + `Verified years: ${EFW2_VERIFIED_TAX_YEARS.join(', ')}.`;
}
 ...
 */
const RW_MONEY: Readonly<Record<string, Span>> = Object.freeze({
  wages: [188, 198],
  fit: [199, 209],
  ssWages: [210, 220],
  ssTax: [221, 231],
 ...
 */
const RT_TOTALS: Readonly<Record<string, Span>> = Object.freeze({
  wages: [10, 24],
  fit: [25, 39],
  ssWages: [40, 54],
  ssTax: [55, 69],
...
const RT_RECORD_COUNT: Span = [3, 9];
/** The count of RW records on the ENTIRE file, across every RE. */
const RF_RECORD_COUNT: Span = [8, 16];

/** Exposed so a guard can assert the two maps stay in step. */
export const EFW2_FIELD_MAPS = Object.freeze({ RW_MONEY, RT_TOTALS, RT_RECORD_COUNT, RF_RECORD_COUNT });
 ...
 */
class Record512 {
  private readonly buf: string[] = new Array(EFW2_RECORD_LEN).fill(' ');
...
/** Who is sending the file. SSA contacts these people when it fails. */
export interface Efw2Submitter {
  /** EIN used to register the BSO User ID. Digits only. */
  ein: string;
  /** The 8-character BSO User ID of the person attesting to the file. */
  userId: string;
  name: string;
  ...
  zip: string;
  contactName: string;
  /** Digits only, including area code. */
  contactPhone: string;
  /** Must not be blank — the one field the specification singles out. */
  contactEmail: string;
  /** 'L' self-prepared, 'A' accounting firm, 'S' service bureau, 'P' parent, 'O' other. */
  preparerCode?: string;
}

/** The employer the wages belong to. */
export interface Efw2Employer {
  ...
  contactPhone: string;
  contactEmail: string;
  /** F federal / S state-local / T 501c / Y state-local 501c / N none apply. */
  kindOfEmployer?: string;
  /** R regular 941 (the payroll default), A agriculture, H household, F 944, M military, Q MQGE, X railroad. */
  employmentCode?: string;
  /** True when this is the last year W-2s are filed under this EIN. */
  terminating?: boolean;
}

/** One employee's W-2 amounts, in cents. Unset keys are written as zeros. */
export interface Efw2Employee {
  ...
  stateCode?: string;
  zip?: string;
  /** Keys must be a subset of the RW money map. */
  amounts: Partial<Record<keyof typeof RW_MONEY, number>>;
  retirementPlan?: boolean;
  statutoryEmployee?: boolean;
  thirdPartySickPay?: boolean;
}

/** The generated submission plus what a human should check before uploading. */
export interface Efw2File {
  content: string;
  taxYear: number;
  recordCount: number;
  employeeCount: number;
  /** The RT totals, as written. */
...
function efw2Ssn(ssn: unknown): { value: string; problem?: string } {
  const d = String(ssn ?? '').replace(/\D/g, '');
  if (d.length !== 9) return { value: '000000000', problem: 'no valid SSN on file — filed as zeros' };
...
export function buildEfw2(
  ...
  if (!EFW2_VERIFIED_TAX_YEARS.includes(taxYear)) {
    return {
      content: '',
      taxYear,
      recordCount: 0,
  ...
  for (const [label, ein] of [['submitter', submitter.ein], ['employer', employer.ein]] as const) {
    if (String(ein || '').replace(/\D/g, '').length !== 9) problems.push(`The ${label} EIN must be 9 digits.`);
  }
  if (String(employer.ein || '').replace(/\D/g, '').startsWith('00')) {
    problems.push('An employer EIN may not begin with 00.');
  }
  if (!employees.length) problems.push('No employees — there is nothing to file.');
  ...
  /* ── RW  one per employee ──────────────────────────────────────────────── */
  const totals: Record<string, number> = Object.fromEntries(Object.keys(RT_TOTALS).map((k) => [k, 0]));

  for (const e of employees) {
    const { value: ssn, problem } = efw2Ssn(e.ssn);
    if (problem) problems.push(`${e.lastName}, ${e.firstName}: ${problem}.`);
    ...
    for (const [key, span] of Object.entries(RW_MONEY)) {
      const cents = Math.max(0, Math.trunc(Number(e.amounts[key as keyof typeof RW_MONEY]) || 0));
      rw.numeric(span, cents);
      if (key in totals) totals[key] += cents;
    }
  ...
  for (const [key, span] of Object.entries(RT_TOTALS)) rt.numeric(span, totals[key]);
  ...
  lines.forEach((l, i) => {
    if (l.length !== EFW2_RECORD_LEN) {
      problems.push(`Record ${i + 1} is ${l.length} characters, must be exactly ${EFW2_RECORD_LEN}.`);
    }
  });
  ...
  if (totals.medicareWages < totals.ssWages + totals.ssTips) {
    problems.push(
      'Total Medicare wages are less than Social Security wages plus tips, which SSA rejects. '
  ...
  return {
    ...
    problems,
    caveat:
      'This file has NOT been submitted. Upload it to SSA Business Services Online, and run it '
 ...
 */
export function readEfw2Totals(content: string): {
  employeeCountRt: number;
  employeeCountRf: number;
  rwRecords: number;
  totals: Record<string, number>;
} {
  ...
  const read = (line: string, span: Span) => Number(line.slice(span[0] - 1, span[1]) || 0);
  ...
  return {
    ...
    totals: Object.fromEntries(
      Object.entries(RT_TOTALS).map(([key, span]) => [key, read(rt, span)]),
    ),
  };
}
