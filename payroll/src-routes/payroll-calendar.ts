...
export const IRS_HOLIDAYS_VERIFIED_THROUGH = 2026;
...
/** How a federal holiday's statutory date is determined. */
interface HolidayRule {
  name: string;
  /** Fixed calendar date [month, day], or an nth-weekday rule. */
  fixed?: [number, number];
 ...
 */
const FEDERAL_HOLIDAYS: readonly HolidayRule[] = Object.freeze([
...
function statutoryDates(year: number): Array<{ name: string; date: string }> {
...
/** One observed closure or legal holiday. */
export interface Observance {
  /** The date actually observed. */
  date: string;
  name: string;
  /** The statutory date, when it differs from the observed one. */
 ...
 */
export function fedBankingClosures(year: number): Observance[] {
  const out: Observance[] = [];
  for (const h of statutoryDates(year)) {
    const day = dow(h.date);
    if (day === 6) continue;                       // Saturday — Banks stay open Friday
...
const fedClosureCache = new Map<number, Set<string>>();

/** Memoised closure lookup for a year. */
function fedClosureSet(year: number): Set<string> {
 ...
 */
export function isFedBankingDay(date: string): boolean {
 ...
 */
export function nextFedBankingDay(date: string): string {
  let d = date;
  for (let i = 0; i < 30; i += 1) {
    if (isFedBankingDay(d)) return d;
    d = addDays(d, 1);
  }
  return d;
}
...
export function previousFedBankingDay(date: string): string {
  let d = date;
  for (let i = 0; i < 30; i += 1) {
    if (isFedBankingDay(d)) return d;
    d = addDays(d, -1);
 ...
 */
function emancipationDay(year: number): Observance {
...
/** IRS legal holidays for a year, plus whether that year's list was actually read. */
export interface IrsHolidayYear {
  holidays: Observance[];
  /** True only for a year whose Pub 15 legal-holiday list was retrieved and matched. */
  verified: boolean;
  note: string;
}
 ...
 */
export function irsLegalHolidays(year: number): IrsHolidayYear {
  const holidays: Observance[] = statutoryDates(year).map((h) => {
    const day = dow(h.date);
  ...
  const verified = year <= IRS_HOLIDAYS_VERIFIED_THROUGH;
  return {
    holidays,
    verified,
    note: verified
...
const irsHolidayCache = new Map<number, Set<string>>();
 ...
 * within the year.
 */
function irsHolidaySet(year: number): Set<string> {
  let s = irsHolidayCache.get(year);
  if (!s) {
    s = new Set<string>();
    for (const y of [year - 1, year, year + 1]) {
      for (const o of irsLegalHolidays(y).holidays) {
        if (o.date.startsWith(`${year}-`)) s.add(o.date);
      }
    }
 ...
 */
export function isIrsBusinessDay(date: string): boolean {
  if (isWeekend(date)) return false;
...
export function nextIrsBusinessDay(date: string): string {
  let d = date;
  for (let i = 0; i < 30; i += 1) {
    if (isIrsBusinessDay(d)) return d;
    d = addDays(d, 1);
  }
  return d;
}
...
/** The outcome of checking a pay date against the banking calendar. */
export interface PayDateCheck {
  /** The date that will actually fund. */
  payDate: string;
  /** The date originally requested. */
  requested: string;
  shifted: boolean;
  /** Plain-language why, for the operator. Empty when nothing moved. */
  reason: string;
}
 ...
 */
export function resolvePayDate(date: string, direction: 'earlier' | 'later' = 'earlier'): PayDateCheck {
...
/** A computed federal deposit due date and the rule that produced it. */
export interface DepositDue {
  dueDate: string;
  rule: string;
  /** Extra days granted because a legal holiday fell in the semiweekly window. */
  extraDaysForHolidays: number;
  /** False when the year's Pub 15 list has not been read. */
  verified: boolean;
}
...
export function depositDueDate(
  ...
  if (depositorStatus === 'semiweekly') return semiweeklyDue(payDate, verified);
 ...
 */
function semiweeklyDue(payDate: string, verified: boolean): DepositDue {
  ...
  for (let i = 0; i < 14 && dow(due) !== target; i += 1) due = addDays(due, 1);
  ...
  for (let counted = 0; counted < 3;) {
    ...
    counted += 1;
    if (!isIrsBusinessDay(cursor)) extraDaysForHolidays += 1;
  }
  for (let i = 0; i < extraDaysForHolidays; i += 1) due = addDays(due, 1);
...
function closeOfSemiweeklyPeriod(payDate: string, wedToFri: boolean): string {
  let d = payDate;
  // Wed–Fri periods close on Friday(5); Sat–Tue periods close on Tuesday(2).
  const closeDow = wedToFri ? 5 : 2;
  for (let i = 0; i < 7 && dow(d) !== closeDow; i += 1) d = addDays(d, 1);
  return d;
}
 ...
 */
export function futaDepositDue(year: number, quarter: number): string {
