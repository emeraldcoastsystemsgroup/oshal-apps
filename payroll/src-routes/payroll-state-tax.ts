...
import { STANDARD_DEDUCTION_CENTS, type FilingStatus, type TaxBracket, type W4LikeProfile } from './payroll-tax-tables';

/** What the engine carries per employee for state withholding. */
export interface StateTaxProfile {
  /** Two-letter state code, upper case. Empty string when unset. */
  code: string;
  /** Operator-entered flat rate, used when we have no verified table. */
  manualRatePct: number;
  /** State withholding-certificate allowances (e.g. Illinois IL-W-4). */
  allowances: number;
}

/** The outcome of a state computation, including how much to trust it. */
...
/** A verified state rule. Absent from STATE_RULES = unsupported, by design. */
interface StateTaxRule {
  kind: 'none' | 'flat' | 'brackets';
  /** kind 'flat': the statutory withholding rate, percent. */
  ratePct?: number;
  /** kind 'brackets': annual thresholds in cents, last one Infinity. */
  brackets?: TaxBracket[];
  /** Annual state standard deduction by filing status. Applies to BOTH kinds. */
  standardDeductionCents?: Record<FilingStatus, number>;
  /** Deduction used instead when the filer is married AND the spouse works. */
  marriedSpouseWorksDeductionCents?: number;
  /** Annual value of ONE withholding allowance, cents. */
  allowanceValueCents?: number;
  /** State requires the period withholding rounded to the nearest whole dollar. */
  roundToDollar?: boolean;
  note: string;
  source: string;
}

/** A state researched and DELIBERATELY not shipped, with the reason. */
interface UnsupportedReason { code: string; reason: string; source: string }
 ...
 */
const STATE_RULES: Record<string, StateTaxRule> = {
  ...
  MO: {
    ...
    standardDeductionCents: STANDARD_DEDUCTION_CENTS,
    marriedSpouseWorksDeductionCents: STANDARD_DEDUCTION_CENTS.single,
    roundToDollar: true,
...
const KNOWN_UNSUPPORTED: UnsupportedReason[] = [
  {
    code: 'IN',
...
function stateBracketTax(annualCents: number, brackets: TaxBracket[]): number {
  ...
  for (const b of brackets) {
    ...
    if (annualCents <= b.upToCents) break;
    prevTop = b.upToCents;
  }
  return tax;
}
...
function shelterCents(rule: StateTaxRule, profile: StateTaxProfile, w4: W4LikeProfile): number {
  let deduction = 0;
 ...
 */
export function stateSupport(code: string): { supported: boolean; kind: string; note: string } {
  const key = String(code || '').toUpperCase();
  const rule = STATE_RULES[key];
  if (rule) return { supported: true, kind: rule.kind, note: rule.note };
 ...
 */
export function stateWithholding(
  profile: StateTaxProfile,
  taxablePeriodCents: number,
  w4: W4LikeProfile,
  ...
  if (rule.kind === 'none') return { cents: 0, supported: true, warnings: [] };
...
export function unsupportedStates(): UnsupportedReason[] {
