...
/** What an RDFI is telling us. */
export type AchEventKind = 'return' | 'noc';

/** How an employer should respond to a return. */
export type ReturnCategory =
  | 'funding'         // the money was not there; the account is fine
  | 'account-data'    // the account details are wrong or gone
  | 'authorization'   // the receiver disputes the entry
  | 'deceased'
  | 'format'          // our file was malformed
  | 'other';

/** One return reason code and what it obliges the employer to do. */
export interface ReturnCode {
  code: string;
  title: string;
  /** Nacha timeframe within which the RDFI must return. */
  timeframe: string;
  category: ReturnCategory;
  /**
   * Whether the Originator must STOP initiating entries to this account.
   * R01 is deliberately excluded: insufficient funds may be retried.
   */
  stopFutureEntries: boolean;
  /** Whether re-presenting the same entry is permitted at all. */
  reinitiable: boolean;
  /** What a payroll operator actually does next. */
  action: string;
}

const R = (
  code: string, title: string, timeframe: string, category: ReturnCategory,
  stopFutureEntries: boolean, reinitiable: boolean, action: string,
): ReturnCode => ({ code, title, timeframe, category, stopFutureEntries, reinitiable, action });
 ...
 */
export const RETURN_CODES: Readonly<Record<string, ReturnCode>> = Object.freeze(Object.fromEntries([
  R('R01', 'Insufficient Funds', TWO_DAYS, 'funding', false, true,
    'The employer account was short. Fund it and reinitiate — at most twice after the return.'),
  ...
  R('R05', 'Unauthorized Debit to Consumer Account Using Corporate SEC Code', SIXTY_DAYS, 'authorization', true, false,
    'Not expected on payroll credits — check the entry was originated as PPD.'),
  R('R06', "Returned per ODFI's Request", 'determined by the ODFI and RDFI', 'other', false, false,
    'Your own bank asked for this back. Ask them why.'),
  R('R07', 'Authorization Revoked by Customer', SIXTY_DAYS, 'authorization', true, false, PAY_ANOTHER_WAY),
  R('R08', 'Payment Stopped', TWO_DAYS, 'authorization', true, false,
    'The receiver stopped it. Do not re-send; resolve with the employee first.'),
  R('R09', 'Uncollected Funds', TWO_DAYS, 'funding', false, true,
    'Funds were on deposit but uncollected. Reinitiate — at most twice after the return.'),
  R('R10', 'Customer Advises Originator Not Known and/or Not Authorized', SIXTY_DAYS, 'authorization', true, false, PAY_ANOTHER_WAY),
  R('R11', 'Customer Advises Entry Not in Accordance with the Terms of the Authorization', SIXTY_DAYS, 'authorization', true, false,
    'Resolve the dispute, then originate a corrected entry.'),
  R('R12', 'Account Sold to Another DFI', TWO_DAYS, 'account-data', true, false,
    "Get the new institution's routing and account number from the employee."),
  R('R13', 'Invalid ACH Routing Number', NEXT_FILE, 'account-data', true, false, FIX_ACCOUNT),
  R('R14', 'Representative Payee Deceased or Unable to Continue in that Capacity', TWO_DAYS, 'deceased', true, false,
    'Stop payments to this account and handle as a deceased-employee final payment.'),
  R('R15', 'Beneficiary or Account Holder Deceased', TWO_DAYS, 'deceased', true, false,
    'Stop payments to this account and handle as a deceased-employee final payment.'),
  R('R16', 'Account Frozen / Entry Returned per OFAC Instruction', TWO_DAYS, 'other', true, false,
    'Legal or sanctions action on the account. Do not re-send; seek advice.'),
  R('R17', 'File Record Edit Criteria / Entry Initiated Under Questionable Circumstances', TWO_DAYS, 'format', true, false,
    'The RDFI could not process the entry, or flagged it. Check the addenda information for QUESTIONABLE.'),
  R('R18', 'Improper Effective Entry Date', NEXT_FILE, 'format', false, true,
    'The effective entry date fell outside the permitted window. Re-send with a valid date.'),
  R('R19', 'Amount Field Error', NEXT_FILE, 'format', false, true, 'Correct the amount and re-send.'),
  R('R20', 'Non-Transaction Account', TWO_DAYS, 'account-data', true, false,
    'The account cannot receive ACH entries (e.g. a savings-only or escrow account).'),
  R('R21', 'Invalid Company Identification', TWO_DAYS, 'format', false, true,
    'Your ACH company identification does not match what the bank has on file. Fix it in settings.'),
  R('R22', 'Invalid Individual ID Number', TWO_DAYS, 'format', false, true, 'Correct the individual identification number.'),
  R('R23', 'Credit Entry Refused by Receiver', 'by opening of business on the 2nd banking day after the refusal notice', 'authorization', false, false,
    'The employee refused the credit. Ask them why before re-sending.'),
  R('R24', 'Duplicate Entry', TWO_DAYS, 'format', false, false,
    'The bank saw this entry twice. Confirm you did not upload the same file twice.'),
  ...
  R('R31', 'Permissible Return Entry (CCD and CTX only)', 'determined by the ODFI and RDFI', 'other', false, false,
    'Agreed between the banks. Confirm with your bank.'),
  ...
  R('R35', 'Return of Improper Debit Entry', NEXT_FILE, 'format', false, false,
    'A debit was sent where only credits are permitted. Payroll batches must be credits only.'),
...
export function mustStopFutureEntries(code: string): boolean {
  return RETURN_CODES[String(code).toUpperCase()]?.stopFutureEntries === true;
}
...
export const REINITIATION_WINDOW_DAYS = 180;

/* ── Notification of Change ───────────────────────────────────────────────── */

/** What an NOC is telling us to change, decoded from the corrected-data field. */
export interface NocCorrection {
  changeCode: string;
  title: string;
  ...
  individualId?: string;
  /** True when we can safely rewrite the employee's bank account from this. */
 ...
 */
const CHANGE_CODES: Readonly<Record<string, {
  title: string;
  routing?: [number, number];
  account?: [number, number];
  tranCode?: [number, number];
  individualId?: [number, number];
  autoApplicable: boolean;
  note: string;
}>> = Object.freeze({
...
function field(record: string, from: number, to: number): string {
  return record.slice(from - 1, to).trim();
}

/** Decode the 29-byte corrected-data field according to the change code. */
function decodeCorrection(changeCode: string, corrected: string): NocCorrection | null {
  const spec = CHANGE_CODES[changeCode];
  if (!spec) return null;
  const at = (range?: [number, number]) => (range ? corrected.slice(range[0] - 1, range[1]).trim() : undefined);
  return {
    changeCode,
    title: spec.title,
...
/** One return or NOC, joined to the entry detail record it describes. */
export interface AchEvent {
  kind: AchEventKind;
  /** R01… or C01… */
  code: string;
  /** Our trace number from the original file — the join key back to a payment. */
  originalTrace: string;
  originalDfi: string;
  /** The trace of this return entry, assigned by the RDFI. */
  returnTrace: string;
  amountCents: number;
  routingNumber: string;
  accountNumber: string;
  ...
  individualId: string;
  /** YYMMDD, populated only for R14/R15. */
  dateOfDeath: string;
  addendaInformation: string;
  /** Present for a return whose code we recognise. */
  reason?: ReturnCode;
  /** Present for an NOC whose change code we recognise. */
  correction?: NocCorrection;
  problems: string[];
}

/** The outcome of reading a file the bank handed back. */
export interface AchReturnFile {
  events: AchEvent[];
  returnCount: number;
  nocCount: number;
 ...
 */
export function parseAchReturnFile(content: string): AchReturnFile {
  const problems: string[] = [];
  const lines = String(content || '')
    .split(/\r\n|\r|\n/)
    .filter((l) => l.trim().length > 0 && !/^9{94}$/.test(l));

  if (!lines.length) problems.push('The file is empty.');
  ...
  lines.forEach((raw, i) => {
    ...
    if (type === '6') { lastDetail = line; return; }
    if (type !== '7') return;

    const addendaType = line.slice(1, 3);
    if (addendaType !== '99' && addendaType !== '98') return;

    const kind: AchEventKind = addendaType === '99' ? 'return' : 'noc';
    ...
    if (kind === 'return') {
      event.reason = RETURN_CODES[code];
      if (!event.reason) {
      ...
      if (correction) event.correction = correction;
      else eventProblems.push(`Unrecognised change code ${code}. Do not apply it blind.`);
    }
...
/** What a parsed event means for one payment row. */
export interface PaymentOutcome {
  status: 'returned' | 'corrected';
  /** Whether the employee should now be treated as NOT paid. */
  unpaid: boolean;
  stopFutureEntries: boolean;
  reinitiable: boolean;
  summary: string;
 ...
 */
export function outcomeFor(event: AchEvent): PaymentOutcome {
  if (event.kind === 'noc') {
    return {
      status: 'corrected',
      unpaid: false,
