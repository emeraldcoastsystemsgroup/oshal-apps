 ...
 */
export function normalizeRouting(v: unknown): string | null {
  const d = digits(v);
  if (d.length !== 9) return null;
...
export function last4(v: unknown): string {
  const d = digits(v);
...
export function maskAccount(last: string): string {
  return last ? `••••${last}` : '';
}
 ...
 */
export function sealIdentifier(ownerSub: string, plain: string | null): string | null {
  if (!plain) return null;
  if (isEncrypted(plain)) return plain;
  return encryptField(ownerSub, plain);
}
 ...
 */
export function openIdentifier(ownerSub: string, stored: unknown): string | null {
  if (stored === null || stored === undefined || stored === '') return null;
  const out = decryptField(ownerSub, String(stored));
  return out ? String(out) : null;
}

/** What a W-2 needs before it can stop being a preview. */
export interface W2Readiness {
  ready: boolean;
  missing: string[];
}
 ...
 */
export function w2Readiness(company: Record<string, unknown>, employee: Record<string, unknown>): W2Readiness {
  const missing: string[] = [];
  if (!company.ein_encrypted) missing.push('employer EIN');
  ...
  if (!String(company.city || '').trim() || !String(company.state_code || '').trim()
      || !String(company.postal_code || '').trim()) missing.push('employer city/state/ZIP');
  if (!employee.ssn_encrypted) missing.push('employee SSN');
  ...
  if (!String(employee.city || '').trim() || !String(employee.state_code || '').trim()
      || !String(employee.postal_code || '').trim()) missing.push('employee city/state/ZIP');
