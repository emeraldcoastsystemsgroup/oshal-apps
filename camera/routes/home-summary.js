...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        if (failed)
            items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: "camera-ops" });
        else if (!items.length)
            items.push({ text: 'No saved work yet. Open the app to begin.', tone: 'neutral', fix: "camera-ops" });
