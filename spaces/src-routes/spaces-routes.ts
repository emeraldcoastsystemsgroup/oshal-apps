...
function callerSub(req: Request): string | null {
  const oidc = (req as unknown as { oidc?: { user?: { sub?: string; oid?: string } } }).oidc;
...
function callerEmail(req: Request): string | null {
  const user = (req as unknown as { oidc?: { user?: { email?: string; preferred_username?: string } } }).oidc?.user;
...
function clampPairingTtlMinutes(raw: unknown): number {
  const n = Number(raw);
  if (!Number.isFinite(n) || n <= 0) return DEFAULT_PAIRING_TTL_MIN;
...
/** Per-request scratch: the pre-generated scan id + caller sub for multer. */
interface UploadScratch {
  _spacesSub?: string;
  _spacesScanId?: string;
}
...
/** What one completed sim scan flight yields — persisted as the scan's source manifest. */
interface SimScanFlight {
  home: { lat: number; lon: number };
  pattern: { perRing: number; stepDeg: number; ringCount: number };
  plan: MissionPlan;
  captures: unknown[];
  virtualFlightS: number;
}
 ...
 */
async function flySimScanMission(): Promise<SimScanFlight> {
  const pattern = droneScanPattern({
  ...
  const home: GeoPoint = { ...SIM_SCAN_HOME, alt: 0 };
  const errors = validateMission(plan, { maxRadiusM: 100, maxAltM: 120, minAltM: 1 }, home);
  if (errors.length > 0) throw new Error(`generated scan mission failed validation: ${errors.join('; ')}`);
  let virtualNow = 0;
  const drone = new SimDroneProvider({
    droneId: `spaces-scan-${randomUUID().slice(0, 8)}`,
    home: SIM_SCAN_HOME,
    clock: () => virtualNow,
  });
  await drone.arm();
 ...
 */
export function createSpacesRoutes(ctx: AppContext): Router {
  const router = Router();
  const service = new SpatialMappingService(ctx.pool);
  const rf = new RfOverlayService(ctx.pool);
  ...
  const prepUpload = async (req: Request, res: Response, next: () => void): Promise<void> => {
  ...
  router.post('/scans', prepUpload, uploadVideo, async (req: Request, res: Response) => {
    const s = req as unknown as UploadScratch;
    const sub = s._spacesSub as string;
    const scanId = s._spacesScanId as string;
    const file = req.file;
    if (!file) { res.status(400).json({ error: 'a video file is required (multipart field "video")' }); return; }
  ...
  router.post('/scans/import', prepUpload, uploadModel, async (req: Request, res: Response) => {
    const s = req as unknown as UploadScratch;
    const sub = s._spacesSub as string;
    const scanId = s._spacesScanId as string;
    const cleanup = (): void => {
    ...
    const file = req.file;
    if (!file) { res.status(400).json({ error: 'a 3D capture file is required (multipart field "model")' }); return; }
    const ext = path.extname(file.originalname || '').toLowerCase();
    if (!(IMPORT_EXTENSIONS as readonly string[]).includes(ext)) {
      cleanup();
      res.status(400).json({ error: 'unsupported_import_format', accepts: IMPORT_EXTENSIONS });
      return;
    }
    const rawTitle = typeof req.body?.title === 'string' ? req.body.title.trim() : '';
  ...
  router.post('/pair', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const ttlMinutes = clampPairingTtlMinutes((req.body as { ttlMinutes?: unknown } | undefined)?.ttlMinutes);
    const device = String((req.body as { device?: unknown } | undefined)?.device ?? '').slice(0, 40).trim();
    try {
      const minted = await insertCliToken(ctx.pool, {
  ...
  router.get('/scans/:id', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!scan) { res.status(404).json({ error: 'not_found' }); return; }
      res.json({ scan });
    } catch (err) {
  ...
  router.get('/scans/:id/artifact', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const artifact = await service.getArtifactPath(sub, id);
      if (!artifact || !fs.existsSync(artifact)) { res.status(404).json({ error: 'artifact_not_ready' }); return; }
  ...
  router.get('/scans/:id/geometry', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const geo = await service.getGeometryPath(sub, id);
      if (!geo || !fs.existsSync(geo.path)) { res.status(404).json({ error: 'no_geometry' }); return; }
  ...
  router.get('/scans/:id/dimensions', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const dimensions = await service.getDimensions(sub, id);
      if (!dimensions) { res.status(404).json({ error: 'no_dimensions' }); return; }
      res.setHeader('Cache-Control', 'private, no-store');
      res.json({ dimensions });
    } catch (err) {
  ...
  router.get('/scans/:id/poses', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const poses = await service.getPosesPath(sub, id);
      if (!poses || !fs.existsSync(poses)) { res.status(404).json({ error: 'no_poses' }); return; }
      res.setHeader('Cache-Control', 'private, no-store');
      res.sendFile(poses, (err) => { if (err && !res.headersSent) res.status(404).json({ error: 'no_poses' }); });
    } catch (err) {
  ...
  router.post('/scans/:id/rf', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    const demo = req.body?.demo === true;
    const samples = Array.isArray(req.body?.samples) ? req.body.samples : undefined;
    if (!demo && !samples) { res.status(400).json({ error: 'provide { samples: [...] } or { demo: true }' }); return; }
    try {
      const result = await rf.computeOverlay(sub, id, { samples, demo });
      if (!result) { res.status(404).json({ error: 'not_found_or_not_ready' }); return; }
      ...
      if (err instanceof RfInputError) {
  ...
  router.get('/scans/:id/rf/overlay', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const overlay = await rf.getOverlayPath(sub, id);
      if (!overlay || !fs.existsSync(overlay)) { res.status(404).json({ error: 'no_overlay' }); return; }
  ...
  router.get('/scans/:id/rf/summary', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const id = String(req.params.id);
    try {
      const summary = await rf.getSummaryPath(sub, id);
      if (!summary || !fs.existsSync(summary)) { res.status(404).json({ error: 'no_summary' }); return; }
      res.setHeader('Cache-Control', 'private, no-store');
      res.sendFile(summary, (err) => { if (err && !res.headersSent) res.status(404).json({ error: 'no_summary' }); });
    } catch (err) {
  ...
  router.get('/capture-plan', (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const raw = String(req.query.target ?? 'room');
    const targets: CaptureTarget[] = ['room', 'large-room', 'object', 'facade'];
    const target = (targets as string[]).includes(raw) ? (raw as CaptureTarget) : 'room';
    res.json({ plan: generateCapturePlan(target) });
  });
  ...
  router.post('/capture-telemetry', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    const record = sanitizeCaptureTelemetry(req.body);
    if (!record) { res.status(400).json({ error: 'invalid_telemetry' }); return; }
    try {
      const file = captureTelemetryPath(sub, record.sessionId);
      await fs.promises.mkdir(path.dirname(file), { recursive: true });
      const size = await fs.promises.stat(file).then((st) => st.size).catch(() => 0);
      if (size >= CAPTURE_TELEMETRY_MAX_BYTES) {
  ...
  router.post('/drone-scan', async (req: Request, res: Response) => {
    const sub = callerSub(req);
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    if (!(await service.canAcceptScan(sub))) {
...
function sendPage(res: Response, filePath: string): void {
...
function streamArtifact(req: Request, res: Response, filePath: string, id: string): void {
  res.setHeader('Content-Type', 'application/octet-stream');
