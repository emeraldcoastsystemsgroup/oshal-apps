...
const logger = (0, logger_1.createChildLogger)({ module: 'switchboard-compose-routes' });
...
function servePage(dir, file) {
    return (_req, res) => {
        res.sendFile(path.join(dir, file), (err) => {
            if (err) {
                logger.error({ err, file }, 'Failed to serve switchboard compose surface');
...
function platformProviders(platform) {
...
async function withTimeout(work, ms, label) {
    let timer;
    const deadline = new Promise((_, rej) => { timer = setTimeout(() => rej(new Error(`${label} timed out after ${ms}ms`)), ms); });
    try {
 ...
 */
async function generateComposeImage(ctx, sub, prompt) {
    const provider = await (0, video_generation_1.resolveStoryboardImageProvider)();
    const result = await withTimeout(provider.generateWithMeta
        ? provider.generateWithMeta(prompt, null)
        : provider.generate(prompt, null).then((image) => ({ image, costUsd: null, model: `${provider.id}-default` })), IMAGE_TIMEOUT_MS, 'image generation');
...
async function publishToX(ctx, sub, text) {
    ...
    if (r.status >= 200 && r.status < 300)
        return { ok: true, target: 'x', tweetId: j?.data?.id ?? null };
...
async function publishToFacebook(ctx, sub, text) {
    ...
    if (!userTok)
        return { ok: false, code: 409, error: 'no_facebook_connection', message: 'Connect Facebook at /utilities first.' };
    const pagesRes = await fetch(`${graph}/me/accounts?fields=id,name,access_token&access_token=${encodeURIComponent(userTok)}`);
    if (!pagesRes.ok)
        return { ok: false, code: 502, status: pagesRes.status, error: (await pagesRes.text()).slice(0, 300) };
    const page = (await pagesRes.json()).data?.[0];
    if (!page)
        return { ok: false, code: 409, error: 'no_facebook_pages', message: 'You manage no Facebook Page on this account.' };
 ...
 */
function publishTo(ctx, sub, platform, text) {
    if (platform === 'x' || platform === 'twitter')
        return publishToX(ctx, sub, text);
    if (platform === 'facebook')
        return publishToFacebook(ctx, sub, text);
    if (platform === 'linkedin')
        return publishToLinkedIn(ctx, sub, text);
 ...
 */
function createComposeRoutes(ctx) {
    ...
    router.get('/targets', async (req, res) => {
        ...
        try {
            const connectedRows = (await ctx.pool.query("SELECT provider FROM oshal_connections WHERE user_sub = $1 AND status = 'connected'", [sub])).rows;
