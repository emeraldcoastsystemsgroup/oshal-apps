...
exports.createInboxRoutes = createInboxRoutes;
...
const logger = (0, logger_1.createChildLogger)({ module: 'switchboard-inbox-routes' });
const GRAPH_MESSAGES = 'https://graph.microsoft.com/v1.0/me/messages?$select=id,subject,bodyPreview,from,receivedDateTime,isRead,importance&$top=25&$orderby=receivedDateTime%20desc';
 ...
 */
async function storedInboxItems(pool, sub, days) {
    const rows = (await pool.query(`SELECT msg_id, from_addr, subject, snippet, category, received_at FROM oshal_inbox_messages
      WHERE user_sub = $1 AND category <> 'promotions' AND received_at > NOW() - ($2 || ' days')::interval
      ORDER BY received_at DESC LIMIT 120`, [sub, String(days)])).rows;
    const out = [];
    for (const r of rows) {
        const from = r.from_addr || '';
        const subject = r.subject || '(no subject)';
        if (isBulk(from, subject))
            continue;
 ...
 */
async function slackItems(token) {
    const me = await (0, slack_client_1.slackIdentity)(token);
    if (!me.ok)
        throw new Error('slack auth failed');
    const mention = me.userId ? `<@${me.userId}>` : null;
    const { messages } = await (0, slack_client_1.pullSlackFeed)(token, { maxChannels: 15, perChannel: 10, maxUsers: 40 });
    const out = [];
    for (const m of messages) {
        if (me.userId && m.userId === me.userId)
            continue; // skip the caller's own messages
 ...
 */
async function assembleInbox(ctx, sub, opts) {
    const want = (p) => !opts.providers || opts.providers.has(p);
    const msTok = (want('microsoft') || want('outlook'))
        ? (await (0, connectors_routes_1.getValidAccessToken)(ctx.pool, sub, 'microsoft')) || (await (0, connectors_routes_1.getValidAccessToken)(ctx.pool, sub, 'outlook'))
        : null;
    const slackTok = want('slack') ? await (0, connectors_routes_1.getValidAccessToken)(ctx.pool, sub, 'slack') : null;
    ...
    if (opts.workspaceId)
        for (const it of items)
            it.workspaceId = opts.workspaceId;
 ...
 */
function createInboxRoutes(ctx) {
        ...
        try {
            ...
            if (!connected) {
                if (req.query.surface === '1') {
                    res.json({ connected: false, message: 'Connect Gmail, Outlook, or Slack at /utilities to fill your inbox.' });
                    return;
                }
