...
exports.counterpartKey = counterpartKey;
exports.isBulk = isBulk;
 ...
 */
function buildThreads(rows, options) {
    ...
    for (const row of rows || []) {
        ...
        if (ts >= acc.latestTs) {
            ...
            acc.latestFrom = from;
            if (!acc.address)
                acc.address = emailAddressOf(from);
        }
    }
    const threads = [];
    for (const acc of byKey.values()) {
        ...
        for (const it of items)
            if (!channels.includes(it.source))
                channels.push(it.source);
        threads.push({
            key: acc.key,
