...
/** One channel submission in a broadcast: the exact text to publish on that platform. */
export interface BroadcastPost {
  platform: string;
  text: string;
}

/** A validated broadcast plan, or the single error that rejects the whole request. */
export interface BroadcastPlan {
  posts: BroadcastPost[];
  error?: string;
}

/** One channel's outcome. Extra keys (tweetId, postId, …) pass through from the publisher. */
...
export const MAX_BROADCAST_CHANNELS = 6;
 ...
 */
export function canonicalChannel(platform: string): string {
...
export function normalizeBroadcast(
  body: unknown,
  publishable: ReadonlySet<string> | string[],
  limits: Record<string, number>,
): BroadcastPlan {
  const allowed = publishable instanceof Set ? publishable : new Set(publishable);
  const raw = (body as { posts?: unknown })?.posts;
  if (!Array.isArray(raw) || !raw.length) return { posts: [], error: 'posts array required (one entry per channel)' };
  if (raw.length > MAX_BROADCAST_CHANNELS) return { posts: [], error: `too many channels (max ${MAX_BROADCAST_CHANNELS})` };
  const posts: BroadcastPost[] = [];
  const seen = new Set<string>();
  for (const entry of raw) {
    const platform = canonicalChannel((entry as { platform?: string })?.platform || '');
    const text = String((entry as { text?: string })?.text || '').trim();
    if (!platform || !allowed.has(platform)) return { posts: [], error: `unsupported platform: ${platform || '(missing)'}` };
    if (seen.has(platform)) return { posts: [], error: `duplicate channel: ${platform} (one send per channel per broadcast)` };
    if (!text) return { posts: [], error: `empty text for ${platform}` };
    const limit = limits[platform];
    if (typeof limit === 'number' && text.length > limit) return { posts: [], error: `${platform} text is ${text.length} chars (max ${limit})` };
    seen.add(platform);
...
export async function runFanout(
  posts: BroadcastPost[],
  publish: (platform: string, text: string) => Promise<Record<string, unknown>>,
): Promise<{ results: FanoutResult[]; summary: FanoutSummary }> {
  ...
  for (const post of posts) {
    try {
      ...
      results.push({ platform: post.platform, ok: false, error: (err as Error).message || 'publish threw' });
    }
  }
