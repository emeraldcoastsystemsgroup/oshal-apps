...
exports.DRONE_FITS = void 0;
exports.buildDrone = buildDrone;
exports.fitForSensorSet = fitForSensorSet;
exports.fitById = fitById;
const sensor_set_1 = require("../drone/sensor-set");
const airframe_1 = require("./airframe");
...
function printedParts(l, fit) {
    const mastMass = Math.round(10 + l.mastMm * 0.3);
    return [
        { id: 'centre-plate', name: 'Centre plate (top and bottom)', qty: 2, material: 'PETG or PA-CF', printNotes: 'flat, 6 walls, 40 % gyroid', massEachG: 38, cad: (0, airframe_1.centrePlate)(l) },
        { id: 'arm', name: 'Arm with integral motor mount', qty: 4, material: 'PA-CF (PETG acceptable)', printNotes: 'flat, solid infill, 0.2 mm layers', massEachG: fit.propIn >= 7 ? 18 : 15, cad: (0, airframe_1.arm)(l) },
        { id: 'guard', name: 'Prop guard quarter ring', qty: 4, material: 'TPU 95A or PETG', printNotes: 'mandatory indoors; sits above the prop plane as well as around it', massEachG: fit.propIn >= 7 ? 13 : 11.25, cad: (0, airframe_1.guardSegment)(l) },
        { id: 'mast', name: 'Sensor mast', qty: 1, material: 'PETG', printNotes: 'vertical, 4 walls; the seat at 0.12 mm layers', massEachG: mastMass, cad: (0, airframe_1.mast)(l) },
        { id: 'tof-mount', name: 'ToF camera and flow mount', qty: 1, material: 'PETG', printNotes: 'nadir bracket under the bottom plate', massEachG: 12, cad: (0, airframe_1.tofMount)() },
        { id: 'battery-tray', name: 'Battery tray', qty: 1, material: 'PETG', printNotes: 'two strap slots', massEachG: 18, cad: (0, airframe_1.batteryTray)() },
        { id: 'foot', name: 'Landing foot', qty: 4, material: 'TPU', printNotes: 'on the arm underside', massEachG: 3, cad: (0, airframe_1.landingFoot)() },
        { id: 'pad', name: 'Landing pad', qty: 1, material: 'PETG', printNotes: 'the AprilTag printed on paper under a clear insert; not flown', massEachG: 380, cad: (0, airframe_1.landingPad)() },
    ];
}
const PRINT_RULES = [
 ...
 * @returns The design.
 */
function buildDrone(fitId) {
    const fit = exports.DRONE_FITS[fitId];
    const sensorSet = sensor_set_1.DRONE_SENSOR_SETS[fit.sensorSet];
    const layout = (0, airframe_1.layoutFor)(fit.propIn, Math.max(40, Math.round(sensorSet.mastM * 1000)));
    const parts = printedParts(layout, fit);
    const props = { id: 'props', name: `${fit.propIn} in two-blade props (printed ~${fit.propIn >= 7 ? 9 : 7} g each; commercial ~5 g)`, qty: 4, massEachG: fit.propIn >= 7 ? 9 : 7, role: 'aero-lab designs the printed blade', approxUsdEach: 3 };
    ...
    const sizing = (0, propulsion_1.sizeHover)({ propIn: fit.propIn, auwG: allUpG, cells: fit.cells, mAh: fit.mAh });
    const battery = { id: 'battery', name: `${fit.cells}S ${fit.mAh} mAh LiPo`, qty: 1, massEachG: fit.batteryG, role: '', approxUsdEach: fit.cells >= 6 ? 45 : 30 };
    const approxUsd = Math.round([...bought, battery].reduce((a, p) => a + p.qty * p.approxUsdEach, 0));
    return { fit, sensorSet, layout, parts, bought: [...bought, battery], massBudget: { printedG, boughtG, batteryG: fit.batteryG, allUpG }, sizing, tipSpeedMps: (0, propulsion_1.tipSpeedMps)(fit.propIn, fit.hoverRpm), approxUsd, printRules: PRINT_RULES };
}
/**
...
function fitForSensorSet(sensorSet) {
    return (Object.values(exports.DRONE_FITS).find((f) => f.sensorSet === sensorSet) ?? exports.DRONE_FITS['recon-mini']).id;
}
function fitById(id) {
