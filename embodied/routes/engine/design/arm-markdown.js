...
exports.jointTable = jointTable;
exports.armSizingTable = armSizingTable;
exports.armPartsTable = armPartsTable;
exports.armMassTable = armMassTable;
exports.armBoughtTable = armBoughtTable;
exports.armDesignMarkdown = armDesignMarkdown;
const arm_design_1 = require("./arm-design");
...
function armSizingTable(fitId) {
    const base = arm_design_1.ARM_FITS[fitId];
    const rows = ['| Upper arm / forearm | Payload | Reach | Shoulder needs | Shoulder drive | Elbow drive | Moving mass |', '|---|---|---|---|---|---|---|'];
    for (const [u, f] of [[180, 180], [170, 170], [160, 160], [150, 160]])
        for (const pay of [0.15, 0.1]) {
            const d = (0, arm_design_1.buildArmFrom)({ ...base, layout: { ...base.layout, upperArmMm: u, forearmMm: f }, payloadKg: pay });
            const chosen = u === base.layout.upperArmMm && f === base.layout.forearmMm && pay === base.payloadKg;
            const drive = (i) => { const j = d.joints[i]; return j.drive ? `${j.drive.cfg.servos} × servo${j.drive.cfg.ratio > 1 ? `, ${j.drive.cfg.ratio}:1 belt` : ''}, margin ${n2(j.drive.margin)}` : 'none'; };
 ...
 * @returns Markdown.
 */
function armDesignMarkdown(fitId) {
    const d = (0, arm_design_1.buildArm)(fitId);
    const s = d.fit.servo;
    return [
        `# ${d.fit.label}`,
        '',
        ...
        `- A servo holds at most **${Math.round(servos_1.CONTINUOUS_FRACTION * 100)} %** of its rated stall continuously: the bench test ran stable at half its rating and tripped its overload protection at two thirds.`,
        `- A joint must deliver its worst static gravity torque × **${servos_1.DYNAMIC_ALLOWANCE}** (to accelerate the same load), plus, on a vertical axis, the stretched arm's inertia × 4 rad/s². The worst pose is searched over the joint limits, not assumed.`,
        `- The node commands **${Math.round(servos_1.SPEED_FRACTION * 100)} %** of the no-load speed. Each joint takes the simplest drive that holds: one servo, two in parallel (only the shoulder has two cheeks), then a printed belt stage.`,
        '',
        jointTable(d),
        '',
        ...
        '## Repeatability',
        '',
        `The servo's measured backlash (about ${servos_1.BACKLASH_DEG}°) at each pitch and yaw joint, times its lever to the tool with the arm stretched: **${d.repeatability.worstMm} mm** stacked worst case, **${d.repeatability.rssMm} mm** root sum of squares. A grasp is planned with a jaw opening wider than the object by at least the stacked figure; the swarm's map and the wrist camera close the rest.`,
        '',
        '## Print rules',
        '',
...
//# sourceMappingURL=arm-markdown.js.map