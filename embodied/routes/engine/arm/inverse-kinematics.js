...
exports.ikSolutions = ikSolutions;
exports.solveIk = solveIk;
const transform_1 = require("../math/transform");
const vec_1 = require("../math/vec");
const arm_model_1 = require("./arm-model");
const DEFAULTS = {
    maxIterations: 150,
...
function poseError(target, current, w) {
    const pt = (0, transform_1.position)(target);
    const pc = (0, transform_1.position)(current);
    const rv = (0, transform_1.rotationVector)((0, transform_1.multiply)(target, (0, transform_1.rotationInverse)(current)));
...
function jacobian(spec, q, target, base, w) {
    const h = 1e-6;
    const cols = q.map((_, j) => {
        const qj = q.slice();
        qj[j] += h;
        const e = poseError(target, (0, arm_model_1.forwardKinematics)(spec, qj).tcp, w);
...
function solveDamped(J, e, lambda) {
    const n = J[0].length;
    const A = Array.from({ length: n }, (_, r) => Array.from({ length: n + 1 }, (_, c) => {
        if (c === n)
            return J.reduce((s, row, k) => s + row[r] * e[k], 0);
...
function descend(spec, target, seed, o) {
    let q = (0, arm_model_1.clampToLimits)(spec, seed);
    let lambda = 0.01;
    let e = poseError(target, (0, arm_model_1.forwardKinematics)(spec, q).tcp, o.orientationWeight);
    let cost = (0, vec_1.norm)([e[0], e[1], e[2]]) + (0, vec_1.norm)([e[3], e[4], e[5]]);
    for (let it = 1; it <= o.maxIterations; it += 1) {
        const pErr = (0, vec_1.norm)([e[0], e[1], e[2]]);
        const rErr = (0, vec_1.norm)([e[3], e[4], e[5]]) / o.orientationWeight;
        if (pErr < o.positionTolerance && rErr < o.orientationTolerance)
            return { ok: true, q, iterations: it - 1, positionError: pErr, orientationError: rErr };
        const J = jacobian(spec, q, target, e, o.orientationWeight);
        const delta = solveDamped(J, e, lambda).map((d) => Math.max(-0.3, Math.min(0.3, d)));
        const candidate = (0, arm_model_1.clampToLimits)(spec, q.map((v, i) => v + delta[i]));
        const eNew = poseError(target, (0, arm_model_1.forwardKinematics)(spec, candidate).tcp, o.orientationWeight);
        const costNew = (0, vec_1.norm)([eNew[0], eNew[1], eNew[2]]) + (0, vec_1.norm)([eNew[3], eNew[4], eNew[5]]);
        if (costNew < cost) {
            q = candidate;
            e = eNew;
            cost = costNew;
 ...
 */
function ikSolutions(spec, target, seed, opts = {}) {
    const T = Array.isArray(target) ? target : (0, transform_1.fromPose)(target);
    const o = { ...DEFAULTS, ...opts };
    const reach = (0, vec_1.distance)((0, transform_1.position)(T), (0, arm_model_1.shoulderPoint)(spec));
    if (reach > (0, arm_model_1.maxReach)(spec) - 1e-6) {
        return { solutions: [], bestFailure: { ok: false, q: (0, arm_model_1.clampToLimits)(spec, seed), iterations: 0, positionError: reach - (0, arm_model_1.maxReach)(spec), orientationError: Math.PI, reason: 'out_of_reach' } };
    }
    const solutions = [];
    ...
    for (const s of [seed, ...(opts.seeds ?? []), ...FALLBACK_SEEDS]) {
        ...
        if (!r.ok) {
            if (!bestFailure || r.positionError + r.orientationError < bestFailure.positionError + bestFailure.orientationError)
                bestFailure = r;
            continue;
        }
        if (!solutions.some((k) => k.q.every((v, i) => Math.abs(v - r.q[i]) < 1e-3)))
            solutions.push(r);
    }
 ...
 */
function solveIk(spec, target, seed, opts = {}) {
    const T = Array.isArray(target) ? target : (0, transform_1.fromPose)(target);
    const o = { ...DEFAULTS, ...opts };
    const reach = (0, vec_1.distance)((0, transform_1.position)(T), (0, arm_model_1.shoulderPoint)(spec));
    if (reach > (0, arm_model_1.maxReach)(spec) - 1e-6) {
        return { ok: false, q: (0, arm_model_1.clampToLimits)(spec, seed), iterations: 0, positionError: reach - (0, arm_model_1.maxReach)(spec), orientationError: Math.PI, reason: 'out_of_reach' };
    }
    ...
    for (const s of seeds) {
        ...
        if (r.ok)
            return r;
        if (!best || r.positionError + r.orientationError < best.positionError + best.orientationError)
            best = r;
    }
    return best;
}
//# sourceMappingURL=inverse-kinematics.js.map