...
exports.DEFAULT_RELOCALIZE = exports.wideRegister = exports.WIDE_RADII = exports.PLANAR_REGISTER = exports.DEFAULT_REGISTER = void 0;
exports.toBodyFrame = toBodyFrame;
exports.fromBodyFrame = fromBodyFrame;
exports.concatBody = concatBody;
exports.registerSweep = registerSweep;
...
exports.DEFAULT_REGISTER = { radii: [0.15, 0.15, 0.12, 0.10, 0.08, 0.06, 0.05, 0.04, 0.03, 0.03], stride: 6, minMatched: 300, settleM: 0.002, mode: 'full', minObservability: 40 };
...
exports.PLANAR_REGISTER = { ...exports.DEFAULT_REGISTER, mode: 'planar', stride: 1, minMatched: 100 };
...
exports.WIDE_RADII = [0.30, 0.30, 0.25, 0.20, 0.15, 0.12, 0.10, 0.08, 0.06, 0.05, 0.04, 0.03, 0.03];
/** @description A preset widened for recovery. @param base - The set's usual preset. @returns The same rules with the wide capture. */
const wideRegister = (base) => ({ ...base, radii: exports.WIDE_RADII });
exports.wideRegister = wideRegister;
/**
 ...
 */
function toBodyFrame(sweep, pose) {
    ...
    sweep.hits.forEach((h, i) => {
        ...
        names.push(h.name);
        paints.push(h.paint);
        normals[3 * i] = c * h.normal[0] + s * h.normal[1];
        normals[3 * i + 1] = -s * h.normal[0] + c * h.normal[1];
        normals[3 * i + 2] = h.normal[2];
    });
    const misses = new Float32Array(sweep.misses.length * 3);
    sweep.misses.forEach((p, i) => into(misses, i, p));
    const o = new Float32Array(3);
    into(o, 0, sweep.origin);
 ...
 */
function fromBodyFrame(body, pose) {
    const c = Math.cos(pose.yaw);
    const s = Math.sin(pose.yaw);
    const place = (bx, by, bz) => [pose.x + c * bx - s * by, pose.y + s * bx + c * by, pose.z + bz];
    ...
    for (let i = 0; i < body.n; i += 1) {
        ...
        const normal = [c * body.normals[3 * i] - s * body.normals[3 * i + 1], s * body.normals[3 * i] + c * body.normals[3 * i + 1], body.normals[3 * i + 2]];
    ...
    const misses = [];
    for (let i = 0; i < body.m; i += 1)
        misses.push(place(body.misses[3 * i], body.misses[3 * i + 1], body.misses[3 * i + 2]));
 ...
 */
function concatBody(parts) {
    const n = parts.reduce((a, p) => a + p.n, 0);
    const m = parts.reduce((a, p) => a + p.m, 0);
    const hits = new Float32Array(3 * n);
    const normals = new Float32Array(3 * n);
    const misses = new Float32Array(3 * m);
    const names = [];
    const paints = [];
    let hi = 0;
    let mi = 0;
    for (const p of parts) {
        hits.set(p.hits.subarray(0, 3 * p.n), 3 * hi);
        normals.set(p.normals.subarray(0, 3 * p.n), 3 * hi);
        hi += p.n;
        misses.set(p.misses.subarray(0, 3 * p.m), 3 * mi);
        mi += p.m;
        names.push(...p.names);
        paints.push(...p.paints);
    }
 ...
 */
function matchPass(map, body, pose, radius, opts) {
    const c = Math.cos(pose.yaw);
    const s = Math.sin(pose.yaw);
    ...
    for (let i = 0; i < body.n; i += opts.stride) {
        ...
        if (opts.mode === 'planar' && Math.abs(body.normals[3 * i + 2]) > 0.5)
            continue;
        usable += 1;
        const bx = body.hits[3 * i];
        const by = body.hits[3 * i + 1];
        const bz = body.hits[3 * i + 2];
        const px = pose.x + c * bx - s * by;
        const py = pose.y + s * bx + c * by;
        const pz = pose.z + bz;
        ...
        const qx = c * body.normals[3 * i] - s * body.normals[3 * i + 1];
        const qy = s * body.normals[3 * i] + c * body.normals[3 * i + 1];
        const qz = body.normals[3 * i + 2];
        if (qx * a.n[0] + qy * a.n[1] + qz * a.n[2] < 0.7)
            continue;
        const r = opts.mode === 'planar' ? a.n[0] * (px - a.p[0]) + a.n[1] * (py - a.p[1]) : a.n[0] * (px - a.p[0]) + a.n[1] * (py - a.p[1]) + a.n[2] * (pz - a.p[2]);
        row[0] = a.n[0];
        row[1] = a.n[1];
        row[2] = a.n[2];
        row[3] = a.n[0] * -(py - pose.y) + a.n[1] * (px - pose.x);
        for (let u = 0; u < 4; u += 1) {
            atb[u] += -r * row[u];
            for (let v = 0; v < 4; v += 1)
                ata[4 * u + v] += row[u] * row[v];
        }
        n += 1;
        sq += r * r;
    }
 ...
 */
function registerSweep(map, body, guess, opts = exports.DEFAULT_REGISTER) {
    ...
    for (const radius of opts.radii) {
        ...
        const d = [0, 0, 0, 0];
        unknowns.forEach((u, i) => { d[u] = x[i]; });
        lastStep = Math.hypot(d[0], d[1], d[2]);
        pose = { x: pose.x + d[0], y: pose.y + d[1], z: pose.z + d[2], yaw: pose.yaw + d[3] };
        if (opts.trace)
            opts.trace({ radius, matched: pass.n, residualM: pass.rms, stepM: lastStep, yawStepRad: d[3] });
    }
    ...
    if (converged && !observable && spectrum.weakAxisRad !== null) {
        ...
        const wx = Math.cos(spectrum.weakAxisRad);
        const wy = Math.sin(spectrum.weakAxisRad);
...
function xySpectrum(ata) {
    ...
    const big = (tr + disc) / 2;
    const small = Math.max(0, (tr - disc) / 2);
    if (big <= 1e-9)
        return { ratio: 0, small: 0, weakAxisRad: null };
    const vx = Math.abs(b) > 1e-12 ? b : (small - d);
    const vy = Math.abs(b) > 1e-12 ? small - a : b;
...
exports.DEFAULT_RELOCALIZE = { searchM: 0.6, stepM: 0.05, yawSearchRad: 0.35, yawStepRad: 0.035, sample: 300, minScore: 0.5, uniqueness: 0.85 };
/**
 ...
 */
function relocalize(map, body, guess, opts = exports.DEFAULT_REGISTER, ropts = exports.DEFAULT_RELOCALIZE) {
    ...
    const score = (p) => {
        ...
        let hit = 0;
        for (const i of pts) {
            const bx = body.hits[3 * i] - RELOC_INWARD_M * body.normals[3 * i];
            const by = body.hits[3 * i + 1] - RELOC_INWARD_M * body.normals[3 * i + 1];
            const bz = body.hits[3 * i + 2] - RELOC_INWARD_M * body.normals[3 * i + 2];
            const q = [p.x + c * bx - s * by, p.y + s * bx + c * by, p.z + bz];
            const [ci, cj, ck] = map.toCell(q);
            if (map.inside(ci, cj, ck) && map.stateAt(q) === voxel_map_1.OCCUPIED)
                hit += 1;
        }
        return hit / pts.length;
    };
    const cells = [];
    const n = Math.round(ropts.searchM / ropts.stepM);
    const m = Math.round(ropts.yawSearchRad / ropts.yawStepRad);
    for (let ix = -n; ix <= n; ix += 1)
        for (let iy = -n; iy <= n; iy += 1)
            for (let iw = -m; iw <= m; iw += 1) {
    ...
    const best = cells[0];
    const apart = (p) => Math.hypot(p.x - best.pose.x, p.y - best.pose.y) > 2 * ropts.stepM || Math.abs(p.yaw - best.pose.yaw) > 2 * ropts.yawStepRad;
