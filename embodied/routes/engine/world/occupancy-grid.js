 ...
 * @returns The grid.
 */
function buildGrid(room, obstacles, inflate = 0.325, res = 0.05) {
...
const cellToWorld = (g, i, j) => ({ x: g.originX + (i + 0.5) * g.res, y: g.originY + (j + 0.5) * g.res });
exports.cellToWorld = cellToWorld;
/** @description Is a world point in a free cell? */
function isFree(g, p) {
    if (p.x < g.originX || p.y < g.originY || p.x >= g.originX + g.width * g.res || p.y >= g.originY + g.height * g.res)
        return false;
 ...
 */
function lineClear(g, a, b, supercover = false) {
    if (!supercover)
        return lineClearBresenham(g, a, b);
    const ax = (a.x - g.originX) / g.res;
    const ay = (a.y - g.originY) / g.res;
    const bx = (b.x - g.originX) / g.res;
    const by = (b.y - g.originY) / g.res;
    const end = worldToCell(g, b);
    let i = Math.min(g.width - 1, Math.max(0, Math.floor(ax)));
    let j = Math.min(g.height - 1, Math.max(0, Math.floor(ay)));
    const dx = bx - ax;
    const dy = by - ay;
    ...
    const tDeltaX = stepI === 0 ? Number.POSITIVE_INFINITY : Math.abs(1 / dx);
    const tDeltaY = stepJ === 0 ? Number.POSITIVE_INFINITY : Math.abs(1 / dy);
    for (let guard = 0; guard < g.width + g.height + 2; guard += 1) {
        if (i < 0 || j < 0 || i >= g.width || j >= g.height || g.cells[j * g.width + i] !== 0)
            return false;
        if (i === end.i && j === end.j)
            return true;
        if (tMaxX < tMaxY) {
...
function lineClearBresenham(g, a, b) {
...
class OpenSet {
    ...
    less(a, b) {
        const A = this.items[a];
        const B = this.items[b];
    ...
    sink(k) {
        for (;;) {
            ...
            const r = l + 1;
            let m = k;
            if (l < this.items.length && this.less(l, m))
                m = l;
            if (r < this.items.length && this.less(r, m))
                m = r;
            if (m === k)
                return;
 ...
 */
function planPath(g, start, goal) {
    if (!isFree(g, start) || !isFree(g, goal))
        return null;
    const s = worldToCell(g, start);
    const t = worldToCell(g, goal);
    const sIdx = s.j * g.width + s.i;
    const tIdx = t.j * g.width + t.i;
    ...
    const heuristic = (idx) => {
        const dx = Math.abs((idx % g.width) - t.i);
        const dy = Math.abs(Math.floor(idx / g.width) - t.j);
    ...
    while (open.size) {
        ...
        closed[cur] = 1;
        if (cur === tIdx)
            return reconstruct(g, parent, cur, start, goal);
        expand(g, cur, gCost, parent, closed, open, heuristic);
    }
    return null;
}
/** @description Push every admissible neighbour of a cell. */
function expand(g, cur, gCost, parent, closed, open, heuristic) {
    const ci = cur % g.width;
    const cj = Math.floor(cur / g.width);
    for (const [di, dj, cost] of NEIGHBOURS) {
        const ni = ci + di;
        const nj = cj + dj;
        if (ni < 0 || nj < 0 || ni >= g.width || nj >= g.height)
            continue;
        const nIdx = nj * g.width + ni;
        if (g.cells[nIdx] || closed[nIdx])
            continue;
        if (di && dj && (g.cells[cj * g.width + ni] || g.cells[nj * g.width + ci]))
            continue;
...
function reconstruct(g, parent, goalIdx, start, goal) {
    const cells = [];
    for (let c = goalIdx; c !== -1; c = parent[c])
        cells.push(c);
    cells.reverse();
    const pts = cells.map((c) => (0, exports.cellToWorld)(g, c % g.width, Math.floor(c / g.width)));
 ...
 */
function simplifyPath(g, path, supercover = false) {
    if (path.length <= 2)
        return path.slice();
    const out = [path[0]];
    let i = 0;
    while (i < path.length - 1) {
        let j = path.length - 1;
        while (j > i + 1 && !lineClear(g, path[i], path[j], supercover))
            j -= 1;
        out.push(path[j]);
        i = j;
    }
