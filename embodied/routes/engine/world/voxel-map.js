...
exports.VoxelMap = exports.SURFACE_Z_CAP = exports.OCCUPIED = exports.FREE = exports.UNKNOWN = void 0;
/** @description Voxel states. */
exports.UNKNOWN = 0;
exports.FREE = 1;
exports.OCCUPIED = 2;
/** @description Hits above this height never count toward the height map (ceilings, upper walls). */
exports.SURFACE_Z_CAP = 1.6;
...
class VoxelMap {
    ...
    column(x, y) {
        const i = Math.floor((x - this.bounds.minX) / this.res);
        const j = Math.floor((y - this.bounds.minY) / this.res);
    ...
    stateAt(p) {
        ...
        if (!this.inside(i, j, k))
            return exports.OCCUPIED;
    ...
    set(i, j, k, state) {
        ...
        if (this.cells[idx] !== state) {
            if (this.cells[idx] === exports.OCCUPIED)
                this.anchors.delete(idx);
            this.cells[idx] = state;
            this.version += 1;
        }
    }
    /** @description How many registration anchors the map holds (one per sensed occupied voxel). */
    get anchorCount() { return this.anchors.size; }
    wallIndex = null;
    ...
    wallAnchors() {
        if (this.wallIndex && this.wallIndex.version === this.version)
            return this.wallIndex.cells;
        const cells = new Map();
        for (const a of this.anchors.values()) {
            if (Math.abs(a.n[2]) > 0.5)
                continue;
     ...
     */
    nearestWallAnchor(p, radius) {
        const walls = this.wallAnchors();
        const i0 = Math.floor((p[0] - this.bounds.minX) / this.res);
        const j0 = Math.floor((p[1] - this.bounds.minY) / this.res);
        ...
        for (let j = j0 - r; j <= j0 + r; j += 1)
            for (let i = i0 - r; i <= i0 + r; i += 1) {
                if (i < 0 || j < 0 || i >= this.nx || j >= this.ny)
                    continue;
                const list = walls.get(j * this.nx + i);
                if (!list)
                    continue;
                for (const a of list) {
                    const dd = (a.p[0] - p[0]) ** 2 + (a.p[1] - p[1]) ** 2;
                    if (dd < bestD) {
                        bestD = dd;
                        best = a;
                    }
     ...
     */
    nearestAnchor(p, radius) {
        const [i0, j0, k0] = this.toCell(p);
        const r = Math.ceil(radius / this.res);
        let best = null;
        let bestD = radius * radius;
        for (let k = k0 - r; k <= k0 + r; k += 1)
            for (let j = j0 - r; j <= j0 + r; j += 1)
                for (let i = i0 - r; i <= i0 + r; i += 1) {
                    if (!this.inside(i, j, k))
                        continue;
     ...
     */
    integrateRay(origin, end, hit, normal) {
        ...
        if (len < 1e-9) {
            if (hit)
                this.set(ei, ej, ek, exports.OCCUPIED);
            return;
        }
        let i = ci;
        let j = cj;
        ...
        const bound = (c, o, d, min, step) => (step === 0 ? Number.POSITIVE_INFINITY : ((min + (c + (step > 0 ? 1 : 0)) * this.res) - o) / d);
        let tMaxX = bound(i, origin[0], dx, this.bounds.minX, stepI);
        let tMaxY = bound(j, origin[1], dy, this.bounds.minY, stepJ);
        let tMaxZ = bound(k, origin[2], dz, this.bounds.minZ, stepK);
        const tDeltaX = stepI ? Math.abs(this.res / dx) : Number.POSITIVE_INFINITY;
        const tDeltaY = stepJ ? Math.abs(this.res / dy) : Number.POSITIVE_INFINITY;
        const tDeltaZ = stepK ? Math.abs(this.res / dz) : Number.POSITIVE_INFINITY;
        let guard = 0;
        while (!(i === ei && j === ej && k === ek) && guard < 4000) {
            if (this.inside(i, j, k)) {
                const idx = this.index(i, j, k);
                if (this.cells[idx] !== exports.FREE) {
                    if (this.cells[idx] === exports.OCCUPIED)
                        this.anchors.delete(idx);
                    this.cells[idx] = exports.FREE;
                    this.version += 1;
                }
            }
            ...
            guard += 1;
            if (tMaxX > 1 && tMaxY > 1 && tMaxZ > 1 && !(i === ei && j === ej && k === ek))
                break;
        }
        if (hit) {
            ...
            if (!(hi === ai && hj === aj && hk === ak) && this.inside(ai, aj, ak) && this.cells[this.index(ai, aj, ak)] !== exports.OCCUPIED)
                this.set(ai, aj, ak, exports.FREE);
            this.set(hi, hj, hk, exports.OCCUPIED);
            if (normal && this.inside(hi, hj, hk)) {
                const ai = this.index(hi, hj, hk);
                if (!this.anchors.has(ai))
                    this.anchors.set(ai, { p: [end[0], end[1], end[2]], n: normal });
            }
            ...
            if (isTop && col >= 0)
                this.topSeen[col] = 1;
            if (isTop && col >= 0 && into[2] <= exports.SURFACE_Z_CAP && into[2] > this.topZ[col]) {
                this.topZ[col] = into[2];
                this.version += 1;
            }
        }
        else
            this.set(ei, ej, ek, exports.FREE);
    }
    /** @description Integrate a whole sweep and count it as one scan. */
    integrateSweep(sweep) {
    ...
    integrateSweeps(sweeps) {
        for (const sweep of sweeps) {
            for (const h of sweep.hits)
                this.integrateRay(sweep.origin, h.point, true, h.normal);
            for (const m of sweep.misses)
                this.integrateRay(sweep.origin, m, false);
        }
        this.scans += 1;
        this.version += 1;
    }
     ...
     */
    clearBox(min, max, restZ) {
        const [i0, j0, k0] = this.toCell(min);
        const [i1, j1, k1] = this.toCell(max);
        for (let k = k0; k <= k1; k += 1)
            for (let j = j0; j <= j1; j += 1)
                for (let i = i0; i <= i1; i += 1)
                    this.set(i, j, k, exports.FREE);
        for (let j = j0; j <= j1; j += 1)
            for (let i = i0; i <= i1; i += 1)
                if (i >= 0 && j >= 0 && i < this.nx && j < this.ny)
                    this.topZ[j * this.nx + i] = Math.min(this.topZ[j * this.nx + i], restZ);
        this.version += 1;
    }
    ...
    markBox(min, max) {
        const [i0, j0, k0] = this.toCell(min);
        const [i1, j1, k1] = this.toCell([max[0] - 1e-6, max[1] - 1e-6, max[2] - 1e-6]);
        for (let k = k0; k <= k1; k += 1)
            for (let j = j0; j <= j1; j += 1)
                for (let i = i0; i <= i1; i += 1)
                    this.set(i, j, k, exports.OCCUPIED);
        for (let j = j0; j <= j1; j += 1)
            for (let i = i0; i <= i1; i += 1)
                if (i >= 0 && j >= 0 && i < this.nx && j < this.ny && max[2] <= exports.SURFACE_Z_CAP)
                    this.topZ[j * this.nx + i] = Math.max(this.topZ[j * this.nx + i], max[2] - 0.002);
        this.version += 1;
    }
    /**
     ...
     */
    markPanel(hinge, dir, normal, width, thickness, height, state) {
        ...
        for (let j = j0; j <= j1; j += 1)
            for (let i = i0; i <= i1; i += 1) {
                if (i < 0 || j < 0 || i >= this.nx || j >= this.ny)
                    continue;
                const cx = this.bounds.minX + (i + 0.5) * this.res - hinge[0];
                const cy = this.bounds.minY + (j + 0.5) * this.res - hinge[1];
                const along = cx * dir[0] + cy * dir[1];
                const across = cx * normal[0] + cy * normal[1];
                if (along < -this.res / 2 || along > width + this.res / 2 || across < -this.res / 2 || across > thickness + this.res / 2)
                    continue;
                for (let k = 0; k <= kTop; k += 1)
                    this.set(i, j, k, state);
            }
        this.version += 1;
    }
    /** @description A grid marking the columns whose voxel at an altitude is UNKNOWN (for frontier search). */
    unknownGrid(alt, dilate = 0) {
        ...
        for (let j = 0; j < this.ny; j += 1)
            for (let i = 0; i < this.nx; i += 1)
                if (this.cells[this.index(i, j, k)] === exports.UNKNOWN)
                    raw[j * this.nx + i] = 1;
     ...
     */
    topWithClearance(col) {
        const z = this.topZ[col];
        if (!Number.isFinite(z))
            return Number.NaN;
        const i = col % this.nx;
        const j = Math.floor(col / this.nx);
        const k = Math.floor((z - this.bounds.minZ) / this.res);
        return k + 1 < this.nz && this.cells[this.index(i, j, k + 1)] === exports.FREE ? z : Number.NaN;
    }
            ...
            for (let i = 0; i < this.nx; i += 1) {
                ...
                for (let dj = -r; dj <= r; dj += 1)
                    for (let di = -r; di <= r; di += 1) {
                        ...
                        const ii = i + di;
                        const jj = j + dj;
                        if (ii >= 0 && jj >= 0 && ii < this.nx && jj < this.ny)
                            cells[jj * this.nx + ii] = 1;
                    }
            }
        return cells;
    }
    ...
    stats() {
        ...
        for (let n = 0; n < this.cells.length; n += 1) {
            const v = this.cells[n];
            if (v === exports.FREE)
                free += 1;
            else if (v === exports.OCCUPIED)
                occupied += 1;
        }
    ...
    occupiedList() {
        ...
        for (let k = 0; k < this.nz; k += 1)
            for (let j = 0; j < this.ny; j += 1)
                for (let i = 0; i < this.nx; i += 1)
                    if (this.cells[this.index(i, j, k)] === exports.OCCUPIED)
                        out.push([i, j, k]);
        return out;
    }
    /**
     ...
     */
    navGrid(zMin = 0.10, zMax = 1.6, inflate = 0.325, minKnownFraction = 0.25) {
        ...
        for (let j = 0; j < this.ny; j += 1) {
            for (let i = 0; i < this.nx; i += 1) {
                ...
                let known = 0;
                for (let k = k0; k <= k1; k += 1) {
                    const v = this.cells[this.index(i, j, k)];
                    if (v === exports.OCCUPIED) {
                        occ = true;
                        break;
                    }
                    if (v === exports.FREE)
                ...
                if (occ || known / (k1 - k0 + 1) < minKnownFraction)
                    raw[j * this.nx + i] = 1;
            }
        }
     ...
     */
    flightGrid(alt, clearance = 0.15, knownRing = 0.10, clearanceZ = clearance) {
        ...
        const occ = new Uint8Array(this.nx * this.ny);
        const unk = new Uint8Array(this.nx * this.ny);
        const notFree = new Uint8Array(this.nx * this.ny);
        for (let j = 0; j < this.ny; j += 1)
            for (let i = 0; i < this.nx; i += 1) {
                const at = this.cells[this.index(i, j, kAlt)];
                if (at !== exports.FREE)
                    notFree[j * this.nx + i] = 1;
                if (at === exports.UNKNOWN)
                    unk[j * this.nx + i] = 1;
                for (let k = k0; k <= k1; k += 1)
                    if (this.cells[this.index(i, j, k)] === exports.OCCUPIED) {
                        occ[j * this.nx + i] = 1;
        ...
        for (let c = 0; c < cells.length; c += 1)
            if (notFree[c] || occWide[c] || unkNear[c])
                cells[c] = 1;
        return { res: this.res, width: this.nx, height: this.ny, originX: this.bounds.minX, originY: this.bounds.minY, cells };
    }
}
exports.VoxelMap = VoxelMap;
//# sourceMappingURL=voxel-map.js.map