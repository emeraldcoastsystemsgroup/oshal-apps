...
exports.MAX_WORK_REACH = exports.SHELF_LIFT = exports.DOOR_ARC_STEP = exports.backOff = exports.toolLevel = exports.toolDown = exports.CARRY_CLEARANCE = exports.APPROACH_CLEARANCE = exports.WORK_LIFT_Z = void 0;
exports.gridWithDoors = gridWithDoors;
exports.driveLegs = driveLegs;
exports.placementSlots = placementSlots;
exports.slotsNearest = slotsNearest;
exports.planClearSceneSurface = planClearSceneSurface;
exports.findStandoff = findStandoff;
exports.discoveredSlots = discoveredSlots;
exports.planExplore = planExplore;
exports.planClearSurface = planClearSurface;
exports.towardArmBase = towardArmBase;
exports.lineSteps = lineSteps;
exports.carryPose = carryPose;
exports.planFetchFromAppliance = planFetchFromAppliance;
...
exports.MAX_WORK_REACH = 0.56;
/** @description Sequential step ids. */
class StepIds {
    n = 0;
    next(prefix) { this.n += 1; return `${prefix}-${String(this.n).padStart(3, '0')}`; }
}
/**
 ...
 */
function gridWithDoors(sim, doorAngles) {
    const scene = JSON.parse(JSON.stringify(sim.scene));
    for (const a of scene.appliances)
        if (a.id in doorAngles)
            a.angle = doorAngles[a.id];
    return (0, occupancy_grid_1.buildGrid)(scene.room, (0, scene_1.sceneSolids)(scene));
}
/**
 ...
 * @returns The legs.
 */
function driveLegs(sim, from, to, doorAngles = {}) {
    const grid = Object.keys(doorAngles).length ? gridWithDoors(sim, doorAngles) : sim.grid;
    const path = (0, occupancy_grid_1.planPath)(grid, from, to);
    if (!path)
        throw new Error(`no drivable path from (${from.x.toFixed(2)}, ${from.y.toFixed(2)}) to (${to.x.toFixed(2)}, ${to.y.toFixed(2)})`);
    const pts = (0, occupancy_grid_1.simplifyPath)(grid, path);
    const legs = [];
    ...
    if (!legs.length)
        legs.push({ x: to.x, y: to.y, yaw: to.yaw });
    return legs;
}
/** @description Free placement slots across a surface for a class, left to right, 15 cm pitch, avoiding occupied ones. */
function placementSlots(scene, surface, cls) {
    const size = scene_1.OBJECT_CLASSES[cls].size;
    const pitch = Math.max(size.l, 0.12) + 0.03;
    const y = (surface.area.minY + surface.area.maxY) / 2;
    const slots = [];
    for (let x = surface.area.minX + size.l / 2 + 0.01; x <= surface.area.maxX - size.l / 2; x += pitch)
        slots.push({ x, y });
    const occupied = (0, scene_1.objectsOn)(scene, surface.id);
...
function slotsNearest(slots, standoff) {
    const ax = standoff.x + world_sim_1.ARM_MOUNT_X * Math.cos(standoff.yaw);
    const ay = standoff.y + world_sim_1.ARM_MOUNT_X * Math.sin(standoff.yaw);
...
function surveySteps(ids, zone, present, absent, label) {
    return [
...
function pickSteps(ids, o) {
    const yaw = o.pose.yaw;
    const size = scene_1.OBJECT_CLASSES[o.cls].size;
    const contact = Math.max(size.l, size.w) / 2 + 0.04;
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `approach above ${o.id}`, target: (0, exports.toolDown)(o.pose.x, o.pose.y, o.pose.z + exports.APPROACH_CLEARANCE, yaw), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.move', label: `descend onto ${o.id}`, target: (0, exports.toolDown)(o.pose.x, o.pose.y, o.pose.z, yaw), expectedForceN: 0, contactRadius: contact },
        { id: ids.next('arm'), kind: 'arm.grasp', label: `grasp ${o.id}`, objectId: o.id },
        { id: ids.next('arm'), kind: 'arm.move', label: `lift ${o.id} clear`, target: (0, exports.toolDown)(o.pose.x, o.pose.y, o.pose.z + exports.CARRY_CLEARANCE, yaw), expectedForceN: 0, contactRadius: contact },
        { id: ids.next('arm'), kind: 'arm.joints', label: 'tuck for carrying', q: [...arm_model_1.READY_Q] },
    ];
}
/** @description Steps that place a held object at a slot. */
function placeSteps(ids, o, surface, slot) {
    const z = (0, scene_1.restingZ)(o.cls, surface.z) + 0.005;
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `approach above the ${surface.name} slot`, target: (0, exports.toolDown)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, 0), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.move', label: `lower ${o.id} onto the ${surface.name}`, target: (0, exports.toolDown)(slot.x, slot.y, z, 0), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.release', label: `release ${o.id}`, surfaceId: surface.id },
        { id: ids.next('arm'), kind: 'arm.move', label: 'retract', target: (0, exports.toolDown)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, 0), expectedForceN: 0, contactRadius: exports.APPROACH_CLEARANCE + scene_1.OBJECT_CLASSES[o.cls].size.h + 0.08 },
        { id: ids.next('arm'), kind: 'arm.joints', label: 'ready pose', q: [...arm_model_1.READY_Q] },
    ];
}
/**
 ...
 * @returns The plan.
 */
function planClearSceneSurface(sim, fromZoneId, toZoneId) {
    const scene = sim.scene;
    const from = (0, scene_1.zoneById)(scene, fromZoneId);
    const to = (0, scene_1.zoneById)(scene, toZoneId);
    const fromSurface = (0, scene_1.surfaceById)(scene, from.surfaceId);
    const toSurface = (0, scene_1.surfaceById)(scene, to.surfaceId);
    const objects = (0, scene_1.objectsOn)(scene, fromSurface.id).filter((o) => !(0, scene_1.isEnclosed)(scene, o)).sort((a, b) => a.id.localeCompare(b.id));
    if (!objects.length)
        throw new Error(`nothing to move: ${fromSurface.name} holds no reachable object`);
    const ids = new StepIds();
    const steps = [{ id: ids.next('drone'), kind: 'drone.takeoff', label: 'drone takes off' }];
    ...
    steps.push({ id: ids.next('base'), kind: 'arm.joints', label: 'fold the arm', q: [...arm_model_1.STOW_Q] });
    let basePose = sim.basePose();
    const slots = slotsNearest(placementSlots(scene, toSurface, objects[0].cls), to.standoff);
    if (slots.length < objects.length)
        throw new Error(`${toSurface.name} has ${slots.length} free slot(s) for ${objects.length} object(s)`);
    /** A standoff searched in the discovered grid for a point, falling back to the zone's declared one. */
    const standoffFor = (p, fallback) => { try {
        return findStandoff(sim, p, basePose);
    }
    catch {
        return fallback;
    } };
    objects.forEach((o, i) => {
        ...
        basePose = pickAt;
        steps.push({ id: ids.next('base'), kind: 'base.lift', label: 'raise the lift to work', z: exports.WORK_LIFT_Z });
        steps.push({ id: ids.next('arm'), kind: 'arm.joints', label: 'ready pose', q: [...arm_model_1.READY_Q] });
        ...
        steps.push({ id: ids.next('arm'), kind: 'arm.joints', label: 'fold the arm', q: [...arm_model_1.STOW_Q] });
        steps.push({ id: ids.next('base'), kind: 'base.lift', label: 'stow the lift for travel', z: sim.baseLimits.liftMin });
    });
...
function findStandoff(sim, target, from) {
    const candidates = [];
    for (const d of [0.55, 0.62, 0.70, 0.76, 0.80])
        for (let k = 0; k < 24; k += 1) {
            const yaw = (2 * Math.PI * k) / 24;
            candidates.push({ x: target.x - d * Math.cos(yaw), y: target.y - d * Math.sin(yaw), yaw });
        }
    const reach = (p) => {
        const ax = p.x + world_sim_1.ARM_MOUNT_X * Math.cos(p.yaw);
        const ay = p.y + world_sim_1.ARM_MOUNT_X * Math.sin(p.yaw);
        return Math.hypot(target.x - ax, target.y - ay);
    };
    const free = candidates.filter((p) => (0, occupancy_grid_1.isFree)(sim.grid, p) && reach(p) >= 0.25 && reach(p) <= exports.MAX_WORK_REACH);
    ...
    if (!free.length)
        throw new Error(`no free standoff around (${target.x.toFixed(2)}, ${target.y.toFixed(2)}) in the discovered map — explore more, or the area is blocked`);
    return free[0];
}
/** @description Free slots on a discovered surface for an object of a given extent, nearest a standoff first. */
function discoveredSlots(sim, surface, extent, near) {
    ...
    const slots = [];
    for (let x = surface.bbox.minX + margin; x <= surface.bbox.maxX - margin; x += 0.10)
        for (let y = surface.bbox.minY + margin; y <= surface.bbox.maxY - margin; y += 0.10) {
            let inside = true;
            for (const [dx, dy] of [[-margin, -margin], [margin, -margin], [-margin, margin], [margin, margin], [0, 0]])
                if (!columns.has(map.column(x + dx, y + dy))) {
                    inside = false;
                    break;
                }
            if (!inside)
            ...
            if (others.some((o) => Math.hypot(o.centroid[0] - x, o.centroid[1] - y) < margin + Math.max(o.size.l, o.size.w) / 2 + 0.10))
                continue;
            slots.push({ x, y });
        }
    return slotsNearest(slots, near);
}
function planExplore(sim, maxScans = 12, opts = {}) {
    const ids = new StepIds();
    const home = sim.scene.droneHome;
    const steps = [
        ...(opts.droneFirst ? [] : [{ id: ids.next('scan'), kind: 'rover.scan', label: 'base LiDAR sweep' }]),
...
function discoveredPickSteps(ids, o, standoff) {
    const [x, y, z] = o.centroid;
    const top = o.max[2];
    const contact = Math.max(o.size.l, o.size.w) / 2 + 0.04;
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `look at ${o.id} with the wrist camera`, target: (0, exports.toolDown)(x, y, top + 0.35, 0), expectedForceN: 0 },
        { id: ids.next('scan'), kind: 'wrist.scan', label: `wrist depth picture of ${o.id}` },
        { id: ids.next('arm'), kind: 'arm.move', label: `approach above ${o.id}`, target: (0, exports.toolDown)(x, y, top + exports.APPROACH_CLEARANCE, 0), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.move', label: `descend onto ${o.id}`, target: (0, exports.toolDown)(x, y, z, 0), expectedForceN: 0, contactRadius: contact },
        { id: ids.next('arm'), kind: 'arm.grasp', label: `grasp ${o.id}`, objectId: '*' },
        { id: ids.next('arm'), kind: 'arm.move', label: `lift ${o.id} clear`, target: (0, exports.toolDown)(x, y, top + exports.CARRY_CLEARANCE, 0), expectedForceN: 0, contactRadius: contact },
        { id: ids.next('arm'), kind: 'arm.move', label: 'tuck for carrying', target: carryPose(standoff, exports.WORK_LIFT_Z), expectedForceN: 0 },
    ];
}
...
function surveySurfaceSteps(ids, s, at) {
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `look over ${s.label ?? s.id} with the wrist camera`, target: (0, exports.toolDown)(at.x, at.y, s.z + 0.45, 0), expectedForceN: 0 },
...
function discoveredPlaceSteps(ids, o, surface, slot, standoff) {
    const z = surface.z + o.size.h / 2 + 0.005;
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `approach above the ${surface.label ?? surface.id} slot`, target: (0, exports.toolDown)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, 0), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.move', label: `lower ${o.id} onto ${surface.label ?? surface.id}`, target: (0, exports.toolDown)(slot.x, slot.y, z, 0), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.release', label: `release ${o.id}`, surfaceId: '*' },
        { id: ids.next('arm'), kind: 'arm.move', label: 'retract', target: (0, exports.toolDown)(slot.x, slot.y, z + 0.35, 0), expectedForceN: 0, contactRadius: 0.35 + o.size.h + 0.08 },
        { id: ids.next('scan'), kind: 'wrist.scan', label: 'wrist depth picture of the placed object' },
        { id: ids.next('arm'), kind: 'arm.move', label: 'tool back ahead of the base', target: carryPose(standoff, exports.WORK_LIFT_Z), expectedForceN: 0 },
    ];
}
/**
 ...
 * @returns The plan.
 */
function planClearSurface(sim, fromId, toId) {
    const world = sim.world;
    const from = world.surface(fromId);
    const to = world.surface(toId);
    if (!from)
        throw new Error(`no discovered surface ${fromId} — explore first`);
    if (!to)
        throw new Error(`no discovered surface ${toId} — explore first`);
    const objects = [...world.objectsOn(fromId)].sort((a, b) => a.id.localeCompare(b.id));
    if (!objects.length)
        throw new Error(`nothing discovered on ${from.label ?? fromId}`);
    const ids = new StepIds();
    const steps = [];
    ...
    objects.forEach((o) => {
        let standoff;
        try {
            standoff = findStandoff(sim, { x: o.centroid[0], y: o.centroid[1] }, basePose);
        }
        catch (error) {
            skipped.push({ id: o.id, reason: error instanceof Error ? error.message : String(error) });
            return;
        }
        const index = surveyed ? 1 : 0;
        surveyed = true;
        ...
        steps.push({ id: ids.next('base'), kind: 'base.lift', label: 'raise the lift to work', z: exports.WORK_LIFT_Z });
        if (index === 0)
            steps.push(...surveySurfaceSteps(ids, from, { x: o.centroid[0], y: o.centroid[1] }), ...surveySurfaceSteps(ids, from, { x: (o.centroid[0] + from.centroid[0]) / 2, y: (o.centroid[1] + from.centroid[1]) / 2 }));
        ...
        if (!chosen)
            throw new Error(`${to.label ?? toId} has no free slot for ${o.id} that the arm can reach from the discovered map`);
    ...
    if (!moved)
        throw new Error(`no object on ${from.label ?? fromId} has a reachable standoff: ${skipped.map((s) => s.id).join(', ')}`);
...
function reachableSlot(sim, slots, from) {
    for (const slot of slots) {
        try {
            return { slot, standoff: findStandoff(sim, slot, from) };
        }
        catch { /* try the next slot */ }
    }
    return null;
}
...
function verifyBySurveySteps(ids, sim, from, to, moved, left) {
    const above = (s) => [s.centroid[0], s.centroid[1] - 0.3, sim.droneLimits.cruiseAlt];
    const name = (s) => s.label ?? s.id;
    return [
...
function clearSurfaceSummary(objects, skipped, stepCount) {
...
function closeDoorAndVerifySteps(ids, sim, a, o, to, doorStandoff, from, open) {
    const door = a.door;
    const steps = [{ id: ids.next('base'), kind: 'base.drive', label: `return beside the ${a.name} door`, legs: driveLegs(sim, from, doorStandoff, open) }];
    steps.push({ id: ids.next('base'), kind: 'base.lift', label: 'raise the lift to the handle', z: exports.WORK_LIFT_Z });
...
function handlePose(a, angle) {
    const g = (0, scene_1.handleGrasp)(a.door, angle);
    return (0, exports.toolLevel)(g.point[0], g.point[1], g.point[2], g.yaw);
}
/**
 ...
 */
function doorArcSteps(ids, a, from, to, verb) {
    const door = a.door;
    const n = Math.max(1, Math.ceil(Math.abs(to - from) / exports.DOOR_ARC_STEP));
    const steps = [];
    for (let k = 1; k <= n; k += 1) {
 ...
 */
function towardArmBase(p, standoff, d) {
    const ax = standoff.x + world_sim_1.ARM_MOUNT_X * Math.cos(standoff.yaw);
    const ay = standoff.y + world_sim_1.ARM_MOUNT_X * Math.sin(standoff.yaw);
 ...
 */
function doorWaypoints(a, angle) {
    const door = a.door;
    const d = (0, scene_1.doorDirection)(door, angle);
    const n = (0, scene_1.doorNormal)(door, angle);
    const g = (0, scene_1.handleGrasp)(door, angle);
    const fx = door.hinge.x + door.width * d[0];
    const fy = door.hinge.y + door.width * d[1];
    return {
        beyond: (0, exports.toolLevel)(fx + 0.12 * d[0] + 0.10 * n[0], fy + 0.12 * d[1] + 0.10 * n[1], door.handleZ, g.yaw),
        near: (0, exports.toolLevel)(g.point[0] + 0.06 * n[0], g.point[1] + 0.06 * n[1], g.point[2], g.yaw),
        grasp: (0, exports.toolLevel)(g.point[0], g.point[1], g.point[2], g.yaw),
    };
}
/**
 ...
 */
function lineSteps(ids, from, to, label, stepM = 0.10) {
    const n = Math.max(1, Math.ceil(Math.hypot(to.x - from.x, to.y - from.y, to.z - from.z) / stepM));
    const steps = [];
    for (let k = 1; k <= n; k += 1) {
        const t = k / n;
        const target = { x: from.x + (to.x - from.x) * t, y: from.y + (to.y - from.y) * t, z: from.z + (to.z - from.z) * t, roll: to.roll, pitch: to.pitch, yaw: from.yaw + (to.yaw - from.yaw) * t };
...
function swingDoorSteps(ids, a, from, to, verb, standoff) {
    const start = doorWaypoints(a, from);
    const end = doorWaypoints(a, to);
    const tuck = carryPose(standoff, exports.WORK_LIFT_Z);
    return [
...
function carryPose(standoff, liftZ) {
    const ax = standoff.x + world_sim_1.ARM_MOUNT_X * Math.cos(standoff.yaw);
    const ay = standoff.y + world_sim_1.ARM_MOUNT_X * Math.sin(standoff.yaw);
    return (0, exports.toolLevel)(ax + 0.25 * Math.cos(standoff.yaw), ay + 0.25 * Math.sin(standoff.yaw), liftZ + 0.35, standoff.yaw - Math.PI / 2);
}
/** @description Steps that take an object from a shelf with a level (front) grasp, draw it out and tuck it for travel. */
function shelfPickSteps(ids, o, standoff, liftZ) {
    const at = (0, exports.toolLevel)(o.pose.x, o.pose.y, o.pose.z, standoff.yaw - Math.PI / 2);
    const approach = (0, exports.backOff)(at, 0.15);
    return [
        ...
        { id: ids.next('arm'), kind: 'arm.grasp', label: `grasp ${o.id}`, objectId: o.id },
        { id: ids.next('arm'), kind: 'arm.move', label: `lift ${o.id} off the shelf`, target: { ...at, z: at.z + exports.SHELF_LIFT }, expectedForceN: 0, contactRadius: 0.12 },
        { id: ids.next('arm'), kind: 'arm.move', label: `draw ${o.id} out`, target: (0, exports.backOff)({ ...at, z: at.z + exports.SHELF_LIFT }, 0.30), expectedForceN: 0 },
...
function shelfPlaceSteps(ids, o, surface, slot, standoff) {
    const z = (0, scene_1.restingZ)(o.cls, surface.z) + 0.005;
    const yaw = standoff.yaw - Math.PI / 2;
    return [
        { id: ids.next('arm'), kind: 'arm.move', label: `approach above the ${surface.name} slot`, target: (0, exports.toolLevel)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, yaw), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.move', label: `lower ${o.id} onto the ${surface.name}`, target: (0, exports.toolLevel)(slot.x, slot.y, z, yaw), expectedForceN: 0 },
        { id: ids.next('arm'), kind: 'arm.release', label: `release ${o.id}`, surfaceId: surface.id },
        { id: ids.next('arm'), kind: 'arm.move', label: 'retract', target: (0, exports.backOff)((0, exports.toolLevel)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, yaw), 0.15), expectedForceN: 0, contactRadius: 0.22 + scene_1.OBJECT_CLASSES[o.cls].size.h + 0.08 },
        { id: ids.next('arm'), kind: 'arm.move', label: 'draw the tool back', target: towardArmBase((0, exports.backOff)((0, exports.toolLevel)(slot.x, slot.y, z + exports.APPROACH_CLEARANCE, yaw), 0.15), standoff, 0.20), expectedForceN: 0 },
    ];
}
/** @description Park the machine for travel: arm folded, lift down. */
const stowSteps = (ids, sim) => [
    { id: ids.next('base'), kind: 'arm.joints', label: 'fold the arm', q: [...arm_model_1.STOW_Q] },
 ...
 * @returns The plan.
 */
function planFetchFromAppliance(sim, applianceId, objectId, toZoneId) {
    const scene = sim.scene;
    const a = (0, scene_1.applianceById)(scene, applianceId);
    if (!a.door)
        throw new Error(`${a.name} has no door to open`);
    if (a.angle > 0.01)
        throw new Error(`${a.name} is already open (${Math.round((a.angle * 180) / Math.PI)}°); close it first`);
    const o = (0, scene_1.objectById)(scene, objectId);
    if (o.location.kind !== 'surface' || (0, scene_1.surfaceById)(scene, o.location.surfaceId).enclosedBy !== a.id)
        throw new Error(`${objectId} is not inside the ${a.name}`);
    const doorZone = scene.zones.find((z) => z.id === `${a.id}-door`);
    const insideZone = scene.zones.find((z) => z.id === a.id);
    if (!doorZone || !insideZone)
        throw new Error(`no standoffs declared for the ${a.name}`);
    const to = (0, scene_1.zoneById)(scene, toZoneId);
    const toSurface = (0, scene_1.surfaceById)(scene, to.surfaceId);
    const slot = slotsNearest(placementSlots(scene, toSurface, o.cls), to.standoff)[0];
    if (!slot)
        throw new Error(`${toSurface.name} has no free slot`);
    const ids = new StepIds();
    const steps = [{ id: ids.next('drone'), kind: 'drone.takeoff', label: 'drone takes off' }];
    ...
    steps.push({ id: ids.next('base'), kind: 'base.lift', label: 'raise the lift to the shelf', z: exports.WORK_LIFT_Z });
    steps.push(...shelfPickSteps(ids, o, insideZone.standoff, exports.WORK_LIFT_Z));
...
//# sourceMappingURL=skill-planner.js.map