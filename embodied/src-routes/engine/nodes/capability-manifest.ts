...
/** @description The peripheral kinds the swarm knows. New kinds are added here, with a vocabulary. */
export type PeripheralKind = 'drone' | 'camera' | 'mobile-manipulator' | 'manipulator' | 'mobile-base' | 'printer' | 'light' | 'prop';

/** @description 0 sensing · 1 low-energy · 2 kinetic · 3 hazardous. */
export type SafetyClass = 0 | 1 | 2 | 3;

/** @description What a node declares at enrolment. */
...
export const KIND_VOCABULARY: Record<PeripheralKind, { senses: readonly string[]; acts: readonly string[]; minSafetyClass: SafetyClass }> = {
...
export const CONFIRM_EXEMPT_ACTS: ReadonlySet<string> = new Set(['e-stop', 'abort', 'stop', 'land', 'hover', 'observe', 'capture', 'disarm']);

const NODE_ID = /^[a-z0-9][a-z0-9-]{0,63}$/;

/** @description Validation result. */
export type ManifestValidation = { ok: true; manifest: CapabilityManifest } | { ok: false; issues: string[] };

/** @description Collect the issues with a candidate's string-list field. */
function listIssues(field: 'senses' | 'acts', value: unknown, allowed: readonly string[]): string[] {
  if (!Array.isArray(value)) return [`${field} must be a list`];
  const issues: string[] = [];
  for (const v of value) {
    if (typeof v !== 'string') issues.push(`${field} entries must be strings`);
    else if (!allowed.includes(v)) issues.push(`${field} entry "${v}" is outside the kind's vocabulary`);
  }
  if (new Set(value).size !== value.length) issues.push(`${field} repeats an entry`);
  return issues;
}
...
export function validateManifest(input: unknown): ManifestValidation {
  const issues: string[] = [];
  if (!input || typeof input !== 'object') return { ok: false, issues: ['manifest must be an object'] };
  const m = input as Record<string, unknown>;
  if (typeof m.nodeId !== 'string' || !NODE_ID.test(m.nodeId)) issues.push('nodeId must be a short lowercase identifier');
  const kind = typeof m.kind === 'string' && m.kind in KIND_VOCABULARY ? (m.kind as PeripheralKind) : null;
  if (!kind) issues.push(`kind "${String(m.kind)}" is unknown`);
  if (typeof m.model !== 'string' || !m.model.trim()) issues.push('model is required');
  const sc = m.safetyClass;
  if (!(sc === 0 || sc === 1 || sc === 2 || sc === 3)) issues.push('safetyClass must be 0, 1, 2 or 3');
  if (kind) {
    const vocab = KIND_VOCABULARY[kind];
    if (typeof sc === 'number' && sc < vocab.minSafetyClass) issues.push(`kind ${kind} is at least safety class ${vocab.minSafetyClass}`);
  ...
  if (!m.envelope || typeof m.envelope !== 'object' || Array.isArray(m.envelope)) issues.push('envelope must be an object of numbers');
  else for (const [k, v] of Object.entries(m.envelope as Record<string, unknown>)) if (typeof v !== 'number' || !Number.isFinite(v)) issues.push(`envelope.${k} must be a finite number`);
  if (issues.length) return { ok: false, issues };
  return {
    ok: true,
    manifest: { nodeId: m.nodeId as string, kind: kind as PeripheralKind, model: (m.model as string).trim(), safetyClass: sc as SafetyClass, senses: [...(m.senses as string[])], acts: [...(m.acts as string[])], envelope: { ...(m.envelope as Record<string, number>) } },
  };
}
 ...
 */
export function actRequiresConfirm(manifest: CapabilityManifest, act: string): boolean {
  if (manifest.safetyClass < 2) return false;
  return !CONFIRM_EXEMPT_ACTS.has(act);
}
