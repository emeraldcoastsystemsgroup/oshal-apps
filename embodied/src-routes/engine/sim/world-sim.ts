...
import { armCenterOfMass, forwardKinematics, jointOrigins, SIM_ARM_6, stepJoints, STOW_Q, type ArmSpec } from '../arm/arm-model';
import { ikSolutions, type IkResult } from '../arm/inverse-kinematics';
import { DEFAULT_BASE_LIMITS, driveStep, integrateUnicycle, liftStep, type BaseLimits, type BaseState, type DriveGoal } from '../base/diff-drive';
import { HARDWARE_BASE_ITEMS, HARDWARE_FOOTPRINT, speedCheck, tipBudget, wrenchCheck, type MassItem, type TipBudget } from '../base/stability';
import { cameraToWorld, DEFAULT_INTRINSICS, localizeOnPlane, observe, projectBoxOutline, type BoxOutline, type CameraIntrinsics, type CameraPose, type Detection } from '../drone/camera-model';
import { DEFAULT_DRONE_LIMITS, droneStep, gotoPoint, insideBox, land, landedDrone, propagateTruth, takeoff, type Box3 as Box3Like, type DroneLimits, type DroneState, type Fence, type Pose3Yaw } from '../drone/quad-model';
import { framesToSweeps, isDroneNode, senseSpec, type PhysicsPlant, type PlantPhase } from '../physics/plant';
import { armSpecMismatch, isArmNode, type ArmPlant } from '../physics/arm-plant';
import { dronePlant } from '../physics/mjcf';
import { fitForSensorSet } from '../design/parts-model';
import { concatBody, DEFAULT_REGISTER, fromBodyFrame, PLANAR_REGISTER, registerSweep, relocalize, toBodyFrame, type BodySweep, type RegisterOptions } from '../sense/register';
import { scenarioById } from '../world/scenes';
import { RECON_3D, type DepthCameraSpec, type DroneSensorSet } from '../drone/sensor-set';
import { applyToPoint, fromPose, multiply, rotationRpy, toPose, translation, type Mat4 } from '../math/transform';
import { distance, round, type Pose2, type Pose6, type Vec3 } from '../math/vec';
import { isFree, lineClear, simplifyPath, type Grid, planPath } from '../world/occupancy-grid';
import { applianceById, doorAngleFromPoint, doorDirection, doorNormal, doorSlices, handleGrasp, isEnclosed, OBJECT_CLASSES, objectById, pointOnSurface, restingZ, sceneSolids, type Obstacle, type Scene, type Surface, type WorldObject } from '../world/scene';
import { castRay, DEFAULT_LIDAR, lidarSweep, renderDepthPicture, type DepthPicture, type LidarSweep, type SensedSolid } from '../sense/raycast';
import { FREE, OCCUPIED, UNKNOWN } from '../world/voxel-map';
import { WorldModel } from '../world/world-model';
import { nextFrontier, standBack, gainFrontier } from '../world/explore';

/** @description Where the arm base sits relative to the base frame origin (floor centre): 0.20 m forward, on the carriage. */
export const ARM_MOUNT_X = 0.20;
/** @description Node identifiers the sim exposes. */
export const UNIT_NODE_ID = 'rover-arm-1';
export const DRONE_NODE_ID = 'mini-drone-1';
/** @description Gripper stroke (m). */
export const GRIPPER_MAX_WIDTH = 0.085;
...
export const GRASP_TOLERANCE = 0.05;
...
export const ARM_GRIP_OPEN_RAD = 0.05;
export const ARM_GRIP_CLOSED_RAD = -0.6;
...
export const ROVER_LIDAR_Z = 0.35;
...
export const SELF_BUBBLE_M = 0.45;
...
export const DRONE_CLEARANCE_MARGIN_M = 0.05;
...
export const MAX_UNREGISTERED_M = 2.0;
...
export const DRONE_BACKOUT_M = 0.3;
/** The most trail points a recovery flies back along (B14). */
export const RECOVERY_TRAIL_MAX = 12;
const TRAIL_CAP = 64;
...
export const DRONE_LIDAR_Z = 0.08;
...
export const MAP_SHELL_M = 0.05;
/** @description A landing this close to the pad (true distance) gets the pad's fiducial fix: the belief snaps to the truth. */
export const PAD_FIX_RADIUS = 0.30;

/** @description How well the drone knows where it is — the machine's own view. */
export interface Localization {
  /** anchored: on the pad; tracking: the last sweep registered; dead-reckoned: the last sweep had nothing to match (no contradiction); lost: it matched and did not settle. */
  status: 'anchored' | 'tracking' | 'dead-reckoned' | 'lost';
  correctionM: number;
  yawCorrectionRad: number;
  matched: number;
  residualM: number;
  registrations: number;
  /** False when the last sweep's face normals spanned one direction only: the shift along `weakAxisRad` was not measured and stands as dead reckoning. */
  observable: boolean;
  weakAxisRad: number | null;
  /** The last registration's final-pass matches over the sweep's usable points (a statistic, not a gate). */
  coverage: number;
}

/** @description Sim-only ground truth: how far the belief is from the truth. */
export interface LocalizationError {
  positionM: number;
  yawRad: number;
}
/** @description Sensor names. */
export type Sensor = 'drone' | 'rover' | 'wrist';
/** @description Joint-space interpolation step for the swept-volume check (rad). */
export const SWEEP_STEP_RAD = 0.1;
/** @description What the gripper reports while it holds a door handle. */
export const handleHold = (applianceId: string): string => `handle:${applianceId}`;

/** @description The mobile manipulator's state. */
export interface UnitState {
  ...
  enabled: boolean;
  estop: boolean;
  driveGoal: DriveGoal | null;
  jog: { v: number; w: number; untilMs: number } | null;
  liftTarget: number | null;
  phase: string;
}

/** @description An event the sim recorded (for the log and the surface). */
...
/** @description What the drone camera returns for the surface. */
...
export interface WorldSimOptions {
  scene?: Scene;
  /** The named hidden scene to start from (B4); default the kitchen. Ignored when `scene` is given. */
  scenario?: string;
  armSpec?: ArmSpec;
  baseLimits?: BaseLimits;
  droneLimits?: DroneLimits;
  intrinsics?: CameraIntrinsics;
  /** What the drone carries; defaults to the 3-D set the earlier suites were written against. */
  sensorSet?: DroneSensorSet;
  /** A physics plant for the drone (ADR-152): the truth is stepped and sensed by it. Absent = the kinematic truth model. */
  plant?: PhysicsPlant;
  /** A physics plant for the ARM (B22): the joint truth is stepped by it. Absent = the kinematic joint model. */
  armPlant?: ArmPlant;
}
...
function insetByFence(flight: Grid, fence: Fence, unknown?: Grid, buffer = FENCE_DRIFT_BUFFER_M): void {
  for (let j = 0; j < flight.height; j += 1) for (let i = 0; i < flight.width; i += 1) {
    const x = flight.originX + (i + 0.5) * flight.res; const y = flight.originY + (j + 0.5) * flight.res;
    if (x < fence.minX + buffer || x > fence.maxX - buffer || y < fence.minY + buffer || y > fence.maxY - buffer) { flight.cells[j * flight.width + i] = 1; if (unknown) unknown.cells[j * flight.width + i] = 0; }
  }
}
...
function openBubble(flight: Grid, at: { x: number; y: number }, r: number): void {
  const cells = Math.ceil(r / flight.res);
  const i0 = Math.floor((at.x - flight.originX) / flight.res); const j0 = Math.floor((at.y - flight.originY) / flight.res);
  for (let dj = -cells; dj <= cells; dj += 1) for (let di = -cells; di <= cells; di += 1) {
    if (di * di + dj * dj > cells * cells) continue;
    const i = i0 + di; const j = j0 + dj;
    if (i >= 0 && j >= 0 && i < flight.width && j < flight.height) flight.cells[j * flight.width + i] = 0;
  }
}

export class WorldSim {
  readonly scene: Scene;
  readonly armSpec: ArmSpec;
  readonly baseLimits: BaseLimits;
  readonly droneLimits: DroneLimits;
  readonly intrinsics: CameraIntrinsics;
  readonly sensorSet: DroneSensorSet;
  ...
  private cmd: Pose3Yaw | null = null;
  gimbalPitch = -1.05;
  events: SimEvent[] = [];
  ...
  lastPictures: Partial<Record<Sensor, DepthPicture>> = {};
  lastSweep: LidarSweep | null = null;
  /** While set, mapped voxels within the radius of the centre are the thing being grasped or set down, not an obstacle. */
  private contact: { center: Vec3; radius: number } | null = null;
  private seq = 0;
  private gridCache: { key: number; grid: Grid } | null = null;

  constructor(opts: WorldSimOptions = {}) {
    this.scene = opts.scene ?? scenarioById(opts.scenario);
    this.armSpec = opts.armSpec ?? SIM_ARM_6;
    this.baseLimits = opts.baseLimits ?? DEFAULT_BASE_LIMITS;
    this.droneLimits = opts.droneLimits ?? DEFAULT_DRONE_LIMITS;
    this.intrinsics = opts.intrinsics ?? DEFAULT_INTRINSICS;
    this.sensorSet = opts.sensorSet ?? RECON_3D;
    const hull = dronePlant(fitForSensorSet(this.sensorSet.id));
    this.droneRadiusM = hull.hullHalfXyM;
    this.droneClearanceM = hull.hullHalfXyM + DRONE_CLEARANCE_MARGIN_M;
    this.droneClearanceZM = hull.hullHalfZM + hull.mastM + DRONE_CLEARANCE_MARGIN_M;
    ...
    this.world = new WorldModel({ minX: room.minX - MAP_SHELL_M, maxX: room.maxX + MAP_SHELL_M, minY: room.minY - MAP_SHELL_M, maxY: room.maxY + MAP_SHELL_M, minZ: -MAP_SHELL_M, maxZ: room.ceiling + MAP_SHELL_M });
    ...
    this.unit = { base: { x: park.x, y: park.y, yaw: park.yaw, v: 0, w: 0, liftZ: this.baseLimits.liftMin }, q: [...STOW_Q], qTarget: null, gripper: { width: GRIPPER_MAX_WIDTH, holding: null }, enabled: true, estop: false, driveGoal: null, jog: null, liftTarget: null, phase: 'idle' };
    this.drone = landedDrone(this.scene.droneHome, 0);
    ...
    if (this.armPlant && this.armPlant.dh.length) {
      ...
      const mismatch = armSpecMismatch(this.armPlant, this.armSpec.joints.map((j) => [j.d, j.a, j.alpha] as [number, number, number]));
      if (mismatch) throw new Error(`refused: the arm plant is not this arm — ${mismatch}`);
    }
  }

  /** @description A deep copy for rehearsal: same scene state, same time, independent afterwards. */
  clone(): WorldSim {
    const copy = new WorldSim({ scene: JSON.parse(JSON.stringify(this.scene)) as Scene, armSpec: this.armSpec, baseLimits: this.baseLimits, droneLimits: this.droneLimits, intrinsics: this.intrinsics, sensorSet: this.sensorSet, plant: this.plant?.clone() ?? undefined, armPlant: this.armPlant?.clone() ?? undefined });
    copy.timeMs = this.timeMs;
    copy.unit = JSON.parse(JSON.stringify(this.unit)) as UnitState;
    copy.drone = JSON.parse(JSON.stringify(this.drone)) as DroneState;
    copy.truth = { ...this.truth };
    copy.localization = { ...this.localization };
    copy.registeredTrail = this.registeredTrail.map((p) => [...p] as Vec3);
  ...
  sensingSolids(): SensedSolid[] {
    const paintOf = (o: Obstacle): string => (o.name.endsWith('-door') ? 'door' : o.kind === 'counter' || o.kind === 'island' ? 'counter' : o.kind === 'fixture' ? 'fixture' : o.kind === 'table' ? 'table' : 'appliance');
    const out: SensedSolid[] = sceneSolids(this.scene).map((o) => ({ name: o.name, min: o.min, max: o.max, paint: paintOf(o) }));
    for (const o of this.scene.objects) {
      if (o.location.kind === 'gripper') continue;
      const s = OBJECT_CLASSES[o.cls].size;
  ...
  wristCamera(): CameraPose {
    const T = multiply(this.armBaseTransform(), forwardKinematics(this.armSpec, this.unit.q).tcp);
    const z: Vec3 = [T[2], T[6], T[10]];
  ...
  scanDrone(opts: { wide?: boolean } = {}): LidarSweep {
    if (this.droneDown) throw new Error(`drone is down: ${this.droneDown}`);
    ...
    const truth: Pose3Yaw = { ...this.truth };
    const bodies = sensed.map((s) => toBodyFrame(s, truth));
    this.registerDrone(concatBody(bodies), nadirM, opts.wide === true);
    const belief: Pose3Yaw = { x: this.drone.x, y: this.drone.y, z: this.drone.z, yaw: this.drone.yaw };
    const placed = bodies.map((b) => fromBodyFrame(b, belief));
    ...
    if (loc.status !== 'lost') { this.world.map.integrateSweeps(placed); this.world.refresh(); }
    const sweep: LidarSweep = { origin: placed[0].origin, hits: placed.flatMap((p) => p.hits), misses: placed.flatMap((p) => p.misses) };
    this.lastSweep = sweep;
    this.record(DRONE_NODE_ID, `${opts.wide ? 'wide ' : ''}${set.id} sweep: ${sweep.hits.length} returns, map ${round(this.world.stats().knownFraction * 100, 0)}% known, localisation ${loc.status}${loc.status === 'tracking' ? ` (corrected ${round(loc.correctionM * 100, 1)} cm, ${loc.matched} matches)` : ''}`, loc.status === 'lost' ? 'warn' : 'info');
    return sweep;
  }
  ...
  private kinematicSensing(set: DroneSensorSet, truth: Pose3Yaw, solids: SensedSolid[]): { sensed: LidarSweep[]; nadirM: number | null } {
    const sensed: LidarSweep[] = [lidarSweep([truth.x, truth.y, truth.z + this.droneMast()], truth.yaw, solids, set.lidar)];
    if (set.depthCamera) sensed.push(this.depthCameraSweep(set.depthCamera, truth, solids));
    let nadirM: number | null = null;
    if (set.nadirRangerM !== null) {
      const ray = lidarSweep([truth.x, truth.y, truth.z - this.underDrop()], truth.yaw, solids, { azimuthCount: 1, elevationsDeg: [-90], maxRange: set.nadirRangerM });
      sensed.push(ray);
      if (ray.hits.length && ray.hits[0].name === 'floor') nadirM = ray.hits[0].t;
    }
  ...
  private plantSensing(solids: SensedSolid[]): { sensed: LidarSweep[]; nadirM: number | null } {
    const frames = (this.plant as PhysicsPlant).sense(senseSpec(this.sensorSet, this.intrinsics));
    this.truth = { x: frames.truth.x, y: frames.truth.y, z: frames.truth.z, yaw: frames.truth.yaw };
    const { sweeps, nadirM } = framesToSweeps(frames, solids);
  ...
  private depthCameraSweep(spec: DepthCameraSpec, body: Pose3Yaw, solids: SensedSolid[]): LidarSweep {
    const intr: CameraIntrinsics = { ...this.intrinsics, fx: this.intrinsics.width / (2 * Math.tan(((spec.fovHDeg / 2) * Math.PI) / 180)), fy: this.intrinsics.height / (2 * Math.tan(((spec.fovVDeg / 2) * Math.PI) / 180)) };
    const pic = renderDepthPicture(intr, { position: [body.x, body.y, body.z - this.underDrop()], yaw: body.yaw, pitch: -Math.PI / 2 + 1e-3 }, solids, spec.maxRange, spec.stride);
    return pic.sweep;
  }
   ...
   */
  private registerDrone(body: BodySweep, nadirM: number | null, wide = false): void {
    const map = this.world.map;
    const guess: Pose3Yaw = { x: this.drone.x, y: this.drone.y, z: this.drone.z, yaw: this.drone.yaw };
    const opts = this.sensorSet.registration === 'planar' ? PLANAR_REGISTER : DEFAULT_REGISTER;
    const airborne = this.drone.mode !== 'landed';
    if (map.anchorCount === 0) { this.localization = { ...this.localization, status: airborne ? 'dead-reckoned' : 'anchored' }; return; }
    if (wide) { this.relocalizeDrone(body, guess, opts); return; }
    const reg = registerSweep(map, body, guess, opts);
    if (reg.unobservable) {
      this.localization = { ...this.localization, status: 'dead-reckoned', matched: reg.matched, residualM: reg.residualM };
      this.record(DRONE_NODE_ID, `registration: nothing to match yet (${reg.matched} usable matches) — dead reckoning${nadirM !== null ? ', altitude from the nadir ranger' : ''}`);
      return;
    }
    if (!reg.converged) {
      this.localization = { ...this.localization, status: 'lost', matched: reg.matched, residualM: reg.residualM };
      this.record(DRONE_NODE_ID, `registration did not converge (${reg.matched} matches) — localisation lost; land and re-anchor`, 'warn');
      return;
    }
    ...
    if (!reg.observable) this.record(DRONE_NODE_ID, `registration: the shift along ${round(((reg.weakAxisRad ?? 0) * 180) / Math.PI, 0)}° could not be measured (one face seen head-on) — dead reckoning stands along it`);
    this.registeredTrail.push([reg.pose.x, reg.pose.y, reg.pose.z]);
    if (this.registeredTrail.length > TRAIL_CAP) this.registeredTrail.splice(0, this.registeredTrail.length - TRAIL_CAP);
  }
   ...
   */
  private relocalizeDrone(body: BodySweep, guess: Pose3Yaw, opts: RegisterOptions): void {
    const r = relocalize(this.world.map, body, guess, opts);
    const reg = r.registration;
    const detail = `best ${round(r.best.score * 100, 0)} % at (${round(r.best.pose.x, 2)}, ${round(r.best.pose.y, 2)}), runner-up ${r.second ? round(r.second.score * 100, 0) : 0} %`;
    if (!r.accepted) {
      ...
      this.record(DRONE_NODE_ID, `relocalisation refused: ${r.unique ? 'the best pose did not refine to a fit' : 'no pose stood clearly above the rest'} (${detail}) — still lost`, 'warn');
      return;
    }
    ...
    this.registeredTrail.push([reg.pose.x, reg.pose.y, reg.pose.z]);
    this.record(DRONE_NODE_ID, `relocalised: belief moved ${round(moved * 100, 1)} cm (${detail})`);
  }

  /** @description Where the machine believes its LiDAR is: the believed body pose plus the sensor mast. */
  droneSensorBelief(): Pose3Yaw {
  ...
  scanRover(): LidarSweep {
    const b = this.unit.base;
    const sweep = lidarSweep([b.x + 0.3 * Math.cos(b.yaw), b.y + 0.3 * Math.sin(b.yaw), ROVER_LIDAR_Z], b.yaw, this.sensingSolids(), DEFAULT_LIDAR);
    this.world.map.integrateSweep(sweep); this.world.refresh(); this.lastSweep = sweep;
    this.record(UNIT_NODE_ID, `base LiDAR sweep: ${sweep.hits.length} returns, map ${round(this.world.stats().knownFraction * 100, 0)}% known`);
    return sweep;
  }

  /** @description A depth picture from the wrist camera, integrated into the map (close range, dense). */
  scanWrist(): DepthPicture {
    const pic = renderDepthPicture(this.intrinsics, this.wristCamera(), this.sensingSolids(), 2.5, 4);
    this.world.map.integrateSweep(pic.sweep); this.world.refresh(); this.lastPictures.wrist = pic;
    this.record(UNIT_NODE_ID, `wrist depth picture: ${pic.sweep.hits.length} returns`);
    return pic;
  }

  /** @description A picture from a camera, NOT integrated — what the surface shows as the live view. */
  picture(sensor: Sensor): DepthPicture {
    const cam = sensor === 'wrist' ? this.wristCamera() : sensor === 'drone' ? this.droneCamera() : { position: [this.unit.base.x + 0.3 * Math.cos(this.unit.base.yaw), this.unit.base.y + 0.3 * Math.sin(this.unit.base.yaw), ROVER_LIDAR_Z] as Vec3, yaw: this.unit.base.yaw, pitch: 0 };
    const pic = renderDepthPicture(this.intrinsics, cam, this.sensingSolids(), sensor === 'wrist' ? 2.5 : 6, 4);
  ...
  droneClear(p: Vec3, knownRing = true): string | null {
    const map = this.world.map;
    const s = map.stateAt(p);
    if (s === UNKNOWN) return 'unknown space';
    if (s === OCCUPIED) return 'a mapped obstacle';
    const cells = Math.ceil(this.droneClearanceM / map.res); const layers = Math.ceil(this.droneClearanceZM / map.res); const ring = Math.ceil(DRONE_KNOWN_RING_M / map.res);
    for (let dk = -layers; dk <= layers; dk += 1) for (let dj = -cells; dj <= cells; dj += 1) for (let di = -cells; di <= cells; di += 1) {
      if (di * di + dj * dj > cells * cells) continue;
      const v = map.stateAt([p[0] + di * map.res, p[1] + dj * map.res, p[2] + dk * map.res]);
      if (v === OCCUPIED) return 'a mapped obstacle';
      if (knownRing && dk === 0 && v === UNKNOWN && di * di + dj * dj <= ring * ring) return 'unknown space beside the path';
    }
    return null;
  }
  ...
  nextExplorationGoal(scannedFrom: readonly Vec3[] = []): { point: Vec3; legs: Vec3[]; frontierCount: number } | null {
    ...
    const flight = this.world.map.flightGrid(alt, this.droneClearanceM, DRONE_KNOWN_RING_M, this.droneClearanceZM);
    const unknown = this.sensorSet.frontier === 'tops' ? this.world.map.topUnknownGrid() : this.world.map.unknownGrid(alt, DRONE_KNOWN_RING_M);
    ...
    openBubble(flight, here, DRONE_BACKOUT_M); // the drone is where it is, and may back out of a spot the map has since closed around
    if (this.sensorSet.frontier === 'tops') {
      ...
      const gain = gainFrontier(flight, unknown, here, Math.round(TOPS_FOOTPRINT_M / flight.res), TOPS_MIN_GAIN_CELLS, 2, scannedFrom.map((p) => ({ x: p[0], y: p[1] })));
      if (!gain) return null;
      const legs = simplifyPath(flight, gain.path, true).slice(1).map((q): Vec3 => [q.x, q.y, alt]);
    ...
    const goal = nextFrontier(flight, unknown, here);
    if (!goal) return null;
    const p = standBack(goal);
    const end = goal.path.findIndex((q) => q.x === p.x && q.y === p.y);
    const legs = simplifyPath(flight, goal.path.slice(0, end + 1), true).slice(1).map((q): Vec3 => [q.x, q.y, alt]);
   ...
   */
  flightLegs(target: Vec3): Vec3[] {
    const flight = this.world.map.flightGrid(target[2], this.droneClearanceM, DRONE_KNOWN_RING_M, this.droneClearanceZM);
    insetByFence(flight, this.fence);
    const here = { x: this.drone.x, y: this.drone.y };
    openBubble(flight, here, DRONE_BACKOUT_M);
    const goal = { x: target[0], y: target[1] };
    const path = planPath(flight, here, goal);
    if (!path) return [target];
    const pts = simplifyPath(flight, path, true).slice(1);
    if (!pts.length) return [target];
    const legs = pts.map((p): Vec3 => [p.x, p.y, target[2]]);
  ...
  get grid(): Grid {
    ...
    if (!this.gridCache || this.gridCache.key !== key) this.gridCache = { key, grid: this.world.map.navGrid() };
   ...
   */
  armMount(): { x: number; y: number; z: number; yaw: number } {
    const b = this.unit.base;
    return { x: b.x + ARM_MOUNT_X * Math.cos(b.yaw), y: b.y + ARM_MOUNT_X * Math.sin(b.yaw), z: b.liftZ, yaw: b.yaw };
  }

  /** @description World-from-arm-base transform for the current base pose and lift. */
  armBaseTransform(): Mat4 {
    const b = this.unit.base;
    return multiply(multiply(translation(b.x, b.y, b.liftZ), rotationRpy(0, 0, b.yaw)), translation(ARM_MOUNT_X, 0, 0));
  }

  /** @description The tool centre point in the world frame. */
  tcpWorld(): Pose6 {
    return toPose(multiply(this.armBaseTransform(), forwardKinematics(this.armSpec, this.unit.q).tcp));
  }

  /** @description Express a world pose in the arm base frame. */
  worldToArmFrame(pose: Pose6): Mat4 {
    const b = this.unit.base;
    const inv = multiply(translation(-ARM_MOUNT_X, 0, 0), multiply(rotationRpy(0, 0, -b.yaw), translation(-b.x, -b.y, -b.liftZ)));
    return multiply(inv, fromPose(pose));
  }

  /** @description The joint origins in the world frame (for drawing and collision checks). */
  armPointsWorld(q: readonly number[] = this.unit.q): Vec3[] {
    const T = this.armBaseTransform();
    return jointOrigins(this.armSpec, q).map((p) => applyToPoint(T, p));
  }

  // ── Stability ──────────────────────────────────────────────────────────────

  /** @description The lumped masses in the base frame for the current configuration. */
  massItems(q: readonly number[] = this.unit.q, payloadKg = this.heldMass()): MassItem[] {
    const arm = armCenterOfMass(this.armSpec, q);
    const lift = this.unit.base.liftZ;
    const items: MassItem[] = [...HARDWARE_BASE_ITEMS, { name: 'arm', mass: arm.mass, com: [ARM_MOUNT_X + arm.com[0], arm.com[1], lift + arm.com[2]] }];
    if (payloadKg > 0) {
      const tcp = forwardKinematics(this.armSpec, q).tcp;
      items.push({ name: 'payload', mass: payloadKg, com: [ARM_MOUNT_X + tcp[3], tcp[7], lift + tcp[11]] });
    }
    return items;
  }

  /** @description The live tip budget. */
  tipBudget(q: readonly number[] = this.unit.q, payloadKg = this.heldMass()): TipBudget {
    return tipBudget(this.massItems(q, payloadKg), HARDWARE_FOOTPRINT);
  }

  /** @description Mass of whatever the gripper holds (a door handle carries no payload). */
  heldMass(): number {
    const id = this.unit.gripper.holding;
    if (!id || id.startsWith('handle:')) return 0;
    return OBJECT_CLASSES[objectById(this.scene, id).cls].mass;
  }

  /** @description The appliance whose handle the gripper holds, or null. */
  heldAppliance(): string | null {
    const id = this.unit.gripper.holding;
  ...
  private requireLive(): void {
    if (this.unit.estop) throw new Error('refused: e-stop is latched — a human must reset it');
    if (!this.unit.enabled) throw new Error('refused: the unit is disabled');
  }

  private record(node: string, text: string, level: 'info' | 'warn' = 'info'): void {
    this.seq += 1;
   ...
   */
  driveTo(goal: DriveGoal): void {
    this.requireLive();
    const b = this.unit.base;
    if (!isFree(this.grid, goal)) throw new Error(`refused: goal (${round(goal.x, 2)}, ${round(goal.y, 2)}) is inside an inflated obstacle`);
    if (!lineClear(this.grid, b, goal)) throw new Error('refused: the straight leg crosses an obstacle');
    const check = speedCheck(this.tipBudget(), this.unit.base.liftZ <= this.baseLimits.liftMin + 0.01 ? this.baseLimits.vStowed : this.baseLimits.vLifted);
    if (!check.ok) throw new Error(`refused: travel speed fails the tip budget (factor ${round(check.factor, 2)})`);
    this.unit.driveGoal = { ...goal };
    this.unit.jog = null;
    this.unit.phase = 'driving';
    this.record(UNIT_NODE_ID, `drive to (${round(goal.x, 2)}, ${round(goal.y, 2)}) heading ${round(goal.yaw, 2)}`);
  }
   ...
   */
  jog(v: number, w: number, seconds: number): void {
    this.requireLive();
    ...
    if (!isFree(this.grid, predicted) || !lineClear(this.grid, b, predicted)) throw new Error('refused: the jog would enter an obstacle');
    const check = speedCheck(this.tipBudget(), vc);
    if (!check.ok) throw new Error(`refused: jog speed fails the tip budget (factor ${round(check.factor, 2)})`);
  ...
  setLift(z: number): void {
    this.requireLive();
    if (!Number.isFinite(z)) throw new Error('refused: lift height must be a number');
    this.unit.liftTarget = Math.min(this.baseLimits.liftMax, Math.max(this.baseLimits.liftMin, z));
    this.record(UNIT_NODE_ID, `lift to ${round(this.unit.liftTarget, 2)} m`);
  }
   ...
   */
  moveArmToWorldPose(pose: Pose6, expectedForceN = 0, contactRadius = 0): IkResult {
    this.requireLive();
    ...
    const check = wrenchCheck(budget, expectedForceN, 0, pose.z);
    if (!check.ok) throw new Error(`refused: tip budget factor ${round(check.factor, 2)} at the ${check.edge} edge for ${round(expectedForceN, 0)} N at ${round(pose.z, 2)} m`);
    this.unit.qTarget = ik.q;
    this.unit.phase = 'arm-moving';
    this.record(UNIT_NODE_ID, `tool to (${round(pose.x, 2)}, ${round(pose.y, 2)}, ${round(pose.z, 2)})`);
    return ik;
  }

  /** @description Move the joints to a configuration (must be clear of obstacles). */
  moveArmJoints(q: readonly number[]): void {
    this.requireLive();
    if (q.length !== this.armSpec.joints.length) throw new Error('refused: wrong joint count');
    this.assertArmConfigurationClear(q);
    this.assertSweepClear(this.unit.q, q);
    this.unit.qTarget = [...q];
    this.unit.phase = 'arm-moving';
    this.record(UNIT_NODE_ID, 'joints to a named configuration');
  }
   ...
   */
  private chooseClearSolution(pose: Pose6): IkResult {
    const { solutions, bestFailure } = ikSolutions(this.armSpec, this.worldToArmFrame(pose), this.unit.q);
    if (!solutions.length) throw new Error(`refused: tool pose unreachable (${bestFailure?.reason ?? 'no convergence'}, ${round(bestFailure?.positionError ?? 0, 3)} m off)`);
    let firstHit: string | null = null;
    const clear = solutions.filter((s) => { const hit = this.configurationCollision(s.q); if (hit && !firstHit) firstHit = hit; return !hit; });
    if (!clear.length) throw new Error(`refused: the arm would pass through ${firstHit}`);
    const jointDistance = (q: number[]): number => q.reduce((sum, v, i) => sum + Math.abs(v - this.unit.q[i]), 0);
    clear.sort((a, b) => jointDistance(a.q) - jointDistance(b.q));
    let sweepHit: string | null = null;
    for (const s of clear) {
      const hit = this.sweepCollision(this.unit.q, s.q);
      if (!hit) return s;
      if (!sweepHit) sweepHit = hit;
    }
   ...
   */
  private configurationCollision(q: readonly number[]): string | null {
    ...
    const heldDoor = held ? doorSlices(applianceById(this.scene, held).door as NonNullable<ReturnType<typeof applianceById>['door']>, applianceById(this.scene, held).angle, `${held}-door`) : [];
    const inHeldDoor = (p: Vec3): boolean => heldDoor.some((s) => p[0] >= s.min[0] - 0.06 && p[0] <= s.max[0] + 0.06 && p[1] >= s.min[1] - 0.06 && p[1] <= s.max[1] + 0.06 && p[2] >= s.min[2] && p[2] <= s.max[2]);
    for (let i = 1; i < pts.length; i += 1) {
      const a = pts[i - 1]; const b = pts[i];
      const n = Math.max(1, Math.ceil(distance(a, b) / 0.05));
      for (let k = i === 1 ? n : 1; k <= n; k += 1) {
        const p: Vec3 = [a[0] + ((b[0] - a[0]) * k) / n, a[1] + ((b[1] - a[1]) * k) / n, a[2] + ((b[2] - a[2]) * k) / n];
        if (p[2] < 0.02) return 'the floor';
        if (inHeldDoor(p)) continue;
        if (this.contact && distance(p, this.contact.center) <= this.contact.radius) continue;
        const s = this.world.map.stateAt(p);
        if (s === OCCUPIED) return `a mapped obstacle at (${round(p[0], 2)}, ${round(p[1], 2)}, ${round(p[2], 2)})`;
        if (s === UNKNOWN && distance(p, base) > SELF_BUBBLE_M) return `unknown space at (${round(p[0], 2)}, ${round(p[1], 2)}, ${round(p[2], 2)})`;
      }
    }
    return null;
  }
  ...
  private assertArmConfigurationClear(q: readonly number[]): void {
    const hit = this.configurationCollision(q);
    if (hit) throw new Error(`refused: the arm would pass through ${hit}`);
  }
   ...
   */
  private assertSweepClear(from: readonly number[], to: readonly number[]): void {
    const hit = this.sweepCollision(from, to);
    if (hit) throw new Error(`refused: the arm would sweep through ${hit} on the way`);
  }

  /** @description The first solid the interpolated joint path passes through, or null when the sweep is clear. */
  private sweepCollision(from: readonly number[], to: readonly number[]): string | null {
    const span = Math.max(...to.map((v, i) => Math.abs(v - from[i])));
    const n = Math.max(1, Math.ceil(span / SWEEP_STEP_RAD));
    for (let k = 1; k < n; k += 1) {
  ...
  handleTarget(applianceId: string): { point: Vec3; yaw: number } {
    const a = applianceById(this.scene, applianceId);
    if (!a.door) throw new Error(`refused: ${applianceId} has no door`);
    return handleGrasp(a.door, a.angle);
  }
   ...
   */
  graspHandle(applianceId: string): void {
    this.requireLive();
    if (this.unit.gripper.holding) throw new Error('refused: the gripper already holds something');
    const { point } = this.handleTarget(applianceId);
    const tcp = this.tcpWorld();
    if (distance([tcp.x, tcp.y, tcp.z], point) > GRASP_TOLERANCE) throw new Error(`refused: the tool is ${round(distance([tcp.x, tcp.y, tcp.z], point), 3)} m from the ${applianceId} handle`);
    this.unit.gripper = { width: 0.03, holding: handleHold(applianceId) };
    this.record(UNIT_NODE_ID, `took the ${applianceId} handle`);
  }
  ...
  releaseHandle(): string | null {
    ...
    if (!id) return null;
    this.unit.gripper = { width: GRIPPER_MAX_WIDTH, holding: null };
    const a = applianceById(this.scene, id);
    this.record(UNIT_NODE_ID, `released the ${id} handle at ${round(a.angle * 180 / Math.PI, 0)}°`);
    return id;
  }
  ...
  grasp(): string {
    this.requireLive();
    if (this.unit.gripper.holding) throw new Error('refused: the gripper already holds an object');
    const tcp = this.tcpWorld();
    const here: Vec3 = [tcp.x, tcp.y, tcp.z];
    const candidate = this.scene.objects.find((o) => o.location.kind === 'surface' && !isEnclosed(this.scene, o) && distance(here, [o.pose.x, o.pose.y, o.pose.z]) <= GRASP_TOLERANCE);
    if (!candidate) throw new Error('refused: nothing within grasp tolerance of the tool');
    candidate.location = { kind: 'gripper', nodeId: UNIT_NODE_ID };
    this.unit.gripper = { width: Math.min(OBJECT_CLASSES[candidate.cls].size.w, GRIPPER_MAX_WIDTH), holding: candidate.id };
    // The machine knows it lifted whatever it had mapped under the tool: drop it from the world.
    const known = this.world.objects.map((o) => ({ o, d: distance(here, o.centroid) })).filter((x) => x.d <= 0.12).sort((a, b) => a.d - b.d)[0];
    if (known) this.world.lift(known.o.id);
    this.record(UNIT_NODE_ID, `grasped ${candidate.id}${known ? ` (mapped as ${known.o.id})` : ''}`);
    return candidate.id;
  }

  /** @description The discovered object nearest the tool, within a radius. */
  discoveredNearTool(radius = 0.12): string | null {
    const tcp = this.tcpWorld();
    const here: Vec3 = [tcp.x, tcp.y, tcp.z];
    const best = this.world.objects.map((o) => ({ o, d: distance(here, o.centroid) })).filter((x) => x.d <= radius).sort((a, b) => a.d - b.d)[0];
   ...
   */
  release(): { objectId: string; surfaceId: string | null } | null {
    this.requireLive();
    if (this.heldAppliance()) { const id = this.releaseHandle() as string; return { objectId: handleHold(id), surfaceId: null }; }
    const id = this.unit.gripper.holding;
    this.unit.gripper = { width: GRIPPER_MAX_WIDTH, holding: null };
    if (!id) return null;
    const o = objectById(this.scene, id);
    const tcp = this.tcpWorld();
    const surface = this.surfaceUnder(tcp.x, tcp.y, tcp.z);
    if (surface) {
      o.pose = { x: tcp.x, y: tcp.y, z: restingZ(o.cls, surface.z), yaw: tcp.yaw };
      o.location = { kind: 'surface', surfaceId: surface.id };
      this.rememberPlaced(o);
      this.record(UNIT_NODE_ID, `placed ${id} on ${surface.name}`);
      return { objectId: id, surfaceId: surface.id };
    }
    o.pose = { x: tcp.x, y: tcp.y, z: OBJECT_CLASSES[o.cls].size.h / 2, yaw: tcp.yaw };
    o.location = { kind: 'floor' };
    this.rememberPlaced(o);
    this.record(UNIT_NODE_ID, `DROPPED ${id} on the floor`, 'warn');
  ...
  private rememberPlaced(o: WorldObject): void {
    const s = OBJECT_CLASSES[o.cls].size;
   ...
   */
  private surfaceUnder(x: number, y: number, z: number): Pick<Surface, 'id' | 'name' | 'z'> | null {
    const declared = this.scene.surfaces.filter((s) => pointOnSurface(s, x, y) && s.z <= z + 0.005 && z - s.z <= 0.35).map((s) => ({ id: s.id, name: s.name, z: s.z }));
  ...
  eStop(): void {
    ...
    this.record(UNIT_NODE_ID, 'E-STOP latched', 'warn');
  }
  ...
  resetEstop(): void {
    ...
    this.unit.phase = 'idle';
    this.record(UNIT_NODE_ID, 'e-stop reset by a human');
  }
  ...
  droneTakeoff(): void {
    ...
    for (let z = this.drone.z + 0.4; z <= this.droneLimits.cruiseAlt; z += 0.05) {
      ...
      if (why) throw new Error(`takeoff refused: ${why} at ${round(z, 2)} m — scan first`);
    }
    this.drone = takeoff(this.drone, this.droneLimits);
    this.cmd = null;
    this.record(DRONE_NODE_ID, 'takeoff');
  }

  /** @description Fly to a point inside the room along a straight line that the DISCOVERED map shows clear. */
  droneGoto(target: Vec3, opts: { recovering?: boolean } = {}): void {
    if (this.droneDown) throw new Error(`drone is down: ${this.droneDown}`);
    if (this.localization.status === 'lost' && !opts.recovering) throw new Error('goto refused: localisation lost — land and re-anchor on the pad');
    if (this.drone.mode === 'landed') throw new Error('goto refused: the drone is not airborne — take off first');
    const roomOnly: Fence = { ...this.fence, keepOut: [] };
    const from: Vec3 = [this.drone.x, this.drone.y, this.drone.z];
    const len = distance(from, target);
    ...
    for (let i = 1; i <= n; i += 1) {
      const p: Vec3 = [from[0] + ((target[0] - from[0]) * i) / n, from[1] + ((target[1] - from[1]) * i) / n, from[2] + ((target[2] - from[2]) * i) / n];
      if (stuck && distance(p, from) <= DRONE_BACKOUT_M) continue;
      const why = this.droneClear(p);
      if (why) throw new Error(`goto refused: path through ${why} at (${round(p[0], 2)}, ${round(p[1], 2)}, ${round(p[2], 2)})`);
    }
    this.drone = gotoPoint(this.drone, target, roomOnly, this.droneLimits);
    this.cmd = null;
    this.record(DRONE_NODE_ID, `goto (${round(target[0], 2)}, ${round(target[1], 2)}, ${round(target[2], 2)})`);
  }
  ...
  droneLand(force = false): void {
    if (!force && this.drone.mode !== 'landed') {
      for (let z = this.drone.z - 0.05; z >= 0.4; z -= 0.05) {
        if (this.world.map.stateAt([this.drone.x, this.drone.y, z]) === OCCUPIED) throw new Error(`land refused: a mapped obstacle below at ${round(z, 2)} m`);
      }
    }
    this.drone = land(this.drone);
    this.cmd = null;
    this.record(DRONE_NODE_ID, 'land');
  }
   ...
   */
  recoveryTrail(): Vec3[] {
    const here: Vec3 = [this.drone.x, this.drone.y, this.drone.z];
    const cruise = this.droneLimits.cruiseAlt;
    const trail: Vec3[] = [];
    for (const p of [...this.registeredTrail].reverse()) {
      const q: Vec3 = [p[0], p[1], cruise];
      if (distance(q, here) > 0.15 && (!trail.length || distance(trail[trail.length - 1], q) > 0.15)) trail.push(q);
    }
    const home = this.scene.droneHome;
    trail.push([home[0], home[1], cruise]);
    return trail.slice(-RECOVERY_TRAIL_MAX);
  }
  ...
  groundLost(reason: string): void {
    if (this.drone.mode !== 'landed') this.droneLand(true);
    this.droneDown = `grounded: ${reason}`;
    this.record(DRONE_NODE_ID, this.droneDown, 'warn');
  }

  /** @description The camera pose for the current drone pose and gimbal pitch. */
  droneCamera(): CameraPose {
  ...
  believedDroneCamera(): CameraPose {
   ...
   * @returns The frame.
   */
  droneObserve(pitch?: number, yaw?: number): CameraFrame {
    if (typeof pitch === 'number') this.gimbalPitch = pitch;
    if (typeof yaw === 'number') { this.truth = { ...this.truth, yaw: this.truth.yaw + (yaw - this.drone.yaw) }; this.drone = { ...this.drone, yaw }; }
    const frame = this.viewFrame();
    this.record(DRONE_NODE_ID, `observed ${frame.detections.length} object(s)`);
    return frame;
  }

  /** @description The camera's current view with no side effect — what the surface renders as the live picture. */
  viewFrame(): CameraFrame {
    const cam = this.droneCamera();
    const observable = this.scene.objects.map((o) => ({ id: o.id, cls: o.cls, center: [o.pose.x, o.pose.y, o.pose.z] as Vec3, size: OBJECT_CLASSES[o.cls].size, enclosed: isEnclosed(this.scene, o) || o.location.kind === 'gripper' }));
    const detections = observe(this.intrinsics, cam, observable);
    const T = cameraToWorld(cam);
    const outlines = this.solids().map((b) => projectBoxOutline(this.intrinsics, T, b));
   ...
   */
  localize(det: Detection, cam: CameraPose): { position: Vec3; surfaceId: string } | null {
    const cls = det.cls as keyof typeof OBJECT_CLASSES;
    if (!(cls in OBJECT_CLASSES)) return null;
    const ordered = [...this.scene.surfaces].sort((a, b) => b.z - a.z);
    for (const s of ordered) {
      const hit = localizeOnPlane(this.intrinsics, cam, det.u, det.v, restingZ(cls, s.z));
      if (hit && pointOnSurface(s, hit[0], hit[1])) return { position: hit, surfaceId: s.id };
    }
    return null;
  }
  ...
  advance(ms: number): void {
    const steps = Math.max(0, Math.ceil(ms / (STEP_S * 1000)));
    for (let i = 0; i < steps; i += 1) {
      this.timeMs += STEP_S * 1000;
      this.stepBase();
      this.stepLift();
      this.stepArm();
      this.stepDrone();
  ...
  private stepDrone(): void {
    const before = this.drone;
    const struck = this.plant ? this.stepPlant(before) : this.stepKinematic(before);
    if (this.drone === before) return;
    if (before.mode === 'landing' && this.drone.mode === 'landed') { this.padFix(); return; }
    if (this.drone.mode !== 'landed') this.altitudeHold();
    if (!struck) return;
    const p = this.truth;
    this.droneDown = `struck ${struck} at true position (${round(p.x, 2)}, ${round(p.y, 2)}, ${round(p.z, 2)}) while it believed it was at (${round(this.drone.x, 2)}, ${round(this.drone.y, 2)}, ${round(this.drone.z, 2)})`;
  ...
  private stepKinematic(before: DroneState): string | null {
    this.drone = droneStep(before, this.droneLimits, STEP_S);
    if (this.drone === before) return null;
    this.truth = propagateTruth(this.truth, before, this.drone, this.droneLimits.odometry);
    if (this.drone.mode === 'landed' || this.truth.z < this.droneLimits.minMovingAlt) return null;
    const p: Vec3 = [this.truth.x, this.truth.y, this.truth.z];
    const hit = this.sensingSolids().find((b) => b.max[2] > 0.05 && insideBox(p, { min: b.min, max: b.max }, this.droneRadiusM));
   ...
   */
  private stepPlant(before: DroneState): string | null {
    ...
    const ramp = droneStep({ ...before, ...cmd }, this.droneLimits, STEP_S);
    if (before.mode === 'landed' && ramp.mode === 'landed') return null;
    ...
    const phase: PlantPhase = before.mode;
    const r = (this.plant as PhysicsPlant).step(this.cmd, phase, STEP_S);
    this.truth = r.pose;
    ...
    const inLayer = this.drone.mode !== 'hover' || Math.abs(r.pose.z - this.cmd.z) < this.world.map.res / 2 - LAYER_HOLD_MARGIN_M;
    if (finished && !(r.settled && inLayer)) this.drone = { ...this.drone, mode: before.mode, target: before.target };
    return r.contact;
  }
   ...
   */
  private altitudeHold(): void {
    const range = this.sensorSet.nadirRangerM;
    if (range === null) return;
    const drop = this.underDrop();
    const hit = castRay([this.truth.x, this.truth.y, this.truth.z - drop], [0, 0, -1], this.sensingSolids(), range);
    if (!hit) return;
    ...
    for (let dj = -1; dj <= 1; dj += 1) for (let di = -1; di <= 1; di += 1) {
      const i = i0 + di; const j = j0 + dj;
      if (i < 0 || j < 0 || i >= map.nx || j >= map.ny) continue;
      const t = map.topWithClearance(j * map.nx + i);
      if (Number.isFinite(t) && t > 0) candidates.push(t);
    }
    let best = Number.NaN; let bestGap = 0.5;
    for (const t of candidates) { const z = hit.t + drop + t; const gap = Math.abs(z - this.drone.z); if (gap < bestGap) { bestGap = gap; best = z; } }
    if (Number.isNaN(best)) return;
  ...
  private padFix(): void {
    const home = this.scene.droneHome;
    const onPad = Math.hypot(this.truth.x - home[0], this.truth.y - home[1]) <= PAD_FIX_RADIUS;
    if (!onPad) return;
    ...
    this.record(DRONE_NODE_ID, `pad fix: belief corrected ${round(err.positionM * 100, 1)} cm, ${round((err.yawRad * 180) / Math.PI, 2)}°`);
  }

  private stepBase(): void {
    const u = this.unit;
    if (u.jog) {
      if (this.timeMs >= u.jog.untilMs) { this.stopBase(); return; }
      u.base = integrateUnicycle(u.base, u.jog.v, u.jog.w, STEP_S, this.baseLimits);
      return;
    }
    if (!u.driveGoal) return;
    const r = driveStep(u.base, u.driveGoal, this.baseLimits, STEP_S);
    u.base = r.next;
    u.phase = r.phase === 'done' ? 'idle' : `driving:${r.phase}`;
    if (r.done) { u.driveGoal = null; this.record(UNIT_NODE_ID, 'arrived'); }
  }

  private stepLift(): void {
    const u = this.unit;
    if (u.liftTarget === null) return;
    const r = liftStep(u.base, u.liftTarget, this.baseLimits, STEP_S);
    u.base = r.next;
    if (r.done) u.liftTarget = null;
  }
   ...
   */
  private stepArmPlant(): void {
    const u = this.unit;
    const plant = this.armPlant as ArmPlant;
    const command = u.qTarget ?? u.q;
    const grip = u.gripper.holding ? ARM_GRIP_CLOSED_RAD : ARM_GRIP_OPEN_RAD;
    const r = plant.step(command, grip, STEP_S);
    ...
    if (r.contact && r.contact !== this.lastArmContact) {
      ...
      this.record(UNIT_NODE_ID, `the arm touched ${r.contact}`, 'warn');
    } else if (!r.contact) this.lastArmContact = null;
    if (u.qTarget && r.settled) { u.qTarget = null; if (u.phase === 'arm-moving') u.phase = 'idle'; }
  }

  private stepArm(): void {
    const u = this.unit;
    if (this.armPlant) this.stepArmPlant();
    else if (u.qTarget) {
      const r = stepJoints(this.armSpec, u.q, u.qTarget, STEP_S);
      u.q = r.q;
      if (r.arrived) { u.qTarget = null; if (u.phase === 'arm-moving') u.phase = 'idle'; }
    }
    const appliance = this.heldAppliance();
    if (appliance) {
      const a = applianceById(this.scene, appliance);
      const tcp = this.tcpWorld();
      if (a.door) {
        const before = a.angle;
        a.angle = doorAngleFromPoint(a.door, tcp.x, tcp.y);
        if (Math.abs(a.angle - before) > 1e-6) this.relocateDoor(a.id, before, a.angle);
      }
    } else if (u.gripper.holding) {
      const o = objectById(this.scene, u.gripper.holding);
      const tcp = this.tcpWorld();
      o.pose = { x: tcp.x, y: tcp.y, z: tcp.z, yaw: tcp.yaw };
    }
    for (const a of this.scene.appliances) if (a.door) a.open = a.angle >= a.door.openThreshold;
  }

  /** @description The machine moved a door it holds: its old panel voxels are known free now and the new ones occupied. */
  private relocateDoor(applianceId: string, from: number, to: number): void {
    const a = applianceById(this.scene, applianceId);
    if (!a.door) return;
    const d = a.door;
    this.world.map.markPanel([d.hinge.x, d.hinge.y], doorDirection(d, from), doorNormal(d, from), d.width, d.thickness, d.height, FREE);
    this.world.map.markPanel([d.hinge.x, d.hinge.y], doorDirection(d, to), doorNormal(d, to), d.width, d.thickness, d.height, OCCUPIED);
    this.gridCache = null;
  }
  ...
  snapshot(): Record<string, unknown> {
    ...
    const tcp = this.tcpWorld();
    const objects = this.scene.objects.map((o: WorldObject) => ({ ...o, enclosed: isEnclosed(this.scene, o) }));
    return {
      simulated: true,
      timeMs: this.timeMs,
      scenario: this.scene.name,
      scene: { name: this.scene.name, room: this.scene.room, obstacles: this.scene.obstacles, solids: this.solids(), surfaces: this.scene.surfaces, zones: this.scene.zones, appliances: this.scene.appliances.map((a) => ({ ...a, handle: a.door ? handleGrasp(a.door, a.angle).point : null })) },
      objects,
      unit: { nodeId: UNIT_NODE_ID, ...this.unit, tcp, armPoints: this.armPointsWorld(), tipBudget: budget, payloadKg: this.heldMass(), stowed: this.unit.base.liftZ <= this.baseLimits.liftMin + 0.01, speedLimit: this.unit.base.liftZ <= this.baseLimits.liftMin + 0.01 ? this.baseLimits.vStowed : this.baseLimits.vLifted, backend: this.armPlant ? this.armPlant.backend : 'kinematic', plant: this.armPlant ? { engine: this.armPlant.engine, version: this.armPlant.version, seed: this.armPlant.seed } : null, node: isArmNode(this.armPlant) ? { nodeId: this.armPlant.nodeId, link: this.armPlant.link, endpoint: this.armPlant.endpoint } : null, servos: this.armTelemetry },
      drone: { nodeId: DRONE_NODE_ID, ...this.drone, gimbalPitch: this.gimbalPitch, down: this.droneDown, sensorSet: this.sensorSet.id, radiusM: this.droneRadiusM, clearanceM: this.droneClearanceM, clearanceZM: this.droneClearanceZM, backend: this.plant ? (isDroneNode(this.plant) && this.plant.link === 'rail' ? 'node' : 'physics') : 'kinematic', plant: this.plant ? { engine: this.plant.engine, version: this.plant.version, seed: this.plant.seed, controller: this.plant.controller } : null, node: isDroneNode(this.plant) ? { nodeId: this.plant.nodeId, link: this.plant.link, endpoint: this.plant.endpoint } : null, truth: { ...this.truth, simOnly: true }, localization: { ...this.localization, errorM: this.localizationError().positionM, yawErrorRad: this.localizationError().yawRad, trail: this.registeredTrail.length } },
      world: this.world.snapshot(),
      lastSweep: this.lastSweep ? { origin: this.lastSweep.origin, points: this.lastSweep.hits.filter((_, i) => i % 4 === 0).map((h) => h.point.map((v) => round(v, 3))) } : null,
