...
import { PlanExecutor, type CommandRecord } from './plan-executor';
...
export interface PlanValidation {
  ok: boolean;
  issues: string[];
  /** Simulated seconds the rehearsal took. */
  durationS: number;
  stepsCompleted: number;
  stepsTotal: number;
  /** Object id → surface id (or 'floor' / 'gripper') after the rehearsal. */
  finalLocations: Record<string, string>;
  /** Minimum tip-budget factor seen for the front edge against the design's fridge case (60 N at 1.1 m). */
  minFrontFactor: number;
}

/** @description Upper bound on rehearsal time so a stuck plan cannot spin (simulated seconds). */
export const REHEARSAL_LIMIT_S = 1800;
 ...
 */
export function validatePlan(sim: WorldSim, plan: SkillPlan): PlanValidation {
  const world = sim.clone();
  const records: CommandRecord[] = [];
  const executor = new PlanExecutor(world, (r) => records.push(r), 'rehearsal');
  executor.start(plan);
  let minFrontFactor = Number.POSITIVE_INFINITY;
  const start = world.timeMs;
  while (executor.state === 'running' && (world.timeMs - start) / 1000 < REHEARSAL_LIMIT_S) {
    executor.tick();
    if (executor.state !== 'running') break;
    world.advance(200);
  ...
  const issues: string[] = [];
  if (executor.state === 'failed') issues.push(executor.failure ?? 'the rehearsal failed');
  else if (executor.state !== 'done') issues.push(`the rehearsal did not finish within ${REHEARSAL_LIMIT_S} simulated seconds`);
  const dropped = world.scene.objects.filter((o) => o.location.kind === 'floor').map((o) => o.id);
  if (dropped.length) issues.push(`objects ended on the floor: ${dropped.join(', ')}`);
  const completed = records.filter((r) => r.outcome === 'completed' && r.command !== 'drone.register').length;
  const finalLocations: Record<string, string> = {};
  for (const o of world.scene.objects) finalLocations[o.id] = o.location.kind === 'surface' ? o.location.surfaceId : o.location.kind;
