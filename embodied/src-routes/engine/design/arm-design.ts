...
import { forwardKinematics, maxReach, type ArmSpec } from '../arm/arm-model';
import { G_MPS2 } from '../physics/mjcf';
import { position } from '../math/transform';
import type { Vec3 } from '../math/vec';
import type { BoughtPart } from './parts-model';
import { armPrintedParts, type ArmLayout, type ArmPrintedPart } from './arm-parts';
import { BACKLASH_DEG, chooseDrive, DRIVE_OPTIONS, DYNAMIC_ALLOWANCE, STS3215_12V, type DriveChoice, type ServoSpec } from './servos';

/** The angular acceleration a vertical-axis joint is sized to deliver with the arm stretched (rad/s²). */
export const ALPHA_DESIGN_RADS2 = 4;
const HALF_PI = Math.PI / 2;
const DEG = Math.PI / 180;

/** @description The fits: one arm for now — the desk arm we print first. */
export type ArmFit = 'desk-6';

/** @description What a fit fixes: its layout, its servo, the payload it is sized for, its joint limits. */
export interface ArmFitSpec {
  id: ArmFit;
  label: string;
  layout: ArmLayout;
  servo: ServoSpec;
  payloadKg: number;
  /** Joint limits (rad), J1..J6, set by the printed brackets, not by the servos. */
  limits: [number, number][];
}

export const ARM_FITS: Record<ArmFit, ArmFitSpec> = {
  'desk-6': {
    id: 'desk-6',
...
/** @description One joint as designed: what it must hold, what drives it, how fast it may go. */
export interface ArmJointDesign {
  joint: number;
  name: string;
  /** The worst gravity torque over the pose search (N·m) and the pose it occurs in. */
  gravityNm: number;
  worstQ: number[];
  /** The torque to accelerate the stretched arm about this axis, vertical axes only (N·m). */
  inertialNm: number;
  requiredNm: number;
  drive: DriveChoice | null;
}

/** @description The whole arm: layout, parts, joints, spec, budgets, bill of materials. */
export interface ArmDesign {
  fit: ArmFitSpec;
  parts: ArmPrintedPart[];
  bought: BoughtPart[];
  joints: ArmJointDesign[];
  /** The kinematic description the sim, the inverse kinematics and the physics model all read. */
  spec: ArmSpec;
  /** Mass rotating with each joint (kg), J1..J6, and where each servo sits. */
  linkMassesKg: number[];
  massBudget: { printedG: number; servosG: number; movingG: number; payloadG: number };
  reachM: number;
  repeatability: { worstMm: number; rssMm: number };
  servoCount: number;
  approxUsd: number;
  undersized: string[];
  printRules: string[];
 ...
 */
export function armSpecFor(fit: ArmFitSpec, linkMassesKg: number[], speeds: number[]): ArmSpec {
  const l = fit.layout;
  const rows: [number, number, number][] = [[l.shoulderMm / 1000, 0, HALF_PI], [0, l.upperArmMm / 1000, 0], [0, 0, -HALF_PI], [l.forearmMm / 1000, 0, HALF_PI], [0, 0, -HALF_PI], [l.toolMm / 1000, 0, 0]];
  return {
    id: fit.id,
...
function pointMasses(spec: ArmSpec, q: readonly number[], payloadKg: number): { origins: Vec3[]; axes: Vec3[]; masses: { k: number; m: number; p: Vec3 }[] } {
  const { frames } = forwardKinematics(spec, q);
  const origins: Vec3[] = [[0, 0, 0], ...frames.map(position)];
  const axes: Vec3[] = [[0, 0, 1], ...frames.map((f) => [f[2], f[6], f[10]] as Vec3)];
  const masses = spec.joints.map((j, i) => ({ k: i + 1, m: j.linkMass, p: [(origins[i][0] + origins[i + 1][0]) / 2, (origins[i][1] + origins[i + 1][1]) / 2, (origins[i][2] + origins[i + 1][2]) / 2] as Vec3 }));
 ...
 */
export function gravityTorques(spec: ArmSpec, q: readonly number[], payloadKg: number): number[] {
  const { origins, axes, masses } = pointMasses(spec, q, payloadKg);
  return spec.joints.map((_, i) => {
    const o = origins[i]; const z = axes[i];
    let tau = 0;
    for (const pm of masses) {
      if (pm.k < i + 1) continue;
      const r: Vec3 = [pm.p[0] - o[0], pm.p[1] - o[1], pm.p[2] - o[2]];
      // (r × F)·z with F = (0, 0, −m g): r × F = (−r_y m g, r_x m g, 0).
      tau += (-r[1] * pm.m * G_MPS2) * z[0] + (r[0] * pm.m * G_MPS2) * z[1];
    }
    return tau;
  });
}
...
export const BENCH_CLEARANCE_M = 0.035;

/** @description True when every joint frame, every link's middle and the tool sit above the bench in a configuration. */
export function aboveTheBench(spec: ArmSpec, q: readonly number[]): boolean {
  const { frames } = forwardKinematics(spec, q);
  const origins = [[0, 0, 0] as Vec3, ...frames.map(position)];
  for (let i = 1; i < origins.length; i += 1) {
    if (origins[i][2] < BENCH_CLEARANCE_M) return false;
    if ((origins[i][2] + origins[i - 1][2]) / 2 < BENCH_CLEARANCE_M) return false;
  }
  return true;
}

/** @description The moment of inertia of every point mass about a vertical axis through the base (kg·m²). */
function verticalInertia(spec: ArmSpec, q: readonly number[], payloadKg: number): number {
 ...
 */
export function worstLoads(spec: ArmSpec, payloadKg: number): { torque: { nm: number; q: number[] }[]; inertiaKgM2: number } {
  const lim = spec.joints.map((j) => [j.min, j.max]);
  const grid = (i: number, step: number): number[] => { const out: number[] = []; for (let v = lim[i][0]; v <= lim[i][1] + 1e-9; v += step) out.push(v); return out; };
  const best = spec.joints.map(() => ({ nm: 0, q: [0, 0, 0, 0, 0, 0] }));
  let inertia = 0;
  const consider = (q: number[]): void => {
    if (!aboveTheBench(spec, q)) return;
    gravityTorques(spec, q, payloadKg).forEach((t, i) => { if (Math.abs(t) > best[i].nm) best[i] = { nm: Math.abs(t), q: [...q] }; });
    inertia = Math.max(inertia, verticalInertia(spec, q, payloadKg));
  };
  for (const q2 of grid(1, 15 * DEG)) for (const q3 of grid(2, 15 * DEG)) for (const q4 of [0, HALF_PI, -HALF_PI]) for (const q5 of grid(4, 15 * DEG)) consider([0, q2, q3, q4, q5, 0]);
  // Refine each joint's own worst pose, one coordinate at a time.
  best.forEach((b, i) => {
    let q = [...b.q];
    for (let round = 0; round < 3; round += 1) for (const c of [1, 2, 3, 4]) {
      for (let dv = -15; dv <= 15; dv += 1) {
        const cand = [...q]; cand[c] = Math.min(lim[c][1], Math.max(lim[c][0], q[c] + dv * DEG));
        if (!aboveTheBench(spec, cand)) continue;
        const t = Math.abs(gravityTorques(spec, cand, payloadKg)[i]);
        if (t > best[i].nm) { best[i] = { nm: t, q: cand }; }
      }
      q = [...best[i].q];
    }
  });
...
const bought = (id: string, name: string, qty: number, massEachG: number, role: string, approxUsdEach: number): BoughtPart => ({ id, name, qty, massEachG, role, approxUsdEach });

/** @description Where the servos sit: the link each one rotates with (the base-yaw servo is in the fixed base). */
function servoLinks(shoulderServos: 1 | 2): number[] {
...
function linkMasses(parts: ArmPrintedPart[], servo: ServoSpec, shoulderServos: 1 | 2, beltJoints: number[]): number[] {
  const g = [0, 0, 0, 0, 0, 0, 0];
  for (const p of parts) g[p.link] += p.qty * p.massEachG;
  for (const k of servoLinks(shoulderServos)) g[k] += servo.massG;
  for (const j of beltJoints) g[j] += BELT_KIT_G;
...
function boughtParts(fit: ArmFitSpec, servoCount: number, shoulderServos: 1 | 2, beltJoints: number[]): BoughtPart[] {
  const s = fit.servo;
  const list: BoughtPart[] = [
  ...
  if (shoulderServos === 1) list.push(bought('idler', '608ZZ bearing + 8 × 40 mm shaft', 1, 28, 'the shoulder\'s far pivot', 3));
  if (beltJoints.length) list.push(bought('belt-kit', 'GT2 belt stage: 20T and 40T/60T pulleys, closed 6 mm belt, two 608ZZ, 8 mm shaft', beltJoints.length, BELT_KIT_G, `the reduction at joint${beltJoints.length > 1 ? 's' : ''} ${beltJoints.map((j) => `J${j}`).join(', ')}`, 12));
  list.push(
    bought('bus', 'serial-bus servo adapter (USB to TTL half-duplex)', 1, 10, 'the node computer\'s link to the servo bus', 12),
    bought('node', 'Raspberry Pi Zero 2 W', 1, 11, 'the arm-node client only; plans, maps and guards run in the swarm', 18),
    bought('psu', '12 V 10 A supply', 1, 0, 'off the arm; headroom for the shoulder and elbow at their continuous limit', 20),
    bought('estop', 'latching mushroom e-stop, ≥ 10 A DC, in the 12 V servo feed', 1, 0, 'the hardware e-stop a kinetic (class 2) node needs: it cuts servo power, not software', 12),
    bought('hardware', 'M2/M3 screws, M4 bench bolts, zip ties, bus extension leads', 1, 30, '', 12),
  );
  return list;
}
 ...
 */
export function buildArm(fitId: ArmFit): ArmDesign {
  const cached = CACHE.get(fitId);
  if (cached) return cached;
  const design = buildArmFrom(ARM_FITS[fitId]);
 ...
 * @returns The design.
 */
export function buildArmFrom(fit: ArmFitSpec): ArmDesign {
  const s = fit.servo;
  let shoulderServos: 1 | 2 = 1;
  let beltJoints: number[] = [];
  let design: ArmDesign | null = null;
  for (let round = 0; round < 3; round += 1) {
    const parts = armPrintedParts(fit.layout, s, shoulderServos);
    ...
    const joints: ArmJointDesign[] = loads.torque.map((t, i) => {
      const inertialNm = i === 0 ? loads.inertiaKgM2 * ALPHA_DESIGN_RADS2 : 0;
      const requiredNm = t.nm * DYNAMIC_ALLOWANCE + inertialNm;
      return { joint: i + 1, name: JOINT_NAMES[i], gravityNm: t.nm, worstQ: t.q, inertialNm, requiredNm, drive: chooseDrive(s, requiredNm, i === 1 ? DRIVE_OPTIONS : DRIVE_OPTIONS.filter((o) => o.servos === 1)) };
    });
    const nextShoulder: 1 | 2 = joints[1].drive?.cfg.servos ?? 1;
    ...
    design = {
      ...
      reachM: maxReach(spec), repeatability: repeatabilityBudget(spec), servoCount,
    ...
    if (nextShoulder === shoulderServos && nextBelts.join() === beltJoints.join()) break;
    shoulderServos = nextShoulder; beltJoints = nextBelts;
  }
  return design as ArmDesign;
}

const CACHE = new Map<ArmFit, ArmDesign>();
 ...
 */
export function repeatabilityBudget(spec: ArmSpec): { worstMm: number; rssMm: number } {
  const b = BACKLASH_DEG * DEG;
  const upper = spec.joints[1].a; const fore = spec.joints[3].d; const tool = spec.joints[5].d;
...
export function armFitById(id: string | undefined): ArmFit | null {
