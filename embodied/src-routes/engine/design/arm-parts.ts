...
import type { CadFeatureProgram, CadProgram } from './airframe';
import type { ServoSpec } from './servos';

/** @description The arm's driving dimensions, all millimetres. */
export interface ArmLayout {
  /** Bench to the shoulder (J2) axis: base + turntable + shoulder bracket. */
  shoulderMm: number;
  /** Shoulder axis to elbow axis. */
  upperArmMm: number;
  /** Elbow axis to the wrist centre (J5 axis). */
  forearmMm: number;
  /** Wrist centre to the tool centre point between the jaw tips. */
  toolMm: number;
  baseDiameterMm: number;
  baseHeightMm: number;
  turntableDiameterMm: number;
  turntableMm: number;
  /** Width of the link sections (along the pitch axes). */
  linkWidthMm: number;
  linkHeightMm: number;
  wallMm: number;
  /** The widest object the gripper opens around. */
  jawOpeningMm: number;
}

/** Clearance around a servo case in its pocket (mm). */
export const CRADLE_CLEARANCE_MM = 0.3;
...
export function pocketSize(servo: ServoSpec, splineAxis: Axis, lengthAxis: Axis): Vec {
  const [L, W, H] = servo.bodyMm;
  const c = 2 * CRADLE_CLEARANCE_MM;
  const size: Vec = [W + c, W + c, W + c];
 ...
 */
function cradle(id: string, servo: ServoSpec, spline: Vec, splineAxis: Axis, lengthAxis: Axis, through = true, sign: 1 | -1 = 1): CadFeatureProgram[] {
  const centre: Vec = [...spline];
  centre[IDX[lengthAxis]] -= sign * servo.splineOffsetMm;
  const hole: CadFeatureProgram = { id: `${id}-spline`, type: 'hole', params: { diameter: SPLINE_HOLE_MM, axis: splineAxis, x: spline[0], y: spline[1], z: spline[2], ...(through ? {} : { depth: servo.bodyMm[2] }) }, label: `${id}: output spline clearance` };
...
function hubPlus(id: string, servo: ServoSpec, at: Vec, axis: Axis): CadFeatureProgram[] {
  return [
...
function hubMinus(id: string, servo: ServoSpec, at: Vec, axis: Axis, faceCoord: number): CadFeatureProgram {
  const size: Vec = [servo.hornDiscMm + 0.4, servo.hornDiscMm + 0.4, servo.hornDiscMm + 0.4];
...
const bolt = (id: string, axis: Axis, x: number, y: number, z: number, label: string, diameter = M3): CadFeatureProgram => ({ id, type: 'hole', params: { diameter, axis, x, y, z }, label });
 ...
 */
export function printedMassG(envelopeCm3: number, fill: number): number {
  return Math.round(envelopeCm3 * fill * DENSITY_G_CM3);
}

const boxCm3 = (x: number, y: number, z: number): number => (x * y * z) / 1000;
const cylCm3 = (d: number, h: number): number => (Math.PI * d * d * h) / 4000;

/** @description One printed arm part: its program and the estimate that goes into the link masses. */
export interface ArmPrintedPart {
  id: string;
  name: string;
  qty: number;
  material: string;
  printNotes: string;
  massEachG: number;
  /** Which link it rotates with (1 = after the base yaw, … 6 = the gripper). 0 = the fixed base. */
  link: 0 | 1 | 2 | 3 | 4 | 5 | 6;
  cad: CadProgram;
}

/** @description The base: bench bolts, the base-yaw servo standing in its cradle, the 6807 bearing seat above it. */
function base(l: ArmLayout, s: ServoSpec): ArmPrintedPart {
  const h = l.baseHeightMm; const seat = 8;
  // The case stands under the bearing seat, its centre half a case below the seat floor; the spline is on the axis.
  const caseOnAxis: Vec = [0, 0, h - seat - s.bodyMm[2] / 2 - CRADLE_CLEARANCE_MM];
...
function turntable(l: ArmLayout, s: ServoSpec): ArmPrintedPart {
  const t = l.turntableMm; const boss = 8;
  const features: CadFeatureProgram[] = [
    { id: 'edge', type: 'chamfer', params: { length: 1, edges: 'top' } },
...
function shoulderBracket(l: ArmLayout, s: ServoSpec, servos: 1 | 2): ArmPrintedPart {
  const gap = l.linkWidthMm + 3;
  const cheek = s.bodyMm[2] + 2 * CRADLE_CLEARANCE_MM + 2 * CRADLE_WALL_MM;
  const sizeX = gap + 2 * cheek; const sizeY = 60; const floor = 14;
  const axisZ = l.shoulderMm - l.baseHeightMm - l.turntableMm;
  const sizeZ = axisZ + 16;
  const xServo = gap / 2 + CRADLE_WALL_MM + (s.bodyMm[2] + 2 * CRADLE_CLEARANCE_MM) / 2;
  const features: CadFeatureProgram[] = [
  ...
  if (servos === 2) features.push({ id: 'j2-b-pocket', type: 'box-cut', params: { size: pocketSize(s, 'x', 'z'), center: [xServo, 0, axisZ - s.splineOffsetMm] }, label: 'j2-b: second shoulder servo pocket' });
  else features.push({ id: 'idler-seat', type: 'hole', params: { diameter: 22.2, axis: 'x', y: 0, z: axisZ, depth: 7 }, label: '608 bearing seat for the 8 mm idler shaft' });
...
function upperArm(l: ArmLayout, s: ServoSpec, o: { id: string; name: string; lengthMm: number; rootHubs: 1 | 2; tipSpline: Axis | null; link: 2; printNotes: string }): ArmPrintedPart {
  const w = l.linkWidthMm; const h = l.linkHeightMm;
  const sizeX = o.lengthMm + 2 * 22; const root = -o.lengthMm / 2; const tip = o.lengthMm / 2;
  const features: CadFeatureProgram[] = [{ id: 'edges', type: 'fillet', params: { radius: 3, edges: 'parallel-x' } }];
  if (o.tipSpline) {
    const block = pocketSize(s, o.tipSpline, 'x').map((v) => v + 2 * CRADLE_WALL_MM) as Vec;
  ...
  if (o.rootHubs === 2) features.push(hubMinus('root', s, [root, 0, h / 2], 'y', -w / 2));
  if (o.tipSpline) features.push(...cradle('tip', s, [tip, 0, h / 2], o.tipSpline, 'x'));
  const envelope = boxCm3(sizeX, w, h) + (o.tipSpline ? boxCm3(...(pocketSize(s, o.tipSpline, 'x').map((v) => v + 2 * CRADLE_WALL_MM) as Vec)) * 0.5 : 0);
 ...
 */
function forearm(l: ArmLayout, s: ServoSpec, lengthMm: number): ArmPrintedPart {
  const w = l.linkWidthMm; const h = l.linkHeightMm;
  const sizeX = lengthMm + 2 * 22; const wrist = -lengthMm / 2;
  const block = pocketSize(s, 'y', 'x').map((v) => v + 2 * CRADLE_WALL_MM) as Vec;
  const features: CadFeatureProgram[] = [
...
function bracket(l: ArmLayout, s: ServoSpec, o: { id: string; name: string; link: 3 | 5 }): ArmPrintedPart {
  const pocket = pocketSize(s, 'x', 'y');
  const sizeX = pocket[0] + 2 * CRADLE_WALL_MM + 12; const sizeY = pocket[1] + 2 * CRADLE_WALL_MM; const sizeZ = pocket[2] + 2 * CRADLE_WALL_MM;
 ...
 */
function gripperBody(l: ArmLayout, s: ServoSpec): ArmPrintedPart {
  const pocket = pocketSize(s, 'y', 'x');
  const sizeX = 72; const sizeY = 60; const floor = 6; const sizeZ = floor + pocket[2] + CRADLE_WALL_MM;
...
function movingJaw(l: ArmLayout, s: ServoSpec): ArmPrintedPart {
...
function jawPad(): ArmPrintedPart {
 ...
 */
export function armPrintedParts(l: ArmLayout, s: ServoSpec, shoulderServos: 1 | 2): ArmPrintedPart[] {
  const elbowBlock = 30; const wristBlock = 30;
  return [
    base(l, s),
    turntable(l, s),
