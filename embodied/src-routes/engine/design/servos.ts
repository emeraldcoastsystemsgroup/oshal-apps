...
/** @description One servo class: the figures a joint is sized from, where they come from, and what it costs. */
export interface ServoSpec {
  id: string;
  name: string;
  /** Rated stall torque (N·m). */
  stallNm: number;
  /** No-load output speed (rad/s). */
  noLoadRadS: number;
  massG: number;
  /** The case, length × width × height (mm): what a printed pocket is cut to. */
  bodyMm: [number, number, number];
  /** Case centre to the output axis along the length (mm): where a pocket puts the spline. Measured, not published. */
  splineOffsetMm: number;
  /** The disc horn supplied with the servo (mm): the recess a hub is cut to. Measured, not published. */
  hornDiscMm: number;
  encoder: string;
 ...
 */
export const STS3215_12V: ServoSpec = {
  id: 'sts3215-12v',
...
export const DRIVE_OPTIONS: readonly DriveConfig[] = [
...
/** @description What a drive delivers at the joint. */
export interface DriveOutput {
  stallNm: number;
  usableNm: number;
  commandedRadS: number;
}
 ...
 */
export function driveOutput(servo: ServoSpec, cfg: DriveConfig): DriveOutput {
  const efficiency = cfg.ratio === 1 ? 1 : BELT_EFFICIENCY;
  const stallNm = servo.stallNm * cfg.servos * cfg.ratio * efficiency;
  return { stallNm, usableNm: stallNm * CONTINUOUS_FRACTION, commandedRadS: (servo.noLoadRadS * SPEED_FRACTION) / cfg.ratio };
}

/** @description A joint's chosen drive and how much it has in hand. */
export interface DriveChoice {
  cfg: DriveConfig;
  output: DriveOutput;
  /** What the joint must deliver continuously (N·m). */
  requiredNm: number;
  /** usable / required: at least 1 or the drive was not chosen. */
  margin: number;
}
 ...
 */
export function chooseDrive(servo: ServoSpec, requiredNm: number, options: readonly DriveConfig[] = DRIVE_OPTIONS): DriveChoice | null {
  for (const cfg of options) {
    ...
    if (output.usableNm >= requiredNm) return { cfg, output, requiredNm, margin: Math.min(99, requiredNm > 0 ? output.usableNm / requiredNm : 99) };
  }
  return null;
}
