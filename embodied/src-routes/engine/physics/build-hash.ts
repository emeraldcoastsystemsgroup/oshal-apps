...
export const ENGINE_RUNTIME_FILES = ['embodied_worker.py', 'container/embodied_engine_bridge.py', 'requirements.txt', 'requirements-lock.txt', 'tasks/hover_leg.py', 'container/embodied_engine_node.py', 'container/embodied_px4_node.py', 'embodied_arm.py', 'tasks/reach_grasp.py'] as const;
...
export function engineBuildHash(engineDir: string): string | null {
  if (!fs.existsSync(path.join(engineDir, 'embodied_worker.py'))) return null;
  const digest = createHash('sha256');
  for (const rel of ENGINE_RUNTIME_FILES) {
    const file = path.join(engineDir, rel);
    digest.update(Buffer.from(`${rel}\0`, 'utf8'));
    if (fs.existsSync(file)) digest.update(fs.readFileSync(file).toString('utf8').replace(/\r\n/g, '\n'), 'utf8');
