...
import type { Grid, Point2 } from './occupancy-grid';
import { cellToWorld, isFree, worldToCell } from './occupancy-grid';

/** @description Frontier search result. */
export interface FrontierGoal {
  point: Point2;
  /** Cells from the start to the goal, centre points. */
  path: Point2[];
  /** Number of frontier cells that existed. */
 ...
 */
export function nextFrontier(grid: Grid, unknownGrid: Grid, start: Point2, minCells = 6): FrontierGoal | null {
  if (!isFree(grid, start)) return null;
  const s = worldToCell(grid, start);
  ...
  let frontierCount = 0;
  let goal = -1;
  const isFrontier = (idx: number): boolean => {
    const i = idx % grid.width; const j = Math.floor(idx / grid.width);
    for (const [di, dj] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      const ii = i + di; const jj = j + dj;
      if (ii < 0 || jj < 0 || ii >= grid.width || jj >= grid.height) continue;
      if (unknownGrid.cells[jj * grid.width + ii]) return true;
    }
    return false;
  };
  while (head < queue.length) {
    const cur = queue[head]; head += 1;
    if (isFrontier(cur)) { frontierCount += 1; if (goal < 0 && dist[cur] >= minCells) goal = cur; }
    const i = cur % grid.width; const j = Math.floor(cur / grid.width);
    for (const [di, dj] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      const ii = i + di; const jj = j + dj;
      if (ii < 0 || jj < 0 || ii >= grid.width || jj >= grid.height) continue;
      const n = jj * grid.width + ii;
      if (grid.cells[n] || dist[n] >= 0) continue;
      dist[n] = dist[cur] + 1; parent[n] = cur; queue.push(n);
    }
  }
  if (goal < 0) return null;
  const path: Point2[] = [];
  for (let c = goal; c !== -1; c = parent[c]) path.push(cellToWorld(grid, c % grid.width, Math.floor(c / grid.width)));
  path.reverse();
...
/**
 * @description Pull the goal back along the path so it stays `backCells` cells inside known-free
 * space; the sweep from there still sees past the frontier.
 */
/** @description A goal chosen by what the camera will learn there. */
export interface GainGoal {
  goal: Point2;
  path: Point2[];
  gain: number;
  frontierCount: number;
 ...
 */
export function gainFrontier(grid: Grid, unseen: Grid, start: Point2, radiusCells: number, minGain: number, stride = 2, avoid: readonly Point2[] = [], avoidM = 0.4): GainGoal | null {
  if (!isFree(grid, start)) return null;
  const s = worldToCell(grid, start);
  const sIdx = s.j * grid.width + s.i;
  const dist = new Int32Array(grid.cells.length).fill(-1);
  const parent = new Int32Array(grid.cells.length).fill(-1);
  const queue: number[] = [sIdx]; dist[sIdx] = 0; let head = 0;
  while (head < queue.length) {
    const cur = queue[head]; head += 1;
    const i = cur % grid.width; const j = Math.floor(cur / grid.width);
    for (const [di, dj] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
  ...
  for (let dj = -radiusCells; dj <= radiusCells; dj += 1) for (let di = -radiusCells; di <= radiusCells; di += 1) if (di * di + dj * dj <= radiusCells * radiusCells) offs.push([di, dj]);
  let best = -1; let bestScore = 0; let bestGain = 0; let frontierCount = 0;
  for (let j = 0; j < grid.height; j += stride) for (let i = 0; i < grid.width; i += stride) {
    ...
    const here = cellToWorld(grid, i, j);
    if (avoid.some((a) => Math.hypot(a.x - here.x, a.y - here.y) < avoidM)) continue;
    let gain = 0;
    for (const [di, dj] of offs) { const ii = i + di; const jj = j + dj; if (ii < 0 || jj < 0 || ii >= grid.width || jj >= grid.height) continue; if (unseen.cells[jj * grid.width + ii]) gain += 1; }
    if (gain < minGain) continue;
    frontierCount += 1;
    const score = gain / (dist[idx] + 4);
    if (score > bestScore) { bestScore = score; best = idx; bestGain = gain; }
  }
  if (best < 0) return null;
  const path: Point2[] = [];
  for (let c = best; c !== -1; c = parent[c]) path.push(cellToWorld(grid, c % grid.width, Math.floor(c / grid.width)));
  path.reverse();
  path[0] = { x: start.x, y: start.y };
  return { goal: path[path.length - 1], path, gain: bestGain, frontierCount };
}

export function standBack(goal: FrontierGoal, backCells = 3): Point2 {
