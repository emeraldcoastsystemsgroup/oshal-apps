...
import { DEFAULT_DISCOVER, discoverObjects, discoverSurfaces, heightField, type DiscoverOptions, type RawObject, type RawSurface } from './discover';
import { FREE, VoxelMap, type MapBounds, type MapStats } from './voxel-map';

/** @description A tracked surface. */
export interface DiscoveredSurface {
  ...
  lastSeenScan: number;
  /** A name a bot or person attached (the map itself has none). */
  label: string | null;
}
...
function footprintOverlap(a: { min: readonly number[]; max: readonly number[] }, b: { min: readonly number[]; max: readonly number[] }): number {
  const w = Math.min(a.max[0], b.max[0]) - Math.max(a.min[0], b.min[0]); const h = Math.min(a.max[1], b.max[1]) - Math.max(a.min[1], b.min[1]);
  if (w <= 0 || h <= 0) return 0;
  const areaA = (a.max[0] - a.min[0]) * (a.max[1] - a.min[1]); const areaB = (b.max[0] - b.min[0]) * (b.max[1] - b.min[1]);
...
export class WorldModel {
  ...
  private refreshedAt = -1;

  constructor(bounds: MapBounds, readonly opts: DiscoverOptions = DEFAULT_DISCOVER, map?: VoxelMap) {
    this.map = map ?? new VoxelMap(bounds);
  }

  /** @description A deep copy with the same ids. */
  clone(): WorldModel {
    const w = new WorldModel(this.map.bounds, this.opts, this.map.clone());
    w.surfaces = JSON.parse(JSON.stringify(this.surfaces)) as DiscoveredSurface[];
    w.objects = JSON.parse(JSON.stringify(this.objects)) as DiscoveredObject[];
    w.nextSurface = this.nextSurface; w.nextObject = this.nextObject; w.refreshedAt = this.refreshedAt;
    return w;
  }

  /** @description Re-run discovery when the map changed since the last refresh; match to tracked entities. */
  refresh(): void {
    if (this.refreshedAt === this.map.version) return;
    this.refreshedAt = this.map.version;
    const tops = heightField(this.map);
    const rawS = discoverSurfaces(this.map, this.opts, tops);
    const surfaceIds = rawS.map((s) => this.matchSurface(s));
    const rawO = discoverObjects(this.map, rawS, this.opts, tops);
  ...
  private matchSurface(s: RawSurface): string {
    const hit = this.surfaces.find((t) => Math.abs(t.z - s.z) <= this.map.res * 1.5 && overlap(t.bbox, s.bbox) > 0.3);
    if (hit) { Object.assign(hit, { z: s.z, bbox: s.bbox, centroid: s.centroid, areaM2: s.areaM2, columns: s.columns, lastSeenScan: this.map.scans }); return hit.id; }
    const id = `surf-${this.nextSurface}`; this.nextSurface += 1;
  ...
  private matchObject(o: RawObject, surfaceId: string): string {
    ...
    if (hit) { Object.assign(hit, { centroid: o.centroid, min: o.min, max: o.max, size, surfaceId, supportZ: o.supportZ, guess: o.guess, voxels: o.voxels, lastSeenScan: this.map.scans }); return hit.id; }
    const id = `obj-${this.nextObject}`; this.nextObject += 1;
  ...
  private spaceIsFree(o: DiscoveredObject): boolean {
    const [i0, j0, k0] = this.map.toCell(o.min); const [i1, j1, k1] = this.map.toCell([o.max[0] - 1e-6, o.max[1] - 1e-6, o.max[2] - 1e-6]);
    let total = 0; let free = 0;
    for (let k = k0; k <= k1; k += 1) for (let j = j0; j <= j1; j += 1) for (let i = i0; i <= i1; i += 1) { if (!this.map.inside(i, j, k)) continue; total += 1; if (this.map.cells[this.map.index(i, j, k)] === FREE) free += 1; }
    return total > 0 && free / total > 0.6;
  }

  /** @description Forget an object (it is in the gripper) and free its voxels. */
  lift(objectId: string): DiscoveredObject | null {
  ...
  surface(id: string): DiscoveredSurface | null { return this.surfaces.find((s) => s.id === id) ?? null; }
  object(id: string): DiscoveredObject | null { return this.objects.find((o) => o.id === id) ?? null; }
  objectsOn(surfaceId: string): DiscoveredObject[] { return this.objects.filter((o) => o.surfaceId === surfaceId); }
  stats(): MapStats { return this.map.stats(); }
...
function overlap(a: { minX: number; maxX: number; minY: number; maxY: number }, b: { minX: number; maxX: number; minY: number; maxY: number }): number {
  const ix = Math.max(0, Math.min(a.maxX, b.maxX) - Math.max(a.minX, b.minX));
  const iy = Math.max(0, Math.min(a.maxY, b.maxY) - Math.max(a.minY, b.minY));
  const areaA = (a.maxX - a.minX) * (a.maxY - a.minY); const areaB = (b.maxX - b.minX) * (b.maxY - b.minY);
