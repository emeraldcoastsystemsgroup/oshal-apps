...
exports.createEducationRosterRoutes = createEducationRosterRoutes;
const express_1 = require("express");
const logger_1 = require("@/shared/logger");
const education_access_1 = require("./education-access");
const logger = (0, logger_1.createChildLogger)({ module: 'education-roster-routes' });
...
async function assertLockedRosterAccess(client, actor, classId) {
    const lockedClass = (await client.query(`SELECT c.status
       FROM lm_classes c
      ...
      FOR UPDATE OF c, a`, [classId, actor.studentId])).rows[0];
    if (!lockedClass)
        throw new education_access_1.EducationAccessError('You do not teach this class', 403);
    if (lockedClass.status !== 'active') {
        throw new education_access_1.EducationAccessError('Students can only be added to an active class', 409);
    }
}
/** Find or provision one tenant-local placeholder after serializing its email key. */
async function findOrProvisionStudent(client, actor, email) {
    ...
    await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [`${actor.tenantId}:${email}`]);
    const existing = (await client.query('SELECT student_id, name FROM lm_students WHERE lower(email) = $1 AND tenant_id = $2', [email, actor.tenantId])).rows[0];
    if (existing)
        return { student: existing, created: false };
    const student = (await client.query(`INSERT INTO lm_students (name, email, role, tenant_id)
     VALUES ($1, $2, 'student', $3)
     RETURNING student_id, name`, [email.split('@')[0], email, actor.tenantId])).rows[0];
    return { student, created: true };
}
/** Append one database-timestamped audit fact inside the caller's open transaction. */
async function recordRosterAudit(client, actorStudentId, targetStudentId, classId, action) {
    await client.query(`INSERT INTO lm_authorization_audit
       (actor_student_id, student_id, class_id, action)
     VALUES ($1, $2, $3, $4)`, [actorStudentId, targetStudentId, classId, action]);
}
/** Commit one roster enrollment only after its tenant and ownership checks are locked. */
async function provisionEnrollment(pool, actor, classId, email) {
    ...
    try {
        ...
        if (provisioned.created) {
            await recordRosterAudit(client, actor.studentId, provisioned.student.student_id, classId, 'roster.student_provisioned');
        }
        const enrollment = await client.query(`INSERT INTO lm_enrollments (student_id, class_id, tenant_id)
       VALUES ($1, $2, $3)
       ON CONFLICT (student_id, class_id) DO NOTHING
       RETURNING student_id`, [provisioned.student.student_id, classId, actor.tenantId]);
        if (enrollment.rows.length > 0) {
            await recordRosterAudit(client, actor.studentId, provisioned.student.student_id, classId, 'roster.enrollment_created');
        }
...
async function listClassStudents(ctx, req, res) {
    try {
        ...
        const result = await ctx.pool.query(`SELECT s.student_id, s.name, s.email, e.enrolled_at
        ...
        WHERE e.class_id = $1
          AND (a.role = 'admin' OR (a.role = 'teacher' AND c.teacher_student_id = a.student_id))
        ORDER BY s.name`, [classId, actor.studentId]);
...
async function addClassStudent(ctx, req, res) {
    try {
        const classId = String(req.params.classId);
        const actor = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
        await (0, education_access_1.assertTeacherOfClass)(ctx.pool, actor, classId);
...
async function removeClassStudent(ctx, req, res) {
    try {
        const classId = String(req.params.classId);
        const actor = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
        await (0, education_access_1.assertTeacherOfClass)(ctx.pool, actor, classId);
        ...
        const result = await ctx.pool.query(`WITH authorized AS MATERIALIZED (
       ...
       SELECT EXISTS (SELECT 1 FROM authorized) AS authorized,
              ...
              EXISTS (SELECT 1 FROM audited) AS audited`, [classId, req.params.studentId, actor.studentId]);
...
function retireLegacyRosterWrite(_req, res) {
    res.status(410).json({
 ...
 */
function createEducationRosterRoutes(ctx) {
...
//# sourceMappingURL=education-roster-routes.js.map