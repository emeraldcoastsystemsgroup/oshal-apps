...
const logger = (0, logger_1.createChildLogger)({ module: 'education-schema' });
...
END $$`;
async function applyUpgrade(pool, name, statements) {
    try {
...
async function assertIdentityBindingReady(pool) {
    const result = await pool.query(`SELECT i.indisunique,
    ...
    if (result.rows.length !== 1 || !state.indisunique || !correctKeys
        ...
        throw new Error('education routes schema is not ready: migration 032 must install the issuer-bound ' +
...
async function assertSecurityInvariantsReady(pool) {
...
async function validateSchema(pool) {
    await (0, database_1.assertSchemaReady)(pool, 'education routes', SCHEMA_REQUIREMENTS);
...
async function bootstrapBaseSchema(pool) {
...
async function upgradeLectureAndCalendar(pool) {
...
async function upgradeIdentityAndPublishing(pool) {
...
async function upgradeMaterials(pool) {
...
async function upgradeTenancy(pool) {
...
async function upgradeOwnedStudyAndMaterials(pool) {
...
async function upgradeEnrollmentTenancy(pool) {
...
async function upgradeAuthoritativeProgress(pool) {
...
async function upgradeAuthorizationAudit(pool) {
...
async function ensureEducationSchema(pool) {
    ...
    if (!await bootstrapBaseSchema(pool))
        return;
