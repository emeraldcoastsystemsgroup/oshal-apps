...
function createEducationCatalogRoutes(ctx) {
    ...
    router.get('/catalog', async (req, res) => {
        try {
         ...
         ORDER BY c.subject, c.name`, [me.studentId, me.tenantId]);
    ...
    router.post('/classes/:classId/enroll', async (req, res) => {
        try {
            ...
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            const row = (await ctx.pool.query('SELECT published, status, teacher_student_id FROM lm_classes WHERE class_id = $1', [classId])).rows[0];
            if (!row) {
            ...
            await ctx.pool.query(`INSERT INTO lm_enrollments (student_id, class_id) VALUES ($1, $2) ON CONFLICT (student_id, class_id) DO NOTHING`, [me.studentId, classId]);
    ...
    router.post('/classes/:classId/leave', async (req, res) => {
        try {
            const classId = String(req.params.classId);
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            const row = (await ctx.pool.query('SELECT teacher_student_id FROM lm_classes WHERE class_id = $1', [classId])).rows[0];
            if (!row) {
                res.status(404).json({ error: 'class not found' });
                return;
            }
