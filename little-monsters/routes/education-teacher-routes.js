...
async function listTeacherClasses(ctx, req, res) {
    try {
       ...
       ORDER BY c.name`, [teacher.studentId, teacher.tenantId]);
...
async function loadClassAnalytics(ctx, classId, teacher) {
    const [classResult, rosterResult] = await Promise.all([
        ctx.pool.query(`SELECT class_id, name, subject, grade_level FROM lm_classes
        WHERE class_id = $1 AND tenant_id = $2
          AND ($3::boolean OR teacher_student_id = $4)`, [classId, teacher.tenantId, teacher.role === 'admin', teacher.studentId]),
        ctx.pool.query(`SELECT s.student_id, s.name, s.email, s.xp, s.level, s.streak_days, s.last_active_date,
       ...
       WHERE e.class_id = $1 AND s.tenant_id = $2 AND c.tenant_id = $2
         AND ($3::boolean OR c.teacher_student_id = $4) ORDER BY s.name`, [classId, teacher.tenantId, teacher.role === 'admin', teacher.studentId]),
    ]);
...
async function getClassAnalytics(ctx, req, res) {
    try {
