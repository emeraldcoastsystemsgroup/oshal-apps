...
const logger = (0, logger_1.createChildLogger)({ module: 'education-study-routes' });
...
function parseJsonArray(raw) {
    ...
    try {
        const v = JSON.parse(candidate.replace(/,(\s*[\]}])/g, '$1'));
        return Array.isArray(v) ? v : [];
    }
    catch { /* give up */ }
    return [];
...
function createEducationStudyRoutes(ctx) {
    ...
    router.get('/flashcards/sets/:setId/cards', async (req, res) => {
        try {
            ...
            if (own.rows[0].class_id)
                await (0, education_access_1.assertClassAccess)(ctx.pool, me, own.rows[0].class_id);
            const result = await ctx.pool.query(`SELECT * FROM lm_flashcards WHERE set_id = $1 ORDER BY difficulty, card_id`, [req.params.setId]);
    ...
    const classIdForSet = async (setId) => {
    ...
    const classIdForCard = async (cardId) => {
        const r = await ctx.pool.query('SELECT s.class_id FROM lm_flashcards c JOIN lm_flashcard_sets s ON c.set_id = s.set_id WHERE c.card_id = $1', [cardId]);
        return r.rows.length ? (r.rows[0].class_id ?? null) : undefined;
    };
    ...
    const cardField = (v) => String(v ?? '').slice(0, 1000);
    /** PATCH /api/education/flashcards/cards/:cardId — edit a card's front/back (enrollment-gated) */
    router.patch('/flashcards/cards/:cardId', async (req, res) => {
        try {
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            ...
            if (cls)
                await (0, education_access_1.assertClassAccess)(ctx.pool, me, cls);
    ...
    router.delete('/flashcards/cards/:cardId', async (req, res) => {
        try {
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            const cls = await classIdForCard(String(req.params.cardId));
            if (cls === undefined) {
    ...
    router.post('/flashcards/sets/:setId/cards', async (req, res) => {
        try {
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            ...
            await ctx.pool.query(`INSERT INTO lm_flashcards (set_id, front, back, card_type, difficulty) VALUES ($1, $2, $3, 'concept', 2)`, [req.params.setId, front, back]);
    ...
    router.delete('/flashcards/sets/:setId', async (req, res) => {
        try {
            const me = await (0, education_access_1.resolveAuthedStudent)(req, ctx.pool);
            const cls = await classIdForSet(String(req.params.setId));
            if (cls === undefined) {
    ...
    router.post('/flashcards/sets', async (req, res) => {
        try {
            ...
            try {
                ...
                await client.query(`INSERT INTO lm_flashcard_sets (set_id, class_id, title, topic, source_type, source_reference, card_count)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [setId, classId || null, title, topic || null, sourceType || 'manual', sourceReference || null, cards.length]);
                for (const card of cards) {
                    await client.query(`INSERT INTO lm_flashcards (set_id, front, back, card_type, difficulty, topic, hints)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`, [setId, card.front, card.back, card.type || 'concept', card.difficulty || 2, card.topic || topic || '', JSON.stringify(card.hints || [])]);
                }
    ...
    router.post('/flashcards/generate', async (req, res) => {
        try {
            ...
            const groups = await Promise.all(collections.map((c) => rag.search(broadQuery, c, 8).catch(() => [])));
            const chunks = groups.flat().map((h) => String(h.text || '')).filter(Boolean);
            if (chunks.length === 0) {
            ...
            try {
                await client.query('BEGIN');
                await client.query(`INSERT INTO lm_flashcard_sets (set_id, class_id, title, topic, source_type, source_reference, card_count)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [setId, classId, `Auto: ${className} key concepts`, subject || null, 'textbook', 'auto-generated from class materials', cards.length]);
                for (const card of cards) {
                    await client.query(`INSERT INTO lm_flashcards (set_id, front, back, card_type, difficulty, topic, hints)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`, [setId, String(card.front).slice(0, 500), String(card.back).slice(0, 1000), 'concept', Math.min(Math.max(parseInt(String(card.difficulty)) || 2, 1), 3), String(card.topic || subject || '').slice(0, 120), JSON.stringify([])]);
                }
                await client.query('COMMIT');
            }
            catch (txErr) {
    ...
    router.post('/quiz/generate', async (req, res) => {
        try {
            ...
            let questions = parseJsonArray(raw);
    ...
    router.post('/flashcards/review', async (req, res) => {
        try {
            ...
            // SM-2 algorithm
            const existing = await ctx.pool.query('SELECT * FROM lm_flashcard_progress WHERE student_id = $1 AND card_id = $2', [studentId, cardId]);
            ...
            await ctx.pool.query(`INSERT INTO lm_flashcard_progress (student_id, card_id, repetitions, ease_factor, interval_days, next_review, last_reviewed)
         VALUES ($1, $2, $3, $4, $5, $6, NOW())
         ON CONFLICT (student_id, card_id) DO UPDATE SET
           repetitions = $3, ease_factor = $4, interval_days = $5, next_review = $6, last_reviewed = NOW()`, [studentId, cardId, reps, ef, interval, nextReview.toISOString().split('T')[0]]);
