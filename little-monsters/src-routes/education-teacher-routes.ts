...
export function createEducationTeacherRoutes(ctx: AppContext): Router {
  ...
  router.get('/teacher/classes/:classId/analytics', async (req: Request, res: Response) => {
    try {
      ...
      const roster = await ctx.pool.query(
      ...
      const withQuiz = students.filter((s: any) => Number(s.quiz_count) > 0);
      const classQuizAverage = withQuiz.length
        ? Math.round(withQuiz.reduce((sum: number, s: any) => sum + Number(s.quiz_average), 0) / withQuiz.length)
        : null;
      ...
      res.json({
        ...
        summary: {
          ...
          classQuizAverage,
          studentsWithActivity: students.filter((s: any) => Number(s.quiz_count) > 0 || Number(s.cards_reviewed) > 0).length,
          totalCardsReviewed: students.reduce((sum: number, s: any) => sum + Number(s.cards_reviewed), 0),
        },
        students,
      });
    } catch (err: any) {
