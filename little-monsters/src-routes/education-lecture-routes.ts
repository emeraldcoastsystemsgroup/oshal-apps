...
import { renderAndSaveLectureDeck, type SavedLectureDeck } from './education-pptx';
import {
...
export function createEducationLectureRoutes(ctx: AppContext): Router {
  ...
  const recordingUpload = multer({
    ...
    fileFilter: (_req: any, file: any, cb: any) => {
      if (file.mimetype.startsWith('audio/') || file.mimetype === 'video/webm' || file.mimetype === 'application/octet-stream') cb(null, true);
  ...
  router.post('/process-lecture', upload.single('audio'), async (req: Request, res: Response) => {
    try {
      ...
      await ctx.pool.query(
        ...
        [lectureId, classId, date, audioPath]
      );
      ...
      const transcription = await runSynchronousTranscription({
      ...
      const ticketInput = CreateInternalTicketSchema.parse({
        ...
        description: buildLectureTicketDescription({
          className: cls.name,
          subject: cls.subject,
          date,
          audioPath,
      ...
      logger.info(
        { ticketId: ticket.ticketId, lectureId, classId, transcriptStatus: transcription.status },
        'Lecture processing ticket created',
      );
  ...
  router.post('/process-transcript', async (req: Request, res: Response) => {
    try {
      ...
      let ingest: { collection: string; chunkCount: number } | null = null;
      try {
      ...
      } catch (e: any) { logger.warn({ err: e, classId }, 'Lecture transcript ingest failed'); }
      ...
      let obj: any = {};
      try { let t = raw.trim(); const f = t.match(/```(?:json)?\s*([\s\S]*?)```/i); if (f) t = f[1].trim(); const m = t.match(/\{[\s\S]*\}/); obj = JSON.parse(m ? m[0] : t); } catch { obj = {}; }
      const notes = typeof obj.notes === 'string' ? obj.notes : '';
      const flashcards = Array.isArray(obj.flashcards) ? obj.flashcards.filter((c: any) => c && c.front && c.back).slice(0, 12) : [];
      ...
      const slides = Array.isArray(obj.slides)
        ? obj.slides
            ...
            .map((s: any) => ({
              ...
              bullets: Array.isArray(s.bullets) ? s.bullets.map((b: any) => String(b).slice(0, 200)).slice(0, 6) : [],
      ...
      if (notes) fs.writeFileSync(notesPath, notes);
      if (slides.length > 0) {
      ...
      if (flashcards.length > 0) {
        flashcardSetId = randomUUID();
        const client = await ctx.pool.connect();
        try {
          await client.query('BEGIN');
          await client.query(
            `INSERT INTO lm_flashcard_sets (set_id, class_id, title, topic, source_type, source_reference, card_count) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [flashcardSetId, classId, `Lecture: ${title || new Date(stamp).toISOString().slice(0, 10)}`, subject || null, 'lecture', 'auto-generated from lecture transcript', flashcards.length],
          );
          for (const c of flashcards) {
            await client.query(
              `INSERT INTO lm_flashcards (set_id, front, back, card_type, difficulty, topic, hints) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
              [flashcardSetId, String(c.front).slice(0, 500), String(c.back).slice(0, 1000), 'concept', 2, subject || '', JSON.stringify([])],
            );
          }
      ...
      try {
        const lr = await ctx.pool.query(
          `INSERT INTO lm_lectures (class_id, transcript_path, notes_path, slides_path, flashcard_set_id, status) VALUES ($1,$2,$3,$4,$5,'complete') RETURNING lecture_id`,
          [classId, transcriptPath, notes ? notesPath : null, slides.length > 0 ? slidesPath : null, flashcardSetId],
        );
        lectureId = lr.rows[0]?.lecture_id ?? null;
      } catch (e: any) { logger.warn({ err: e, classId }, 'Lecture row insert failed'); }
      ...
      let pptx: SavedLectureDeck | null = null;
      const deckTitle = title || `Lecture ${new Date(stamp).toLocaleDateString()}`;
      const sub = (req as any).oidc?.user?.sub;
      if (slides.length > 0 && sub) {
        try { pptx = await renderAndSaveLectureDeck(ctx, String(sub), deckTitle, slides); }
        catch (e: any) { logger.warn({ err: e?.message, classId, lectureId }, 'Lecture .pptx auto-emit failed (deck still available on demand)'); }
      }
  ...
  router.get('/lectures', async (req: Request, res: Response) => {
    try {
      ...
      await assertClassAccess(ctx.pool, student, String(qClassId)); // 403 unless enrolled/teacher
      const result = await ctx.pool.query(
        `SELECT * FROM lm_lectures WHERE class_id = $1 ORDER BY lecture_date DESC`,
        [qClassId]
      );
  ...
  router.get('/lectures/recent', async (req: Request, res: Response) => {
    try {
  ...
  router.get('/lectures/:lectureId/slides', async (req: Request, res: Response) => {
    try {
      const result = await ctx.pool.query(
        `SELECT l.slides_path, l.lecture_date, l.class_id, c.name AS class_name
  ...
  router.post('/lectures/:lectureId/pptx', async (req: Request, res: Response) => {
    const sub = (req as any).oidc?.user?.sub;
    if (!sub) { res.status(401).json({ error: 'not_authenticated' }); return; }
    try {
      const result = await ctx.pool.query(
        `SELECT l.slides_path, c.name AS class_name
         FROM lm_lectures l LEFT JOIN lm_classes c ON l.class_id = c.class_id
         WHERE l.lecture_id = $1`,
        [req.params.lectureId],
      );
      ...
      const saved = await renderAndSaveLectureDeck(ctx, String(sub), title, deck.slides);
  ...
  router.post('/lectures/:lectureId/audio', recordingUpload.single('audio'), async (req: Request, res: Response) => {
    try {
      const file = (req as Request & { file?: { mimetype: string; buffer: Buffer; size: number } }).file;
      if (!file) {
        res.status(400).json({ error: 'audio file is required' });
        return;
      }
      ...
      await ctx.pool.query(
        'UPDATE lm_lectures SET audio_path = $1, duration_seconds = COALESCE($2, duration_seconds) WHERE lecture_id = $3',
        [audioPath, Number.isFinite(durationSeconds) ? durationSeconds : null, req.params.lectureId],
      );
  ...
  router.get('/lectures/:lectureId/audio', async (req: Request, res: Response) => {
    try {
