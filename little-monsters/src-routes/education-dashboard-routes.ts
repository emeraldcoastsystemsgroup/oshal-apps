...
const logger = createChildLogger({ module: 'education-dashboard-routes' });

interface DashboardScope {
  params: string[];
  teacherClause: string;
}
...
function dashboardScope(viewer: AuthedStudent, studentId: string): DashboardScope {
...
async function loadDashboardStudent(
  pool: Pool,
  viewer: AuthedStudent,
  studentId: string,
): Promise<any | null> {
  const result = await pool.query(
    `SELECT target.student_id, target.name, target.email, target.xp,
...
async function loadDashboardClasses(pool: Pool, scope: DashboardScope): Promise<any[]> {
  const result = await pool.query(
    `SELECT c.class_id, c.name, c.subject, c.grade_level, c.teacher_name,
      ...
      WHERE e.student_id = $1 AND c.tenant_id = $2
        AND c.status = 'active'${scope.teacherClause}`,
    scope.params,
  );
  return result.rows;
}
...
async function loadUpcomingAssignments(pool: Pool, scope: DashboardScope): Promise<any[]> {
  const result = await pool.query(
...
async function loadQuizStats(pool: Pool, viewer: AuthedStudent, scope: DashboardScope): Promise<any> {
  if (viewer.role === 'teacher') {
    return (await pool.query(
      `SELECT COALESCE(AVG(q.score_percent), 0) as avg_score, COUNT(*) as quiz_count
         FROM lm_quiz_results q JOIN lm_classes c ON c.class_id = q.class_id
        WHERE q.student_id = $1 AND c.tenant_id = $2 AND c.teacher_student_id = $3`,
      scope.params,
    )).rows[0];
  }
  return (await pool.query(
    `SELECT COALESCE(AVG(q.score_percent), 0) as avg_score, COUNT(*) as quiz_count
      ...
      WHERE q.student_id = $1 AND (q.class_id IS NULL OR c.tenant_id = $2)`,
    scope.params,
  )).rows[0];
}

/** Count reviewed cards inside the viewer's permitted class boundary. */
async function loadFlashcardStats(pool: Pool, viewer: AuthedStudent, scope: DashboardScope): Promise<any> {
  ...
  return (await pool.query(
    `SELECT COUNT(*) as reviewed ${joins}
     ...
     WHERE fp.student_id = $1 AND fp.last_reviewed IS NOT NULL
       AND (fs.class_id IS NULL OR c.tenant_id = $2)`,
    scope.params,
  )).rows[0];
}
...
async function getStudentDashboard(
  ...
  try {
    const viewer = await resolveAuthedStudent(req, ctx.pool);
    const studentId = String(req.params.studentId);
    await assertCanViewStudent(ctx.pool, viewer, studentId);
 ...
 */
export function createEducationDashboardRoutes(
  ctx: AppContext,
  levelFromXP: (xp: number) => number,
): Router {
  const router = Router();
