...
/** Collectible catalog. Monsters are hue-shifted skins of the base pink monster
 *  (so no extra art needed); accessories are emoji worn on the avatar. */
export interface RewardItem {
  id: string;
  type: 'monster' | 'accessory';
  name: string;
  rarity: 'common' | 'uncommon' | 'rare' | 'legendary';
  hue?: number; sat?: number; rainbow?: boolean; // monster skin
  emoji?: string; pos?: 'top' | 'eyes' | 'side';  // accessory
}
...
const RARITY_WEIGHT: Record<string, number> = { common: 60, uncommon: 25, rare: 12, legendary: 3 };

/** Weighted-random pick from the catalog (rarity-weighted). Exported for tests. */
export function rollItem(): RewardItem {
  const total = REWARD_CATALOG.reduce((s, it) => s + (RARITY_WEIGHT[it.rarity] || 1), 0);
  // Deterministic-enough randomness; Math.random is fine here (no replay concern).
  let r = Math.random() * total;
  for (const it of REWARD_CATALOG) {
...
export function createEducationRewardsRoutes(ctx: AppContext): Router {
  ...
  router.get('/rewards', async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        boxes: st.boxes, inventory, equipped: st.equipped, catalog: REWARD_CATALOG,
  ...
  router.post('/rewards/open', async (req: Request, res: Response) => {
    try {
      const me = await resolveAuthedStudent(req, ctx.pool);
      ...
      const dec = await ctx.pool.query(
        `UPDATE lm_rewards SET boxes = boxes - 1, updated_at = NOW()
         WHERE student_id = $1 AND boxes > 0 RETURNING boxes, inventory`,
        [me.studentId],
      );
      ...
      const inventory: string[] = Array.isArray(dec.rows[0].inventory) ? dec.rows[0].inventory : [];
  ...
  router.post('/rewards/equip', async (req: Request, res: Response) => {
    try {
      const me = await resolveAuthedStudent(req, ctx.pool);
      ...
      if (itemId === 'none-accessory') {
        ...
        if (!owned) { res.status(403).json({ error: 'You have not unlocked that yet' }); return; }
        equipped[item.type] = itemId; // 'monster' or 'accessory'
      }
      await ctx.pool.query(
        `UPDATE lm_rewards SET equipped = $2::jsonb, updated_at = NOW() WHERE student_id = $1`,
        [me.studentId, JSON.stringify(equipped)],
      );
      res.json({ equipped });
    } catch (err: any) {
