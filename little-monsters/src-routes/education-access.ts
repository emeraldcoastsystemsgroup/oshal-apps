...
async function resolveTenantForEmail(pool: Pool, email: string | null): Promise<string> {
 ...
 */
function readIdentity(req: Request): { sub?: string; email?: string; name?: string } | null {
...
export async function resolveAuthedStudent(req: Request, pool: Pool): Promise<AuthedStudent> {
  ...
  if (!row) {
    ...
    const r = await pool.query(
      `INSERT INTO lm_students (name, email, external_id, role, tenant_id)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [id.name || 'Student', email, id.sub || null, role, tenantId],
    );
    row = r.rows[0];
...
export async function hasClassAccess(pool: Pool, student: AuthedStudent, classId: string): Promise<boolean> {
  ...
  if (student.role === 'teacher') {
    ...
    if (t.rowCount && t.rowCount > 0) return true;
...
export async function listAccessibleClassIds(pool: Pool, student: AuthedStudent): Promise<string[]> {
  ...
  const r = await pool.query(
    `SELECT class_id FROM lm_enrollments WHERE student_id = $1
     UNION
     SELECT class_id FROM lm_classes WHERE teacher_student_id = $1`,
    [student.studentId],
  );
  return r.rows.map((x) => x.class_id);
}
