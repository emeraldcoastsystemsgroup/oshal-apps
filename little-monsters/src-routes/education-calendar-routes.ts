...
import { getGoogleAccessToken, probeGoogleOAuth, resolveOAuthProfilePath } from '@/features/voice-providers';
import { resolveAuthedStudent, assertTeacher, assertTeacherOfClass, assertClassAccess, listAccessibleClassIds, EducationAccessError } from './education-access';
import { registerDynamicToolUI, deregisterDynamicToolUI } from '@/app/routes/tool-routes';

const logger = createChildLogger({ module: 'education-calendar' });
 ...
 */
function formatDueUrgency(dueDate: string | Date): string {
...
export function createEducationCalendarRoutes(ctx: AppContext): Router {
  ...
  router.get('/calendar', async (req: Request, res: Response) => {
    try {
      ...
      if (classId) {
        ...
        await assertClassAccess(ctx.pool, me, String(classId));
        params.push(classId);
        ...
        const accessible = await listAccessibleClassIds(ctx.pool, me);
  ...
  router.post('/calendar', async (req: Request, res: Response) => {
    try {
      ...
      if (classId) await assertClassAccess(ctx.pool, me, String(classId));
      ...
      const result = await ctx.pool.query(
        `INSERT INTO lm_calendar_events
         ...
         RETURNING event_id`,
        [classId || null, me.studentId, title, description || '', eventType || 'custom',
  ...
  router.post('/assignments-with-events', async (req: Request, res: Response) => {
    try {
      ...
      const me = await resolveAuthedStudent(req, ctx.pool);
      await assertTeacherOfClass(ctx.pool, me, String(classId));
        ...
        if (dueDate) {
          ...
          const eventResult = await client.query(
            `INSERT INTO lm_calendar_events
             ...
             RETURNING event_id`,
            [classId, eventTitle, `${description || ''}\n\n${urgency}`,
  ...
  router.get('/notifications', async (req: Request, res: Response) => {
    try {
      ...
      const result = await ctx.pool.query(
        `SELECT * FROM lm_notifications n WHERE n.student_id = $1 ${readFilter}
         ORDER BY n.sent_at DESC LIMIT 50`,
        [me.studentId]
      );

      res.json({ notifications: result.rows, unreadCount: result.rows.filter((n: any) => !n.read).length });
    } catch (err: any) {
      if (sendAccessError(res, err)) return;
  ...
  router.post('/notify', async (req: Request, res: Response) => {
    try {
      ...
      if (String(studentId) !== me.studentId) assertTeacher(me);
      ...
      const result = await ctx.pool.query(
        ...
        [studentId, title, body, notifChannel]
      );
  ...
  router.post('/check-reminders', async (req: Request, res: Response) => {
    try {
      ...
      const dueReminders = await ctx.pool.query(
      ...
      for (const event of dueReminders.rows) {
        ...
        const students = await ctx.pool.query(
          'SELECT student_id FROM lm_enrollments WHERE class_id = $1',
          [event.class_id]
        );
        ...
        for (const student of students.rows) {
          await ctx.pool.query(
            ...
            [student.student_id, reminderTitle, reminderBody,
  ...
  router.get('/classes/:classId/info', async (req: Request, res: Response) => {
    try {
      const me = await resolveAuthedStudent(req, ctx.pool);
      await assertClassAccess(ctx.pool, me, String(req.params.classId));
      ...
      const result = await ctx.pool.query(
        `SELECT c.*,
         ...
         FROM lm_classes c WHERE c.class_id = $1`,
        [req.params.classId]
      );
      ...
      const events = await ctx.pool.query(
        `SELECT * FROM lm_calendar_events WHERE class_id = $1 AND event_date >= CURRENT_DATE
      ...
      res.json({
        ...cls,
        upcomingEvents: events.rows.map((e: any) => ({ ...e, urgency: formatDueUrgency(e.event_date) })),
      });
    } catch (err: any) {
      if (sendAccessError(res, err)) return;
  ...
  router.patch('/classes/:classId', async (req: Request, res: Response) => {
    try {
      ...
      const editor = await resolveAuthedStudent(req, ctx.pool);
      await assertTeacherOfClass(ctx.pool, editor, classId); // 403 unless owner/admin
      ...
      if (nextPublished === true) assertTeacher(editor);
      ...
      await ctx.pool.query(
        `UPDATE lm_classes SET
          ...
          updated_at = NOW()
         WHERE class_id = $1`,
        [classId, name, subject, teacherName, gradeLevel, description, JSON.stringify(metadata), nextStatus, nextPublished]
      );
      ...
      if (nextStatus === 'archived') {
        ...
        if (r.rows[0]) registerDynamicToolUI(iconName, r.rows[0].name, 'codicon codicon-book', `/api/education/class?classId=${classId}`, `class-tutor-${classId.substring(0, 8)}`);
      }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  const calendar = new GoogleCalendarService(async () => (await getGoogleAccessToken()).accessToken);

  /** GET /api/education/calendar/google/status — is a Google account connected? */
  router.get('/calendar/google/status', async (_req: Request, res: Response) => {
    try {
      const probe = probeGoogleOAuth();
      let accountEmail: string | null = null;
      if (probe.ready) {
        try {
          const profile = JSON.parse(fs.readFileSync(resolveOAuthProfilePath(), 'utf8'));
  ...
  router.post('/calendar/google/push', async (req: Request, res: Response) => {
    try {
      // Pushing to the shared operator Google account is a staff action.
      const me = await resolveAuthedStudent(req, ctx.pool);
      const { classId, studentId, fromDate } = req.body || {};
      if (classId) await assertTeacherOfClass(ctx.pool, me, String(classId));
      else assertTeacher(me);
      const params: any[] = [];
      ...
      if (studentId) { params.push(studentId); sql += ` AND (e.student_id = $${params.length} OR e.student_id IS NULL)`; }
      ...
      for (const ev of rows) {
        try {
          ...
          await ctx.pool.query(
            'UPDATE lm_calendar_events SET google_event_id = $1, google_synced_at = NOW() WHERE event_id = $2',
            [created.id, ev.event_id],
          );
          pushed += 1;
        } catch (err: any) {
          failures.push({ eventId: ev.event_id, error: err.message });
          // A token/connection failure dooms the whole batch — stop early.
          if (/not connected|401|invalid_grant/i.test(err.message)) break;
        }
      }
  ...
  router.get('/calendar/google/pull', async (req: Request, res: Response) => {
    try {
