...
const logger = createChildLogger({ module: 'education-voice-routes' });
 ...
 */
function serveEducationFile(fileName: string): RequestHandler {
...
async function buildVoiceConfigPayload() {
  const config = loadSwarmVoiceConfig();
  const ttsRegistry = getTTSProviderRegistry();
  const sttRegistry = getSTTProviderRegistry();
  const ttsProviders = await Promise.all(
    ttsRegistry.list().map(async (p) => ({
      id: p.id,
 ...
 */
export function createEducationVoiceRoutes(ctx?: { appPackageDir?: string }): Router {
  ...
  router.get('/voice-config', async (_req: Request, res: Response) => {
    try {
      ...
      logger.info(
        { ttsDefault: payload.config.tts.default, sttDefault: payload.config.stt.default },
...
/**
 * @description Result of the synchronous transcription step consumed by
 * `/api/education/process-lecture`. `status` is the high-level outcome the
 * UI + bot ticket care about.
 */
export interface SynchronousTranscriptionResult {
  status: 'transcribed' | 'unconfigured' | 'failed';
  providerId: string;
 ...
 */
export async function runSynchronousTranscription(params: {
  audioBuffer: Buffer;
  mimeType: string;
  workspaceDir: string;
  date: string;
  ...
  lectureId: string;
  subject: string;
  pool: { query: (sql: string, args?: unknown[]) => Promise<unknown> };
}): Promise<SynchronousTranscriptionResult> {
  const provider = getSTTProviderRegistry().resolveForApp();
  ...
  try {
    ...
    const transcriptPath = path.join(params.workspaceDir, `TRANSCRIPT-${params.date}.md`);
    fs.writeFileSync(transcriptPath, formatTranscriptMarkdown(params, result.providerId, result.text), 'utf8');
    await params.pool.query(
      `UPDATE lm_lectures SET transcript_path = $1 WHERE lecture_id = $2`,
      [transcriptPath, params.lectureId],
    );
    logger.info(
      { providerId: result.providerId, chars: (result.text || '').length, transcriptPath },
      'Synchronous STT succeeded',
    );
    return {
      status: 'transcribed',
...
function formatTranscriptMarkdown(
  params: { date: string; subject: string; classId: string },
  providerId: string,
 ...
 */
function mapTranscriptionError(providerId: string, error: unknown): SynchronousTranscriptionResult {
  if (error instanceof GoogleAuthNotConfiguredError) {
    logger.warn(
      { providerId, reason: error.reason },
      'STT provider unconfigured — ticket will rely on bot-side fallback',
    );
    return { status: 'unconfigured', providerId, message: error.reason };
  }
  logger.error({ err: error, providerId }, 'Synchronous STT failed');
  return { status: 'failed', providerId, message: (error as Error).message };
}
 ...
 */
export function buildLectureTicketDescription(params: {
  className: string;
  subject: string;
  date: string;
  audioPath: string;
  ...
  if (params.transcription.status === 'transcribed' && params.transcription.transcriptPath) {
    lines.push(
      `**Transcript:** ${params.transcription.transcriptPath}`,
      ``,
      `### Instructions (transcript already available)`,
      `1. Read the transcript at the path above`,
      `2. Generate study notes using generate-study-notes tool`,
      `3. Extract assignments using extract-assignments tool`,
      `4. Generate flashcards using generate-flashcards tool`,
      `5. Index content into ChromaDB for class ${params.classId}`,
    );
  } else {
    lines.push(
      `**Transcription note:** ${params.transcription.message || 'unknown'}`,
      ``,
      `### Instructions (transcript unavailable — try BYOK fallback)`,
      `1. Transcribe audio using transcribe-lecture tool (requires OPENAI_API_KEY)`,
      `2. Generate study notes using generate-study-notes tool`,
      `3. Extract assignments using extract-assignments tool`,
      `4. Generate flashcards using generate-flashcards tool`,
      `5. Index content into ChromaDB for class ${params.classId}`,
