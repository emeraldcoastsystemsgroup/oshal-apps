...
export function createHomeSummaryRoutes(ctx: AppContext): Router {
 ...
 router.get('/', async (req, res) => {
  ...
  try {
   const actor = await resolveAuthedStudent(req, ctx.pool, {readOnly:true});
   const now = new Date();
   ...
   const result = await Promise.allSettled(queries.map((text,i)=>ctx.pool.query({text,values:i?[actor.studentId,actor.tenantId,actor.role,now]:[actor.studentId,actor.tenantId,actor.role],query_timeout:1800} as any)));
   const rows=(i:number):any[]=>result[i].status==='fulfilled'?(result[i] as PromiseFulfilledResult<any>).value.rows:[];
   ...
   const schoolIdentityMissing=error instanceof EducationAccessError && error.message==='Authenticated OIDC identity is missing issuer or subject';
   res.status(schoolIdentityMissing?403:error instanceof EducationAccessError?error.status:503).json({error:schoolIdentityMissing?'Open Little Monsters with your school identity to complete setup':error instanceof EducationAccessError?error.message:'School evidence unavailable'});
  }
 });
 return router;
}
