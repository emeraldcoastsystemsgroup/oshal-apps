...
/** The three artifact kinds and the purpose groups each one carries. */
export interface StarterKind {
  id: 'pptx' | 'docx' | 'xlsx';
  name: string;
  noun: string;
  /** What the file opens in — the honest promise on the card. */
  opens: string;
  groups: Array<{ id: string; label: string }>;
}

/** One purpose: the shape a person actually needs, as a real outline in the engine's micro-syntax. */
export interface Starter {
  ...
  icon: string;
  name: string;
  /** One line: what it is FOR — never a restatement of the name. */
  desc: string;
  title: string;
  /** A deck theme id the purpose reads best in; applied only when the caller has not chosen a look. */
  theme: string;
  /** The one line the AI-draft box asks for, in the purpose's own words (falls back to the kind's). */
  prompt?: string;
  outline: string;
}

export const STARTER_KINDS: StarterKind[] = [
...
export const OFFICE_STARTERS: Starter[] = [...DECKS, ...DOCUMENTS, ...SPREADSHEETS];
 ...
 */
export function starterCatalog(): { kinds: StarterKind[]; starters: Starter[] } {
  return { kinds: STARTER_KINDS, starters: OFFICE_STARTERS };
}
