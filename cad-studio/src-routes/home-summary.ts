...
export function createHomeSummaryRoutes(ctx: AppContext): Router {
  ...
  router.get('/', async (req, res) => {
    ...
    rows(1).forEach((r) => {
      ...
      const size = report.extentsMm?.size ? report.extentsMm.size.map((v: unknown) => Number(v).toFixed(1)).join(' × ') + ' mm' : 'not built yet';
    ...
    if (failed) items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: 'cad-studio' });
    else if (!items.length) items.push({ text: 'No parts yet. Open CAD Studio to begin.', tone: 'neutral', fix: 'cad-studio' });
