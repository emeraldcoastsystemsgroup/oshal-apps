...
const logger = createChildLogger({ module: 'aero-engine-build-hash' });

/** Top-level files the image bakes in — mirrors RUNTIME_FILES in aero_engine_bridge.py. */
export const ENGINE_RUNTIME_FILES = [
...
export const ENGINE_RUNTIME_TREES = ['aerosim', 'container'] as const;
...
function walkTree(engineDir: string, rel: string, out: string[]): void {
  const abs = path.join(engineDir, rel);
  if (!fs.existsSync(abs)) return;
  for (const entry of fs.readdirSync(abs, { withFileTypes: true })) {
    const childRel = `${rel}/${entry.name}`;
    if (entry.isDirectory()) {
      if (!SKIP_DIRS.has(entry.name)) walkTree(engineDir, childRel, out);
 ...
 */
export function engineBuildFiles(engineDir: string): string[] {
  const isFile = (p: string): boolean => fs.existsSync(p) && fs.statSync(p).isFile();
  const out: string[] = ENGINE_RUNTIME_FILES.filter((name) => isFile(path.join(engineDir, name)));
  for (const tree of ENGINE_RUNTIME_TREES) walkTree(engineDir, tree, out);
  return out.sort(byCodePoint);
}

/** Fold CRLF to LF so a Windows checkout and the deployed Linux copy of one commit agree. */
function foldCrlf(data: Buffer): Buffer {
  if (!data.includes(0x0d)) return data;
...
export function engineBuildHash(engineDir: string): string | null {
  try {
    ...
    logger.error({ err, stack: (err as Error).stack, engineDir }, 'engine tree unreadable — cannot compute its build hash');
    return null;
  }
}
