...
export function createHomeSummaryRoutes(ctx: AppContext): Router {
  ...
  router.get('/', async (req, res) => {
    ...
    const items: Array<{ metricId?: string; text: string; detail?: string; tone: string; fix: string; actions?: Array<{integration: string; context: {title: string; notes: string}}> }> = [];
    if (recent.status === 'fulfilled') for (const entry of recent.value.rows) {
    ...
    else items.push({text: 'Recent feed entries cannot be checked.', tone: 'warn', fix: 'feeds-dashboard'});
    if (index.status === 'rejected') items.push({ text: 'The saved Slack index cannot be checked.', tone: 'warn', fix: 'feeds-dashboard' });
    if (settings.status === 'rejected') items.push({ metricId: 'sync-age', text: 'The last feed sync cannot be checked.', tone: 'warn', fix: 'feeds-dashboard' });
    else items.push({ metricId: 'sync-age', text: validSync ? `Last recorded sync: ${new Date(sync).toISOString()}.` : 'No valid sync time is recorded. Open Feeds to connect or sync Slack.', tone: 'neutral', fix: 'feeds-dashboard' });
