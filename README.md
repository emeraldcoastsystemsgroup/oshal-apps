...
All **49 packages**, shelved by ADR-097 suite. Versions and status come from
...
| **Identity Hub** | [`identity/`](identity/) | 1.0.1 | ready | Every account you've connected, in one place - click to jump straight into Gmail, LinkedIn, your smart-home app, and more, reconnect a login that's expired, or connect a new one, plus an optional access review that flags what needs attention. |
| **Marketing Engine** | [`marketing-engine/`](marketing-engine/) | 0.1.0 | ready | Takes a built oshal product to traffic, users, and revenue: campaign board with stage-gated intake, per-channel consent (explicit opt-in, default OFF), UTM-tagged links, a deterministic metrics scorecard, experiment registry, budget proposals a human approves, and weekly review tickets worked by inline concierge bots only after human approval. |
...
| **AI Office** | [`presentations/`](presentations/) | 2.6.0 | ready | One outline, three artifacts — a themed PowerPoint deck, Word document, or live Excel workbook. |
...
| **Social** | [`social/`](social/) | 1.1.0 | ready | Draft, review, and publish across your networks from one surface - the comms bot drafts a post in your voice, you approve, and it publishes on your per-user LinkedIn / X / Facebook token (nothing posts until you click Publish). |
...
| **Video Studio** | [`video/`](video/) | 1.1.1 | ready | Make prompted short-form videos (TikTok / YouTube Shorts / Instagram Reels) from an idea - the director bot storyboards it and the studio renders a real .mp4 with voiceover and captions, saved to your Files storage; a series is written by the screenplay-writer, approval-gated on the script, then rendered one episode at a time on the remote Vids node. |
...
| **Little Monsters** | [`little-monsters/`](little-monsters/) | 1.1.0 | ready | Voice-first ADHD study companion for K-12: lecture capture, flashcards, authoritative quizzes, and a Socratic tutor grounded in caller-owned or teacher/admin-approved class material. |
