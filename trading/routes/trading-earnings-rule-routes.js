...
exports.watcherPosture = watcherPosture;
exports.registerTradingEarningsRuleRoutes = registerTradingEarningsRuleRoutes;
...
const trading_earnings_rules_1 = require("@/app/trading-earnings-rules");
const trading_manual_order_routes_1 = require("./trading-manual-order-routes");
const logger = (0, logger_1.createChildLogger)({ module: 'trading-earnings-rule-routes' });
/** Why arming needs the leg — said in the 503 so the operator knows nothing was stored. */
const SCHEDULER_UNAVAILABLE = 'The agent scheduler is not running (ENABLE_AGENT_SCHEDULER) — an earnings rule rides the trading-events leg, and without it the rule would never be looked at.';
/** The note an armed rule carries while the watcher flag is off. */
const WATCHER_OFF_NOTE = 'TRADING_EARNINGS_RULES is off on this server — the rule is stored and armed, but nothing will read EDGAR for it until it is enabled.';
/** The note an armed rule carries while the LEG itself is off (the watcher never gets a tick). */
const LEG_OFF_NOTE = 'TRADING_EVENT_PLANS is off on this server — the trading-events leg does not act, so the earnings watcher never runs.';
/**
 ...
 */
const VIEW_ONLY_WARNING = 'This account is view-only for the autopilot — a rule that decides to BUY will be refused, and the rule ends there. A rule that decides to SELL still runs.';
/**
...
function fail(res, err, what) {
    ...
    res.status(502).json({ error: err.message || 'earnings_rule_failed' });
}
/**
 ...
 */
function watcherPosture() {
    if (!(0, trading_event_plans_1.eventPlansEnabled)())
        return { enabled: false, note: LEG_OFF_NOTE };
    if (!(0, trading_earnings_rules_1.earningsRulesEnabled)())
        return { enabled: false, note: WATCHER_OFF_NOTE };
...
const ruleLimits = () => ({ maxDays: (0, trading_earnings_rules_1.ruleMaxDays)(), windowBeforeDays: (0, trading_earnings_rules_1.ruleWindowBeforeDays)(), windowAfterDays: (0, trading_earnings_rules_1.ruleWindowAfterDays)() });
/**
 ...
 */
function registerTradingEarningsRuleRoutes(router, ctx) {
    ...
    router.get('/events/rules', async (req, res) => {
        ...
        try {
            ...
            const rules = await (0, trading_earnings_rules_1.listEventRules)(ctx.pool, s, { bookId: book.bookId });
            res.json({ rules, book: book.ref, ...watcherPosture(), limits: ruleLimits(), basis: trading_earnings_rules_1.CLASSIFICATION_BASIS });
        }
        catch (err) {
            fail(res, err, 'list');
        }
    ...
    router.post('/events/rules', async (req, res) => {
        ...
        try {
            const book = await resolveRequestBook(ctx, s, req);
            const rule = (0, trading_earnings_rules_1.normalizeEventRule)(b.rule ?? req.body);
            ...
            await (0, trading_manual_order_routes_1.ensureEventSchedule)(s, SCHEDULER_UNAVAILABLE);
            const stored = await (0, trading_earnings_rules_1.createEventRule)(ctx.pool, s, { book, rule });
            ...
            res.status(201).json({ rule: stored, scheduled: true, ...posture, basis: trading_earnings_rules_1.CLASSIFICATION_BASIS, warning: book.enabled ? null : VIEW_ONLY_WARNING });
        }
        catch (err) {
            fail(res, err, 'arm');
        }
    ...
    router.post('/events/rules/:id/cancel', async (req, res) => {
        const s = sub(req, res);
        if (!s)
            return;
        try {
            const rule = await (0, trading_earnings_rules_1.cancelEventRule)(ctx.pool, s, String(req.params.id));
...
//# sourceMappingURL=trading-earnings-rule-routes.js.map