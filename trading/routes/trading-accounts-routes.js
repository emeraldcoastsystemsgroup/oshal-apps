...
function registerTradingAccountRoutes(router, ctx) {
        ...
        try {
            ...
            res.json({
                ...
                books: books.map((b) => ({
                    ...
                    armAckRequired: (0, trading_books_store_1.requiresArmAcknowledgement)(b), armAckAt: b.armAckAt ?? null, armAckBy: b.armAckBy ?? null,
                })),
            });
        }
        catch (err) {
    ...
    router.post('/accounts/books/:bookId/arm-ack', async (req, res) => {
        const s = sub(req, res);
        if (!s)
            return;
        const b = (req.body || {});
        ...
        try {
            const book = await (0, trading_books_store_1.recordArmAck)(ctx.pool, s, String(req.params.bookId), acknowledge, s, b.note ? String(b.note) : null);
            if (!book) {
                res.status(404).json({ error: 'unknown_book' });
                return;
            }
