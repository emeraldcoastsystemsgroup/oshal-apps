...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        const metrics = ['live', 'paper'].flatMap(mode => ['day', 'five'].map(period => ({
            ...
            value: orders.status === 'fulfilled' ? String(orders.value.rows.find((r) => r.mode === mode)?.[period] ?? '0') : 'Unavailable',
        })));
        const items = books.status === 'fulfilled' ? books.value.rows.map((b) => {
        ...
        if (failed)
            items.push({ text: 'Some saved trading sources cannot be checked.', tone: 'warn', fix: 'intelligent-trades-home' });
        else if (!items.length)
            items.push({ text: 'No saved trading books yet.', tone: 'neutral', fix: 'intelligent-trades-home' });
