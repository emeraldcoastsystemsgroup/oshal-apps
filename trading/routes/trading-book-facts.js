...
exports.TRADING_FACT_KEYS = exports.TRADING_BOOK_SLOTS = exports.EQUITY_WINDOW_DAYS = exports.TRADING_FACTS_BUDGET_MS = void 0;
exports.easternDay = easternDay;
exports.assignBookSlots = assignBookSlots;
exports.readTradingBookFacts = readTradingBookFacts;
...
const logger = (0, logger_1.createChildLogger)({ module: 'trading-book-facts' });
/**
 ...
 */
exports.TRADING_BOOK_SLOTS = ['paper', 'live', 'live2', 'live3', 'live4'];
/** The live slots, in fill order - a paper book may never occupy one of these. */
const LIVE_SLOTS = ['live', 'live2', 'live3', 'live4'];
/** The single paper slot. */
const PAPER_SLOTS = ['paper'];
...
const SHAPE_KEYS = ['books.live', 'books.paper', 'books.unreported'];
/**
 ...
 */
exports.TRADING_FACT_KEYS = Object.freeze([
    ...exports.TRADING_BOOK_SLOTS.flatMap((slot) => SUFFIXES.map((suffix) => `${slot}.${suffix}`)),
    ...SHAPE_KEYS,
    'facts.complete',
]);
/**
 ...
 */
function assignBookSlots(books) {
    const ordered = [...books].sort((left, right) => Number(LEGACY_REFS.has(right.ref)) - Number(LEGACY_REFS.has(left.ref)));
    const free = {
 ...
 */
async function readBook(ctx, sub, book, today) {
    const series = await (0, trading_daily_equity_store_1.loadDailyEquitySeries)(ctx.pool, sub, book, exports.EQUITY_WINDOW_DAYS);
    ...
    const realized = await (0, trading_realized_1.realizedReport)(ctx, sub, book.bookId);
    return {
        present: true,
 ...
 */
async function fillSlots(ctx, sub, assignment, out, spent, now) {
    let complete = true;
    ...
    for (const [slot, book] of assignment) {
        ...
        try {
            ...
            logger.error({ err: error, slot, bookId: book.bookId, kind: book.kind }, 'Trading specialist facts: one book could not be read - reported as present but unknown');
            emit(slot, { ...emptyBook(), present: true }, out);
            complete = false;
        }
    }
 ...
 */
async function readTradingBookFacts(ctx, input, options = {}) {
    const now = options.now ?? Date.now;
    const budgetMs = options.budgetMs ?? exports.TRADING_FACTS_BUDGET_MS;
    ...
    const out = {};
    for (const slot of exports.TRADING_BOOK_SLOTS)
        emit(slot, emptyBook(), out);
    for (const key of SHAPE_KEYS)
        out[key] = 0;
    out['facts.complete'] = false;
    const spent = () => input.signal?.aborted === true || now() >= expiresAt;
    const work = async () => {
        const books = await (0, trading_books_store_1.listBooks)(ctx.pool, input.sub);
...
//# sourceMappingURL=trading-book-facts.js.map