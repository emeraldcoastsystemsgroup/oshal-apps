...
const logger = createChildLogger({ module: 'trading-charts-routes' });

/** Alpaca bar timeframes the chart offers, mapped to a sensible lookback window (calendar days). */
const TIMEFRAMES: Record<string, { tf: string; days: number; limit: number }> = {
...
function alpacaKeys(): { id: string; secret: string } {
  const first = (...names: string[]): string => { for (const n of names) { const v = process.env[n]; if (v) return v; } return ''; };
  return {
 ...
 */
export function createTradingChartsRoutes(ctx: AppContext): Router {
  const router = Router();
  ...
  router.get('/vendor/lightweight-charts.js', (_req: Request, res: Response) => {
    const candidates = [
      appPackageDir ? path.join(appPackageDir, 'tools', 'vendor', 'lightweight-charts.js') : '',
      LOAD_TIME_PACKAGE_DIR ? path.join(LOAD_TIME_PACKAGE_DIR, 'tools', 'vendor', 'lightweight-charts.js') : '',
      path.resolve(__dirname, '../tools/vendor/lightweight-charts.js'),
    ].filter(Boolean) as string[];
    const file = candidates.find((p) => existsSync(p));
    if (!file) { res.status(404).type('text/plain').send('// chart lib not vendored'); return; }
  ...
  router.get('/bars', async (req: Request, res: Response) => {
    ...
    if (!id || !secret) { res.status(503).json({ error: 'broker_not_configured', message: 'Set the Alpaca paper keys to load charts.' }); return; }
    ...
    try {
      ...
      if (!r.ok) { res.status(502).json({ error: `alpaca_${r.status}`, message: await r.text().catch(() => '') }); return; }
      const j = (await r.json()) as { bars?: Array<{ t: string; o: number; h: number; l: number; c: number; v: number }> };
      ...
      res.status(aborted ? 504 : 502).json({
        error: aborted ? 'data_feed_timeout' : ((err as Error).message || 'chart_fetch_failed'),
