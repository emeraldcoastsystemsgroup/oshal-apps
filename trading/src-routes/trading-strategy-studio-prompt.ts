...
/* ─── Event playbooks (ADR-136 D6) ─────────────────────────────────────────────── */

/** The event plan being refined — name + the stored plan params fed back into the prompt. */
export interface EventPlanCurrent {
  name: string;
  plan: unknown;
}
 ...
 */
export function isEventIntent(message: string, current?: { kind?: string }): boolean {
  if (current?.kind === 'event') return true;
 ...
 * @returns The prompt text.
 */
export function eventPlanPrompt(message: string, findings: ResearchFinding[], accountLabel: string, equity: number | null, current?: EventPlanCurrent): string {
  const refs = findings.map((f) => `- [${f.id}] ${f.name} — ${f.authors} (${f.year}), ${f.journal}: ${f.finding} Maps: ${f.maps.note}`).join('\n');
 ...
 */
export function parseEventPlanReply(raw: string, findings: ResearchFinding[]): {
  name: string; description: string; hypothesis: string; plan: unknown; citations: ResearchFinding[]; narration: string; manualSteps: string[];
} {
  const fenced = raw.match(/```(?:json)?\s*([\s\S]*?)```/i);
  const jsonText = (fenced ? fenced[1] : raw.match(/\{[\s\S]*\}/)?.[0] ?? '').trim();
  const parsed = JSON.parse(jsonText) as {
    name?: string; description?: string; hypothesis?: string; plan?: unknown; citations?: unknown; narration?: string; manualSteps?: unknown;
  };
  const allowed = new Set(findings.map((f) => f.id));
  const citeIds = Array.isArray(parsed.citations) ? parsed.citations.map((x) => String(x)).filter((id) => allowed.has(id)) : [];
  const cited = citeIds.map((id) => findingById(id)).filter((f): f is ResearchFinding => !!f);
