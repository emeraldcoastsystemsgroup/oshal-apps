...
function expand(steps, t, cur, ctx, field, touched) {
    let now = t;
    steps.forEach((step, i) => {
        ...
        if (step.kind === 'move') {
            for (const [axis, to] of Object.entries(targetsOf(step, ctx.lib))) {
                ...
                if (!ch)
                    throw new rig_contract_1.ContractError(`${here} moves unknown axis ${axis}`, here);
                if (to < ch.minDeg || to > ch.maxDeg)
                    throw new rig_contract_1.ContractError(`${here}.${axis} = ${to}° is outside ${ch.id}'s limits`, here);
                touched?.add(axis);
            ...
            for (let n = 0; n < step.times; n += 1)
                now = expand(step.steps, now, cur, ctx, `${here}.steps`, touched);
        }
        else {
            now = together(step.steps, now, cur, ctx, here, touched);
        }
        if (now > rig_contract_1.LIMITS.durationMs)
            throw new rig_contract_1.ContractError(`${here} takes the scenario past ${rig_contract_1.LIMITS.durationMs} ms`, here);
    });
    return now;
}
/** Children start together; the same axis in two of them is a conflict. */
function together(children, t, cur, ctx, field, outer) {
    let end = t;
    const seen = new Map();
    const merged = { ...cur };
    children.forEach((child, i) => {
        ...
        for (const axis of mine) {
            if (seen.has(axis))
                throw new rig_contract_1.ContractError(`${field}.steps[${i}] and steps[${seen.get(axis)}] both move ${axis} at the same time`, `${field}.steps[${i}]`);
            seen.set(axis, i);
...
function neutralPose(rig) {
    const out = {};
    for (const [axis, ch] of (0, rig_contract_1.axisMap)(rig))
        out[axis] = ch.neutralDeg;
    return out;
}
function sample(segs, t, start) {
    let value = start;
    for (const s of segs) {
        if (s.t0 > t)
            break;
        value = t >= s.t1 ? s.to : s.from + (s.to - s.from) * (0, scenario_1.ease)(s.ease, (t - s.t0) / (s.t1 - s.t0));
    }
    return value;
}
/**
 ...
 */
function compileSteps(rig, lib, steps, start = {}) {
    const axes = (0, rig_contract_1.axisMap)(rig);
    const cur = { ...neutralPose(rig) };
    for (const [axis, deg] of Object.entries(start)) {
        const ch = axes.get(axis);
        if (!ch)
            throw new rig_contract_1.ContractError(`start.${axis} is not an axis of this rig`, `start.${axis}`);
        if (!Number.isFinite(deg) || deg < ch.minDeg || deg > ch.maxDeg)
            throw new rig_contract_1.ContractError(`start.${axis} is outside ${ch.id}'s limits`, `start.${axis}`);
        cur[axis] = deg;
    }
    const initial = { ...cur };
    const ctx = { rig, lib, axes, segments: [] };
    const durationMs = expand(steps, 0, cur, ctx, 'steps');
    const frameCount = Math.floor(durationMs / rig_contract_1.FRAME_MS) + 1;
    if (frameCount > rig_contract_1.LIMITS.frames)
        throw new rig_contract_1.ContractError(`the stream would be ${frameCount} frames; the cap is ${rig_contract_1.LIMITS.frames}`, 'steps');
    const channelIds = rig.channels.map((c) => c.id);
    const axisIndex = {};
    const byChannel = new Map();
    for (const [axis, ch] of axes) {
        axisIndex[axis] = channelIds.indexOf(ch.id);
        byChannel.set(ch.id, { axis, segs: [] });
    }
    for (const s of ctx.segments)
        byChannel.get(axes.get(s.axis).id).segs.push(s);
    for (const entry of byChannel.values())
        entry.segs.sort((a, b) => a.t0 - b.t0 || a.t1 - b.t1);
    const angles = [];
    const pulses = [];
    const demand = new Array(channelIds.length).fill(0);
    let previous = null;
    for (let f = 0; f < frameCount; f += 1) {
        const t = f * rig_contract_1.FRAME_MS;
        const row = rig.channels.map((ch) => { const e = byChannel.get(ch.id); return e ? sample(e.segs, t, initial[e.axis]) : ch.neutralDeg; });
        if (previous)
            row.forEach((deg, i) => { demand[i] = Math.max(demand[i], Math.abs(deg - previous[i]) * (1000 / rig_contract_1.FRAME_MS)); });
        angles.push(row.map((d) => Math.round(d * 100) / 100));
        pulses.push(row.map((deg, i) => (0, rig_contract_1.pulseFor)(rig.channels[i], deg)));
        previous = row;
    }
    return { frameMs: rig_contract_1.FRAME_MS, durationMs, channelIds, outputs: rig.channels.map((c) => c.channel), angles, pulses, axisIndex, end: cur, demandDegPerS: demand.map((d) => Math.round(d)) };
}
