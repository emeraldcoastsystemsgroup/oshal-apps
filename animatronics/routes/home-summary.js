...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        if (failed)
            items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: 'animatronics' });
        else if (!items.length)
            items.push({ text: 'No rigs yet. Open Animatronics to begin.', tone: 'neutral', fix: 'animatronics' });
