...
export const PROTOCOL_NAME = 'oshal-animatronics';
...
/** @description Messages in either direction. */
export type Message =
  | { kind: 'hello' }
  | { kind: 'limits'; channel: number; minUs: number; maxUs: number }
  | { kind: 'frame'; seq: number; pairs: Array<[number, number]> }
  | { kind: 'estop' }
  | { kind: 'release' }
  | { kind: 'ok'; ref: string }
  | { kind: 'err'; ref: string; reason: string }
  | { kind: 'hello-reply'; name: string; version: number; board: string; channels: number }
  | { kind: 'estopped' }
...
export function checksum(body: string): string {
  let x = 0;
  for (let i = 0; i < body.length; i += 1) x ^= body.charCodeAt(i) & 0xff;
  return x.toString(16).toUpperCase().padStart(2, '0');
}

/** @description Terminate a body with its checksum and newline. */
export function withChecksum(body: string): string { return `${body}*${checksum(body)}\n`; }

export const encodeHello = (): string => withChecksum('H');
export const encodeEstop = (): string => withChecksum('E');
export const encodeRelease = (): string => withChecksum('R');
/** @description Controller-side hard clamps for one output. */
export function encodeLimits(channel: number, minUs: number, maxUs: number): string { return withChecksum(`L ${channel} ${minUs} ${maxUs}`); }
/** @description One frame: sequence number and the outputs it sets (may be empty = heartbeat). */
export function encodeFrame(seq: number, pairs: Array<[number, number]>): string {
  return withChecksum(pairs.length ? `F ${seq} ${pairs.map(([c, us]) => `${c}=${us}`).join(',')}` : `F ${seq}`);
}
/** @description Controller replies. */
export const encodeOk = (ref: string): string => withChecksum(`OK ${ref}`);
export const encodeErr = (ref: string, reason: string): string => withChecksum(`ERR ${ref} ${reason.replace(/[*\r\n]/g, ' ')}`);
export function encodeHelloReply(board: string, channels: number): string { return withChecksum(`HELLO ${PROTOCOL_NAME}/${PROTOCOL_VERSION} board=${board} channels=${channels}`); }
export const encodeEstopped = (): string => withChecksum('ESTOP');

const INT = /^\d{1,6}$/;
const int = (s: string | undefined): number | null => (s !== undefined && INT.test(s) ? Number(s) : null);
 ...
 */
export function parseLine(raw: string): Message {
  const line = raw.replace(/\r?\n$/, '');
  const star = line.lastIndexOf('*');
  if (star < 0 || line.length - star !== 3) return { kind: 'invalid', reason: 'missing checksum', line };
  const body = line.slice(0, star);
  if (checksum(body) !== line.slice(star + 1).toUpperCase()) return { kind: 'invalid', reason: 'bad checksum', line };
  ...
  switch (parts[0]) {
    ...
    case 'L': { const c = int(parts[1]); const lo = int(parts[2]); const hi = int(parts[3]); return c !== null && lo !== null && hi !== null && lo < hi && parts.length === 4 ? { kind: 'limits', channel: c, minUs: lo, maxUs: hi } : { kind: 'invalid', reason: 'limits need channel min max', line }; }
...
function parseFrame(parts: string[], line: string): Message {
  const seq = int(parts[1]);
  if (seq === null || parts.length > 3) return { kind: 'invalid', reason: 'frame needs a sequence number', line };
  const pairs: Array<[number, number]> = [];
  if (parts.length === 3) {
    for (const item of parts[2].split(',')) {
      const m = /^(\d{1,3})=(\d{3,4})$/.exec(item);
      if (!m) return { kind: 'invalid', reason: `bad output ${item}`, line };
...
function parseHello(parts: string[], line: string): Message {
  const m = /^([a-z-]+)\/(\d+)$/.exec(parts[1] ?? '');
  const board = /^board=([a-z0-9-]+)$/.exec(parts[2] ?? '');
  const channels = /^channels=(\d{1,3})$/.exec(parts[3] ?? '');
  if (!m || !board || !channels) return { kind: 'invalid', reason: 'hello reply needs name/version board= channels=', line };
 ...
 */
export function frameLines(outputs: number[], pulses: number[][], startSeq = 1): string[] {
  const lines: string[] = [];
  let previous: number[] | null = null;
  pulses.forEach((row, f) => {
    const full = previous === null || f % KEYFRAME_EVERY === 0;
    const pairs: Array<[number, number]> = [];
    row.forEach((us, i) => { if (full || us !== (previous as number[])[i]) pairs.push([outputs[i], us]); });
...
export function describeProtocol(): Record<string, unknown> {
  return {
    name: PROTOCOL_NAME, version: PROTOCOL_VERSION, framing: 'ASCII line, `*XX` XOR checksum of the body, LF', baud: 115200, keyframeEvery: KEYFRAME_EVERY, watchdogMs: WATCHDOG_MS,
    ...
    example: { frame: encodeFrame(7, [[0, 1500], [1, 1720]]).trim(), hello: encodeHello().trim() },
  };
}
