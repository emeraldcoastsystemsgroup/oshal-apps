...
exports.parseEngineJson = parseEngineJson;
exports.registerCareerTargetRoutes = registerCareerTargetRoutes;
const logger_1 = require("@/shared/logger");
const career_engine_dispatch_1 = require("./career-engine-dispatch");
const career_user_store_1 = require("./career-user-store");
const logger = (0, logger_1.createChildLogger)({ module: 'career-targets' });
...
async function classifyUrl(ctx, userSub, url) {
    const result = await (0, career_engine_dispatch_1.runCareerCliAwait)(ctx.pool, userSub, ['classify', '--url', url], {}, { slot: 'classify' });
...
async function resolveTarget(ctx, userSub, id, url) {
    const startedAt = Date.now();
    try {
        const result = await (0, career_engine_dispatch_1.runCareerCliAwait)(ctx.pool, userSub, ['add-target', '--url', url, '--source', `user:${userSub}`], {}, { slot: 'add-target' });
        const json = result.ok ? parseEngineJson(result.out) : null;
        if (json && json.ok === true) {
            await ctx.pool.query(`UPDATE career_user_targets
            SET status='resolved', company_id=$3, company_name=$4, postings=$5, reason=$6, updated_at=NOW()
          WHERE id=$1 AND user_sub=$2`, [id, userSub, Number(json.company_id) || null, typeof json.name === 'string' ? json.name : null,
        ...
        await ctx.pool.query(`UPDATE career_user_targets SET status='unresolved', reason=$3, updated_at=NOW() WHERE id=$1 AND user_sub=$2`, [id, userSub, reason.slice(0, 400)]);
...
async function listTargets(ctx, req, res) {
    ...
    try {
        const result = await ctx.pool.query(`SELECT id, url, ats_type, ats_token, status, company_id, company_name, postings, reason, created_at, updated_at
         FROM career_user_targets WHERE user_sub=$1 ORDER BY created_at DESC`, [userSub]);
        res.json({ targets: result.rows, max: MAX_TARGETS_PER_USER, adminTable: ADMIN_TABLE_PATH });
    }
    catch (err) {
...
async function countTargets(ctx, userSub) {
    const result = await ctx.pool.query('SELECT COUNT(*) AS n FROM career_user_targets WHERE user_sub=$1', [userSub]);
    return Number(result.rows[0]?.n ?? 0);
}
async function acceptTarget(ctx, userSub, url, match) {
    const result = await ctx.pool.query(`INSERT INTO career_user_targets (user_sub, url, ats_type, ats_token, status)
       ...
       RETURNING id, url, ats_type, ats_token, status, company_id, company_name, postings, reason, created_at, updated_at`, [userSub, url, match.atsType, match.token || null]);
    return result.rows[0];
}
async function addTarget(ctx, req, res) {
    const userSub = (0, career_user_store_1.callerSub)(req);
    if (!userSub) {
        res.status(401).json({ error: 'unauthorized' });
        return;
    ...
    if (!url) {
        res.status(400).json({ ok: false, rejected: true, reason: 'invalid_url' });
        return;
    }
    try {
...
async function removeTarget(ctx, req, res) {
    const userSub = (0, career_user_store_1.callerSub)(req);
    if (!userSub) {
        res.status(401).json({ error: 'unauthorized' });
        return;
 ...
 */
function registerCareerTargetRoutes(router, ctx) {
...
//# sourceMappingURL=career-targets.js.map