...
const logger = (0, logger_1.createChildLogger)({ module: 'career-hunter-routes' });
const TOOL_DIR = path_1.default.resolve(__dirname, '..', 'tools'); // ADR-085: surfaces ship in this package's tools/ (compiled file lives in <pkg>/routes/)
/**
...
function resolveEngineCli() {
    ...
    return [packaged, legacy].find((p) => fs_1.default.existsSync(p)) || packaged;
}
const CLI = resolveEngineCli();
const STORE_ROOT = process.env.JOBHUNTER_STORE_ROOT || path_1.default.resolve(process.cwd(), 'apps', 'career-hunter', 'data');
...
const caller_sub_2 = require("@/app/routes/caller-sub");
function userPaths(userSub) {
    const tenantDir = path_1.default.join(STORE_ROOT, TENANT);
    const userDir = path_1.default.join(tenantDir, userSub);
    return {
        userDir,
        corpusDb: path_1.default.join(tenantDir, 'corpus.db'),
        userDb: path_1.default.join(userDir, `user-${userSub}.db`),
    };
}
...
const requireCareerAdmin = (req, res, next) => {
    if (!isCareerAdmin((0, caller_sub_2.callerSub)(req))) {
...
function serveFile(fileName) {
    return (_req, res) => {
        ...
        res.sendFile(path_1.default.join(TOOL_DIR, fileName), (err) => {
            if (err) {
                logger.error({ err, fileName }, 'serve failed');
                res.status(404).send('Not found');
            }
        });
    };
}
function cliEnv(userSub, extra = {}) {
...
function runCli(userSub, args, extraEnv = {}) {
    const r = (0, child_process_1.spawnSync)('node', [CLI, ...args], { env: cliEnv(userSub, extraEnv), encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
...
function runCliAsync(userSub, args, extraEnv = {}) {
    const proc = (0, child_process_1.spawn)('node', [CLI, ...args], { env: cliEnv(userSub, extraEnv), stdio: 'ignore', detached: false });
...
function runCliAwait(userSub, args, extraEnv = {}) {
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)('node', [CLI, ...args], { env: cliEnv(userSub, extraEnv), stdio: ['ignore', 'pipe', 'pipe'] });
        let out = '';
        let err = '';
        proc.stdout?.on('data', (d) => { out += String(d); if (out.length > 400000)
            out = out.slice(-200000); });
        proc.stderr?.on('data', (d) => { err += String(d); if (err.length > 8000)
            err = err.slice(-4000); });
        proc.on('exit', (code) => resolve({ ok: code === 0, out: out.slice(-200000), err: err.slice(-4000) }));
        proc.on('error', (e) => resolve({ ok: false, out, err: String(e?.message || e) }));
    });
}
 ...
 */
function tryAcquireRun(userSub, verb) {
    const key = `${userSub}:${verb}`;
    if (runsInFlight.has(key))
        return 'inflight';
    if (runsInFlight.size >= MAX_CONCURRENT_RUNS)
        return 'busy';
...
async function enqueueForUser(ctx, userSub, limit = 10, opts = {}) {
    if (opts.trigger !== 'manual') {
        const auto = await (0, career_automation_1.readAutomationSettingsSystem)(ctx, userSub);
    ...
    let rows;
    try {
        rows = db.prepare(`SELECT pc.id, pc.title, co.name AS company, us.ai_fit_score AS fit, pc.salary_max, pc.url
    ...
    let created = 0;
    for (const r of rows) {
        const exists = (await pool.query(`SELECT 1 FROM career_hunter_applications WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`, [TENANT, userSub, r.id])).rowCount;
        if (exists)
            continue;
        ...
        await pool.query(`INSERT INTO career_hunter_applications (tenant_id, user_sub, ticket_id, posting_id, company, title, status)
       VALUES ($1,$2,$3,$4,$5,$6,'approval_required')
       ON CONFLICT (tenant_id, user_sub, posting_id) DO NOTHING`, [TENANT, userSub, ticket.ticketId, r.id, r.company, r.title]);
        created++;
    }
    return created;
}
...
function listStoreUsers() {
    const tenantDir = path_1.default.join(STORE_ROOT, TENANT);
    try {
        return fs_1.default.readdirSync(tenantDir, { withFileTypes: true })
...
function runSharedPull(anyUserSub) {
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)('node', [CLI, 'pull'], {
...
function runUserMatch(userSub) {
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)('node', [CLI, 'match'], {
...
function runUserScore(userSub, opts = {}) {
    ...
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)('node', [CLI, ...args], {
            env: { ...process.env, OSHAL_USER_SUB: userSub, OSHAL_TENANT: TENANT, JOBHUNTER_STORE_ROOT: STORE_ROOT },
            stdio: 'ignore',
        });
        proc.on('exit', (code) => resolve({ ok: code === 0 }));
...
/**
 * The board feed's filter composition and query plan live in ./career-board-feed. Re-exported here
 * because that is where the dismissed-exclusion rule has always been imported from, and the rule
 * itself is load-bearing: dismissed jobs are excluded unless the caller asks for a specific status.
 * The board's Dismiss button persists `user_signals.status='dismissed'` and animates the card away
 * WITHOUT reloading, so a feed that still returned dismissed rows looked correct until the next
 * refresh — then every dismissed job reappeared, and rendered with no Dismiss button (the surface
 * hides it at that status), leaving un-clearable cards. The "Dismissed" pipeline tab sends
 * `status=dismissed`, which takes the explicit branch and still lists them.
 */
function buildJobFilters(query) {
    return (0, career_board_feed_1.buildJobFilters)(query);
}
function createCareerHunterRoutes(ctx) {
    ...
    try {
        ...
        require('./career-hunter-cron').startCareerHunterCron(ctx);
    }
    catch (err) {
    ...
    router.get('/static/:file', (req, res) => {
        const file = path_1.default.basename(req.params.file);
        if (!/\.(css|js|png|svg)$/.test(file)) {
            res.status(404).end();
            return;
        }
        res.sendFile(path_1.default.join(TOOL_DIR, file), (err) => { if (err)
            res.status(404).end(); });
    });
    ...
    const proxyBoard = async (req, res) => {
        const userSub = (0, caller_sub_2.callerSub)(req);
        if (!userSub) {
            res.status(401).send('Sign in first.');
            return;
        }
        if (!openUserDb(userSub)) {
            res.set('Content-Type', 'text/html').send('<body style="font:14px system-ui;padding:24px;color:#33414f">Your job board is empty. ' +
        ...
        try {
            ...
            if (!upstream) {
                res.status(502).send('Board starting, retry in a moment.');
                return;
            }
    ...
    router.get('/companies-admin/list', requireCareerAdmin, (req, res) => {
        const sub = (0, caller_sub_2.callerSub)(req);
    ...
    router.get('/applications', async (req, res) => {
        ...
        const rows = (await pool.query(`SELECT posting_id, company, title, include_oshal, status, ticket_id, updated_at
    ...
    router.get('/jobs/stats', (req, res) => {
        ...
        try {
            ...
            // the separate COUNT(*), so the whole endpoint is one index pass (402ms -> 180ms).
            const raw = db.prepare('SELECT status, COUNT(*) AS n FROM user_signals GROUP BY status').all();
    ...
    router.get('/analytics', (req, res) => {
        ...
        try {
           ...
           (SELECT COUNT(*) FROM user_signals WHERE status='promoted') AS promoted`).get();
            const topCompanies = db.prepare(`SELECT c.name AS company, COUNT(*) AS roles, CAST(AVG(${SCORE}) AS INT) AS avg_fit, MAX(COALESCE(c.referral,0)) AS referral
         FROM corpus.postings_corpus p JOIN corpus.companies c ON c.id=p.company_id LEFT JOIN user_signals s ON s.posting_id=p.id
         WHERE p.active=1 AND COALESCE(p.target_role,0)=1 GROUP BY c.id ORDER BY roles DESC, avg_fit DESC LIMIT 12`).all();
            const states = db.prepare(`SELECT COALESCE(NULLIF(p.state,''),'(remote/—)') AS state, COUNT(*) AS n ${LANE} GROUP BY 1 ORDER BY n DESC LIMIT 12`).all();
            const salaryBands = db.prepare(`SELECT CASE WHEN ${PAY} IS NULL THEN 'n/a' WHEN ${PAY}<150000 THEN '<150k' WHEN ${PAY}<200000 THEN '150-200k'
    ...
    const serveResumeFile = (req, res, postingId) => {
        const userSub = (0, caller_sub_2.callerSub)(req);
        if (!userSub) {
            res.status(401).json({ error: 'unauthorized' });
            return;
        ...
        let filePath = null;
        try {
            const row = db.prepare(`SELECT ${kind} AS p FROM user_signals WHERE posting_id=?`).get(postingId);
            filePath = row?.p || null;
        }
        finally {
            try {
        ...
        if (!filePath || !path_1.default.resolve(filePath).startsWith(safeRoot) || !fs_1.default.existsSync(filePath)) {
            res.status(404).json({ error: 'not found' });
            return;
        }
        // `as=html` serves the generated HTML the PDF was printed from, for in-surface preview:
        // no mobile browser renders a PDF in an iframe, so the PDF path can only ever produce an
        // empty frame there. 404 when the sibling is absent — falling back to the PDF would
        // silently restore the invisible preview. See ./career-resume-preview.
        if (req.query.as === 'html') {
            const htmlPath = (0, career_resume_preview_1.resolvePreviewPath)(filePath, userDir);
            if (!htmlPath) {
    ...
    router.post('/jobs/:id/status', (req, res) => {
        ...
        try {
            ...
            const info = db.prepare(`UPDATE user_signals SET status=@status, applied_at=${appliedAt} WHERE posting_id=@id`).run({ status, id: postingId });
    ...
    router.get('/jobs/:id', (req, res) => {
        ...
        try {
            const row = db.prepare(`SELECT p.*, c.name AS company, c.industry, c.homepage, c.careers_url, COALESCE(c.referral,0) AS referral,
          ...
          WHERE p.id = ?`).get(postingId);
            ...
            row.matched = jl(row.ai_fit_matched);
            row.gaps = jl(row.ai_fit_gaps);
            res.json({ job: row });
        }
        catch (err) {
    ...
    router.get('/recruiters', (req, res) => {
        ...
        try {
            ...
            const buckets = db.prepare("SELECT DISTINCT bucket FROM recruiter_firms WHERE bucket IS NOT NULL AND bucket<>'' ORDER BY bucket").all().map((r) => r.bucket);
            ...
            logger.error({ err }, 'recruiters list failed');
            res.status(500).json({ error: 'read failed' });
        }
        finally {
            try {
    ...
    router.post('/recruiters', (req, res) => {
        ...
        try {
            const info = db.prepare(`INSERT INTO recruiter_firms (firm, bucket, website, contact_name, contact_link, status, sort_order, updated_at)
         VALUES (?,?,?,?,?,?, (SELECT COALESCE(MAX(sort_order),0)+1 FROM recruiter_firms), datetime('now'))`).run(firm, String(req.body?.bucket || 'Other').trim(), String(req.body?.website || '').trim(), String(req.body?.contact_name || '').trim(), String(req.body?.contact_link || '').trim(), String(req.body?.status || 'To contact').trim());
            res.status(201).json({ ok: true, id: info.lastInsertRowid });
        }
        catch (err) {
            logger.error({ err }, 'recruiter add failed');
    ...
    router.post('/recruiters/:id', (req, res) => {
        ...
        try {
            ...
            logger.error({ err }, 'recruiter update failed');
            res.status(500).json({ error: 'update failed' });
        }
        finally {
            try {
    ...
    router.delete('/recruiters/:id', (req, res) => {
        ...
        try {
            ...
            logger.error({ err }, 'recruiter delete failed');
    ...
    router.get('/strengthen', (req, res) => {
        ...
        if (!r.ok) {
            logger.error({ err: r.err }, 'strengthen list failed');
        ...
        try {
            ...
            logger.error({ err, tail: r.out.slice(-300) }, 'strengthen parse failed');
    ...
    router.post('/applications/:postingId/approve', async (req, res) => {
        ...
        const oshal = req.body?.oshal === true || req.body?.oshal === 'true';
        const appRow = (await pool.query(`SELECT ticket_id FROM career_hunter_applications WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`, [TENANT, userSub, postingId])).rows[0];
        const ticketId = appRow?.ticket_id;
        if (ticketId) {
            try {
                await ctx.ticketService.updateStatus(ticketId, 'in_process_build');
            }
            catch { /* */ }
        }
        await pool.query(`UPDATE career_hunter_applications SET status='drafting', include_oshal=$4, updated_at=NOW()
        ...
        if (r.ok) {
            if (ticketId) {
                try {
                    await ctx.ticketService.updateStatus(ticketId, 'customer_action');
                }
                catch { /* */ }
            }
            await pool.query(`UPDATE career_hunter_applications SET status='drafted', updated_at=NOW()
          WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`, [TENANT, userSub, postingId]);
            ...
            await pool.query(`UPDATE career_hunter_applications SET status='error', updated_at=NOW()
          WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`, [TENANT, userSub, postingId]);
    ...
    router.post('/applications/:postingId/applied', async (req, res) => {
        ...
        if (appRow?.ticket_id) {
            try {
                await ctx.ticketService.updateStatus(appRow.ticket_id, 'complete');
            }
            catch { /* */ }
        }
        const db = openUserDb(userSub, false);
        if (db) {
            try {
                db.prepare(`INSERT INTO user_signals (posting_id,status,applied_at) VALUES (?, 'applied', datetime('now')) ON CONFLICT(posting_id) DO UPDATE SET status='applied', applied_at=datetime('now')`).run(postingId);
            }
            finally {
                db.close();
            }
    ...
    router.post('/applications/:postingId/deny', async (req, res) => {
        ...
        if (appRow?.ticket_id) {
            try {
                await ctx.ticketService.updateStatus(appRow.ticket_id, 'cancelled');
            }
            catch { /* */ }
        }
        const db = openUserDb(userSub, false);
        if (db) {
            try {
                db.prepare(`INSERT INTO user_signals (posting_id,status) VALUES (?, 'dismissed') ON CONFLICT(posting_id) DO UPDATE SET status='dismissed'`).run(postingId);
            }
            finally {
                db.close();
            }
    ...
    router.get('/settings/state', async (req, res) => {
        ...
        const conns = (await pool.query(`SELECT provider FROM oshal_connections WHERE user_sub=$1 AND provider IN ('anthropic','firecrawl')`, [userSub])).rows;
        const hasHostClaude = fs_1.default.existsSync(path_1.default.join(process.env.HOME || process.env.USERPROFILE || '', '.claude', '.credentials.json'));
        res.json({
            anthropicConnected: conns.some((c) => c.provider === 'anthropic') || hasHostClaude,
            firecrawlSet: conns.some((c) => c.provider === 'firecrawl'),
        });
    });
    async function saveKey(req, res, provider) {
        const userSub = (0, caller_sub_2.callerSub)(req);
        if (!userSub) {
            res.status(401).json({ error: 'unauthorized' });
            return;
        ...
        const enc = await (0, connector_token_crypto_1.encryptToken)(pool, userSub, key);
        await pool.query(`INSERT INTO oshal_connections (user_sub, provider, access_token, status)
       VALUES ($1,$2,$3,'connected')
       ON CONFLICT (user_sub, provider) DO UPDATE SET access_token=EXCLUDED.access_token, status='connected', updated_at=NOW()`, [userSub, provider, enc]);
        res.json({ ok: true });
    }
    ...
    const refreshCallerSub = (req) => (0, caller_sub_2.callerSub)(req) ?? (0, authz_1.getTrustedServiceUserSub)(req);
    ...
    router.get('/run/refresh', (req, res) => {
        ...
        const cron = require('./career-hunter-cron');
        let corpusFreshAt = null;
        const db = openUserDb(sub) || openUserDb(listStoreUsers()[0] || '');
        if (db) {
            try {
                corpusFreshAt = db.prepare('SELECT MAX(last_seen_at) AS m FROM corpus.postings_corpus').get()?.m ?? null;
            }
    ...
    router.post('/run/:verb', async (req, res) => {
        ...
        if (acquired === 'inflight') {
            res.status(409).json({ ok: false, err: `${verb} already running` });
            return;
        }
        if (acquired === 'busy') {
            res.status(429).json({ ok: false, err: 'busy — too many runs in progress, try again shortly' });
            return;
        }
    ...
    const resumeUpload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage(), limits: { fileSize: 8 * 1024 * 1024 } });
    router.post('/resume/upload', resumeUpload.single('resume'), (req, res) => {
        ...
        const { userDir } = userPaths(userSub);
        try {
            const upDir = path_1.default.join(userDir, 'uploads');
            fs_1.default.mkdirSync(upDir, { recursive: true });
            const dest = path_1.default.join(upDir, 'resume' + ext);
            fs_1.default.writeFileSync(dest, file.buffer);
            fs_1.default.writeFileSync(path_1.default.join(userDir, '.indexing'), String(Date.now()));
    ...
    router.get('/resume/state', (req, res) => {
        ...
        let name = '';
        try {
            if (fs_1.default.existsSync(careerDb)) {
                const d = JSON.parse(fs_1.default.readFileSync(careerDb, 'utf8'));
        ...
        let indexing = false;
        try {
            const marker = path_1.default.join(userDir, '.indexing');
            if (fs_1.default.existsSync(marker)) {
                if (hasResume) {
                    fs_1.default.rmSync(marker, { force: true });
                }
                else {
                    indexing = (Date.now() - Number(fs_1.default.readFileSync(marker, 'utf8') || 0)) < 5 * 60 * 1000;
                }
            }
        }
        catch { /* */ }
