...
/**
 * Native Career Hunter board routes over the shared corpus and caller-owned signals database.
 * @module career-board-routes
 */
...
function getJobs(req: Request, res: Response): void {
  ...
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'career jobs database close failed'); } }
}

function getJobStats(req: Request, res: Response): void {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const db = openUserDb(userSub);
  ...
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'career stats database close failed'); } }
}

function readAnalytics(db: any): Record<string, unknown> {
  ...
  const funnel = db.prepare(
    `SELECT (SELECT COUNT(*) ${lane}) AS sourced,
  ...
  const topCompanies = db.prepare(
    `SELECT c.name AS company, COUNT(*) AS roles, CAST(AVG(${score}) AS INT) AS avg_fit,
      ...
      GROUP BY c.id ORDER BY roles DESC, avg_fit DESC LIMIT 12`,
  ).all();
  const states = db.prepare(
    `SELECT COALESCE(NULLIF(p.state,''),'(remote/â€”)') AS state, COUNT(*) AS n
       ${lane} GROUP BY 1 ORDER BY n DESC LIMIT 12`,
  ).all();
  const salaryBands = db.prepare(
    `SELECT CASE WHEN ${pay} IS NULL THEN 'n/a' WHEN ${pay}<150000 THEN '<150k'
...
function getAnalytics(req: Request, res: Response): void {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const db = openUserDb(userSub);
  if (!db) { res.json({ empty: true }); return; }
  try { res.json(readAnalytics(db)); }
  catch (err) {
    logger.error({ err, userSub }, 'career analytics failed');
    res.status(500).json({ error: 'analytics failed' });
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'analytics database close failed'); } }
}
...
function readSignalFile(
  ...
  try {
    const row = db.prepare(`SELECT ${kind} AS p FROM user_signals WHERE posting_id=?`)
      .get(postingId) as { p?: string } | undefined;
    return row?.p || null;
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'resume database close failed'); } }
}

function resolvePacketPath(filePath: string, userDir: string, preview = false): string | null {
  try {
    return preview
      ? resolvePreviewPath(filePath, userDir)
      : resolveContainedRegularFile(filePath, userDir);
  } catch (err) {
...
function serveHtmlPreview(res: Response, filePath: string, userDir: string): void {
  const htmlPath = resolvePacketPath(filePath, userDir, true);
  if (!htmlPath) { res.status(404).json({ error: 'no html preview' }); return; }
  try { res.type('html').send(buildPreviewHtml(htmlPath)); }
  catch (err) {
...
function serveResumeFile(req: Request, res: Response, postingId: number): void {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  ...
  if (filePath === undefined) { res.status(404).json({ error: 'no data' }); return; }
  ...
  if (req.query.as === 'html') { serveHtmlPreview(res, resolved, userDir); return; }
  res.sendFile(resolved, (err: unknown) => {
    if (err) { logger.error({ err }, 'resume serve failed'); res.status(404).end(); }
  });
}

async function updateJobStatus(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  ...
  try {
    if (status === 'applied') {
      const exists = db.prepare('SELECT 1 AS found FROM user_signals WHERE posting_id=?')
        .get(postingId);
      if (!exists) { res.status(404).json({ error: 'job signal not found' }); return; }
    ...
    const info = db.prepare(
      `UPDATE user_signals SET status=@status, applied_at=${appliedAt}${provenance} WHERE posting_id=@id`,
  ...
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'status database close failed'); } }
}

function parseJsonList(value: unknown): unknown[] {
  try { return value ? JSON.parse(String(value)) : []; }
  catch (err) { logger.warn({ err }, 'career job JSON field is malformed'); return []; }
}

function getJob(req: Request, res: Response): void {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const postingId = Number(req.params.id);
  ...
  try {
    const row = db.prepare(
      `SELECT p.*, c.name AS company, c.industry, c.homepage, c.careers_url,
         ...
         FROM corpus.postings_corpus p JOIN corpus.companies c ON c.id=p.company_id
         LEFT JOIN user_signals s ON s.posting_id=p.id WHERE p.id=?`,
    ...
    res.status(500).json({ error: 'read failed' });
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'job database close failed'); } }
}

async function generatePacket(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const postingId = Number(req.params.id);
  if (!Number.isFinite(postingId)) { res.status(400).json({ error: 'bad id' }); return; }
  ...
  const started = await runCareerCliAsync(ctx.pool, userSub, ['tailor'], {
    CH_JOB: String(postingId),
    CH_OSHAL: includeOshal ? '1' : '',
    CH_GUIDANCE: guidance,
  });
  if (rejectEngineStart(res, started, 'tailor')) return;
...
function setReferral(req: Request, res: Response): void {
  ...
  try {
    ...
    res.status(500).json({ error: 'update failed' });
  } finally { try { db.close(); } catch (err) { logger.warn({ err }, 'referral database close failed'); } }
}
 ...
 * @returns Nothing.
 */
export function registerCareerResumeAlias(router: Router): void {
