...
/** @description A browse request's predicate, split by which table each rule belongs to. */
export interface BrowseFilters {
  /** Everything, joined -- what the outer query applies after the corpus join. */
  where: string;
  args: unknown[];
  /**
   * The `postings_corpus`-only half, which a search arm can apply BEFORE it bounds itself.
   * This is a correctness fix as much as a speed one: filtering after a bounded candidate
   * pool means a narrow filter searches only the pool, so "nurse" + state=FL returned ZERO
   * against a corpus that holds plenty of both. Pushed into the arm, the 600 candidates are
   * drawn from rows that already match.
   */
  postingWhere: string;
  postingArgs: unknown[];
}
 ...
 */
export function browseFilters(query: BrowseQuery): BrowseFilters {
  const g = (k: string) => param(query, k);
  const posting: string[] = ['p.active = 1'];
  const postingArgs: unknown[] = [];
  const company: string[] = [];
  const companyArgs: unknown[] = [];
  ...
  if (g('lane') === 'target') posting.push('p.target_role = 1');
  if (g('company')) { company.push('c.name LIKE ?'); companyArgs.push(`%${g('company')}%`); }
  if (g('remote') === '0' || g('remote') === '1') { posting.push('p.remote = ?'); postingArgs.push(Number(g('remote'))); }
  if (g('state')) { posting.push('p.state = ?'); postingArgs.push(g('state').toUpperCase()); }
  if (g('type')) { posting.push('p.job_type = ?'); postingArgs.push(g('type')); }
  const days = parseInt(g('days'), 10);
  if (days > 0) { posting.push("p.posted_date IS NOT NULL AND p.posted_date >= date('now', ?)"); postingArgs.push(`-${days} days`); }
  const minPay = parseInt(g('min_pay'), 10);
  if (minPay > 0) { posting.push('COALESCE(p.salary_max,p.salary_min,0) >= ?'); postingArgs.push(minPay); }
  return {
...
export interface BrowsePage {
  ...
  /** True when the search ran through the full-text index rather than LIKE arms. */
  fullText: boolean;
  /** True when this search also matched against posting descriptions. */
  deep: boolean;
 ...
 */
export function searchCandidateSql(
  deep: boolean,
  postingWhere = 'active = 1',
): { sql: string; arms: number; parts: string[] } {
  ...
  const arm = (body: string) =>
    `SELECT cid AS id FROM (${body} ORDER BY cid + 0 DESC LIMIT ${ARM_POOL})`;
  ...
  arms.push(arm(
    `SELECT p.id AS cid FROM corpus.companies c
  ...
  if (deep) {
    arms.push(arm(
      `SELECT id AS cid FROM corpus.postings_corpus p WHERE ${postingWhere} AND p.description LIKE ?`,
    ));
  }
...
function runSearch(
  ...
  const sql = `WITH cand AS (
        ${candidates.sql}
         LIMIT ${BROWSE_POOL})   /* backstop only — each arm is already bounded */
      SELECT ${SELECT_COLS}
        FROM cand
  ...
  candidates.parts.forEach((part, index) => {
    ...
    if (employerArm) bindings.push(like, ...filters.postingArgs);
    else bindings.push(...filters.postingArgs, like);
  });
  return db.prepare(sql).all(
    ...bindings, ...filters.args, plan.per, plan.offset,
...
function runFullText(
  ...
  const employerArm = `
        ...
        SELECT id FROM (SELECT p2.id AS id FROM corpus.companies c2
               ...
               ORDER BY p2.id DESC LIMIT ${ARM_POOL})`;
  const candidates = `SELECT rowid FROM corpus.${FTS_TABLE} WHERE ${FTS_TABLE} MATCH ?`
  ...
  if (employer) args.push(`%${employer}%`);
  return db.prepare(sql).all(
    ...args, ...filters.args, plan.per, plan.offset,
  ) as Record<string, unknown>[];
}
...
export function fetchBrowsePage(db: SqliteDb, query: BrowseQuery): BrowsePage {
  ...
  if (fullText && expression) {
    ...
    const ftsPlan = plan.order === BROWSE_SORT_MAP.recent
  ...
  return {
    ...
    poolSize: wantsText && !fullText ? BROWSE_POOL : null,
...
export const FTS_TABLE = 'postings_fts';
 ...
 */
export function hasFullText(db: SqliteDb): boolean {
  try {
    const row = db.prepare(
      "SELECT 1 AS ok FROM corpus.sqlite_master WHERE type='table' AND name=?",
 ...
 */
export function ftsMatchExpression(column: string, text: string): string {
 ...
 */
export function ftsExpressionFor(query: BrowseQuery): string {
  const parts = [
