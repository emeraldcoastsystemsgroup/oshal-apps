...
/**
 * Guest-mode demo-profile seeding for career-hunter.
 * @module career-guest-seed-routes
 */
...
const logger = createChildLogger({ module: 'career-guest-seed-routes' });
...
const DEMO_PROFILE = path.resolve(__dirname, '..', 'engine', 'seeds', 'career_db.default.json');
 ...
 */
async function seedGuestProfile(req: Request, res: Response): Promise<void> {
  ...
  if (!userSub.startsWith('guest-')) { res.status(403).json({ error: 'guest_only' }); return; }
  try {
    const { userDir } = userPaths(userSub); // creates the guest's store dir
    await fs.copyFile(DEMO_PROFILE, path.join(userDir, 'career_db.json'));
 ...
 * @returns Nothing.
 */
export function registerCareerGuestSeedRoutes(router: Router): void {
