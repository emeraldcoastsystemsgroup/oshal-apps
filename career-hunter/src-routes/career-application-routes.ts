...
/**
 * Human-in-the-loop Career Hunter application queue routes.
 * @module career-application-routes
 */
...
import { callerSub, careerTenant, openUserDb } from './career-user-store';

const logger = createChildLogger({ module: 'career-application-routes' });
const TENANT = careerTenant();

/** SQL rank for the four explicit application-evidence sources. */
function provenanceRankSql(value: string): string {
...
function loadEnqueueRows(userSub: string, limit: number): EnqueueRow[] {
  ...
  try {
    return db.prepare(
      `SELECT pc.id, pc.title, co.name AS company, us.ai_fit_score AS fit,
        ...
        ORDER BY us.ai_fit_score DESC LIMIT ?`,
    ).all(Math.min(50, Math.max(1, limit))) as EnqueueRow[];
  } finally { db.close(); }
}

async function applicationExists(ctx: AppContext, userSub: string, postingId: number): Promise<boolean> {
  const result = await ctx.pool.query(
    `SELECT 1 FROM career_hunter_applications
      WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`,
    [TENANT, userSub, postingId],
  );
  return !!result.rowCount;
}

/** Build a bounded external key so concurrent replicas ask TicketService for the same ticket. */
function applicationTicketKey(userSub: string, postingId: number): string {
  const owner = createHash('sha256').update(userSub, 'utf8').digest('hex');
...
async function createApplication(ctx: AppContext, userSub: string, row: EnqueueRow): Promise<boolean> {
  ...
  const inserted = await ctx.pool.query(
    `INSERT INTO career_hunter_applications
     ...
     RETURNING id`,
    [TENANT, userSub, ticket.ticketId, row.id, row.company, row.title],
  );
...
export async function enqueueForUser(
  ...
  for (const row of loadEnqueueRows(userSub, limit)) {
    if (await applicationExists(ctx, userSub, row.id)) continue;
    if (await createApplication(ctx, userSub, row)) created += 1;
  }
  return created;
}

async function ticketIdFor(ctx: AppContext, userSub: string, postingId: number): Promise<string | null> {
  const result = await ctx.pool.query(
    `SELECT ticket_id FROM career_hunter_applications
      WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`,
    [TENANT, userSub, postingId],
  );
...
async function updateTicketStatus(
  ...
  if (!ticketId) throw new Error('career application ticket is missing');
  if (metadata) await ctx.ticketService.updateStatus(ticketId, status as never, metadata);
  else await ctx.ticketService.updateStatus(ticketId, status as never);
}
    ...
    updated = await ctx.pool.query(
      `UPDATE career_hunter_applications
          SET status=$4,
              ...
              updated_at=NOW()
        WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`,
      [TENANT, userSub, postingId, status, applicationSource],
    );
  } else if (includeOshal === undefined) {
    updated = await ctx.pool.query(
      `UPDATE career_hunter_applications SET status=$4, updated_at=NOW()
        WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`,
      [TENANT, userSub, postingId, status],
    );
  } else {
    updated = await ctx.pool.query(
      `UPDATE career_hunter_applications SET status=$4, include_oshal=$5, updated_at=NOW()
        WHERE tenant_id=$1 AND user_sub=$2 AND posting_id=$3`,
      [TENANT, userSub, postingId, status, includeOshal],
    );
  }
  if (updated.rowCount !== 1) throw new Error('career application state row is missing');
}
...
async function approveApplication(ctx: AppContext, req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (includeOshal) args.push('--oshal');
    const result = await runCareerCliAwait(ctx.pool, userSub, args, {}, { preclaimed: lease });
    if (result.ok) {
...
function setSignalStatus(userSub: string, postingId: number, status: string): void {
  const db = openUserDb(userSub, false);
  if (!db) throw new Error('career user store is unavailable');
  try {
    if (status === 'applied') {
      db.prepare(
        `INSERT INTO user_signals (posting_id,status,applied_at,application_source)
         ...
         ON CONFLICT(posting_id) DO UPDATE SET status='applied',
           ...
           application_source=CASE
             ...
             ELSE 'manual-mark' END`,
      ).run(postingId);
    } else {
      db.prepare(
        `INSERT INTO user_signals (posting_id,status) VALUES (?, 'dismissed')
         ON CONFLICT(posting_id) DO UPDATE SET status='dismissed'`,
      ).run(postingId);
    }
  } finally { db.close(); }
}

async function markApplicationApplied(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const postingId = Number(req.params.postingId);
  if (!Number.isSafeInteger(postingId) || postingId <= 0) {
  ...
  try {
    const ticketId = await ticketIdFor(ctx, userSub, postingId);
    if (!ticketId) throw new Error('career application ticket is missing');
    await recordManualApplyRun(ctx.pool, {
      ownerSub: userSub,
      postingId,
      ticketId,
...
async function denyApplication(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const postingId = Number(req.params.postingId);
  if (!Number.isSafeInteger(postingId) || postingId <= 0) {
...
async function listApplications(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  const result = await ctx.pool.query(
    `SELECT posting_id, company, title, include_oshal, status, ticket_id,
...
async function enqueueDrafts(ctx: AppContext, req: Request, res: Response): Promise<void> {
  const userSub = callerSub(req);
  if (!userSub) { res.status(401).json({ error: 'unauthorized' }); return; }
  try {
    const created = await enqueueForUser(
      ctx, userSub, Number(req.body?.limit) || 10, { trigger: 'manual' },
    );
    res.json({ created });
  } catch (error) {
 ...
 * @returns Nothing.
 */
export function registerCareerApplicationReadRoutes(router: Router, ctx: AppContext): void {
 ...
 * @returns Nothing.
 */
export function registerCareerApplicationMutationRoutes(router: Router, ctx: AppContext): void {
