...
export async function ingestJobsGraphForUser(
  userSub: string,
  openDb: typeof defaultOpenUserDb = defaultOpenUserDb,
): Promise<void> {
  try {
    if (!userSub) return;
    const db = openDb(userSub);
    if (!db) return; // store not seeded yet — nothing to mirror
    let rows: Array<{ id: number; title: string; company: string; location: string | null; url: string | null }>;
    try {
      rows = db.prepare(
        `SELECT p.id, p.title, c.name AS company, p.location, p.url
          ...
          LIMIT ?`,
      ).all(`-${GRAPH_INGEST_FIRST_SEEN_DAYS} days`, GRAPH_INGEST_LIMIT) as typeof rows;
    } finally {
      try { db.close(); } catch { /* close failure never masks the read result */ }
    }
    if (!rows.length) return;
    const jobs: JobGraphRecord[] = rows.map((r) => ({
      id: r.id, title: r.title, company: r.company, location: r.location ?? null, url: r.url ?? null,
    }));
    await getGraphIngestionService().ingestJobsForPerson(userSub, jobs);
    logger.info({ userSub, jobs: jobs.length }, 'jobs mirrored into person graph');
  } catch (err) {
    logger.error({ err, stack: (err as Error)?.stack, userSub }, 'jobs graph ingestion failed (host flow unaffected)');
  }
}
...
function buildModel(userPath: string, corpusPath: string): { nodes: GraphNode[]; edges: GraphEdge[] } {
  const db = new Database(userPath, { readonly: true, fileMustExist: true });
  try {
    db.exec(`ATTACH DATABASE '${corpusPath.replace(/'/g, "''")}' AS corpus`);
    const nodes: GraphNode[] = [{ id: 'me', labels: ['user'], props: {} }];
    const edges: GraphEdge[] = [];

    // Skills the market wants that the user is gapped on (demand-weighted).
    for (const g of db.prepare('SELECT key, n_jobs, avg_fit FROM gap_themes').all() as Array<{ key: string; n_jobs: number; avg_fit: number }>) {
    ...
    for (const r of db.prepare('SELECT id, firm, bucket, status FROM recruiter_firms').all() as Array<{ id: number; firm: string; bucket: string; status: string }>) {
    ...
    const rows = db.prepare(
      `SELECT c.id AS cid, c.name, c.industry, c.referral,
       ...
       GROUP BY c.id ORDER BY best_fit DESC LIMIT 150`,
    ).all() as Array<{ cid: number; name: string; industry: string | null; referral: number | null; best_fit: number; applied: number }>;
    for (const c of rows) {
      ...
      if (c.applied > 0) edges.push({ from: 'me', to: `company:${c.cid}`, type: 'applied' });
      if (c.industry) {
...
async function insights(g: GraphHandle): Promise<Record<string, unknown>> {
  const topGaps = await g.rawQuery(
    "FOR e IN edges FILTER e.type=='gap' SORT e.props.n_jobs DESC LIMIT 10 " +
  ...
  const fitIndustries = await g.rawQuery(
    "FOR fe IN edges FILTER fe.type=='fit' LET c = DOCUMENT(fe._to) " +
  ...
  const recruiterBuckets = await g.rawQuery(
    "FOR e IN edges FILTER e.type=='in_bucket' COLLECT bucket = DOCUMENT(e._to).props.name WITH COUNT INTO n " +
 ...
 */
export function createCareerGraphRoutes(): Router {
  const router = Router();
  ...
  async function callerGraph(req: Request, res: Response) {
    ...
    if (!connector) { res.status(503).json({ error: 'graph_engine_unavailable' }); return null; }
  ...
  router.post('/ingest', async (req: Request, res: Response) => {
    const ctx = await callerGraph(req, res); if (!ctx) return;
    const { userPath, corpusPath } = dbPaths(ctx.sub);
    if (!fs.existsSync(userPath)) { res.status(404).json({ error: 'no_career_data', message: 'no career-hunter data for this user' }); return; }
    try {
    ...
    } catch (err) { logger.error({ err }, 'career graph ingest failed'); res.status(502).json({ error: (err as Error).message }); }
  });

  /** GET /insights — graph-backed answers (top gaps, fit-industries w/ warm referrals, recruiter buckets). */
  router.get('/insights', async (req: Request, res: Response) => {
    const ctx = await callerGraph(req, res); if (!ctx) return;
    try { res.json(await insights(ctx.g)); }
    catch (err) { logger.error({ err }, 'career graph insights failed'); res.status(502).json({ error: (err as Error).message }); }
  });

  return router;
}

export { buildModel, insights, dbPaths };
