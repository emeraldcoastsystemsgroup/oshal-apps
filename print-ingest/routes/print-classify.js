 ...
 */
function destinationCatalog(env = process.env) {
    const catalog = [
        {
            id: 'private',
            label: 'Private to me',
    ...
    for (const entry of String(env.PRINT_INGEST_BOT_DESTINATIONS || '').split(';')) {
        ...
        if (!SAFE_ID.test(id || '') || !label || !collection)
            continue;
        if (catalog.some((existing) => existing.id === id))
            continue;
        catalog.push({
            id,
            label,
...
//# sourceMappingURL=print-classify.js.map