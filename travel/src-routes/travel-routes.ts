 ...
 */
function packageToolFile(appPackageDir: string | undefined, fileName: string): string {
  const candidates = [
...
function serveFile(appPackageDir: string | undefined, fileName: string): RequestHandler {
...
function resolveViewerSub(req: Request): string {
  ...
  if (process.env.MOCK_OIDC === 'true') return 'demo-traveller';
...
// ── Concierge brain ────────────────────────────────────────────────────────────
interface ChatEnvelope { say: string; show: string[]; watch: string[]; remember: string[]; }

function buildConciergePrompt(o: {
  message: string; history: Array<{ role: string; content: string }>;
  candidates: any[]; kind: string; profile: any; notes: string[]; price: any;
}): string {
  const p = o.profile || {};
  ...
  const fmt = (c: any) => o.kind === 'flight'
    ? `- id:${c.id} | ${c.airline} $${c.price} ${c.currency} | ${(c.slices || []).map((s: any) => `${s.origin}->${s.destination} ${s.departAt || ''} ${s.duration || ''} ${s.stops === 0 ? 'nonstop' : s.stops + ' stop'}`).join(' / ')}`
...
function parseEnvelope(text: string): ChatEnvelope {
  const fallback: ChatEnvelope = { say: (text || '').trim(), show: [], watch: [], remember: [] };
...
function flightQuery(src: any): any {
  return {
...
// ── Router ─────────────────────────────────────────────────────────────────────
export function createTravelRoutes(ctx: AppContext): Router {
  const router = Router();
  const pool = ctx.pool;
  const appPackageDir = ctx.appPackageDir;

  ensureTravelSchema(pool).catch((err) => logger.warn({ err }, 'Travel schema bootstrap deferred — tables may not exist yet'));
  ...
  const traveller =
    ...
    async (req, res) => {
      ...
      catch (err: any) { logger.error({ err, path: req.path }, 'travel route error'); res.status(500).json({ error: err.message || 'internal error' }); }
    };
  ...
  router.get('/flights', traveller(async (req, res, sub) => {
    ...
    if (!q.origin || !q.destination || !q.departDate) { res.status(400).json({ error: 'origin, destination and departDate are required' }); return; }
    ...
    const routeKey = routeKeyFor('flight', q);
    const price = await priceRead(pool, 'flight', routeKey, best);
    await pool.query(
      `INSERT INTO travel_searches (user_sub, kind, route_key, query, best_price) VALUES ($1,'flight',$2,$3,$4)`,
      [sub, routeKey, JSON.stringify(q), best],
    ).catch(() => {});
  ...
  router.get('/hotels', traveller(async (req, res, sub) => {
    ...
    res.json({ items: r.items || [], source: r.source || 'demo', deepLink: r.deepLink, price: await priceRead(pool, 'hotel', routeKey, best), routeKey });
  }));
  ...
  router.get('/cars', traveller(async (req, res, sub) => {
    ...
    const best = (r.items || []).length ? Math.min(...r.items.map((i: any) => Number(i.price))) : null;
    res.json({ items: r.items || [], source: r.source || 'demo', deepLink: r.deepLink, price: await priceRead(pool, 'car', routeKey, best), routeKey });
  }));
  ...
  router.post('/watches', traveller(async (req, res, sub) => {
    ...
    const routeKey = b.routeKey || routeKeyFor(kind, q);
    const r = await pool.query(
      `INSERT INTO travel_watches (user_sub, kind, route_key, query, target_price, last_price)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [sub, kind, routeKey, JSON.stringify(q), b.targetPrice != null ? Number(b.targetPrice) : null, b.lastPrice != null ? Number(b.lastPrice) : null],
    );
  ...
  router.post('/profile', traveller(async (req, res, sub) => {
    const b = req.body || {};
    const r = await pool.query(
      `INSERT INTO travel_profile (user_sub, display_name, home_airport, preferred_airlines, preferred_cabin, seat_pref, hotel_brands, avoid, budget_band, loyalty, notes, onboarded, updated_at)
       ...
       RETURNING *`,
      [sub, b.displayName ?? null, b.homeAirport ?? null, b.preferredAirlines ?? null, b.preferredCabin ?? null,
  ...
  router.post('/chat', traveller(async (req, res, sub) => {
    ...
    const history = (await pool.query(
      `SELECT role, content FROM travel_messages WHERE conversation_id = $1 ORDER BY created_at DESC LIMIT 12`, [conversationId])).rows.reverse();
    ...
    if (search && kind === 'flight' && search.origin && search.destination && search.departDate) {
      ...
      price = await priceRead(pool, 'flight', routeKeyFor('flight', q), best);
    }
    ...
    for (const id of env.watch) {
      ...
      const w = await pool.query(
        `INSERT INTO travel_watches (user_sub, kind, route_key, query, last_price) VALUES ($1,'flight',$2,$3,$4) RETURNING *`,
        [sub, routeKey, JSON.stringify(q), Number(c.price)],
      );
