...
const definitions = [[0, "watches", "saved-fare-watches", "Saved fare watches"], [2, "day", "flight-searches-24h", "Flight searches / 24h"], [2, "five_days", "flight-searches-5d", "Flight searches / 5 days"]];
...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        const items = recent.status === 'fulfilled' ? recent.value.rows.map(item).filter(Boolean).slice(0, 3).map(({ context, ...entry }) => ({ ...entry, tone: 'neutral', fix: "travel-concierge", actions: ["plan-ride", "plan-meal", "plan-shopping", "plan-music"].map(integration => ({ integration, context })) })) : [];
        const failed = results.filter(r => r.status === 'rejected').length;
        if (failed)
            items.push({ text: 'Some saved data cannot be checked.', tone: 'warn', fix: "travel-concierge" });
        else if (!items.length)
            items.push({ text: "No saved preparation items to review yet.", tone: 'neutral', fix: "travel-concierge" });
