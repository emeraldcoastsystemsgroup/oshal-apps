...
const briefingSubjects = world_data_1.DEFAULT_WORLD_TOPICS.map(s => s.entity);
...
const text = (value, cap = 400) => String(value ?? '').replace(/<[^>]*>/g, ' ').replace(/&(?:nbsp|#160);/gi, ' ').replace(/&amp;/gi, '&').replace(/\s+/g, ' ').trim().slice(0, cap);
...
function articleKey(row) {
    ...
    if (!link)
        return `${text(row.outlet)}:${text(row.title)}`.toLowerCase();
...
function createHomeSummaryRoutes() {
    ...
    router.get('/', async (req, res) => {
        ...
        const service = (0, world_data_1.createWorldIntelligenceService)();
        if (!service) {
            res.status(503).json({ error: 'world_archive_unavailable' });
            return;
        }
        try {
            ...
            const articles = [...(snapshot.articles ?? [])].filter(row => {
                ...
                if (seen.has(key) || topics.has(row.entity) || !text(row.title))
                    return false;
                seen.add(key);
                topics.add(row.entity);
                return true;
            }).slice(0, 3);
            ...
            if (upcoming)
                items.push({ text: text(`Upcoming: ${upcoming.title || upcoming.event_type}`, 120),
