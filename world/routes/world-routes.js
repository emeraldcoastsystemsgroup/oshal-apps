...
const outlet_ratings_1 = require("@/features/world-data/outlet-ratings");
...
function createWorldRoutes() {
    ...
    if (svc)
        logger.info('World-Intelligence Service enabled (/api/world)');
    else
        logger.info('World-Intelligence Service disabled (ENABLE_WORLD_INTELLIGENCE / ARANGO_URL / TSDB_URL)');
    ...
    const sendDisabled = (req, res, payload = {}) => {
    ...
    router.post('/contribute', guard, (req, res) => {
        if (!svc) {
            res.status(503).json({ error: 'world intelligence disabled' });
            return;
        }
        const parsed = world_types_1.WorldContributionSchema.safeParse(req.body || {});
        if (!parsed.success) {
            res.status(400).json({ error: 'invalid contribution', details: parsed.error.issues });
            return;
        }
    ...
    router.get('/metric', (req, res) => {
        if (!svc) {
            sendDisabled(req, res, { value: null, points: 0 });
            return;
        }
    ...
    router.post('/seed-outlets', guard, (req, res) => {
        ...
        svc.ingest((0, outlet_ratings_1.buildOutletSeedContribution)(new Date().toISOString()))
            .then((result) => res.json({ ok: true, result }))
    ...
    router.post('/ingest-news', guard, (req, res) => {
        if (!svc) {
            res.status(503).json({ error: 'world intelligence disabled' });
            return;
        }
        ...
        const rawSources = req.query.sources ?? body.sources;
        const sources = Array.isArray(rawSources)
            ? rawSources
            : (typeof rawSources === 'string' && rawSources.trim() ? rawSources.split(',').map((s) => s.trim()).filter(Boolean) : undefined);
        if (!q || !entity) {
            res.status(400).json({ error: 'q and entity required' });
            return;
        }
        (0, news_fetcher_1.ingestFeeds)(svc, q, entity, label, sources, { limit })
            .then((result) => res.json({ ok: true, result }))
    ...
    router.post('/backtest', guard, (req, res) => {
        if (!svc) {
            res.status(503).json({ error: 'world intelligence disabled' });
            return;
        }
        ...
        (0, news_fetcher_1.backtest)(svc, entity, days, { limit })
    ...
    router.get('/neighbors', (req, res) => {
        if (!svc) {
            sendDisabled(req, res, { neighbors: [] });
            return;
        }
    ...
    router.get('/app', (_req, res) => {
        res.type('html').send(world_app_html_1.WORLD_APP_HTML);
    });
    return router;
}
