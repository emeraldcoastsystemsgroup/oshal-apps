...
const stamp=(v:unknown)=>{const n=new Date(String(v)).getTime();return Number.isFinite(n)?new Date(n).toISOString():'date unknown';};
export function createHomeSummaryRoutes(ctx:AppContext):Router {
 ...
 router.get('/',async(req,res)=>{
  ...
  const now=new Date();const read=(text:string)=>ctx.pool.query({text,values:[String(sub),now],query_timeout:1800} as any);
  ...
  const currencies=new Set(Array.isArray(agg?.accounts)?agg.accounts.map((a:any)=>a.currency).filter(Boolean):[]);
  ...
  const items:any[]=[];
  if(row&&typeof row.brief==='string'&&row.brief.trim()&&row.brief_at&&new Date(row.brief_at)<=now){const notes=clip('Saved finance analysis ('+environment+'), generated '+stamp(row.brief_at)+', account snapshot '+stamp(row.synced_at)+'. '+row.brief,2000);items.push({text:'Your saved finance brief',detail:clip(notes),tone:'neutral',fix:'finance-home',actions:[{integration:'prepare-document',context:{title:'Review my saved finance brief',notes}}]});}
  else if(agg)items.push({text:'Your saved account snapshot',detail:'Source: '+environment+' · Synced '+stamp(row.synced_at)+'. Open Finance to review balances and request analysis.',tone:'neutral',fix:'finance-home'});
  if(transfers.status==='fulfilled')for(const t of transfers.value.rows){const notes=clip((t.test_mode?'TEST':'LIVE')+' transfer '+t.transfer_id+' via '+t.provider+': '+t.amount_cents+' cents ('+t.currency+'), recorded status '+t.status+'. Updated '+stamp(t.updated_at)+'. '+clip(t.description),2000);items.push({text:clip((t.test_mode?'Test':'Live')+' transfer: '+t.status,120),detail:clip(notes),tone:t.status==='failed'?'warn':'neutral',highlight:t.status==='failed',fix:'finance-home',actions:[{integration:'prepare-document',context:{title:'Review a recorded transfer',notes}}]});}
  const failed=[linked,saved,transfers].filter(r=>r.status==='rejected').length;
  if(failed)items.push({text:'Some saved financial sources cannot be checked.',tone:'warn',fix:'finance-home'});
  else if(!items.length)items.push({text:'Connect and sync accounts to build your financial overview.',tone:'neutral',fix:'finance-home'});
