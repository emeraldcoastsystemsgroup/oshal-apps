...
export function createCircuitLabRoutes(ctx: AppContext, opts: CircuitLabRouteOpts = {}): Router {
  ...
  const booted = loadDriverCatalog(catalogFile);
  ...
  const currentCatalog = (): ReturnType<typeof loadDriverCatalog> => {
    try {
      return loadDriverCatalog(catalogFile);
    } catch (err) {
  ...
  router.get('/capabilities', (_req: Request, res: Response) => {
    ...
    res.json({ app: 'circuit-lab', solver: 'ngspice (engine container)', contract: describeContract(), examples: listExamples(), engine: engine.status(), catalog: { drivers: catalog.drivers.length, unresolved: catalog.unresolved, path: '/api/circuit-lab/catalog/drivers' } });
  });
  ...
  router.get('/catalog/drivers', (req: Request, res: Response) => {
    ...
    const catalog = currentCatalog();
    res.json({ drivers: listDrivers(catalog.drivers, type), unresolved: catalog.unresolved.filter((u) => !type || u.type === type) });
  });
