 ...
 * -----------------------------------------------------------------------------
 * 1 | maintainer@emeraldcoastsystemsgroup.com   | Initial creation — copied from cad-studio: the api's client to the Circuit Lab engine
 ...
 *                     |                             | bridge on loopback. A refusal carries the worker's
 *                     |                             | `field` so the routes can name it in the 422 body.
 */

import net from 'node:net';
import { createChildLogger } from '@/shared/logger';

const logger = createChildLogger({ module: 'circuit-lab-engine-client' });

/** The bridge wire version this client speaks (PROTOCOL in circuit_engine_bridge.py and circuit_worker.py). */
export const BRIDGE_PROTOCOL = 1;
export const DEFAULT_ENGINE_ADDR = 'circuit-lab-engine:7413';
...
export class EngineFailure extends Error {
  constructor(readonly code: EngineFailureCode, message: string, readonly reason?: string, readonly field?: string) { super(message); this.name = 'EngineFailure'; }
}
...
export class EngineClient {
  ...
  private verifyHello(line: string): EngineFailure | null {
    ...
    try { msg = JSON.parse(line) as typeof msg; } catch { return this.unavailable(`${this.where()} did not answer the Circuit Lab engine bridge protocol`); }
  ...
  private onLine(line: string): void {
    let msg: { id?: unknown; ok?: boolean; result?: unknown; error?: { code?: string; message?: string; field?: string } };
    ...
    else current.reject(new EngineFailure(msg.error?.code === 'refused' ? 'refused' : 'engine_error', String(msg.error?.message || 'engine error'), undefined, typeof msg.error?.field === 'string' ? msg.error.field : undefined));
    this.touchIdle();
    this.pump();
  }
...
/**
 * @description Parse `host:port` (the CIRCUIT_LAB_ENGINE_ADDR form).
