...
function createHomeSummaryRoutes(ctx) {
    ...
    router.get('/', async (req, res) => {
        ...
        if (failed)
            items.push({ text: 'Some saved sources cannot be checked.', tone: 'warn', fix: 'circuit-lab' });
        else if (!items.length)
            items.push({ text: 'No circuits yet. Open Circuit Lab to begin.', tone: 'neutral', fix: 'circuit-lab' });
