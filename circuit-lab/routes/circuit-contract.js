...
exports.ContractError = exports.SIM_METHODS = exports.SIM_LIMITS = exports.SIM_DEFAULTS = exports.LIMITS = exports.ID_PATTERN = exports.ROTATIONS = exports.DRIVER_TYPES = exports.PART_TYPES = exports.PART_LIBRARY = exports.DEFAULT_SKETCH = exports.SKETCH_MAX_CHARS = exports.SKETCH_PATTERN = exports.SEQUENCE_MAX_STEPS = exports.SEQUENCE_PATTERN = void 0;
exports.pinKind = pinKind;
exports.validateProps = validateProps;
exports.parseSequence = parseSequence;
exports.validatePart = validatePart;
exports.validateWire = validateWire;
exports.validateRoute = validateRoute;
exports.validateCircuit = validateCircuit;
exports.validateSim = validateSim;
exports.describeContract = describeContract;
const num = (unit, def, min, max, optional) => (optional ? { type: 'number', unit, default: def, min, max, optional } : { type: 'number', unit, default: def, min, max });
const en = (values, def) => ({ type: 'enum', values, default: def });
const bool = (def) => ({ type: 'boolean', default: def });
const text = (def, maxLength, pattern) => ({ type: 'text', default: def, maxLength, pattern });
/** One token per step: level (0..1, a fraction of highVolts) : seconds. Shared with engine/circuit_parts.py. */
exports.SEQUENCE_PATTERN = String.raw `^\s*(?:(?:0|1|0?\.\d+):\d*\.?\d+(?:[eE]-?\d+)?\s*)+$`;
exports.SEQUENCE_MAX_STEPS = 64;
/** An Arduino sketch: any text up to the cap (the compiler judges it). Shared with engine/circuit_parts.py. */
exports.SKETCH_PATTERN = String.raw `^[\s\S]*$`;
exports.SKETCH_MAX_CHARS = 8000;
exports.DEFAULT_SKETCH = 'void setup() {\n  pinMode(13, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(13, HIGH);\n  delay(500);\n  digitalWrite(13, LOW);\n  delay(500);\n}\n';
const pins = (...names) => names.map((name) => ({ name, kind: 'electrical' }));
/** The part library — the same object engine/circuit_parts.py publishes with `--contract`. */
exports.PART_LIBRARY = Object.freeze({
    ...
    sequencer: { category: 'electrical', label: 'Sequenced pin', pins: pins('out', 'ref'), props: { highVolts: num('V', 5, 0, 1000), pattern: text('1:0.5 0:0.5', 400, exports.SEQUENCE_PATTERN), repeat: bool(true) } },
    ...
    arduino: { category: 'electrical', label: 'Arduino Uno', pins: pins('5V', 'GND', 'D2', 'D3', 'D4', 'D5', 'D6', 'D7', 'D8', 'D9', 'D10', 'D11', 'D12', 'D13', 'A0', 'A1', 'A2', 'A3', 'A4', 'A5'), props: {
            sketch: text(exports.DEFAULT_SKETCH, exports.SKETCH_MAX_CHARS, exports.SKETCH_PATTERN)
        } },
});
exports.PART_TYPES = Object.freeze(Object.keys(exports.PART_LIBRARY));
/** The part types that drive a shaft train (one per train). */
exports.DRIVER_TYPES = Object.freeze(['motor', 'servo', 'stepper']);
exports.ROTATIONS = Object.freeze([0, 90, 180, 270]);
exports.ID_PATTERN = /^[A-Za-z][A-Za-z0-9_]{0,23}$/;
exports.LIMITS = Object.freeze({ maxParts: 200, maxWires: 400, maxRoutePoints: 16 });
/** The transient defaults; reltol is the lab's own, gmin and method are ngspice's. */
exports.SIM_DEFAULTS = Object.freeze({ stopSeconds: 1.0, maxPoints: 20000, defaultPoints: 2000, startFromRest: true, reltol: 0.003, gmin: 1e-12, method: 'trap' });
exports.SIM_LIMITS = Object.freeze({ stopSeconds: [1e-6, 600], stepSeconds: [1e-9, 600], syncSeconds: [4e-6, 10e-3], reltol: [1e-6, 0.05], gmin: [1e-15, 1e-6] });
/** The integration methods a person may choose (ngspice's trapezoidal default, or Gear for stiff, hard-switched circuits). */
exports.SIM_METHODS = Object.freeze(['trap', 'gear']);
...
function pinKind(type, pin) {
    const spec = exports.PART_LIBRARY[type];
    if (!spec)
        return null;
 ...
 */
function validateProps(type, raw, field) {
    const schema = exports.PART_LIBRARY[type].props;
    const props = obj(raw) ? raw : {};
    for (const key of Object.keys(props))
        if (!Object.prototype.hasOwnProperty.call(schema, key))
            throw new ContractError(`${type} has no property ${key}`, `${field}.props.${key}`);
    const out = {};
    for (const [key, spec] of Object.entries(schema)) {
        ...
        if (spec.type === 'number') {
            ...
            if (!isNum(value))
                throw new ContractError(`${key} must be a number`, `${field}.props.${key}`);
            if (value < spec.min || value > spec.max)
                throw new ContractError(`${key} must be between ${spec.min} and ${spec.max} ${spec.unit}`, `${field}.props.${key}`);
            out[key] = value;
        }
        else if (spec.type === 'enum') {
            if (typeof value !== 'string' || !spec.values.includes(value))
                throw new ContractError(`${key} must be one of ${spec.values.join(', ')}`, `${field}.props.${key}`);
            out[key] = value;
        }
        else if (spec.type === 'boolean') {
            if (typeof value !== 'boolean')
                throw new ContractError(`${key} must be true or false`, `${field}.props.${key}`);
            out[key] = value;
        }
        else {
            if (typeof value !== 'string' || value.length > spec.maxLength || !new RegExp(spec.pattern).test(value))
                throw new ContractError(`${key} must match ${spec.pattern} (at most ${spec.maxLength} characters)`, `${field}.props.${key}`);
            if (key === 'pattern') {
                const steps = parseSequence(value);
                if (steps.length > exports.SEQUENCE_MAX_STEPS)
                    throw new ContractError(`pattern has ${steps.length} steps; at most ${exports.SEQUENCE_MAX_STEPS}`, `${field}.props.${key}`);
                if (steps.some(([, sec]) => !(sec > 0)))
                    throw new ContractError('every step needs a positive duration', `${field}.props.${key}`);
            }
            out[key] = value;
        }
    }
...
function parseSequence(value) {
    return value.trim().split(/\s+/).filter(Boolean).map((token) => { const [level, sec] = token.split(':'); return [Number(level), Number(sec)]; });
}
/** @description Validate one part (id kept when well-formed; `fallbackId` is used for updates). */
function validatePart(raw, field = 'part', fallbackId) {
    if (!obj(raw))
        throw new ContractError('part must be an object', field);
    const id = typeof raw.id === 'string' && exports.ID_PATTERN.test(raw.id) ? raw.id : fallbackId;
    if (!id || !exports.ID_PATTERN.test(id))
        throw new ContractError('id must be a letter followed by up to 23 letters, digits or underscores', `${field}.id`);
    const type = raw.type;
    if (typeof type !== 'string' || !exports.PART_LIBRARY[type])
        throw new ContractError(`unknown part type ${JSON.stringify(type)}`, `${field}.type`);
    const rotation = raw.rotation === undefined ? 0 : raw.rotation;
    if (!exports.ROTATIONS.includes(rotation))
        throw new ContractError('rotation must be 0, 90, 180 or 270', `${field}.rotation`);
    const x = raw.x === undefined ? 0 : raw.x, y = raw.y === undefined ? 0 : raw.y;
    if (!isNum(x))
        throw new ContractError('x must be a number', `${field}.x`);
    if (!isNum(y))
        throw new ContractError('y must be a number', `${field}.y`);
    if (raw.label !== undefined && raw.label !== null && typeof raw.label !== 'string')
        throw new ContractError('label must be text', `${field}.label`);
    return { id, type, x, y, rotation: rotation, label: String(raw.label ?? '').slice(0, 60), props: validateProps(type, raw.props, field) };
}
/** @description Validate a wire against the parts it joins: both ends exist, pins are of one kind, no self-loop. */
function validateWire(raw, parts, field = 'wire', fallbackId) {
    if (!obj(raw))
        throw new ContractError('wire must be an object', field);
    const id = typeof raw.id === 'string' && exports.ID_PATTERN.test(raw.id) ? raw.id : fallbackId;
    if (!id || !exports.ID_PATTERN.test(id))
        throw new ContractError('wire id must be a letter followed by up to 23 letters, digits or underscores', `${field}.id`);
    const byId = new Map(parts.map((p) => [p.id, p]));
    const ends = [];
    for (const endName of ['from', 'to']) {
        const end = raw[endName];
        if (!obj(end) || typeof end.part !== 'string' || !byId.has(end.part))
            throw new ContractError(`${endName}.part must name a part in the circuit`, `${field}.${endName}.part`);
        const kind = pinKind(byId.get(end.part).type, end.pin);
        if (!kind)
            throw new ContractError(`${byId.get(end.part).type} ${end.part} has no pin ${JSON.stringify(end.pin)}`, `${field}.${endName}.pin`);
        ends.push({ end: { part: end.part, pin: String(end.pin) }, kind });
    }
    if (ends[0].kind !== ends[1].kind)
        throw new ContractError(`a wire joins pins of one kind; ${ends[0].end.part}.${ends[0].end.pin} is ${ends[0].kind} and ${ends[1].end.part}.${ends[1].end.pin} is ${ends[1].kind}`, field);
    if (ends[0].end.part === ends[1].end.part && ends[0].end.pin === ends[1].end.pin)
        throw new ContractError('a wire cannot join a pin to itself', field);
    const wire = { id, kind: ends[0].kind, from: ends[0].end, to: ends[1].end };
    if (raw.route !== undefined && raw.route !== null)
        wire.route = validateRoute(raw.route, `${field}.route`);
    return wire;
}
/**
 ...
 */
function validateRoute(raw, field) {
    const route = obj(raw) ? raw : {};
    if (route.points !== undefined && route.mid !== undefined)
        throw new ContractError('a route is either {mid} or {points}, not both', field);
    if (route.points !== undefined) {
        if (!Array.isArray(route.points) || route.points.length < 1 || route.points.length > exports.LIMITS.maxRoutePoints)
            throw new ContractError(`route.points must be a list of 1 to ${exports.LIMITS.maxRoutePoints} bend points [x, y]`, `${field}.points`);
        const points = route.points.map((p, i) => {
            if (!Array.isArray(p) || p.length !== 2 || !isNum(p[0]) || !isNum(p[1]))
                throw new ContractError('a bend point is [x, y] in canvas units', `${field}.points[${i}]`);
            return [p[0], p[1]];
        });
        return { points };
    }
    if (!isNum(route.mid))
        throw new ContractError("route.mid must be a number (where the wire's middle segment sits)", `${field}.mid`);
    return { mid: route.mid };
}
const wireKey = (w) => [`${w.from.part}.${w.from.pin}`, `${w.to.part}.${w.to.pin}`].sort().join('|');
/** @description Validate a whole circuit: unique ids (case-insensitive), the caps, no duplicate wires. */
function validateCircuit(raw) {
    if (!obj(raw))
        throw new ContractError('circuit must be an object', 'circuit');
    const partsIn = raw.parts === undefined ? [] : raw.parts, wiresIn = raw.wires === undefined ? [] : raw.wires;
    if (!Array.isArray(partsIn))
        throw new ContractError('parts must be a list', 'parts');
    if (!Array.isArray(wiresIn))
        throw new ContractError('wires must be a list', 'wires');
    if (partsIn.length > exports.LIMITS.maxParts)
        throw new ContractError(`at most ${exports.LIMITS.maxParts} parts`, 'parts');
    if (wiresIn.length > exports.LIMITS.maxWires)
        throw new ContractError(`at most ${exports.LIMITS.maxWires} wires`, 'wires');
    const parts = [], seen = new Set();
    partsIn.forEach((p, i) => {
        const part = validatePart(p, `parts[${i}]`);
        if (seen.has(part.id.toLowerCase()))
            throw new ContractError(`duplicate part id ${part.id} (ids are case-insensitive)`, `parts[${i}].id`);
        seen.add(part.id.toLowerCase());
        parts.push(part);
    });
    const wires = [], keys = new Set(), ids = new Set();
    wiresIn.forEach((w, i) => {
        const wire = validateWire(w, parts, `wires[${i}]`);
        if (ids.has(wire.id))
            throw new ContractError(`duplicate wire id ${wire.id}`, `wires[${i}].id`);
        if (keys.has(wireKey(wire)))
            throw new ContractError(`duplicate wire between ${wire.from.part}.${wire.from.pin} and ${wire.to.part}.${wire.to.pin}`, `wires[${i}]`);
        ids.add(wire.id);
...
function validateSim(raw) {
    const sim = obj(raw) ? raw : {};
    const stop = sim.stopSeconds === undefined ? exports.SIM_DEFAULTS.stopSeconds : sim.stopSeconds;
    if (!isNum(stop) || stop < exports.SIM_LIMITS.stopSeconds[0] || stop > exports.SIM_LIMITS.stopSeconds[1])
        throw new ContractError(`stopSeconds must be between ${exports.SIM_LIMITS.stopSeconds[0]} and ${exports.SIM_LIMITS.stopSeconds[1]}`, 'sim.stopSeconds');
    let step = null;
    if (sim.stepSeconds !== undefined && sim.stepSeconds !== null) {
        if (!isNum(sim.stepSeconds) || sim.stepSeconds < exports.SIM_LIMITS.stepSeconds[0] || sim.stepSeconds > exports.SIM_LIMITS.stepSeconds[1])
            throw new ContractError(`stepSeconds must be between ${exports.SIM_LIMITS.stepSeconds[0]} and ${exports.SIM_LIMITS.stepSeconds[1]}`, 'sim.stepSeconds');
        step = sim.stepSeconds;
    }
    const points = Math.floor(stop / (step ?? stop / exports.SIM_DEFAULTS.defaultPoints)) + 1;
    if (points > exports.SIM_DEFAULTS.maxPoints)
        throw new ContractError(`stopSeconds / stepSeconds is ${points} points; at most ${exports.SIM_DEFAULTS.maxPoints} — raise stepSeconds`, 'sim.stepSeconds');
    if (points < 4)
        throw new ContractError('stepSeconds must give at least four points', 'sim.stepSeconds');
    const rest = sim.startFromRest === undefined ? exports.SIM_DEFAULTS.startFromRest : sim.startFromRest;
    if (typeof rest !== 'boolean')
        throw new ContractError('startFromRest must be true or false', 'sim.startFromRest');
    const out = { stopSeconds: stop, stepSeconds: step, startFromRest: rest };
    ...
    if (sim.syncSeconds !== undefined && sim.syncSeconds !== null) {
        if (!isNum(sim.syncSeconds) || sim.syncSeconds < exports.SIM_LIMITS.syncSeconds[0] || sim.syncSeconds > exports.SIM_LIMITS.syncSeconds[1])
            throw new ContractError(`syncSeconds (the firmware co-simulation step) must be between ${exports.SIM_LIMITS.syncSeconds[0]} and ${exports.SIM_LIMITS.syncSeconds[1]}`, 'sim.syncSeconds');
 ...
 */
function solverKnobs(sim, out) {
    for (const key of ['reltol', 'gmin']) {
        const value = sim[key];
        if (value === undefined || value === null)
            continue;
        const [lo, hi] = exports.SIM_LIMITS[key];
        if (!isNum(value) || value < lo || value > hi)
            throw new ContractError(`${key} must be between ${lo} and ${hi}`, `sim.${key}`);
        out[key] = value;
    }
    if (sim.method !== undefined && sim.method !== null) {
        if (typeof sim.method !== 'string' || !exports.SIM_METHODS.includes(sim.method))
            throw new ContractError(`method must be one of ${exports.SIM_METHODS.join(', ')}`, 'sim.method');
...
function describeContract() {
    return {
        parts: exports.PART_LIBRARY,
        limits: { maxParts: exports.LIMITS.maxParts, maxWires: exports.LIMITS.maxWires, maxRoutePoints: exports.LIMITS.maxRoutePoints, rotations: [...exports.ROTATIONS], idPattern: exports.ID_PATTERN.source },
        sim: { defaults: exports.SIM_DEFAULTS, limits: { stopSeconds: [...exports.SIM_LIMITS.stopSeconds], stepSeconds: [...exports.SIM_LIMITS.stepSeconds], syncSeconds: [...exports.SIM_LIMITS.syncSeconds], reltol: [...exports.SIM_LIMITS.reltol], gmin: [...exports.SIM_LIMITS.gmin] }, methods: [...exports.SIM_METHODS] },
        rules: [
