...
exports.POSITION_LEVERAGE = exports.STATUS_OUT_PROBABILITY = void 0;
exports.availabilityImpact = availabilityImpact;
exports.restImpact = restImpact;
exports.computeUnitRatings = computeUnitRatings;
exports.matchupEdge = matchupEdge;
 ...
 */
exports.STATUS_OUT_PROBABILITY = {
 ...
 */
function availabilityImpact(injuries, league) {
    const table = exports.POSITION_LEVERAGE[league] || {};
    const losses = [];
    let total = 0;
    for (const inj of injuries) {
        const outProb = exports.STATUS_OUT_PROBABILITY[String(inj.status || '').toLowerCase().trim()];
        if (!outProb)
            continue;
 ...
 */
function restImpact(ctx, league) {
    const days = Number.isFinite(ctx.restDays) ? ctx.restDays : 7;
    let points = 0;
    if (league === 'nba') {
        if (days <= 1)
            points -= 1.8; // back-to-back
        else if (days === 2)
            points -= 0.4; // 3-in-4 territory
        else if (days >= 4)
            points += 0.5; // genuinely rested
    }
    else {
        if (days <= 4)
            points -= 1.0; // short week, e.g. a Thursday game
        else if (days >= 13)
            points += 0.6; // off a bye
    }
    if (ctx.longTravel)
        points -= 0.4;
 ...
 */
function computeUnitRatings(games, league) {
    const half = sports_ratings_1.LEAGUE_CONSTANTS[league].homeAdvantagePoints / 2;
    const scored = {};
    const allowed = {};
    const opponents = {};
 ...
 */
function matchupEdge(home, away) {
    const h = home || { team: '?', offense: 0, defense: 0, games: 0 };
    const a = away || { team: '?', offense: 0, defense: 0, games: 0 };
    const homeEdge = { offenseVsDefense: round2(h.offense - a.defense), defenseVsOffense: round2(h.defense - a.offense) };
    const awayEdge = { offenseVsDefense: round2(a.offense - h.defense), defenseVsOffense: round2(a.defense - h.offense) };
...
//# sourceMappingURL=sports-adjustments.js.map