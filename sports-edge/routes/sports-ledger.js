...
exports.ENSEMBLE_STRATEGY = exports.MIN_GRADED = void 0;
exports.predictionsFrom = predictionsFrom;
exports.gradeOne = gradeOne;
exports.spreadProbability = spreadProbability;
exports.classify = classify;
exports.rollup = rollup;
const sports_odds_1 = require("./sports-odds");
const sports_odds_2 = require("./sports-odds");
/** Settled calls a strategy needs before its record means anything. */
exports.MIN_GRADED = Number(process.env.SPORTS_MIN_GRADED) || 30;
/** The strategy name the voting ensemble records under. */
exports.ENSEMBLE_STRATEGY = 'ensemble';
/**
 ...
 */
function predictionsFrom(preview, stakeAllowed) {
    const base = {
    ...
    const rows = preview.edges.map((e) => ({
        ...base,
        strategy: exports.ENSEMBLE_STRATEGY,
 ...
 */
function shadowRows(preview, base) {
    const out = [];
    const ml = preview.quote.homeMoneyline;
    const opp = preview.quote.awayMoneyline;
    const marketProb = Number.isFinite(ml) && Number.isFinite(opp)
        ? (0, sports_odds_1.devigTwoWay)(ml, opp).home : NaN;
    const add = (strategy, prob, note) => {
        if (!Number.isFinite(prob) || !Number.isFinite(marketProb))
            return;
        out.push({
    ...
    for (const s of preview.line.shadow) {
        ...
        if (s.model === 'espn-predictor' || !Number.isFinite(s.points))
            continue;
        add(s.model, (0, sports_odds_1.winProbabilityFromMargin)(s.points, preview.league), s.basis);
    }
    ...
    if (Number.isFinite(pct)) {
        add('espn-predictor', pct / 100, `ESPN projects ${pct.toFixed(1)}% for ${preview.homeTeam} — a free number this package must beat`);
    }
    return out;
}
/**
...
function gradeOne(row, settle) {
    ...
    const clv = (settle.closingPrice !== undefined && settle.closingOpposingPrice !== undefined && row.priceAtPick !== null)
        ? (0, sports_odds_1.closingLineValue)(row.priceAtPick, settle.closingPrice, settle.closingOpposingPrice)
        : null;
    const dec = row.priceAtPick !== null ? (0, sports_odds_1.decimalOdds)(row.priceAtPick) : NaN;
    const pnlUnits = Number.isFinite(dec) ? (won ? dec - 1 : -1) : 0;
    return { won, brier, marketBrier, clv: Number.isFinite(clv) ? clv : null, pnlUnits };
}
/**
 ...
 */
function spreadProbability(projectedMargin, homeSpread, side, league) {
    const homeCover = (0, sports_odds_2.coverProbability)(projectedMargin, homeSpread, league);
 ...
 */
function classify(rollup) {
    const base = { strategy: rollup.strategy, rollup };
    if (rollup.graded < exports.MIN_GRADED) {
        return {
            ...base, verdict: 'UNPROVEN', mayStake: false,
            reason: `${rollup.graded} of ${exports.MIN_GRADED} settled calls — not enough evidence to stake anything.`,
        };
    }
    ...
    const why = [];
    if (!beatsMarket)
        why.push(`Brier ${rollup.brier.toFixed(4)} does not beat the market's ${rollup.marketBrier.toFixed(4)}`);
    if (!beatsClose) {
 ...
 * @returns The rollup.
 */
function rollup(strategy, rows) {
    const graded = rows.length;
    const clvRows = rows.filter((r) => r.clv !== null && Number.isFinite(r.clv));
    ...
    return {
        ...
        meanClv: clvRows.length ? sum(clvRows.map((r) => r.clv)) / clvRows.length : 0,
        clvSamples: clvRows.length,
        pnlUnits: sum(rows.map((r) => r.pnlUnits)),
    };
}
//# sourceMappingURL=sports-ledger.js.map