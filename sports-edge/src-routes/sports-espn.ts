...
/**
 * Why a read failed, as far as this client can tell — and the distinction a message has to make:
 *
 ...
 */
export type EspnFailureKind = 'transport' | 'unavailable' | 'refused';
 ...
 */
export function classifyFailure(reason: string): EspnFailure {
...
export async function readWithOutcome<T>(
  opts: EspnOptions, run: (opts: EspnOptions) => Promise<T>,
): Promise<{ value: T; failure: EspnFailure | null }> {
  const captured: EspnFailure[] = [];
 ...
 */
export function parseHeadCoach(roster: any): string | null {
  const staff = Array.isArray(roster?.coach) ? roster.coach.filter((coach: any) => coach && typeof coach === 'object') : [];
  const title = (coach: any): string => String(coach.title ?? coach.position?.name ?? coach.type?.text ?? '').trim().toLowerCase();
  const namedHeads = staff.filter((coach: any) => /^(?:(?:interim|acting) )?head[ -]coach$/.test(title(coach)));
 ...
 */
export async function teamHeadCoach(league: League, teamId: string, team: string, opts: EspnOptions = {}): Promise<{ name: string; teamName: string } | null> {
  const p = LEAGUE_PATHS[league];
  if (!p || !/^[0-9]{1,12}$/.test(teamId) || !/^[a-z0-9.-]{1,20}$/i.test(team)) return null;
  ...
  if (!body?.team || String(body.team.id) !== teamId || typeof body.team.abbreviation !== 'string'
    || body.team.abbreviation.toUpperCase() !== team.toUpperCase()) return null;
