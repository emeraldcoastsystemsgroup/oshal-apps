...
/** A subject this package asks World to cover. Mirrors the shape World's own refresh consumes. */
export interface SportsSubject {
  /** Canonical world entity id — `world:<type>:<key>`. Stable across passes, by construction. */
  entity: string;
  /** Display label, which is also what the archive shows in the world dashboard. */
  label: string;
  /** The search string driven into every registered feed. */
  query: string;
  /** What this subject is about, so a caller can pull just the injury feed or just the matchup. */
  kind: 'team' | 'injuries' | 'coach' | 'matchup';
}
 ...
 */
const LEAGUE_TERMS: Readonly<Record<League, string>> = {
  nfl: 'NFL',
  nba: 'NBA',
 ...
 */
export const SPORTS_FEED_IDS: ReadonlyArray<string> = ['google-news', 'bing-news', 'reddit'];
 ...
 */
export function teamEntityId(league: League, team: string): string {
 ...
 */
export function matchupEntityId(league: League, eventId: string): string {
  return `world:matchup:${league}-${slug(eventId)}`;
}

/** The identity of a followed team, as much of it as the subject derivation needs. */
 ...
 */
export function teamSubjects(t: TeamRef): SportsSubject[] {
 ...
 * @returns One subject.
 */
export function matchupSubject(
  league: League,
  eventId: string,
  homeName: string,
  awayName: string,
 ...
 */
export function dedupeSubjects(subjects: SportsSubject[]): SportsSubject[] {
  const seen = new Set<string>();
  const out: SportsSubject[] = [];
 ...
 */
export function dueSubjects(
  subjects: SportsSubject[],
  lastPulled: ReadonlyMap<string, string>,
...
/** How the World layer answers an ingest, reduced to what this package records. */
...
/**
 * The two calls this module needs from the World engine, named as an interface so the ingest pass
 * is testable without a live service, a database, or an outbound request.
 */
export interface WorldIngest {
  (query: string, entity: string, label: string, sources: string[], opts: { limit: number }): Promise<{
    perSource: Array<{ fetched?: number; newItems?: number }>;
  }>;
}
 ...
 */
export async function ingestSubject(
  ingest: WorldIngest,
  subject: SportsSubject,
  limit: number,
): Promise<IngestOutcome> {
  try {
    const r = await ingest(subject.query, subject.entity, subject.label, [...SPORTS_FEED_IDS], { limit });
    ...
    return { entity: subject.entity, fetched: 0, newItems: 0, error: (e as Error).message };
  }
}
