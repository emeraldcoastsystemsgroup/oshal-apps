 ...
 */
export async function ensureFantasySchema(pool: Pool): Promise<void> {
  for (const stmt of DDL) await pool.query(stmt);
}

/** A league a person has linked. */
 ...
 * @returns Nothing.
 */
export async function linkLeague(pool: Pool, userSub: string, league: LinkedLeague): Promise<void> {
  await pool.query(
    `INSERT INTO sports_fantasy_leagues (user_sub, season, league_id, league_name, team_id, team_name)
     VALUES ($1,$2,$3,$4,$5,$6)
     ON CONFLICT (user_sub, season, league_id) DO UPDATE
     SET league_name = EXCLUDED.league_name, team_id = EXCLUDED.team_id, team_name = EXCLUDED.team_name`,
    [userSub, league.season, league.leagueId, league.leagueName, league.teamId, league.teamName],
  );
}
 ...
 */
export async function unlinkLeague(pool: Pool, userSub: string, season: number, leagueId: string): Promise<number> {
  const r = await pool.query(
    'DELETE FROM sports_fantasy_leagues WHERE user_sub = $1 AND season = $2 AND league_id = $3',
    [userSub, season, leagueId],
  );
  return r.rowCount || 0;
}
 ...
 */
export async function listLeagues(pool: Pool, userSub: string): Promise<LinkedLeague[]> {
  const r = await pool.query(
    `SELECT season, league_id, league_name, team_id, team_name FROM sports_fantasy_leagues
     WHERE user_sub = $1 ORDER BY linked_at ASC`,
    [userSub],
  );
...
/** The cached projection feed for one week. */
 ...
 */
export async function readProjections(pool: Pool, season: number, week: number): Promise<CachedProjections | null> {
  const r = await pool.query(
    'SELECT payload, players, generated_at FROM sports_fantasy_projections WHERE season = $1 AND scoring_period = $2',
    [season, week],
  );
  if (!r.rows.length) return null;
  return {
    players: r.rows[0].payload as Record<number, DistilledPlayer>,
...
export async function writeProjections(
  pool: Pool, season: number, week: number, players: Record<number, DistilledPlayer>, buildMs: number,
): Promise<void> {
  await pool.query(
    `INSERT INTO sports_fantasy_projections (season, scoring_period, payload, players, generated_at, build_ms)
...
export async function recordCalls(
  pool: Pool, userSub: string, season: number, leagueId: string, week: number, calls: StartSitCall[],
): Promise<number> {
  let n = 0;
  for (const c of calls) {
    const r = await pool.query(
      `INSERT INTO sports_fantasy_calls
       ...
       WHERE sports_fantasy_calls.settled = FALSE`,
      [userSub, season, leagueId, week, c.start.playerId, c.start.name, c.sit.playerId, c.sit.name,
 ...
 */
export async function openCalls(pool: Pool, season: number, beforeWeek: number, limit = 500): Promise<OpenCall[]> {
  const r = await pool.query(
    `SELECT id, user_sub, season, league_id, week, start_player_id, sit_player_id
     FROM sports_fantasy_calls
     WHERE settled = FALSE AND season = $1 AND week < $2
     ORDER BY week ASC LIMIT $3`,
    [season, beforeWeek, limit],
  );
  return r.rows.map((x: any) => ({
 ...
 * @returns Nothing.
 */
export async function gradeCall(pool: Pool, id: number, actualStart: number, actualSit: number): Promise<void> {
  await pool.query(
    `UPDATE sports_fantasy_calls
     SET settled = TRUE, actual_start = $2, actual_sit = $3, actual_gain = $2 - $3, graded_at = now()
     WHERE id = $1 AND settled = FALSE`,
    [id, actualStart, actualSit],
  );
}

/** What the fantasy record adds up to. */
export interface FantasyRecord {
  graded: number;
  /** Calls whose swap actually gained points. */
  right: number;
  /** Total points the recommendations actually gained (or lost). */
  actualPoints: number;
  /** Total points they were projected to gain — the claim, next to the result. */
 ...
 * @returns The rollup.
 */
export async function fantasyRecord(pool: Pool, userSub: string): Promise<FantasyRecord> {
  const r = await pool.query(
    `SELECT count(*) AS graded,
     ...
     FROM sports_fantasy_calls WHERE user_sub = $1 AND settled = TRUE`,
    [userSub],
  );
  const row = r.rows[0] || {};
  return {
 ...
 */
export async function listCalls(pool: Pool, userSub: string, limit = 100): Promise<any[]> {
  const r = await pool.query(
    `SELECT season, league_id, week, start_player_name, sit_player_name, projected_gain,
            settled, actual_start, actual_sit, actual_gain, reason, created_at, graded_at
     FROM sports_fantasy_calls WHERE user_sub = $1 ORDER BY created_at DESC LIMIT $2`,
    [userSub, limit],
  );
  return r.rows;
}
