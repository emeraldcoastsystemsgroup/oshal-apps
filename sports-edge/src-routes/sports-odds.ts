...
/** The leagues Phase 1 models. Adding one means adding its margin sigma and rating constants. */
export type League = 'nfl' | 'nba' | 'ncaaf';
 ...
 */
export const MARGIN_SIGMA: Record<League, number> = { nfl: 13.4, nba: 11.5, ncaaf: 16.5 };
...
/** A two-way market with the book's margin removed. `overround` is what was taken out. */
export interface DevigResult {
  /** Fair probability of the first side. */
  home: number;
  /** Fair probability of the second side. */
  away: number;
  /** Raw probabilities' sum; 1.0 would be a vig-free market. */
  overround: number;
}
 ...
 * unusable.
 */
export function devigTwoWay(homeAmerican: number, awayAmerican: number): DevigResult {
  const rawHome = impliedProbability(homeAmerican);
  const rawAway = impliedProbability(awayAmerican);
  const sum = rawHome + rawAway;
  if (!Number.isFinite(sum) || sum <= 0) return { home: NaN, away: NaN, overround: NaN };
 ...
 */
export function winProbabilityFromMargin(projectedMargin: number, league: League): number {
  const sigma = MARGIN_SIGMA[league];
  if (!Number.isFinite(projectedMargin) || !sigma) return NaN;
 ...
 */
export function coverProbability(projectedMargin: number, homeSpread: number, league: League): number {
  const sigma = MARGIN_SIGMA[league];
  if (!Number.isFinite(projectedMargin) || !Number.isFinite(homeSpread) || !sigma) return NaN;
 ...
 */
export function kellyFraction(probability: number, american: number): number {
  const dec = decimalOdds(american);
  if (!Number.isFinite(probability) || !Number.isFinite(dec) || dec <= 1) return 0;
  const b = dec - 1;
  const full = (probability * b - (1 - probability)) / b;
  if (!(full > 0)) return 0;
  return Math.min(full * KELLY_MULTIPLIER, MAX_STAKE_FRACTION);
}
...
export function expectedValue(probability: number, american: number): number {
  const dec = decimalOdds(american);
  if (!Number.isFinite(probability) || !Number.isFinite(dec)) return NaN;
...
export function closingLineValue(
  takenAmerican: number, closingAmerican: number, opposingClosingAmerican: number,
): number {
  const close = devigTwoWay(closingAmerican, opposingClosingAmerican);
  if (!Number.isFinite(close.home)) return NaN;
  const takenRaw = impliedProbability(takenAmerican);
  const closeRaw = impliedProbability(closingAmerican);
  if (!Number.isFinite(takenRaw) || !Number.isFinite(closeRaw) || closeRaw <= 0) return NaN;
